...
import {
  GeminiStrategy,
  GeminiCostAnalyzer,
} from "../strategies/gemini-strategy";
import {
  AIRequest,
  AIResponse,
  ChatRequest,
...
export class AIOrchestrator {
  ...
  constructor(config: OrchestratorConfig) {
    ...
    if (config.useSemanticCache) {
      ...
      this.cache =
        config.cache || new CacheManager(process.env.REDIS_URL, false);
    }
  ...
  async process(request: AIRequest): Promise<AIResponse> {
    ...
    while (attempts < maxAttempts) {
      try {
        console.log(
          `[Orchestrator] Processing with ${provider.constructor.name}`,
        );
        ...
        attempts++;
        console.error(
          `[Orchestrator] Provider failed (attempt ${attempts}):`,
          error,
        );
        ...
        if (attempts >= maxAttempts) {
          ...
          if (fallback) {
            console.log(
              `[Orchestrator] Trying fallback provider ${fallback.constructor.name}`,
            );
  ...
  private async cacheResponse(
    ...
    if (this.cache instanceof SemanticCache && "prompt" in request) {
      const embedding = await this.cache.generateEmbedding(request.prompt);
      await this.cache.setWithEmbedding(
        request,
        response,
        embedding,
        ttlOverride,
  ...
  private selectProvider(
    ...
    const requiresStability =
      feature === "communication-helper" || feature === "clara";

    if (
      GeminiStrategy.shouldPreferGemini(
        taskType,
        costSensitive,
        requiresStability,
      )
      ...
      if (geminiProvider) {
        console.log(
          `[Orchestrator] Selected Gemini for ${taskType} (cost savings: ${
            GeminiStrategy.getOptimalConfiguration(taskType, request)
              .costSavings
          }%)`,
        );
