...
class AIModelManager {
   ...
   */
  getBestModelForTask(task: 'text' | 'vision' | 'analysis' | 'fast'): { provider: AIProvider; model: AIModel } | null {
    const taskModels: Record<string, Array<{ provider: AIProvider; model: AIModel }>> = {
      text: [
  ...
  private async generateOpenAIText(
    ...
    return {
      ...
      usage: usage ? {
        ...
        cost: this.calculateOpenAICost(model, usage.prompt_tokens, usage.completion_tokens)
      } : undefined
    }
  }
...
Format as JSON if possible.`
   ...
   */
  private calculateOpenAICost(model: AIModel, promptTokens: number, completionTokens: number): number {
    ...
    return (promptTokens * modelCost.prompt / 1000) + (completionTokens * modelCost.completion / 1000)
  }
...
export async function generateAIText(prompt: string, options?: any): Promise<AIResponse> {
  return aiModelManager.generateText(prompt, options)
}

export async function analyzeAIImage(imageData: string | Buffer, prompt: string, options?: any): Promise<AIImageAnalysis> {
