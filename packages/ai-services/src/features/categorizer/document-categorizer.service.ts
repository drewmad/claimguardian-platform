...
export class DocumentCategorizerService {
  ...
  private analyzeByFilename(fileName: string): CategoryAnalysis {
    ...
    if (lowerName.match(/\.(jpg|jpeg|png|gif)$/)) {
      return {
        category: this.DOCUMENT_CATEGORIES.find(
          (c) => c.id === "damage-photos",
        )!,
        confidence: 0.6,
  ...
  private async saveCategorization(
    ...
    const { error } = await this.supabase.from("categorized_documents").insert({
      id: doc.id,
      user_id: userId,
  ...
  private async checkRequiredDocuments(
    ...
    const missingRequired = this.DOCUMENT_CATEGORIES.filter(
      (cat) => cat.requiredForClaim && !uploadedCategories.has(cat.id),
    ).map((cat) => cat.name);
