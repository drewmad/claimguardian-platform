...
export class ClarityService {
  ...
  async calculateClaim(claimData: ClaimData, userId: string): Promise<ClarityCalculation> {
    ...
    // 4. Generate summary
    const summary = await this.generateSummary(explainedSteps, totalValue, claimData, userId);
  ...
  private async generateCalculationSteps(claimData: ClaimData): Promise<CalculationStep[]> {
    ...
    if (claimData.coverageType === 'ACV') {
      const depreciation = this.calculateDepreciation(replacementCost.value, claimData);
      steps.push({
        stepNumber: 2,
    ...
    if (claimData.damageType === 'total_loss' || claimData.damageType === 'major') {
  ...
  private calculateDepreciation(replacementCost: number, claimData: ClaimData): {
    value: number;
    explanation: string;
    formula: string;
  } {
  ...
  private calculateDeductible(claimData: ClaimData): {
    ...
    if (claimData.damageType === 'hurricane') {
      value = claimData.policyLimits.dwelling! * this.FLORIDA_DEDUCTIBLES.hurricane;
  ...
  private applyPolicyLimits(currentTotal: number, claimData: ClaimData): {
    limited: boolean;
    adjustment: number;
    explanation: string;
  } {
