...
export class SentinelService {
  ...
  async analyzeClaimDeadlines(
    ...
    const deadlines = await this.generateDeadlines(
      claimId,
      fullClaimData,
      userId,
    );
  ...
  private async generateDeadlines(
    ...
    if (claimData.currentStatus === "not_filed") {
      deadlines.push({
        ...
        dueDate: new Date(
          damageDate.getTime() +
            this.FLORIDA_DEADLINES.initial_claim * 24 * 60 * 60 * 1000,
        ),
        status: "pending",
        priority: "critical",
        remindersSent: 0,
    ...
    if (claimData.claimType === "hurricane") {
      deadlines.push({
        ...
        dueDate: new Date(
          damageDate.getTime() +
            this.FLORIDA_DEADLINES.hurricane_notice * 24 * 60 * 60 * 1000,
        ),
        status: "pending",
        priority: "critical",
        remindersSent: 0,
    ...
    if (
      claimData.currentStatus === "submitted" ||
      claimData.currentStatus === "investigating"
    ) {
      ...
      deadlines.push({
        ...
        dueDate: new Date(
          filingDate.getTime() +
            this.FLORIDA_DEADLINES.proof_of_loss * 24 * 60 * 60 * 1000,
        ),
        status: "pending",
        priority: "high",
        remindersSent: 0,
    ...
    const aiDeadlines = await this.analyzeForAdditionalDeadlines(
      claimData,
      userId,
    );
  ...
  private async saveDeadlines(deadlines: ClaimDeadline[]): Promise<void> {
    ...
    const { error } = await this.supabase.from("claim_deadlines").insert(
      deadlines.map((d) => ({
        ...d,
