...
export class MapsIntelligenceService {
  ...
  async updateApiExecution(
    ...
    const { data: execution, error } = await this.supabase
      ...
      .update({
        ...updates,
        executed_at: updates.status === 'completed' ? new Date().toISOString() : undefined,
  ...
  async getAddressIntelligence(
    propertyId: string,
    latest = true
  ): Promise<IntelligenceResponse<AddressIntelligence | AddressIntelligence[]>> {
    let query = this.supabase
      .from('address_intelligence')
      .select('*')
      .eq('property_id', propertyId)
  ...
  ): Promise<IntelligenceResponse<WeatherIntelligence | WeatherIntelligence[]>> {
    let query = this.supabase
      .from('weather_intelligence')
      .select('*')
      .eq('property_id', propertyId)
  ...
  async getPropertyIntelligenceSummary(
    ...
    const { data: summary, error } = await this.supabase
      .rpc('get_property_intelligence_summary', { p_property_id: propertyId })
  ...
  async cleanupExpiredCache(): Promise<IntelligenceResponse<{ deleted_count: number }>> {
    const { data: result, error } = await this.supabase
      .rpc('cleanup_expired_intelligence_cache')
  ...
  async performMaintenance(): Promise<IntelligenceResponse<{ summary: string }>> {
    const { data: result, error } = await this.supabase
      .rpc('maintenance_cleanup_maps_data')
