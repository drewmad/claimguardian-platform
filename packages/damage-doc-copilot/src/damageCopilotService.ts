...
export const damageCopilotService = {
  ...
  generateFallbackGuidance(options: ProcessFrameOptions): ProcessFrameResult {
    ...
    const nextAngle = allAngles.find(
      (angle) => !completedAngles.includes(angle),
    );

    if (!nextAngle || current >= total) {
      return {
        nextStep:
          "✅ Documentation appears complete! Review your images and submit when ready.",
        done: true,
        quality: 75,
        completionMessage:
          "All required documentation angles have been captured.",
      };
    }
    ...
    return {
      nextStep:
        guidanceMap[nextAngle as keyof typeof guidanceMap] ||
        "Continue documenting the damage area.",
      done: false,
      quality: 70,
      completedAngle:
        current > 0 ? completedAngles[completedAngles.length - 1] : undefined,
      suggestions: [
