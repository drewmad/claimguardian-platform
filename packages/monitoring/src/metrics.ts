...
class MetricsCollector {
  ...
  recordDatabaseQuery(metric: DatabaseMetric) {
    ...
    if (metric.duration > 1000) {
      console.warn(
        `Slow database query detected: ${metric.queryName} took ${metric.duration}ms`,
      );
    }
  }
  ...
  recordAPICall(metric: APIMetric) {
    ...
    if (metric.duration > 2000) {
      console.warn(
        `Slow API call detected: ${metric.method} ${metric.endpoint} took ${metric.duration}ms`,
      );
    }
  }
  ...
  getMetricsSummary() {
    ...
    const summary = {
      ...
      api: {
        ...
        errorRate:
          this.apiMetrics.length > 0
            ? this.apiMetrics.filter((m) => m.statusCode >= 400).length /
              this.apiMetrics.length
            : 0,
      },
    };
