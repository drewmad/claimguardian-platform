...
describe('Damage Assessment E2E Tests', () => {
  ...
  describe('Damage Assessment Creation', () => {
    test('should create a comprehensive damage assessment', async () => {
      ...
      await element(by.id('damage-description-input'))
        .typeText('Multiple shingles blown off, exposed underlayment, water intrusion in attic')

      await element(by.id('affected-area-input')).typeText('Approximately 200 sq ft')
      ...
      await element(by.id('photo-title-input')).typeText('Roof Damage Overview')
      await element(by.id('photo-notes-input')).typeText('Wide view showing extent of shingle damage')
      ...
      await element(by.id('photo-notes-input')).typeText('Close-up of water staining in attic')
      await element(by.id('save-photo-button')).tap()
      ...
      await detoxExpect(element(by.text('Hurricane Damage Assessment'))).toBeVisible()
  ...
  describe('Photo Capture and Management', () => {
    beforeEach(async () => {
      ...
      await element(by.id('assessment-title-input')).typeText('Photo Test Assessment')
      await element(by.id('property-selector')).tap()
      await element(by.text('Main Residence')).tap()
      await element(by.id('next-step-button')).tap()
    ...
    test('should capture and organize multiple photos', async () => {
      ...
      await element(by.id('capture-button')).tap()
      await element(by.id('photo-title-input')).typeText('Exterior Wall Damage')
      ...
      await element(by.id('capture-photo-button')).tap()
      await element(by.id('capture-button')).tap()
      await element(by.id('photo-title-input')).typeText('Before Repair Reference')
      await element(by.id('photo-category-picker')).tap()
      ...
      await detoxExpect(element(by.text('Before Repair Reference'))).not.toBeVisible()
      ...
      await detoxExpect(element(by.text('Exterior Wall Damage'))).toBeVisible()
      await detoxExpect(element(by.text('Before Repair Reference'))).toBeVisible()
    })
    ...
    test('should handle camera permissions and errors', async () => {
      ...
      await waitFor(element(by.text('Camera access is required to capture photos')))
        .toBeVisible()
        .withTimeout(5000)
  ...
  describe('AI Damage Analysis', () => {
    beforeEach(async () => {
      ...
      await element(by.id('assessment-title-input')).typeText('AI Analysis Test')
      await element(by.id('property-selector')).tap()
      await element(by.text('Main Residence')).tap()
      await element(by.id('next-step-button')).tap()
    ...
    test('should perform AI damage analysis on photos', async () => {
      ...
      await detoxExpect(element(by.text('Analyzing damage using AI...'))).toBeVisible()
    ...
    test('should compare AI analysis with manual assessment', async () => {
      ...
      await detoxExpect(element(by.id('cost-difference-indicator'))).toBeVisible()
      await detoxExpected(element(by.id('severity-difference-indicator'))).toBeVisible()
  ...
  describe('Assessment History and Tracking', () => {
    test('should track assessment status changes', async () => {
      ...
      await element(by.id('assessment-title-input')).typeText('Status Tracking Test')
      await element(by.id('property-selector')).tap()
      await element(by.text('Main Residence')).tap()
      await element(by.id('next-step-button')).tap()
      ...
      await element(by.id('status-change-notes-input')).typeText('Contractor contacted')
  ...
  describe('Offline Assessment Management', () => {
    test('should create assessments while offline', async () => {
      ...
      await element(by.id('assessment-title-input')).typeText('Offline Assessment')
      await element(by.id('property-selector')).tap()
      await element(by.text('Main Residence')).tap()
      await element(by.id('next-step-button')).tap()
      ...
      await element(by.id('damage-description-input')).typeText('Water damage from dishwasher leak')
      await element(by.id('next-step-button')).tap()

      // Capture photo offline
      await element(by.id('capture-photo-button')).tap()
      await element(by.id('capture-button')).tap()
      await element(by.id('photo-title-input')).typeText('Kitchen Water Damage')
      await element(by.id('save-photo-button')).tap()
      ...
      await detoxExpect(element(by.text('Assessment saved offline'))).toBeVisible()
