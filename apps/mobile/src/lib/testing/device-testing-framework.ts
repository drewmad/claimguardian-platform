...
export class MobileDeviceTestFramework {
  ...
  async runTestSuite(config: DeviceTestConfig): Promise<TestReport> {
    ...
    if (!this.deviceInfo) {
      throw new Error(
        "Device test framework not initialized. Call initialize() first.",
      );
    }
  ...
  private async runPerformanceTests(config: DeviceTestConfig): Promise<void> {
    ...
    await this.runTest(
      ...
      "performance",
      async () => {
        const memoryMetrics = await this.performanceMonitor.measureMemoryUsage(
          async () => {
        ...
        if (memoryMetrics.peak > 200 * 1024 * 1024) {
          // 200MB threshold
          throw new Error(
            `High memory usage: ${memoryMetrics.peak / 1024 / 1024}MB`,
          );
        }
      },
      config,
    ...
    await this.runTest(
      ...
      async () => {
        ...
        const avgRenderTime =
          renderMetrics.reduce((a, b) => a + b, 0) / renderMetrics.length;
        if (avgRenderTime > 50) {
          // 50ms threshold
    ...
    await this.runTest(
      ...
      async () => {
        const scrollMetrics =
          await this.performanceMonitor.measureScrollPerformance();

        if (scrollMetrics.droppedFrames > 5) {
          throw new Error(
            `Poor scroll performance: ${scrollMetrics.droppedFrames} dropped frames`,
          );
        }
      },
      config,
  ...
  private async runLocationTests(config: DeviceTestConfig): Promise<void> {
    ...
    if (this.deviceInfo?.permissions.location !== RESULTS.GRANTED) {
      console.warn(
        "⚠️ Location permission not granted, skipping location tests",
      );
      return;
    }
  ...
  private async runTest(
    ...
    try {
      ...
      if (config.screenshotOnFailure) {
        screenshots =
          await this.screenshotManager.captureFailureScreenshot(testName);
      }
      ...
      Geolocation.getCurrentPosition(
        (position) => {
          if (position.coords.accuracy && position.coords.accuracy > 100) {
            reject(
              new Error(`Poor location accuracy: ${position.coords.accuracy}m`),
            );
          } else {
            resolve();
          }
        ...
        (error) =>
          reject(new Error(`Location accuracy test failed: ${error.message}`)),
  ...
  private async testNetworkErrorHandling(): Promise<void> {
    ...
    throw new Error(
      "Network error handling test failed - should have thrown error",
    );
  }
  ...
  private generateTestReport(
    ...
    const memoryUsages = this.testResults.flatMap(
      (r) => r.performanceMetrics?.memoryUsage || [],
    );
    const renderTimes = this.testResults.flatMap(
      (r) => r.performanceMetrics?.renderTimes || [],
    );
...
class PerformanceMonitor {
  ...
  async measureMemoryUsage<T>(
    operation: () => Promise<T>,
  ): Promise<{ peak: number; average: number; samples: number[] }> {
...
export async function runMobileDeviceTests(
  config?: Partial<DeviceTestConfig>,
): Promise<TestReport> {
