...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    event.waitUntil(
      self.registration.showNotification(data.title || 'ClaimGuardian', options)
    )
  } catch (error) {
...
self.addEventListener('message', (event) => {
  ...
  if (event.data && event.data.type === 'CACHE_PROPERTY') {
    ...
    caches.open(RUNTIME_CACHE).then(cache => {
      cache.put(`/offline/property/${event.data.id}`, new Response(JSON.stringify(event.data.property)))
    })
  }
})
