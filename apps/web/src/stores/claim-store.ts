...
interface ClaimState {
  ...
  updateEvidence: (
    claimId: string,
    evidenceId: string,
    updates: Partial<ClaimEvidence>,
  ) => void;
  deleteEvidence: (claimId: string, evidenceId: string) => void;
  bulkAddEvidence: (
    claimId: string,
    evidence: Omit<ClaimEvidence, "id">[],
  ) => void;
  ...
  addStatusHistory: (
    claimId: string,
    history: Omit<ClaimStatusHistory, "id">,
  ) => void;
  ...
  addSettlementOffer: (
    claimId: string,
    offer: Omit<SettlementOffer, "id" | "claimId">,
  ) => void;
...
export const useClaimStore = create<ClaimState>()(
  persist(
    (set, get) => ({
      ...
      getClaimsByPropertyId: (propertyId) =>
        get().claims.filter((c) => c.property_id === propertyId),

      getActiveClaims: () =>
        get().claims.filter(
          (c) => !["closed", "denied"].includes(c.status || ""),
        ),
      ...
      deleteClaim: (id) => {
        set((state) => ({
          ...
          selectedClaimId:
            state.selectedClaimId === id ? null : state.selectedClaimId,
        }));
      },
        ...
        set((state) => ({
          claims: state.claims.map((c) =>
            c.id === claimId
              ? {
                  ...c,
                  evidence: [...c.evidence, evidenceWithId],
      ...
      bulkAddEvidence: (claimId, evidence) => {
        const evidenceWithIds = evidence.map((e) => ({
          ...e,
          id: generateId(),
        }));
        set((state) => ({
          claims: state.claims.map((c) =>
            c.id === claimId
              ? {
                  ...c,
                  evidence: [...c.evidence, ...evidenceWithIds],
                  updated_at: new Date().toISOString(),
                }
              : c,
          ),
          ...
          claims: state.claims.map((c) =>
            c.id === claimId
              ? {
                  ...
                  updated_at: now,
                  reported_date:
                    status === "submitted" && !c.reported_date
                      ? now
                      : c.reported_date,
                }
              : c,
        ...
        set((state) => ({
          claims: state.claims.map((c) =>
            c.id === claimId
              ? {
                  ...c,
                  statusHistory: [...c.statusHistory, historyWithId],
                  updated_at: new Date().toISOString(),
                }
              : c,
          ),
        ...
        set((state) => ({
          claims: state.claims.map((c) =>
            c.id === claimId
              ? {
                  ...c,
                  contacts: [...(c.contacts || []), contactWithId],
                  updated_at: new Date().toISOString(),
                }
              : c,
          ),
        ...
        set((state) => ({
          claims: state.claims.map((c) =>
            c.id === claimId
              ? {
                  ...c,
                  lineItems: [...(c.lineItems || []), itemWithId],
                  updated_at: new Date().toISOString(),
                }
              : c,
          ),
        ...
        set((state) => ({
          claims: state.claims.map((c) =>
            c.id === claimId
              ? {
                  ...c,
                  lineItems: [...(c.lineItems || []), ...itemsWithIds],
                  updated_at: new Date().toISOString(),
                }
              : c,
          ),
      ...
      deletePayment: (claimId, paymentId) => {
        set((state) => ({
          claims: state.claims.map((c) => {
            if (c.id === claimId) {
              const updatedPayments = (c.payments || []).filter(
                (p) => p.id !== paymentId,
              );
              const paidAmount = updatedPayments.reduce(
                (sum, p) => sum + p.amount,
        ...
        set((state) => ({
          claims: state.claims.map((c) =>
            c.id === claimId
              ? {
                  ...c,
                  settlementAmount: offer.offerAmount,
                  updated_at: new Date().toISOString(),
                }
              : c,
          ),
        ...
        set((state) => ({
          claims: state.claims.map((c) =>
            c.id === claimId
              ? {
                  ...c,
                  status: "investigating" as ClaimStatus,
                  updated_at: new Date().toISOString(),
                }
              : c,
          ),
