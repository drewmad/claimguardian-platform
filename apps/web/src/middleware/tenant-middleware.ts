...
export async function extractTenantContext(
  ...
  const organization =
    await tenantManager.getOrganizationByCode(organizationCode);
  if (!organization) {
    return null;
  }

  // Get customizations
  const customizations = await tenantManager.getOrganizationCustomizations(
    organization.id,
  );
...
export function applyCustomizations(
  ...
  response.headers.set(
    ...
    tenantContext.subscriptionStatus,
  );

  // Add feature flags
  response.headers.set(
    "X-Feature-Flags",
    JSON.stringify(tenantContext.featureFlags),
  );
...
export function handleTenantRouting(
  ...
  if (tenantContext) {
    ...
    for (const [route, allowedTiers] of Object.entries(restrictedRoutes)) {
      if (
        pathname.startsWith(route) &&
        !allowedTiers.includes(tenantContext.subscriptionTier)
      ) {
    ...
    if (
      ...
      if (
        stateParam &&
        !tenantContext.allowedStates.includes(stateParam.toUpperCase())
      ) {
...
function getRateLimitsInternal(tenantContext: TenantContext): {
  ...
  return (
    limits[tenantContext.subscriptionTier as keyof typeof limits] ||
    limits.standard
  );
}
...
function generateTenantCSPInternal(tenantContext: TenantContext): string {
  ...
  if (tenantDomains.length > 0) {
    baseCSP.push(
      `connect-src 'self' https: ${tenantDomains.map((d) => `https://${d}`).join(" ")}`,
    );
  }
...
export async function tenantMiddleware(
  ...
  if (tenantContext) {
    ...
    response.headers.set(
      ...
      generateTenantCSPInternal(tenantContext),
    );
    ...
    response.headers.set(
      "X-RateLimit-Uploads",
      rateLimits.fileUploads.toString(),
    );
  }
