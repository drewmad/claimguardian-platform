...
export class TrackedAIClient {
  ...
  async analyzeImage(
    ...
    try {
      ...
      if (request.provider === "openai") {
        ...
        throw new Error(
          `Unsupported provider for image analysis: ${request.provider}`,
        );
      }
  ...
  private async analyzeGeminiImage(
    ...
    // Estimate tokens
    const inputTokens =
      TokenEstimator.estimateGeminiTokens(prompt) + request.images.length * 85; // Rough image token estimate
    const outputTokens = TokenEstimator.estimateGeminiTokens(text);
