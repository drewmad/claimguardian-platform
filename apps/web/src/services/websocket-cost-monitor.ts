...
class WebSocketCostMonitor {
  ...
  private subscribers: Map<string, (update: RealTimeCostUpdate) => void> = new Map()
  ...
  async initialize(): Promise<boolean> {
    try {
      if (typeof window === 'undefined') {
        console.warn('WebSocket cost monitor can only run in browser environment')
        return false
      }
  ...
  private async checkBudgetThresholds(
    ...
    try {
      ...
      const { data: todaySpending } = await this.supabase.rpc('get_user_daily_cost', {
  ...
  private async createBudgetAlert(userId: string, alertData: {
    ...
    try {
      ...
      const { error } = await this.supabase
        ...
        .insert({
          user_id: userId,
          alert_type: alertData.percentageUsed >= 100 ? 'limit_reached' : 'threshold_reached',
  ...
  private async checkUsageMetrics(): Promise<void> {
    try {
      ...
      const metrics: UsageMetrics = {
        ...
        cost_per_minute: recentLogs.reduce((sum, log) => sum + (log.cost_total || 0), 0),
        error_rate: recentLogs.filter(log => !log.success).length / recentLogs.length,
        avg_response_time: recentLogs.reduce((sum, log) => sum + (log.processing_time_ms || 0), 0) / recentLogs.length
      }
      ...
      if (metrics.requests_per_minute > 100) {
        const update: RealTimeCostUpdate = {
          ...
          data: {
            ...
            severity: metrics.requests_per_minute > 500 ? 'critical' : 'warning',
            metadata: metrics
          }
        }
        this.broadcastUpdate(update)
  ...
  subscribe(subscriberId: string, callback: (update: RealTimeCostUpdate) => void): boolean {
