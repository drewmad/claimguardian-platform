...
class CostTrackingService {
  ...
  async trackUsage(
    ...
    try {
      ...
      const costAudio =
        (usage.inputAudioSeconds * (tool.cost_per_minute || 0)) / 60;
  ...
  async getUserUsageCurrentMonth(userId?: string): Promise<any> {
    try {
      ...
      const { data, error } = await supabase.rpc(
        "get_user_ai_usage_current_month",
        {
  ...
  async getAITools(): Promise<AITool[]> {
    try {
      ...
      return data.map((tool) => ({
        ...
        costPerImage: tool.cost_per_image
          ? parseFloat(tool.cost_per_image)
          : undefined,
        costPerMinute: tool.cost_per_minute
          ? parseFloat(tool.cost_per_minute)
          : undefined,
  ...
  async canUserMakeRequest(toolName: string): Promise<{
    ...
    try {
      ...
      // Check daily limit
      if (
        subscription.current_day_requests >= subscription.daily_request_limit
      ) {
        return {
          allowed: false,
      ...
      // Check monthly limit
      if (
        subscription.current_month_requests >=
  ...
  private async sendBudgetAlerts(
    ...
    try {
      ...
      for (const alertLevel of budgetStatus.alerts_needed) {
        const alertType =
          alertLevel === "critical" ? "limit_reached" : "threshold";
  ...
  async updateUserRequestCount(userId?: string): Promise<void> {
    try {
      ...
      await supabase.rpc("increment_user_request_count", {
...
export class TokenEstimator {
  ...
  static estimateOutputTokens(
    responseType: "short" | "medium" | "long" | "analysis",
  ): number {
    switch (responseType) {
      case "short":
        return 100;
