...
export interface AlertPayload {
  id: string
  type: 'budget_warning' | 'budget_critical' | 'usage_spike' | 'model_error' | 'system_alert'
...
class AlertDeliverySystem {
  ...
  constructor(config?: Partial<AlertDeliveryConfig>) {
    this.config = {
      email: {
        ...
        templates: {
          budget_warning: 'Budget Warning: You have used {percentage}% of your {period} budget ({current}/{budget})',
          budget_critical: 'CRITICAL: Budget limit reached! You have used {percentage}% of your {period} budget ({current}/{budget})',
          usage_spike: 'Usage Spike Alert: {requests} requests in the last minute (cost: {cost})',
          model_error: 'Model Error Alert: {model} has {error_rate}% error rate ({failures}/{total} requests)'
        }
      },
      webhook: {
        enabled: false,
  ...
  private getActiveChannels(alert: AlertPayload): ('email' | 'webhook' | 'push' | 'slack')[] {
  ...
  private async deliverWebhook(alert: AlertPayload): Promise<DeliveryResult> {
    try {
      ...
      const payload = {
        ...
        metadata: this.config.webhook.includeMetadata ? alert.metadata : undefined
      }
        ...
        attachments: [
          {
            ...
            fields: [
              ...
              {
                title: 'Type',
                value: alert.type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
                short: true
              },
              {
                title: 'Time',
  ...
  private async deliverPushNotification(alert: AlertPayload): Promise<DeliveryResult> {
    try {
  ...
  private async logDeliveryResults(alert: AlertPayload, results: DeliveryResult[]): Promise<void> {
    try {
  ...
  async testWebhook(): Promise<DeliveryResult> {
    const testAlert: AlertPayload = {
      ...
      message: 'This is a test webhook from ClaimGuardian Alert Delivery System',
      timestamp: new Date().toISOString(),
      channels: ['webhook']
    }
  ...
  async testSlack(): Promise<DeliveryResult> {
    const testAlert: AlertPayload = {
      ...
      message: 'This is a test message from ClaimGuardian Alert Delivery System',
      timestamp: new Date().toISOString(),
      channels: ['slack']
    }
