...
export class FloridaComplianceManager extends SupabaseService {
  constructor() {
    super();
  }
  ...
  async checkInsuranceCodeCompliance(claimId: string): Promise<{
    ...
    try {
      ...
      const promptPaymentCheck =
        await this.checkPromptPaymentActCompliance(claimId);
      ...
      if (
        badFaithCheck.riskLevel === "high" ||
        badFaithCheck.riskLevel === "critical"
      ) {
            ...
            riskLevel === "low"
              ...
              : riskLevel === "medium"
                ...
                : riskLevel === "high"
                  ? 3
                  : 4,
            badFaithCheck.riskLevel === "high" ? 3 : 4,
          ) === 1
            ? "low"
            : Math.max(
                  riskLevel === "low"
      ...
      if (violations.length === 0) {
        ...
        recommendations.push(
          "Enhance documentation and communication processes",
        );
  ...
  async checkPromptPaymentActCompliance(claimId: string): Promise<{
    ...
    try {
      ...
      const { data: complianceData } = await supabase
        .from("florida_insurance_compliance")
        .select("*")
        .eq("claim_id", claimId)
        .limit(1);
      ...
      const compliance: PromptPaymentActCompliance = {
        ...
        acknowledgmentCompleted: complianceData?.[0]
          ?.initial_contact_completed_at
          ? new Date(complianceData[0].initial_contact_completed_at)
          : undefined,
        investigationDeadline,
        investigationCompleted: complianceData?.[0]?.investigation_completed_at
          ? new Date(complianceData[0].investigation_completed_at)
          : undefined,
        decisionDeadline,
        decisionRendered: complianceData?.[0]?.decision_rendered_at
          ? new Date(complianceData[0].decision_rendered_at)
          : undefined,
        paymentDeadline,
        paymentCompleted: complianceData?.[0]?.payment_completed_at
          ? new Date(complianceData[0].payment_completed_at)
          : undefined,
        daysRemaining: Math.ceil(
          (decisionDeadline.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
        ),
        violationRisk: "low",
      ...
      if (
        compliance.decisionRendered &&
        !compliance.paymentCompleted &&
        now > paymentDeadline
      ) {
  ...
  async assessBadFaithRisk(claimId: string): Promise<{
    ...
    try {
      ...
      if (
        daysSinceReported > 30 &&
        !complianceData?.[0]?.investigation_completed_at
      ) {
      ...
      const communicationEvents =
        complianceData?.[0]?.metadata?.communication_log || [];
      ...
      if (missingDocs.length > 0) {
        riskFactors.push(
          `Missing required documentation: ${missingDocs.join(", ")}`,
        );
      ...
      const protection: BadFaithProtection = {
        claimId,
        protectionMeasures: {
          timelyInvestigation: !riskFactors.some((factor) =>
            factor.includes("Investigation delay"),
          ),
          adequateCommunication: communicationEvents.length > 0,
          fairSettlement:
            claim.status !== "denied" || claim.metadata?.denial_justification,
   ...
   */
  async checkPublicRecordsCompliance(
    entityId: string,
    entityType: string,
  ): Promise<{
    compliant: boolean;
    ...
    try {
      ...
      switch (entityType) {
        case "claim":
          ...
          disclosureObligations.push(
            "Provide claim file to claimant upon written request",
          );
          retentionRequirements.push(
            "Retain all claim documentation for 7 years",
          );
          break;
        ...
        case "user":
          requirements.push({
            ...
            description:
              "Privacy notice must be provided for personal data collection",
          ...
          disclosureObligations.push(
            "Provide privacy notice at data collection",
          );
          break;
      }
   ...
   */
  async validateAOBCompliance(
    aobId: string,
  ): Promise<AssignmentOfBenefitsCompliance> {
    try {
      const supabase = await this.getSupabaseClient();
      const { data: aob, error } = await supabase
      ...
      const documentationComplete =
        Object.values(requiredDisclosures).every(Boolean) &&
        Boolean(aob.contractor_license) &&
        Boolean(aob.assignment_date);

      let complianceStatus: "compliant" | "incomplete" | "non_compliant" =
        "compliant";
   ...
   */
  async validatePublicAdjusterCompliance(
    licenseNumber: string,
  ): Promise<PublicAdjusterCompliance> {
    try {
      ...
      if (error) {
        logger.warn("Public adjuster not found in database", {
          licenseNumber,
          error,
        });
      }

      const licenseExpiry = adjuster?.license_expiry
        ? new Date(adjuster.license_expiry)
        : new Date();
      const now = new Date();
      const isExpired = licenseExpiry < now;

      let complianceStatus: "compliant" | "expired" | "non_compliant" =
        "compliant";
      ...
      if (!licenseNumber || licenseNumber.length !== 7) {
        // Florida PA licenses are 7 characters
        complianceStatus = "non_compliant";
      }
      ...
      const contractTerms = {
        ...
        cancellationRights: Boolean(
          adjuster?.contract_terms?.cancellation_rights,
        ),
   ...
   */
  async checkHurricaneClaimReporting(
    claimId: string,
  ): Promise<HurricaneClaimReporting> {
    try {
      const supabase = await this.getSupabaseClient();
      const { data: claim, error } = await supabase
        .from("claims")
      ...
      const isHurricaneClaim = complianceData?.[0]?.hurricane_claim || false;
      const hurricaneEvent = complianceData?.[0]?.catastrophic_event_id || "";
      const oirReportingRequired =
        isHurricaneClaim &&
        claim.estimated_amount &&
        claim.estimated_amount > 25000;

      let reportingDeadline: Date | undefined;
      if (oirReportingRequired && complianceData?.[0]?.created_at) {
        reportingDeadline = new Date(complianceData[0].created_at);
      ...
      if (isHurricaneClaim) {
        specialRequirements.push(
          "Enhanced documentation requirements for hurricane damage",
        );
        specialRequirements.push(
          "Expedited processing required (72 hours for emergency services)",
        );
        specialRequirements.push(
          "Additional engineer inspection may be required",
        );
        if (oirReportingRequired) {
          specialRequirements.push(
            "Office of Insurance Regulation reporting required",
          );
        }
      }
      ...
      return {
        claimId,
        hurricaneEvent,
        femaDisasterNumber: complianceData?.[0]?.metadata?.fema_disaster_number,
        oirReportingRequired,
        reportingDeadline,
        reportSubmitted: Boolean(complianceData?.[0]?.oir_report_submitted_at),
        reportSubmissionDate: complianceData?.[0]?.oir_report_submitted_at
          ? new Date(complianceData[0].oir_report_submitted_at)
          : undefined,
        specialRequirements,
      };
    } catch (error) {
  ...
  async runComprehensiveComplianceCheck(claimId: string): Promise<{
    ...
    try {
      ...
      const hasViolations =
        !checks.insuranceCode.compliant || !checks.promptPayment.compliant;
      const hasHighRisk =
        checks.insuranceCode.riskLevel === "high" ||
        checks.insuranceCode.riskLevel === "critical" ||
        checks.badFaith.riskLevel === "high" ||
        checks.badFaith.riskLevel === "critical";

      let overallCompliance: "compliant" | "at_risk" | "violation" =
        "compliant";
      if (hasViolations) {
      ...
      if (checks.promptPayment.timeline.violationRisk === "critical") {
        urgentActions.push(
          "Immediate action required: Prompt Payment Act deadline exceeded",
        );
      }
      if (checks.badFaith.riskLevel === "critical") {
        urgentActions.push(
          "Immediate action required: Critical bad faith risk detected",
        );
      }
      if (
        checks.hurricane?.oirReportingRequired &&
        checks.hurricane.reportingDeadline
      ) {
        const daysUntilDeadline = Math.ceil(
          (checks.hurricane.reportingDeadline.getTime() - Date.now()) /
            (1000 * 60 * 60 * 24),
        );
        if (daysUntilDeadline <= 7) {
          urgentActions.push(
            `Urgent: OIR reporting deadline in ${daysUntilDeadline} days`,
          );
        }
      }
...
export class ComplianceChecker {
  ...
  async getComplianceStatus(claimId: string): Promise<{
    ...
    try {
      const comprehensive =
        await this.floridaManager.runComprehensiveComplianceCheck(claimId);
      ...
      if (comprehensive.urgentActions.length > 0) {
        ...
        comprehensive.riskLevel === "critical"
      ) {
        priority = "high";
