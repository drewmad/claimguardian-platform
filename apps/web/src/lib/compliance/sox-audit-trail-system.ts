...
export interface SOXAuditEvent {
  id: string
  eventType: 'financial_transaction' | 'data_access' | 'system_change' | 'user_action' | 'process_control'
...
export class SOXAuditTrailManager {
   ...
   */
  async logAuditEvent(event: Omit<SOXAuditEvent, 'id' | 'eventHash' | 'chainHash' | 'timestamp'>): Promise<string> {
    try {
   ...
   */
  async logFinancialTransaction(transaction: Omit<FinancialTransaction, 'id' | 'auditTrail'>): Promise<string> {
    try {
      ...
      await this.logAuditEvent({
        ...
        controlObjective: 'Accurate and complete recording of financial transactions',
        riskLevel: transaction.amount > 10000 ? 'high' : transaction.amount > 1000 ? 'medium' : 'low',
      ...
      logger.error('Financial transaction logging failed', { transaction, error })
      throw error
    }
  }
  ...
  async updateTransactionStatus(
    ...
    try {
      ...
      await this.logAuditEvent({
        ...
        userId: updatedBy,
        financialImpact: true,
        controlObjective: 'Proper authorization and approval of transaction changes',
        riskLevel: newStatus === 'failed' || newStatus === 'reversed' ? 'high' : 'medium',
      ...
      logger.error('Transaction status update logging failed', { transactionId, newStatus, updatedBy, error })
      throw error
    }
  }
  ...
  private async generateChainHash(currentEventHash: string): Promise<string> {
    try {
      ...
      const { data: lastAudit, error } = await this.supabase
        .from('compliance.audit_logs')
        .select('chain_hash')
   ...
   */
  async verifyAuditTrailIntegrity(fromDate?: Date, toDate?: Date): Promise<{
    valid: boolean
    totalEvents: number
    ...
    try {
      ...
      if (error) {
        logger.error('Failed to retrieve audit events for integrity verification', { error })
        throw error
      }
      ...
      logger.error('Audit trail integrity verification failed', { fromDate, toDate, error })
      throw error
    }
  }
   ...
   */
  private mapRiskLevelToSeverity(riskLevel: 'low' | 'medium' | 'high' | 'critical'): 'info' | 'warning' | 'error' | 'critical' | 'security_alert' {
    switch (riskLevel) {
...
export class FinancialControlsMonitor {
  private supabase: ReturnType<typeof createClient>
  ...
  async monitorControlEffectiveness(): Promise<{
    ...
    try {
      ...
      const controls: InternalControl[] = [
        {
          ...
          monitoring: true,
          controlObjective: 'Ensure all financial transactions are properly authorized',
          risksMitigated: ['Unauthorized payments', 'Fraud', 'Misappropriation'],
          controlProcedures: ['Manager approval for payments > $1000', 'Dual authorization for payments > $10000'],
          keyControl: true,
        ...
        },
        {
          ...
          monitoring: true,
          controlObjective: 'Prevent single person from controlling entire claims process',
          risksMitigated: ['Fraud', 'Errors', 'Conflicts of interest'],
          controlProcedures: ['Separate claim review and payment approval', 'Independent verification'],
          keyControl: true,
          ...
          deficiencies: [
            {
              ...
              description: 'Same individual approving claims and payments in 15% of cases',
              identifiedDate: new Date('2025-06-01'),
              remediationPlan: 'Implement system controls to prevent same user approvals',
      ...
      let overallAssessment: 'effective' | 'needs_improvement' | 'material_weakness' = 'effective'
      if (materialWeaknesses > 0) {
        overallAssessment = 'material_weakness'
      } else if (deficientControls > 0 || controlsNeedingAttention.length > totalControls * 0.2) {
  ...
  async testControl(controlId: string, testResults: {
    ...
    try {
      ...
      logger.error('Control testing logging failed', { controlId, testResults, error })
      throw error
    }
  }
}
...
export class TamperProofLogger {
  ...
  async logCriticalEvent(event: {
    ...
    try {
      return await this.auditManager.logAuditEvent({
        ...
        userId: event.userId,
        financialImpact: event.riskLevel === 'high' || event.riskLevel === 'critical',
        controlObjective: 'Maintain complete and accurate audit trail of critical system events',
  ...
  async verifyEventIntegrity(eventId: string): Promise<{
    ...
    try {
      const { data: event, error } = await this.auditManager.supabase
        .from('compliance.audit_logs')
        .select('*')
        .eq('id', eventId)
        .single()
      ...
      return {
        ...
        hashValid,
        chainValid,
        details: hashValid && chainValid ? 'Event integrity verified' : 'Integrity violation detected'
      }
