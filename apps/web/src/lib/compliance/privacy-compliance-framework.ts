...
export class PrivacyComplianceManager extends SupabaseService {
    ...
    try {
      ...
      if (activeConsents.length === 0) {
        ...
        recommendations.push(
          "Obtain valid consent for all data processing activities",
        );
      }
      ...
      const supabase2 = await this.getSupabaseClient();
      const { data: privacyRequests } = await supabase2
      ...
      if (violations.length === 0) {
        ...
        recommendations.push(
          "Regular review of consent status and data processing activities",
        );
      }
  ...
  async checkCCPACompliance(userId: string): Promise<{
    ...
    try {
      ...
      const supabase3 = await this.getSupabaseClient();
      const { data: userProfile, error } = await supabase3
        .from("user_profiles")
      ...
      const applicable =
        userProfile?.signup_region === "CA" ||
        userProfile?.signup_country === "US";
      ...
      const supabase4 = await this.getSupabaseClient();
      const { data: privacyRequests } = await supabase4
        .from("compliance.privacy_requests")
        .select("*")
        .eq("user_id", userId)
        .order("received_at", { ascending: false });
      ...
      const consumerRights = [
        {
          ...
          available: true,
          exercised: Boolean(
            privacyRequests?.some((r) => r.request_type === "data_access"),
          ),
          lastExercised: privacyRequests?.find(
            (r) => r.request_type === "data_access",
          )?.completed_at
        ...
        },
        {
          ...
          available: true,
          exercised: Boolean(
            privacyRequests?.some((r) => r.request_type === "data_erasure"),
          ),
          lastExercised: privacyRequests?.find(
            (r) => r.request_type === "data_erasure",
          )?.completed_at
        ...
        },
        {
          ...
          available: true,
          exercised: Boolean(
            privacyRequests?.some((r) => r.request_type === "opt_out_sale"),
          ),
          lastExercised: privacyRequests?.find(
            (r) => r.request_type === "opt_out_sale",
          )?.completed_at
            ? new Date(
                privacyRequests.find(
                  (r) => r.request_type === "opt_out_sale",
      ...
      const disclosureRequirements = [
        ...
        },
        {
          ...
          disclosed: true,
          details:
            "Directly from consumer, third-party services, public records",
        },
        {
      ...
      const missingDisclosures = disclosureRequirements.filter(
        (req) => !req.disclosed,
      );
      if (missingDisclosures.length > 0) {
        violations.push(
          `Missing required disclosures: ${missingDisclosures.map((d) => d.category).join(", ")}`,
        );
      }
  ...
  async processPrivacyRequest(
    request: Omit<PrivacyRequest, "id" | "receivedAt" | "responseBy">,
  ): Promise<{
    requestId: string;
    ...
    try {
      ...
      if (
        request.complianceFramework === "gdpr" ||
        request.complianceFramework === "both"
      ) {
      ...
      const supabase5 = await this.getSupabaseClient();
      const { error: insertError } = await supabase5
        .from("compliance.privacy_requests")
        .insert({
          id: requestId,
          ...
          gdpr_applicable:
            ...
            request.complianceFramework === "both",
          ccpa_applicable:
            request.complianceFramework === "ccpa" ||
            request.complianceFramework === "both",
        });
  ...
  private async validatePrivacyRequest(
    ...
    try {
      ...
      const supabase6 = await this.getSupabaseClient();
      const { data: user, error: userError } = await supabase6
        .from("user_profiles")
        .select("user_id")
      ...
      const supabase7 = await this.getSupabaseClient();
      const { data: recentRequests, error: requestError } = await supabase7
        .from("compliance.privacy_requests")
        .select("id")
        .eq("user_id", request.userId)
      ...
      if (recentRequests && recentRequests.length > 0) {
        return {
          valid: false,
          reason: "Similar request made within the last 30 days",
        };
      }
      ...
      if (request.type === "erasure") {
        ...
        if (hasLegalHold) {
          return {
            valid: false,
            reason: "Cannot delete data due to legal retention requirements",
          };
        }
      }
  ...
  private async startPrivacyRequestWorkflow(requestId: string): Promise<void> {
    try {
      ...
      const supabase8 = await this.getSupabaseClient();
      await supabase8
        .from("compliance.privacy_requests")
        .update({
  ...
  private async checkLegalRetentionRequirements(
    ...
    try {
      const supabase9 = await this.getSupabaseClient();
      const { data: retentionRecords, error } = await supabase9
...
export class DataDeletionService extends SupabaseService {
      ...
      const dataMap: DataMapping[] = [
        {
          ...
          personalDataFields: [
            ...
            },
            {
              ...
              category: "contact",
              sensitivity: "internal",
              processingPurpose: ["account_management", "communication"],
              retention: "3 years",
            },
            {
              field: "phone",
          ...
          personalDataFields: [
            {
              ...
              processingPurpose: ["claims_processing"],
              retention: "7 years",
            },
          ],
          relationships: [
      ...
      for (const mapping of dataMap) {
        try {
          ...
          if (mapping.table === "user_profiles") {
            ...
            deletedTables.push(
              `${mapping.schema}.${mapping.table} (anonymized)`,
            );
          } else if (mapping.table === "claims") {
            // Check if claims need to be preserved
            const activeClaimsExist =
              await this.checkActiveClaimsForUser(userId);
          ...
          errors.push(
            `Failed to process ${mapping.schema}.${mapping.table}: ${error}`,
          );
        }
      }
  ...
  private async logDataDeletion(
    ...
    try {
      const supabase = await this.getSupabaseClient();
      await supabase.from("compliance.audit_logs").insert({
