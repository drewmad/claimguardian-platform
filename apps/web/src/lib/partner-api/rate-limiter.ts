...
class PartnerRateLimiter {
  ...
  private async checkWindowLimit(
    ...
    if (window.count >= limit) {
      ...
      const resetTime = Math.floor(
        (oldestRequest + windowSeconds * 1000) / 1000,
      );
  ...
  async getUsageStats(
    ...
    try {
      const baseKey = apiKeyId
        ? `rl:${partnerId}:${apiKeyId}`
        : `rl:${partnerId}`;
      ...
      for (const window of windows) {
        ...
        if (cached) {
          ...
          const validRequests = cached.requests.filter(
            (timestamp) => timestamp > windowStart,
          );

          stats[window.type as keyof typeof stats].current =
