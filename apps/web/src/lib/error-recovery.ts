    ...
    while (this.retryCount <= this.config.maxRetries) {
      try {
        ...
        if (
          ...
          !this.config.retryCondition(error as Error)
        ) {
          throw error;
        }
        ...
        const delay = Math.min(
          this.config.initialDelay *
            Math.pow(this.config.backoffFactor, this.retryCount - 1),
      ...
      try {
        ...
        const result = await retryManager.executeWithRetry(
          ...
          (attempt, error) => {
            ...
            if (showToasts) {
              toast.info(
                `Retrying... (${attempt}/${retryManager.config.maxRetries})`,
              );
            }
        ...
        if (showToasts) {
          toast.error(
            `Operation failed after ${retryManager.config.maxRetries} attempts`,
          );
        }
