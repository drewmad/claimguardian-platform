...
class LegalService {
  ...
  async getDocumentsNeedingAcceptance(
    ...
    try {
      ...
      const { data: consentStatus, error: statusError } =
        await this.supabase.rpc("get_user_consent_status", {
          p_user_id: userId,
        });
  ...
  async recordMultipleConsents({
    ...
    try {
      ...
      logger.error(
        "Failed to record multiple consents",
        { userId },
        error as Error,
      );
      throw error;
  ...
  async hasAcceptedAllRequiredDocuments(userId: string): Promise<boolean> {
    try {
      ...
      return consentStatus.every(
        (status) => status.is_accepted && !status.needs_update,
      );
    } catch (error) {
      logger.error(
        "Failed to check document acceptance",
        { userId },
        error as Error,
      );
      return false;
  ...
  async getDocumentByType(
    ...
    try {
      ...
      logger.error(
        "Failed to fetch document by type",
        { type },
        error as Error,
      );
      throw error;
  ...
  async validateDocumentHash(
    ...
    try {
      ...
      logger.error(
        "Failed to validate document hash",
        { documentId },
        error as Error,
      );
      return false;
  ...
  async getLegalDocumentBySlug(slug: string): Promise<LegalDocument | null> {
    try {
      ...
      logger.error(
        "Failed to fetch document by slug",
        { slug },
        error as Error,
      );
      throw error;
