...
class LegalServiceServer {
  ...
  async getActiveLegalDocuments(): Promise<LegalDocument[]> {
    try {
      ...
      logger.error(
        "Error fetching active legal documents",
        {},
   ...
   */
  async getDocumentsNeedingAcceptance(
    userId: string,
    ...
    try {
      ...
      if (error) {
        logger.error(
          "Failed to fetch documents needing acceptance",
          { userId },
          error,
        );
        throw error;
      ...
      logger.error(
        "Error fetching documents needing acceptance",
        { userId },
        error instanceof Error ? error : new Error(String(error)),
      );
      throw error;
  ...
  async recordAcceptances(
    ...
    try {
      ...
      logger.error(
        "Error recording legal acceptances",
        { userId },
        error instanceof Error ? error : new Error(String(error)),
      );
      throw error;
  ...
  async getUserAcceptanceHistory(userId: string): Promise<UserConsent[]> {
    try {
      ...
      if (error) {
        logger.error(
          "Failed to fetch user acceptance history",
          {},
          error instanceof Error ? error : new Error(String(error)),
        );
        throw error;
      ...
      logger.error(
        "Error fetching user acceptance history",
        { userId },
        error instanceof Error ? error : new Error(String(error)),
      );
      throw error;
  ...
  async hasUserAcceptedAllRequired(userId: string): Promise<boolean> {
    try {
      ...
      logger.error(
        "Error checking user acceptance status",
        { userId },
        error instanceof Error ? error : new Error(String(error)),
      );
      return false;
  ...
  async getLegalDocumentBySlug(slug: string): Promise<LegalDocument | null> {
    try {
      ...
      if (error) {
        ...
        logger.error(
          "Failed to fetch legal document by slug",
          {},
          error instanceof Error ? error : new Error(String(error)),
        );
        throw error;
      ...
      logger.error(
        "Error fetching legal document by slug",
        { slug },
        error instanceof Error ? error : new Error(String(error)),
      );
      throw error;
  ...
  async validateDocumentHash(
    ...
    try {
      ...
      if (error || !data) {
        logger.error(
          "Failed to fetch document for hash validation",
          {},
          error instanceof Error ? error : new Error(String(error)),
        );
        return false;
      ...
      logger.error(
        "Error validating document hash",
        { documentId },
        error instanceof Error ? error : new Error(String(error)),
      );
      return false;
