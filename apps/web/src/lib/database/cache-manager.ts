...
export type CacheStrategy = 'write-through' | 'write-back' | 'write-around' | 'cache-aside'
...
export class CacheManager {
  ...
  constructor(config: Partial<CacheConfig> = {}) {
    ...
    this.metrics = {
      ...
      memoryUsage: { used: 0, available: this.config.maxMemorySize, utilization: 0 },
      levelStats: {
    ...
    try {
      ...
      if (this.config.strategy === 'write-through') {
        ...
        if (memoryLayer) {
          ...
          this.scheduleBackgroundWrite(key, value, ttl, metadata, targetLevels.filter(l => l !== 'memory'))
        }
      } else {
   ...
   */
  async exists(key: string, options: {
    levels?: CacheLevel[]
  } = {}): Promise<boolean> {
    if (!this.isInitialized) {
      await this.initialize()
  ...
  async optimize(): Promise<{
    ...
    try {
      ...
      logger.info('Cache optimization completed', {
        ...
        memoryReduced: beforeMetrics.memoryUsage.used - afterMetrics.memoryUsage.used
      })
  ...
  private getWriteLevels(levels: CacheLevel[], skipLowerLevels: boolean): CacheLevel[] {
    if (skipLowerLevels) {
  ...
  private async promoteToHigherLevels(key: string, entry: CacheEntry, currentLevel: CacheLevel): Promise<void> {
    ...
    for (let i = 0; i < currentIndex; i++) {
      ...
      if (layer) {
        try {
          ...
          logger.warn('Failed to promote cache entry to higher level', { key, level: higherLevel }, error as Error)
        }
      }
    }
  }

  private scheduleBackgroundWrite(key: string, value: any, ttl: number, metadata: any, levels: CacheLevel[]): void {
  ...
  private updateMetrics(): void {
    ...
    this.metrics.memoryUsage.utilization = (totalUsed / this.config.maxMemorySize) * 100
  }
  ...
  private async cleanupExpired(): Promise<void> {
    for (const [level, layer] of this.layers) {
      try {
        ...
        logger.warn('Cleanup failed for cache level', { level }, error as Error)
      }
    }
  }
...
class MemoryCacheLayer implements CacheLayer {
  ...
  async set<T>(key: string, value: T, ttl = this.config.defaultTTL, metadata: any = {}): Promise<void> {
