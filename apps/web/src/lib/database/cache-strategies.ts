...
export class FrequentlyAccessedStrategy extends BaseCacheStrategy {
  ...
  async get<T>(key: CacheKey, options?: CacheOptions): Promise<T | null> {
    ...
    try {
      ...
      if (result) {
        logger.debug("Cache hit for frequently accessed data", {
          key: cacheKey,
        });
      }
      ...
      logger.warn("Cache get failed for frequently accessed data", {
        key: cacheKey,
  ...
  async set<T>(key: CacheKey, value: T, options?: CacheOptions): Promise<void> {
    ...
    try {
      ...
      logger.error(
        "Cache set failed for frequently accessed data",
        { key: cacheKey },
        error as Error,
      );
    }
  ...
  async warmup(keys: CacheKey[] = []): Promise<void> {
    ...
    logger.info("Warming up frequently accessed data cache", {
...
export class RealTimeStrategy extends BaseCacheStrategy {
  ...
  async get<T>(key: CacheKey, options?: CacheOptions): Promise<T | null> {
    ...
    try {
      ...
      logger.warn("Real-time cache get failed", {
        key: cacheKey,
        error: (error as Error).message,
      });
      return null;
  ...
  async invalidate(key: CacheKey): Promise<void> {
    ...
    try {
      ...
      logger.error(
        "Real-time cache invalidation failed",
        { key: cacheKey },
        error as Error,
      );
    }
...
export class ReferenceDataStrategy extends BaseCacheStrategy {
  ...
  async get<T>(key: CacheKey, options?: CacheOptions): Promise<T | null> {
    ...
    try {
      ...
      logger.warn("Reference data cache get failed", {
        key: cacheKey,
        error: (error as Error).message,
      });
      return null;
  ...
  async set<T>(key: CacheKey, value: T, options?: CacheOptions): Promise<void> {
    ...
    try {
      ...
      logger.error(
        "Reference data cache set failed",
        { key: cacheKey },
        error as Error,
      );
    }
  ...
  async invalidate(
    ...
    try {
      ...
      logger.error(
        "Reference data invalidation failed",
        { key: cacheKey },
        error as Error,
      );
    }
  ...
  async warmup(keys: CacheKey[] = []): Promise<void> {
    ...
    for (const key of keys) {
      try {
        ...
        logger.warn("Reference data warmup failed for key", {
          key,
          error: (error as Error).message,
        });
      }
...
export class ComputationalStrategy extends BaseCacheStrategy {
  ...
  async get<T>(key: CacheKey, options?: CacheOptions): Promise<T | null> {
    ...
    try {
      ...
      if (result) {
        logger.debug("Computational cache hit - saved expensive operation", {
          key: cacheKey,
        });
      }
      ...
      logger.warn("Computational cache get failed", {
        key: cacheKey,
        error: (error as Error).message,
      });
      return null;
  ...
  async set<T>(key: CacheKey, value: T, options?: CacheOptions): Promise<void> {
    ...
    try {
      ...
      logger.info("Cached expensive computation result", {
        key: cacheKey,
        ttl,
      });
    } catch (error) {
      logger.error(
        "Computational cache set failed",
        { key: cacheKey },
        error as Error,
      );
    }
  ...
  async invalidate(key: CacheKey): Promise<void> {
    ...
    try {
      ...
      logger.error(
        "Computational cache invalidation failed",
        { key: cacheKey },
        error as Error,
      );
    }
...
export const cacheStrategies = {
  frequentlyAccessed: () =>
    CacheStrategyFactory.getStrategy("frequently-accessed"),
