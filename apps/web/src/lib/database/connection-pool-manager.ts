...
export class ConnectionPoolManager {
  ...
  async execute<T = any>(
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      ...
      try {
        ...
        logger.warn(`Query execution failed (attempt ${attempt + 1}/${retries + 1})`, {
  ...
  async shutdown(timeoutMs = 30000): Promise<void> {
    ...
    while (this.getActiveConnectionCount() > 0 && (Date.now() - shutdownStart) < timeoutMs) {
  ...
  private async createConnection(): Promise<PooledConnection> {
    ...
    try {
      ...
      logger.error('Failed to create connection', { connectionId }, error as Error)
      throw error
    }
  }

  private async destroyConnection(connectionId: string, force = false): Promise<void> {
