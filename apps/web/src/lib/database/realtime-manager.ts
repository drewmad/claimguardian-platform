...
export class RealtimeManager {
  ...
  async subscribe<T = any>(
    ...
    const channelName =
      customChannel || `table:${table}:${this.generateChannelId(filter)}`;
  ...
  async subscribeToPresence(
    ...
    try {
      ...
      realtimeChannel.subscribe(async (status) => {
        if (status === "SUBSCRIBED") {
          logger.info("Presence subscription established", {
            channel,
            subscriptionId,
          });
        }
  ...
  async subscribeToBroadcast(
    ...
    try {
      ...
      realtimeChannel.subscribe((status) => {
        if (status === "SUBSCRIBED") {
          logger.info("Broadcast subscription established", {
            channel,
            eventType,
            subscriptionId,
          });
  ...
  private async updateCache(event: RealtimeEvent): Promise<void> {
    try {
      ...
      logger.debug("Cache updated from realtime event", {
        event: event.type,
        table: event.table,
      });
    } catch (error) {
  ...
  private scheduleReconnect(subscriptionId: string): void {
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        logger.error(
          "Subscription reconnection failed",
          { subscriptionId },
          error as Error,
        );
      }
  ...
  private startHeartbeat(): void {
    this.heartbeatInterval = setInterval(() => {
      ...
      logger.debug("Realtime heartbeat", {
        ...
        bufferedMessages: Array.from(this.messageBuffer.values()).reduce(
