...
export class DatabaseConnectionPool {
  ...
  private async initializePool(): Promise<void> {
    ...
    logger.info(
      `Database pool initialized with ${this.connections.length} connections`,
    );
  }
  ...
  private async createConnection(): Promise<Connection> {
    ...
    try {
      ...
      if (
        error &&
        !error.message.includes('relation "health_checks" does not exist')
      ) {
    ...
    while (this.pendingAcquires.length > 0 && !this.isShuttingDown) {
      ...
      if (
        ...
        try {
          ...
          logger.error(
            "Failed to create connection for queue processing",
            {},
            error as Error,
          );
          break;
  ...
  async shutdown(): Promise<void> {
    ...
    if (this.stats.activeConnections > 0) {
      logger.warn(
        `Forcibly closing ${this.stats.activeConnections} active connections during shutdown`,
      );
    }
