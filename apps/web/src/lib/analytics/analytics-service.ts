...
class AnalyticsService {
   ...
   * Track feature usage
   */
  public trackFeature(featureName: string, category: string, metadata?: Record<string, any>) {
   ...
   */
  public trackPerformance(metric: string, value: number, metadata?: Record<string, any>) {
   ...
   * Track user actions
   */
  public trackAction(action: string, target: string, metadata?: Record<string, any>) {
   ...
   */
  public trackConversion(conversionType: string, value?: number, metadata?: Record<string, any>) {
  ...
  private batchEvent(event: AnalyticsEvent) {
    ...
    if (this.batchedEvents.length >= this.BATCH_SIZE) {
      ...
      this.batchTimer = setTimeout(() => this.flushEvents(), this.BATCH_INTERVAL)
    }
  }
  ...
  private setupPageVisibilityTracking() {
    ...
    window.addEventListener('beforeunload', () => {
      this.track('session_end', {
        session_duration_ms: Date.now() - parseInt(sessionStorage.getItem('session_start') || '0')
      })
      this.flushEvents()
    })
  }
  ...
  private categorizeEvent(eventName: string): AnalyticsEvent['event_category'] {
    ...
    if (eventName.includes('feature') || eventName.includes('ai') || eventName.includes('nims')) {
      return 'feature'
    }
    return 'action'
  }
