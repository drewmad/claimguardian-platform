...
interface StreamEvent {
  id: string
  timestamp: Date
  eventType: 'ai_request' | 'cache_hit' | 'model_switch' | 'error' | 'cost_update'
...
interface AggregatedMetrics {
  ...
  metrics: {
    ...
    modelPerformance: Array<{ model: string; requests: number; avgTime: number; errorRate: number }>
  }
}
...
class AnalyticsStreamProcessor {
   ...
   */
  getRealTimeMetrics(interval: '1m' | '5m' | '1h' | '1d'): AggregatedMetrics | null {
    const now = new Date()
