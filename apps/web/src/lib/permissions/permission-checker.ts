...
class PermissionChecker {
  ...
  async hasPermission(
    ...
    try {
      ...
      return {
        ...
        reason: hasBasePermission
          ? "Permission granted via tier"
          : `Permission not available for ${userPermissions.tier} tier`,
      };
    } catch (error) {
  ...
  async checkUsageLimit(
    ...
    try {
      ...
      return {
        ...
        reason: hasCapacity
          ? "Within usage limits"
          : `Usage limit exceeded (${currentUsage}/${limit})`,
        currentUsage,
   ...
   */
  async getTierUpgradeSuggestions(
    userId: string,
  ): Promise<{
    ...
    try {
      ...
      if (
        !propertiesCheck.hasPermission &&
        propertiesCheck.currentUsage &&
        propertiesCheck.limit
      ) {
        suggestions.push(
