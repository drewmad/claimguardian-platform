...
export class MobilePermissionChecker {
  ...
  static canAccessMobileFeature(
    ...
    if (!tierFeatures.includes(feature)) {
      ...
      result.requirements.userUpgrades.push(
        `Upgrade from ${userTier} to access this feature`,
      );
    ...
    if (config.requiresOnline && !deviceCapabilities.isOnline) {
      result.requirements.deviceCapabilities.push(
    ...
    const aiRequestsNeeded =
      currentUsage.aiRequestsThisMonth + config.aiRequestsRequired;
    ...
    if (
      (config.requiresCamera || config.storageRequired > 20) &&
      deviceCapabilities.batteryLevel < 20
    ) {
      result.requirements.deviceCapabilities.push(
        "Low battery - charge device for intensive operations",
      );
    ...
    for (let i = currentTierIndex + 1; i < allTiers.length; i++) {
      ...
      higherTierFeatures.forEach((feature) => {
        if (
          ...
          !upgrade_required.includes(feature)
        ) {
