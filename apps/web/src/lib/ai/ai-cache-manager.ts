...
class AICacheManager {
  ...
  async getCachedResponse(
    ...
    const exactKey = this.generateCacheKey(
      messages,
      featureId,
      model,
      additionalParams,
  ...
  private async findSimilarCachedResponse(
    ...
    for (const [key, cachedItem] of this.cache.entries()) {
      if (
        cachedItem.featureId !== featureId ||
        !this.isCacheValid(cachedItem)
      ) {
        continue;
      }
  ...
  private updateAverageResponseTime(responseTime: number): void {
    ...
    this.metrics.avgResponseTime =
      (totalTime + responseTime) / this.metrics.totalRequests;
  }
   ...
   */
  getFeatureStats(): Record<
    string,
    { count: number; totalCost: number; avgAge: number }
  > {
    const stats: Record<
      string,
      { count: number; totalCost: number; avgAge: number }
