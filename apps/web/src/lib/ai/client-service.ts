    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          ...
          throw new Error(
            "AI service unavailable. Please try again later or contact support.",
          );
        ...
        throw new Error(
          errorData.error || "AI processing failed. Please try again.",
        );
      }
  ...
  async analyzeImage(request: ImageAnalysisRequest): Promise<string> {
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          throw new Error(
            "Too many image analysis requests. Please wait a moment and try again.",
          );
          ...
          throw new Error(
            "Invalid image format. Please use JPG or PNG images.",
          );
        } else if (response.status >= 500) {
          throw new Error(
            "Image analysis service is temporarily down. Please try again in a few minutes.",
          );
        }
  ...
  async chatWithPolicy(
    ...
    if (!response.ok) {
      const errorData = await response
        ...
        .catch(() => ({ error: response.statusText }));
      throw new Error(
        errorData.error || `Policy chat API error: ${response.statusText}`,
      );
    }
