...
export class EnhancedAIClient extends AIClient {
  ...
  async enhancedImageAnalysis(request: ImageAnalysisRequest): Promise<string> {
    ...
    try {
      ...
      const cacheMessages = [{ role: 'user' as const, content: `${enhancedPrompt}|IMG:${imageHash}` }]
   ...
   */
  private async selectModelForFeature(featureId: string, userId?: string): Promise<ModelSelectionResult | null> {
    try {
      ...
      const modelConfig = await aiModelConfigService.getModelForFeature(featureId)
      if (!modelConfig) {
        return null
      }
   ...
   */
  private async applyCustomPrompts(messages: ChatMessage[], featureId: string): Promise<ChatMessage[]> {
    try {
      const response = await fetch(`/api/admin/custom-prompts?feature_id=${featureId}`)
   ...
   */
  private async applyCustomPromptToText(prompt: string, featureId: string): Promise<string> {
    try {
      const response = await fetch(`/api/admin/custom-prompts?feature_id=${featureId}`)
   ...
   */
  private async getActiveABTest(featureId: string, userId?: string): Promise<{
    testId: string
    selectedModel: string
    fallbackModel: string
  } | null> {
    try {
      const response = await fetch(`/api/admin/ab-tests?feature_id=${featureId}&active_only=true`)
   ...
   */
  private getProviderFromModel(modelId: string): 'openai' | 'gemini' | 'claude' | 'grok' {
   ...
   */
  private getVisionCapableProvider(provider: 'openai' | 'gemini' | 'claude' | 'grok'): 'openai' | 'gemini' {
  ...
  private createImageHash(image: string): string {
    ...
    return Buffer.from(start + end).toString('base64').substring(0, 16)
  }
