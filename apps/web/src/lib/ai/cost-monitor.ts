...
interface CostAlert {
  id: string
  type: 'budget_warning' | 'budget_critical' | 'budget_exceeded' | 'spike_detected' | 'prediction_high'
...
class AICostMonitor {
   ...
   */
  private updateBudgetSpending(cost: number, featureId?: string, userId?: string): void {
  ...
  private createAlert(alertData: Omit<CostAlert, 'id' | 'timestamp' | 'isResolved'>): void {
    ...
    console.log(`🚨 Cost Alert [${alert.level.toUpperCase()}]: ${alert.message}`)
  }
  ...
  getCostMetrics(): CostMetrics {
    ...
    const totalSpent = this.spendingHistory.reduce((sum, entry) => sum + entry.cost, 0)
    ...
    const costPerFeature = this.spendingHistory.reduce((acc, entry) => {
   ...
   * Create new budget
   */
  createBudget(budgetData: Omit<CostBudget, 'id' | 'spent' | 'remaining' | 'resetDate'>): string {
    const id = this.generateBudgetId()
  ...
  getSpendingPatterns(): SpendingPattern[] {
    ...
    const last7Days = this.spendingHistory.filter(
      entry => entry.timestamp > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
    )

    // Group by hour for hourly patterns
    const hourlySpending = last7Days.reduce((acc, entry) => {
