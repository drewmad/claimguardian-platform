...
class AIBatchProcessor {
   ...
   */
  async addRequest(
    request: Omit<BatchRequest, "id" | "timestamp" | "resolve" | "reject">,
  ): Promise<any> {
  ...
  private async processBatch(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Batch processed: ${batch.length} requests in ${processingTime}ms`,
      );
    } catch (error) {
   ...
   */
  private groupRequestsByFeature(
    batch: BatchRequest[],
   ...
   */
  private async processFeatureBatch(
    featureId: string,
    requests: BatchRequest[],
  ): Promise<void> {
    console.log(
      `Processing batch for ${featureId}: ${requests.length} requests`,
    );
      ...
      await Promise.allSettled(
        chunk.map(async (request) => {
          try {
            ...
            const responseTime = Date.now() - startTime;
            const estimatedCost = this.estimateVisionCost(
              featureId,
              responseTime,
            );
  ...
  private estimateCost(featureId: string, responseTime: number): number {
    const baseCosts: Record<string, number> = {
      ...
      "claim-assistant": 0.02,
      "document-generator": 0.03,
    ...
    const estimatedWaitTime =
      this.requestQueue.length > 0
        ? Math.max(
            0,
            this.config.maxWaitTime -
              (Date.now() - (this.requestQueue[0]?.timestamp || Date.now())),
          )
        : 0;
  ...
  getQueueStatus(): {
    ...
    const byPriority = this.requestQueue.reduce(
      (acc, req) => {
    ...
    const byFeature = this.requestQueue.reduce(
      (acc, req) => {
