...
export class ThreatAssessmentEngine {
  ...
  private async initialize() {
    ...
    if (this.isInitialized) {
      ...
      logger.info(
        "Threat Assessment Engine initialized with providers:",
        availableProviders,
      );
    } else {
      logger.warn(
        "No AI providers available. Threat assessment will use simulated data.",
      );
    }
  }
  ...
  private async initializeGrok() {
    try {
      const apiKey =
        process.env.NEXT_PUBLIC_XAI_API_KEY || process.env.XAI_API_KEY;
  ...
  private selectProvider(
    ...
    if (
      ...
      this.providers[preferredProvider].available
    ) {
  ...
  private calculateSpeedScore(
    ...
    const isUrgent =
      urgency === ThreatLevel.CRITICAL || urgency === ThreatLevel.EMERGENCY;
  ...
  private calculateAbilityScore(
    ...
    const strengthMatches = this.getStrengthMatches(
      config.strengths,
      focusAreas,
      urgency,
    );
  ...
  private getStrengthMatches(
    ...
    if (urgency === ThreatLevel.CRITICAL || urgency === ThreatLevel.EMERGENCY) {
      if (
        strengths.includes("safety-analysis") ||
        strengths.includes("real-time-data")
      ) {
  ...
  async assessThreats(
    request: AIThreatAssessmentRequest,
  ): Promise<AIThreatAssessmentResponse> {
    ...
    try {
      ...
      const assessment = this.parseAIResponse(
        response,
        request,
        startTime,
        provider,
      ...
      logger.error(
        `AI threat assessment failed with ${provider}, trying fallback:`,
        toError(error),
      );
      ...
      if (alternateProvider) {
        try {
          ...
          logger.error(
            `Fallback provider ${alternateProvider} also failed:`,
            fallbackError,
          );
        }
      }
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert property threat assessment analyst for ClaimGuardian. Provide comprehensive threat analysis in valid JSON format only.",
        },
        {
          role: "user",
          content: prompt,
...
Respond with valid JSON only, no additional text.
  ...
  private parseAIResponse(
    ...
    try {
      ...
      const threats: ThreatAssessment[] = parsed.threats.map(
        (threat: any, index: number) => ({
          ...
          aiAnalysis: {
            ...
            modelConfidence: Math.min(
              100,
              Math.max(0, threat.aiAnalysis?.modelConfidence || 50),
            ),
      ...
      const intelligenceFeeds: IntelligenceFeed[] =
        parsed.intelligenceFeeds.map((feed: any, index: number) => ({
  ...
  private validateActionPriority(priority: string): ActionPriority {
    ...
    const found = validPriorities.find(
      (p) => p.toUpperCase() === normalizedPriority,
    );
  ...
  private fallbackAssessment(
    ...
    const fallbackThreat: ThreatAssessment = {
      ...
      description:
        "AI threat assessment temporarily unavailable. Manual monitoring recommended.",
      ...
      recommendations: [
        {
          ...
          description:
            "Monitor weather conditions and local emergency services while AI system is unavailable",
      ...
      intelligenceFeeds: [
        {
          ...
          content:
            "AI-powered threat assessment is temporarily unavailable. Using fallback monitoring.",
  ...
  async generateWeatherContext(location: {
    lat: number;
    lng: number;
  }): Promise<WeatherData> {
