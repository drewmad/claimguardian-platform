...
class RedisAICacheService {
  ...
  async set(
    ...
    try {
      ...
      console.log(
        `💾 Redis Cache SET: ${featureId} (${model}) - ${serialized.length} bytes`,
      );
      return true;
    } catch (error) {
  ...
  async findSimilar(
    ...
    try {
      ...
      const keys = Array.from(this.mockRedisStorage.keys()).filter((key) =>
      ...
      for (const key of keys) {
        ...
        if (
          parsed.metadata.featureId !== featureId ||
        ...
        if (similarity >= similarityThreshold) {
          ...
          console.log(
            `🎯 Redis Semantic Cache HIT: ${featureId} (similarity: ${similarity.toFixed(2)})`,
          );
          return parsed;
        }
      }
  ...
  private updateFeatureStats(featureId: string, cost: number): void {
    const existing = this.stats.topFeatures.find(
  ...
  async clearFeatureCache(featureId: string): Promise<number> {
    ...
    try {
      ...
      console.log(
        `🗑️ Cleared ${cleared} cache entries for feature: ${featureId}`,
      );
      return cleared;
    } catch (error) {
