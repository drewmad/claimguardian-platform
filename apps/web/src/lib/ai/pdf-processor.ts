...
export function searchInPDF(
  ...
  const regex = new RegExp(
    `.{0,${contextLength}}${query}.{0,${contextLength}}`,
    "gi",
  );
  ...
  matches.forEach((match) => {
    ...
    const page = pageMatches
      ? parseInt(pageMatches.pop()?.match(/\d+/)?.[0] || "1", 10)
      : 1;
...
export function extractPolicyData(pdfText: string): {
  ...
  const patterns = {
    ...
    standardDeductible:
      /(?:all\s*other\s*perils|standard)\s*deductible:\s*\$?([\d,]+)/i,
    windDeductible:
      /(?:hurricane|wind(?:storm)?)\s*deductible:\s*(\d+%|\$[\d,]+)/i,
  };
