...
export class AIClient {
  ...
  async chat(
    ...
    model: "openai" | "gemini" | "claude" | "grok" = "openai",
  ) {
    switch (model) {
      case "openai":
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        const response = await fetch(
          `${AI_ENDPOINTS.OPENAI}/chat/completions`,
          {
            method: "POST",
            headers: {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 429) {
            ...
            throw new Error(
              "Invalid Claude API key. Please check your Anthropic configuration.",
            );
          } else if (response.status >= 500) {
            if (attempt < retries) {
              await new Promise((resolve) =>
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 429) {
            ...
            throw new Error(
              "Invalid Grok API key. Please check your X.AI configuration.",
            );
          } else if (response.status >= 500) {
            if (attempt < retries) {
              await new Promise((resolve) =>
