...
class BatchAIService {
   ...
   */
  async processBulkImageAnalysis(
    requests: BatchImageRequest[],
  ): Promise<BatchResponse[]> {
    const promises = requests.map((request) =>
      this.processImageAnalysis(request),
    );
    return await Promise.all(promises);
  }
   ...
   */
  async smartProcess(
    request: BatchChatRequest | BatchImageRequest,
  ): Promise<BatchResponse> {
    ...
    const shouldBatch = this.shouldUseBatching(
      metrics,
      queueStatus,
      request.priority,
      isImageRequest,
  ...
  private shouldUseBatching(
    ...
    if (metrics.avgBatchSize > 3 && metrics.avgProcessingTime < 2000)
      return true;
   ...
   */
  async processWithLoadBalancing(
    requests: (BatchChatRequest | BatchImageRequest)[],
  ): Promise<BatchResponse[]> {
    const metrics = aiBatchProcessor.getMetrics();
    const queueStatus = aiBatchProcessor.getQueueStatus();
    ...
    const imageRequests = requests.filter(
      (r) => "image" in r,
    ) as BatchImageRequest[];

    const highPriorityChat = chatRequests.filter((r) => r.priority === "high");
    const normalPriorityChat = chatRequests.filter(
      (r) => r.priority !== "high",
    );
