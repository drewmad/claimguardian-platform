...
export class BotProtection {
  ...
  private checkRequestRate(ip: string): boolean {
    ...
    if (
      !record ||
      now - record.timestamp > SUSPICIOUS_PATTERNS.rapidRequests.windowMs
    ) {
  ...
  private checkSuspiciousHeaders(headers: Headers): boolean {
    return SUSPICIOUS_PATTERNS.suspiciousHeaders.some((check) =>
      check(headers),
    );
  }
   ...
   */
  static validateHoneypot(
    formData: Record<string, unknown>,
    formType: keyof typeof HONEYPOT_FIELDS,
  ): boolean {
