...
export class AsyncErrorHandler {
  ...
  async execute<T>(
    ...
    try {
      ...
      logger.error(
        `Operation failed${contextMessage}`,
        {},
  ...
  async executeWithRetry<T>(
    ...
    const contextMessage = context ? ` in ${context}` : "";
    logger.error(
      `Operation failed after ${finalConfig.maxAttempts} attempts${contextMessage}`,
      {},
      lastError || undefined,
    );
  ...
  async executeWithTimeout<T>(
    ...
    try {
      ...
      const contextMessage = context ? ` in ${context}` : "";
      logger.error(
        `Operation timed out${contextMessage}`,
        {},
        error instanceof Error ? error : new Error(String(error)),
      );
  ...
  static retryConditions = {
    ...
    temporaryErrors: (error: Error): boolean => {
      return (
        AsyncErrorHandler.retryConditions.networkErrors(error) ||
