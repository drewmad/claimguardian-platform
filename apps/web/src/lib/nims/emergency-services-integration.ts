...
export class EmergencyServicesIntegration {
  ...
  async sendNWSAlert(capMessage: CAPMessage): Promise<void> {
    const nwsIntegration = this.integrations.get(
      ServiceProvider.NATIONAL_WEATHER_SERVICE,
    );
    if (!nwsIntegration) {
  ...
  async requestFEMAResources(
    ...
    const resourceMessage = {
      ...
      incidentInformation: {
        ...
        incidentDescription:
          "Property damage incident requiring federal assistance",
      },
      contactInformation: [
        {
   ...
   */
  async getNWSWeatherData(location: {
    lat: number;
    lng: number;
  }): Promise<any> {
    const nwsIntegration = this.integrations.get(
    ...
    try {
      ...
      const pointResponse = await fetch(
        `https://api.weather.gov/points/${location.lat},${location.lng}`,
        {
          headers: {
            "User-Agent":
              "ClaimGuardian-NIMS/1.0 (emergency-management@claimguardian.com)",
          },
        },
      );
  ...
  async getRedCrossShelters(searchArea: {
    ...
    const redCrossIntegration = this.integrations.get(
      ServiceProvider.RED_CROSS,
    );
  ...
  async coordinateWithLocalEM(
    ...
    const localEMIntegration = this.integrations.get(
      ServiceProvider.LOCAL_EMERGENCY_MANAGEMENT,
    );
  ...
  async monitorSystemStatus(): Promise<SystemStatus[]> {
    const statusChecks = Array.from(this.integrations.values()).map(
      async (integration) => {
        ...
        return {
          ...
          status,
          availability:
            status === "operational" ? 100 : status === "degraded" ? 75 : 0,
  ...
  private async validateIntegration(
    ...
    if (!integration.api_endpoint || !integration.authentication) {
      throw new Error(
        "Invalid integration configuration: missing endpoint or authentication",
      );
    }
    ...
    switch (integration.authentication.type) {
      case "api_key":
        headers["Authorization"] =
          `Bearer ${integration.authentication.credentials.api_key}`;
        break;
      case "basic_auth":
        const auth = btoa(
