...
export class ICSIntegrationService {
  ...
  async createIncident(
    incidentData: Partial<ICSIncident>,
  ): Promise<ICSIncident> {
    const incident: ICSIncident = {
      ...
      complexity_level:
        incidentData.complexity_level || IncidentComplexity.TYPE_5,
  ...
  async generateICS201(incidentId: string): Promise<ICSForm> {
    ...
    const form: ICSForm = {
      ...
      data: {
        ...
        current_organization: this.getOrganizationSummary(
  ...
  async generateICS202(
    ...
    const form: ICSForm = {
      ...
      operational_period: incident.operational_period.period_number,
      prepared_by:
        incident.organization.general_staff.planning_chief?.name ||
        incident.organization.incident_commander.name,
      prepared_date: new Date().toISOString(),
      status: "draft",
      data: {
        incident_name: incident.incident_name,
        operational_period: {
          date: new Date(
            incident.operational_period.start,
          ).toLocaleDateString(),
  ...
  private getOrganizationSummary(organization: ICSOrganization): any {
    return {
      ...
      command_staff_assigned: Object.keys(organization.command_staff).filter(
        (key) =>
          organization.command_staff[
            key as keyof typeof organization.command_staff
          ],
      ).length,
      general_staff_assigned: Object.keys(organization.general_staff).filter(
        (key) =>
          organization.general_staff[
            key as keyof typeof organization.general_staff
          ],
      ).length,
    };
  }
  ...
  private buildOrganizationChart(organization: ICSOrganization): any[] {
    ...
    assignments.push({
      ...
      contact:
        organization.incident_commander.contact.phone ||
        organization.incident_commander.contact.radio,
    });
    ...
    if (organization.command_staff.safety_officer) {
      assignments.push({
        ...
        contact:
          organization.command_staff.safety_officer.contact.phone ||
          organization.command_staff.safety_officer.contact.radio,
      });
    }
    ...
    if (organization.command_staff.information_officer) {
      assignments.push({
        ...
        contact:
          organization.command_staff.information_officer.contact.phone ||
          organization.command_staff.information_officer.contact.radio,
      });
    }
    ...
    if (organization.general_staff.operations_chief) {
      assignments.push({
        ...
        contact:
          organization.general_staff.operations_chief.contact.phone ||
          organization.general_staff.operations_chief.contact.radio,
      });
    }
  ...
  private async notifyResourceManagers(
  ...
  private async generateSituationReport(
    ...
    const report: SituationReport = {
      ...
      resource_summary: {
        total_personnel: incident.resources.filter(
          (r) => r.category === "personnel",
        ).length,
        overhead:
          Object.keys(incident.organization.general_staff).length +
          Object.keys(incident.organization.command_staff).length +
          1,
