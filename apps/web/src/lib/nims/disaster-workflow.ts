...
import {
  ...
  IncidentType,
  IncidentComplexity,
  icsIntegrationService,
} from "./ics-integration";
import {
  NIMSResource,
  ResourceStatus,
  nimsResourceManager,
...
export const DISASTER_WORKFLOW_TEMPLATES: Record<
  IncidentType,
  Partial<DisasterWorkflow>
> = {
    ...
    phases: [
      {
        ...
        activities: [
          {
            ...
            description:
              "Continuous monitoring of hurricane track and intensity",
            phase: ResponsePhase.PREPAREDNESS,
            priority: "critical",
            status: "pending",
        ...
        entry_criteria: [
          ...
          "Landfall projected within 72 hours",
        ],
        exit_criteria: [
          "All preparation activities completed",
      ...
      },
      {
        ...
        activities: [
          {
            ...
            status: "pending",
            assigned_position: [
              ICSPosition.INCIDENT_COMMANDER,
              ICSPosition.OPERATIONS_CHIEF,
            ],
            assigned_resources: [],
        ...
        entry_criteria: [
          "Hurricane conditions present",
          "Emergency operations activated",
        ],
        exit_criteria: [
          "Hurricane passed",
          "Immediate life safety threats addressed",
        ],
  ...
  [IncidentType.WINTER_STORM]: {
    workflow_name: "Winter Storm Response Workflow",
  },
  [IncidentType.HAIL_STORM]: { workflow_name: "Hail Storm Response Workflow" },
  [IncidentType.STRUCTURE_FIRE]: {
    workflow_name: "Structure Fire Response Workflow",
  },
  [IncidentType.HAZMAT]: { workflow_name: "Hazmat Response Workflow" },
  [IncidentType.OTHER]: {
    workflow_name: "General Emergency Response Workflow",
  },
};
...
export class DisasterWorkflowManager {
  ...
  async activateWorkflow(workflowId: string, triggerId: string): Promise<void> {
    ...
    if (workflow.status !== WorkflowStatus.INITIATED) {
      throw new Error(
        `Workflow cannot be activated in status: ${workflow.status}`,
      );
    }
  ...
  async transitionPhase(
    ...
    const exitCriteriaMet = await this.validateExitCriteria(
      workflowId,
      currentPhase,
    );
    if (!exitCriteriaMet) {
    ...
    const entryCriteriaMet = await this.validateEntryCriteria(
      workflowId,
      nextPhase,
    );
    if (!entryCriteriaMet) {
  ...
  async updateActivityProgress(
    ...
    if (update.progress_percent >= 100) {
      ...
      activity.actual_start_time =
        activity.actual_start_time || new Date().toISOString();
    }
  ...
  async generatePerformanceReport(
    ...
    const responseTime =
      workflow.current_activities.length > 0
        ? new Date(
            workflow.current_activities[0].actual_start_time!,
          ).getTime() - new Date(workflow.created_at).getTime()
        : 0;
    ...
    const totalActivities = workflow.phases.reduce(
      (sum, phase) => sum + phase.activities.length,
      0,
    );
    const completedActivities = workflow.phases.reduce(
      (sum, phase) =>
        sum + phase.activities.filter((a) => a.status === "completed").length,
      0,
    );
    ...
    const metrics: WorkflowMetrics = {
      ...
      improvement_recommendations:
        await this.generateImprovementRecommendations(workflow),
    };
  ...
  private getDefaultDecisionPoints(
    incidentType: IncidentType,
  ): DecisionPoint[] {
    return [
      {
  ...
  private async validateEntryCriteria(
    workflowId: string,
    phase: WorkflowPhase,
  ): Promise<boolean> {
  ...
  private async completePhaseActivities(
    workflowId: string,
    phase: ResponsePhase,
  ): Promise<void> {
