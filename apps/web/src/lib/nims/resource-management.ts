...
export const NIMS_RESOURCE_TYPES: Record<string, ResourceType> = {
  ...
  EMERGENCY_SUPPLY_KIT: {
    ...
    training_requirements: ["Emergency preparedness training"],
    certification_requirements: [
...
export class NIMSResourceManager {
  ...
  async registerResource(
    resourceData: Partial<NIMSResource>,
  ...
  async typeResource(resourceId: string, resourceType: string): Promise<void> {
    ...
    if (!complianceCheck.compliant) {
      throw new Error(
        `Resource does not meet NIMS standards: ${complianceCheck.deficiencies.join(", ")}`,
      );
    }
  ...
  async searchResources(criteria: {
    ...
    if (criteria.capabilities) {
      resources = resources.filter((resource) => {
        ...
        return criteria.capabilities!.every(
          (capability) =>
  ...
  async deployResource(
    ...
    const deployment: ResourceDeployment = {
      ...
      expected_return_date:
        deploymentData.expected_return_date ||
  ...
  async generateInventoryReport(
    ...
    const inventory: ResourceInventory = {
      ...
      next_inventory_date: new Date(
        Date.now() + 90 * 24 * 60 * 60 * 1000,
      ).toISOString(), // 90 days
      compliance_status: await this.assessInventoryCompliance(
        resources as NIMSResource[],
      ),
      notes: [],
    };
   ...
   */
  private async validateResourceCompliance(
    resource: Partial<NIMSResource>,
  ): Promise<void> {
    ...
    if (nimsType) {
      ...
      const hasCapabilities = nimsType.minimum_capabilities.every(
        (capability) => resource.qualifications?.includes(capability),
      );

      if (!hasCapabilities) {
        throw new Error(
          `Resource must demonstrate all minimum capabilities for type: ${resource.type}`,
        );
      }
    }
  }
  ...
  private async checkTypeCompliance(
    ...
    if (
      nimsType.training_requirements.length > 0 &&
  ...
  private async assessInventoryCompliance(
    resources: NIMSResource[],
  ): Promise<"compliant" | "needs_attention" | "non_compliant"> {
    const totalResources = resources.length;
    const compliantResources = resources.filter(
      (resource) =>
    ...
    const complianceRate =
      totalResources > 0 ? compliantResources / totalResources : 0;
