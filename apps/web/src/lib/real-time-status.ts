...
class RealTimeStatusManager {
  ...
  public subscribeToSystemStatus(callback: (status: SystemStatus) => void) {
    ...
    return () => {
      this.systemStatusCallbacks = this.systemStatusCallbacks.filter(
        (cb) => cb !== callback,
      );
    };
  }
...
export function useRealTimeStatus() {
  ...
  useEffect(() => {
    ...
    const unsubscribeUpdates =
      statusManagerRef.current.subscribeToStatusUpdates((update) => {
    ...
    const unsubscribeStatus = statusManagerRef.current.subscribeToSystemStatus(
      (status) => {
