  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400 && errorBody.includes("API_KEY_INVALID")) {
        return jsonSchema
          ? {}
          : " API Key Required: To use AI features with real analysis, please add your Gemini API key. For now, I'm providing demo responses.";
      }
    ...
    return jsonSchema
      ? {}
      : "I'm currently in demo mode. AI features are limited but you can still explore the interface!";
  }
};
