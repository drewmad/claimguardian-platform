...
class TenantManager {
  ...
  async getOrganizationById(
    organizationId: string,
  ): Promise<EnterpriseOrganization | null> {
    try {
  ...
  async getUserOrganization(
    ...
    try {
      ...
      const { data: userOrg } = await this.supabase!.from("organization_users")
  ...
  async getTenantContext(
    ...
    try {
      ...
      if (!targetOrgId) {
        const { data: userOrg } = await this.supabase!.from(
          "organization_users",
        )
          .select("organization_id")
          .eq("user_id", userId)
      ...
      const { data: userRole } = await this.supabase!.from("organization_users")
        .select("*")
        .eq("user_id", userId)
      ...
      const limits = {
        ...
        properties: {
          current: organization.currentProperties,
          limit: organization.propertyLimit,
        },
        claims: {
          current: organization.currentClaims,
          limit: organization.claimLimit,
        },
        aiRequests: {
          current: organization.currentAiRequests,
          limit: organization.aiRequestLimit,
        },
        storage: {
          current: organization.currentStorageGb,
          limit: organization.storageLimitGb,
        },
      };
  ...
  async updateUserRole(
    ...
    try {
      ...
      const { error } = await this.supabase!.from("organization_users")
        .update({
          role,
   ...
   */
  async getOrganizationCustomizations(
    organizationId: string,
  ...
  async updateOrganizationCustomizations(
    ...
    try {
      ...
      const { error } = await this.supabase!.from("enterprise_organizations")
        .update({
  ...
  async executeTenantQuery<T = unknown>(
    ...
    try {
      ...
      let queryBuilder = this.supabase!.from(tenantQuery.from).select(
        tenantQuery.select,
      );
      ...
      queryBuilder = queryBuilder.eq(
        "organization_id",
        tenantQuery.organizationId,
      );

      // Apply additional filters
      if (tenantQuery.eq) {
        queryBuilder = queryBuilder.eq(
          tenantQuery.eq.column,
          tenantQuery.eq.value,
        );
      }
      if (tenantQuery.in) {
        queryBuilder = queryBuilder.in(
          tenantQuery.in.column,
          tenantQuery.in.values,
        );
      }
  ...
  async updateUsageMetrics(
    ...
    try {
      ...
      if (metrics.properties !== undefined)
        updates.current_properties = metrics.properties;
      if (metrics.claims !== undefined) updates.current_claims = metrics.claims;
      if (metrics.aiRequests !== undefined)
        updates.current_ai_requests = metrics.aiRequests;
      if (metrics.storage !== undefined)
        updates.current_storage_gb = metrics.storage;
  ...
  async getOrganizationUsers(
    ...
    try {
      ...
      const { data, error } = await this.supabase!.from("organization_users")
        .select("*")
        .eq("organization_id", organizationId)
      ...
      return (data || []).map((row: OrganizationUserRow) =>
  ...
  private parseOrganizationUser(data: OrganizationUserRow): OrganizationUser {
    return {
      ...
      invitationExpiresAt: data.invitation_expires_at
        ? new Date(data.invitation_expires_at)
        : undefined,
