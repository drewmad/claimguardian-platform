...
export class LoadTestFramework {
  ...
  async runLoadTest(config: LoadTestConfig): Promise<LoadTestResult> {
    ...
    console.log(`⏳ Waiting for ${this.activeRequests.size} remaining requests...`)
  ...
  private logResults(result: LoadTestResult): void {
    ...
    if (result.errors.length > 0) {
      ...
      const errorCounts = result.errors.reduce((acc, error) => {
        const key = error.statusCode ? `HTTP ${error.statusCode}` : 'Network Error'
        acc[key] = (acc[key] || 0) + 1
        return acc
  ...
  private generateSummaryReport(results: LoadTestResult[]): void {
    ...
    console.log(`  Avg Response Time: ${(avgResponseTimes.reduce((a, b) => a + b, 0) / avgResponseTimes.length).toFixed(2)}ms`)
    console.log(`  Avg Throughput: ${(avgThroughput.reduce((a, b) => a + b, 0) / avgThroughput.length).toFixed(2)} req/s`)
    console.log(`  Avg Error Rate: ${(avgErrorRates.reduce((a, b) => a + b, 0) / avgErrorRates.length).toFixed(2)}%`)
    ...
    results.forEach((result, index) => {
      ...
      console.log(`  ${status} ${result.config.name}: ${result.metrics.requestsPerSecond.toFixed(1)} req/s, ${result.metrics.avgResponseTime.toFixed(1)}ms avg`)
    })
  }
