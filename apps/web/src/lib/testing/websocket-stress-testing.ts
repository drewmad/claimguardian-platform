...
export interface WebSocketStressResult {
  ...
  thresholdResults: {
    connectionSuccessRate: { passed: boolean; actual: number; expected: number }
...
export class WebSocketStressTestFramework {
  ...
  async runStressTest(config: WebSocketStressConfig): Promise<WebSocketStressResult> {
  ...
  private handleMessage(
    ...
    try {
      ...
      switch (data.type) {
        case 'ping':
          // Respond to ping
          this.sendMessage(connection, { type: 'pong', timestamp: data.timestamp }, errors)
          break
        case 'cost_update':
  ...
  private startMessageSending(
    ...
    console.log(`📤 Starting message sending for ${connectedConnections.length} connections...`)
  ...
  private calculateResults(
    ...
    const successfulConnections = connections.filter(c => c.connected || c.connectTime).length
    ...
    // Message metrics
    const totalMessagesSent = connections.reduce((sum, c) => sum + c.messagesSent, 0)
    const totalMessagesReceived = connections.reduce((sum, c) => sum + c.messagesReceived, 0)
    const lostMessages = totalMessagesSent - totalMessagesReceived

    // Latency metrics
    const allLatencies = connections.flatMap(c => c.latencies).sort((a, b) => a - b)
    ...
    // Throughput metrics
    const connectionSuccessRate = (successfulConnections / attemptedConnections) * 100
    const messageSuccessRate = totalMessagesSent > 0 ? (totalMessagesReceived / totalMessagesSent) * 100 : 0
    ...
    const bandwidth = (totalMessagesReceived * config.messageSize) / (duration / 1000)

    // Threshold checks
    const thresholdResults = {
      connectionSuccessRate: {
        passed: connectionSuccessRate >= config.thresholds.connectionSuccessRate,
  ...
  private logResults(result: WebSocketStressResult): void {
    ...
    console.log(`  Connections/sec: ${metrics.connectionsPerSecond.toFixed(2)}`)
    ...
    Object.entries(thresholdResults).forEach(([key, threshold]) => {
      ...
      console.log(`  ${status} ${key}: ${threshold.actual.toFixed(2)} (expected: ${threshold.expected})`)
    })
  ...
  private generateSummaryReport(results: WebSocketStressResult[]): void {
    ...
    const totalConnections = results.reduce((sum, r) => sum + r.connections.attempted, 0)
    const totalMessages = results.reduce((sum, r) => sum + r.messages.sent, 0)
    const avgConnectionSuccessRate = results.reduce((sum, r) => sum + r.metrics.connectionSuccessRate, 0) / results.length
    const avgMessageSuccessRate = results.reduce((sum, r) => sum + r.metrics.messageSuccessRate, 0) / results.length
    const avgLatency = results.reduce((sum, r) => sum + r.metrics.avgLatency, 0) / results.length

    console.log(`\nAggregate Statistics:`)
    console.log(`  Total Connections Tested: ${totalConnections}`)
    console.log(`  Total Messages Sent: ${totalMessages}`)
    console.log(`  Avg Connection Success Rate: ${avgConnectionSuccessRate.toFixed(2)}%`)
    console.log(`  Avg Message Success Rate: ${avgMessageSuccessRate.toFixed(2)}%`)
    console.log(`  Avg Latency: ${avgLatency.toFixed(2)}ms`)

    console.log(`\nIndividual Test Results:`)
    results.forEach((result, index) => {
      const allPassed = Object.values(result.thresholdResults).every(t => t.passed)
      const status = allPassed ? '✅' : '❌'
      console.log(`  ${status} ${result.config.name}: ${result.connections.successful}/${result.connections.attempted} connections, ${result.metrics.avgLatency.toFixed(1)}ms avg latency`)
    })
  }
