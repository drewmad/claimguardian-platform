...
export class LearningAssistant {
  ...
  async searchLearnings(query: LearningQuery): Promise<LearningSuggestion[]> {
    try {
      ...
      const { data: textResults, error: textError } = await this.supabase.rpc(
        "search_learnings",
        {
          p_query: query.query,
          p_limit: 10,
  ...
  async detectPatterns(errorText: string): Promise<PatternMatch[]> {
    try {
      const { data: patterns, error } = await this.supabase.rpc(
    ...
    const enhanced = await Promise.all(
      results.map(async (result) => {
        ...
        return {
          ...
          category:
            (learning?.category as { name?: string })?.name || "general",
          tags: ((tags as Array<{ tag?: { name?: string } }>)
            ?.map((t) => t.tag?.name)
