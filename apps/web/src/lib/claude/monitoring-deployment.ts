...
class MonitoringDeployment {
  ...
  async deploy(): Promise<{
    ...
    try {
      ...
      if (this.config.enabledFeatures.databaseHealth) {
        productionErrorMonitor.startHealthMonitoring(
      ...
      logger.error(
        "Failed to deploy monitoring system",
        {},
  ...
  async performManualHealthCheck(): Promise<{
    ...
    const missingObjects = databaseHealth.filter(
      (check) => !check.exists,
    ).length;
    const criticalErrors = errorPatterns.filter(
      (pattern) => pattern.severity?.high > 0 || pattern.severity?.critical > 0,
    ).length;

    let systemStatus = "healthy";
    if (
      missingObjects > 0 ||
      criticalErrors > this.config.alertThresholds.criticalErrors
    ) {
...
export function getMonitoringStatus(): void {
  ...
  console.log(
    `  Health Check Interval: ${status.config.healthCheckInterval} minutes`,
  );
  console.log(`  Features Enabled:`);
  Object.entries(status.config.enabledFeatures).forEach(
    ([feature, enabled]) => {
