    ...
    const corePatterns: Omit<SharedPattern, "id">[] = [
      {
        ...
        description:
          "Execute multiple independent tool calls in parallel rather than sequentially",
        problem:
          "Sequential tool calls lead to unnecessary waiting time when operations are independent",
        solution:
          "Batch independent tool calls in a single message to run them concurrently",
        confidence: 0.95,
        impact: {
          timeReduction: 65,
          errorReduction: 0,
...
const [file1, file2, file3] = await Promise.all([
  ...
  readFile('schema.json')
            ...
            explanation:
              "Using Promise.all() executes all reads simultaneously, reducing total time from sum to max",
      ...
      },
      {
        ...
        description:
          "Validate inputs and preconditions at the beginning of functions to fail fast",
        problem:
          "Late validation leads to wasted computation and unclear error messages",
        solution:
          "Check all prerequisites and validate inputs immediately upon function entry",
        confidence: 0.92,
        impact: {
          timeReduction: 20,
          errorReduction: 45,
  ...
  return await db.updateUser(userId, processed)
            ...
            explanation:
              "Moving validation to the start prevents unnecessary database calls and processing",
      ...
      },
      {
        ...
        description:
          "Migrate legacy code incrementally with feature flags and dual implementations",
        problem:
          "Big-bang migrations are risky and often fail or cause extended downtime",
        solution:
          "Create new implementation alongside old, use feature flags to gradually switch traffic",
        confidence: 0.88,
        impact: {
          timeReduction: 30,
          errorReduction: 70,
...
class DataService {
  ...
  // Week 1: 5% -> Week 2: 25% -> Week 3: 50% -> Week 4: 100%
            ...
            explanation:
              "Feature flags allow gradual rollout with easy rollback if issues arise",
      ...
      },
      {
        ...
        description:
          "Use builder pattern for creating test data with sensible defaults",
        problem:
          "Test setup code becomes verbose and brittle with complex object creation",
        solution:
          "Create builder classes that provide fluent API for test data creation",
        confidence: 0.91,
        impact: {
          timeReduction: 40,
          errorReduction: 35,
...
const testUser2 = new UserBuilder()
  ...
  .build()`,
            explanation:
              "Builders provide sensible defaults and expose only what needs customization",
    ...
    const templates: PatternTemplate[] = [
      {
        ...
        structure: {
          problem:
            "Operation {operation} takes {current_time} but could be optimized",
          context: "When {trigger_condition}, the system {current_behavior}",
          solution:
            "Implement {optimization_technique} to reduce time to {target_time}",
          implementation: "{implementation_steps}",
          verification:
            "Measure {metric} before and after, expecting {expected_improvement}",
        },
        placeholders: {
          ...
          implementation_steps:
            "e.g., 1. Split data 2. Process in parallel 3. Merge results",
      ...
      },
      {
        ...
        placeholders: {
          ...
          implementation_steps:
            "e.g., 1. Wrap in try-catch 2. Add retry logic 3. Implement fallback",
   ...
   */
  async derivePatternFromLearnings(
    learningIds: string[],
  ...
  private analyzeCommonality(learnings: unknown[]): {
    ...
    const mostCommonCategory = this.findMostCommon(
      categories,
    ) as PatternCategory;
  ...
  private categorizelearning(learning: any): PatternCategory {
    ...
    if (
      ...
      taskDescription.includes("speed")
    ) {
      return "performance_optimization";
    } else if (
      taskDescription.includes("error") ||
      taskDescription.includes("exception")
    ) {
  ...
  searchPatterns(query: {
    ...
    if (query.category) {
      const categoryPatternIds =
        this.categoryIndex.get(query.category) || new Set();
    ...
    return results.sort((a, b) => {
      const scoreA =
        a.confidence * (a.impact.timeReduction + a.impact.errorReduction);
      const scoreB =
        b.confidence * (b.impact.timeReduction + b.impact.errorReduction);
  ...
  async applyPattern(
    ...
    if (context.timeSaved) {
      pattern.metrics.averageTimeSaved =
        (pattern.metrics.averageTimeSaved * (pattern.metrics.timesApplied - 1) +
          context.timeSaved) /
        pattern.metrics.timesApplied;
    }
    ...
    if (context.feedback) {
      pattern.metrics.feedbackScore =
        (pattern.metrics.feedbackScore * (pattern.metrics.timesApplied - 1) +
          context.feedback) /
        pattern.metrics.timesApplied;
    }
    ...
    try {
      ...
      data.patterns.forEach((pattern: any) => {
        try {
          ...
          result.errors.push(
            `Failed to import pattern ${pattern.name}: ${error}`,
          );
        }
      });
    } catch (error) {
  ...
  getStatistics(): {
    ...
    const avgImpact =
      patterns.reduce(
        (sum, p) =>
          sum + (p.impact.timeReduction + p.impact.errorReduction) / 2,
        0,
      ) / patterns.length;
