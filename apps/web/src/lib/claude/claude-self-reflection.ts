...
class ClaudeSelfReflection {
  ...
  startReflection(
    ...
    logger.info("Claude self-reflection started", {
      taskId,
      taskType,
      taskDescription,
    });
  ...
  logStep(
    ...
    if (
      fileAccessed &&
      !this.activeReflection.filesAccessed.includes(fileAccessed)
    ) {
  ...
  async completeReflection(
    ...
    const insights = await this.generateImprovementInsights(
      this.activeReflection,
      analysis,
    );
  ...
  private async analyzeApproach(
    ...
    const approachImprovements = this.generateApproachImprovements(
      context,
      inefficiencies,
    );
    const toolUsageOptimizations = this.generateToolOptimizations(
      context,
      metrics,
    );
  ...
  private calculateEfficiencyMetrics(
    ...
    const toolEfficiency = Math.max(
      0,
      1 - (context.toolsUsed.length - 1) * 0.1,
    );
    ...
    const errorRate =
      (context.errorsEncountered.length /
        Math.max(context.actualSteps.length, 1)) *
      100;
  ...
  private identifyInefficiencies(
    ...
    if (metrics.toolEfficiency < 0.7) {
      inefficiencies.push(
        `Used ${context.toolsUsed.length} tools - could potentially be reduced`,
      );
    }

    if (metrics.contextSwitching > 3) {
      inefficiencies.push(
        `Too many context switches (${metrics.contextSwitching}) - approach lacked focus`,
      );
    }

    if (metrics.errorRate > 20) {
      inefficiencies.push(
        `High error rate (${metrics.errorRate.toFixed(1)}%) - better preparation needed`,
      );
    }

    if (context.filesAccessed.length > 10) {
      inefficiencies.push(
        `Accessed ${context.filesAccessed.length} files - could be more targeted`,
      );
    }
   ...
   */
  private async identifyBetterApproaches(
    context: ReflectionContext,
  ): Promise<string[]> {
    ...
    if (
      context.toolsUsed.includes("Edit") &&
      !context.toolsUsed.includes("Read")
    ) {
      betterApproaches.push(
        "Should have used Read tool before Edit to understand file structure",
      );
    }

    // Check if Glob could have been used instead of multiple Read calls
    const readCount = context.actualSteps.filter((step) =>
      step.includes("Read"),
    ).length;
    if (readCount > 3 && !context.toolsUsed.includes("Glob")) {
      betterApproaches.push(
        "Could have used Glob tool to find files more efficiently",
      );
    }

    // Check if Task tool could have been used for complex searches
    const grepCount = context.actualSteps.filter((step) =>
      step.includes("Grep"),
    ).length;
    if (grepCount > 5 && !context.toolsUsed.includes("Task")) {
      betterApproaches.push(
        "Could have used Task tool for complex search operations",
      );
    }
    ...
    try {
      ...
      learnings.forEach((learning) => {
        if (
          ...
          learning.solution_pattern !== context.initialApproach
        ) {
          betterApproaches.push(
            `Previous learning suggests: ${learning.solution_pattern}`,
          );
        }
      });
    } catch (error) {
      logger.warn("Could not fetch learnings for approach analysis", {
  ...
  private async generateImprovementInsights(
    ...
    if (analysis.overallEfficiencyScore < 70) {
      insights.push({
        ...
        improvementAction:
          analysis.approachImprovements[0] || "Focus on more direct approach",
        measurableGain: "Estimated 20-30% time reduction",
        confidenceLevel: 0.8,
        applicableContexts: [
          context.taskType,
          context.codebase.framework || "general",
        ].filter(Boolean),
        priority: "high",
        created_at: new Date(),
      });
    ...
    if (analysis.metrics.toolEfficiency < 0.7) {
      insights.push({
        ...
        improvementAction:
          analysis.toolUsageOptimizations[0] ||
          "Use fewer, more targeted tools",
    ...
    if (context.errorOccurred) {
      insights.push({
        ...
        improvementAction:
          "Apply better preparation and validation before execution",
   ...
   */
  private async saveInsightsToLearningSystem(
    ...
    for (const insight of insights) {
      try {
        ...
        logger.error(
          "Failed to save self-improvement insight",
          { insight },
  ...
  getReflectionStats(): {
    ...
    this.improvementInsights.forEach((insight) => {
      categoryCount.set(
        insight.category,
        (categoryCount.get(insight.category) || 0) + 1,
      );
    });
  ...
  private identifyMissedOpportunities(context: ReflectionContext): string[] {
    ...
    if (context.filesAccessed.length > 5) {
      const duplicateAccess = this.findDuplicateFileAccess(
        context.filesAccessed,
      );
      if (duplicateAccess.length > 0) {
        opportunities.push(
          "Could have cached file contents to avoid re-reading",
        );
      }
    }
  ...
  private identifyWastedSteps(context: ReflectionContext): string[] {
    ...
    if (!context.success && context.actualSteps.length > 5) {
      ...
      wastedSteps.push(
        ...lastSteps.map((step) => `Potentially wasted: ${step}`),
      );
    }
  ...
  private extractNewLearnings(context: ReflectionContext): string[] {
    ...
    if (context.success && context.completionQuality === "excellent") {
      learnings.push(
        `Successful approach for ${context.taskType}: ${context.initialApproach}`,
      );
    }
  ...
  private identifyChallengedAssumptions(context: ReflectionContext): string[] {
    ...
    if (
      context.alternativeApproaches &&
      context.alternativeApproaches.length > 0
    ) {
  ...
  private generateApproachImprovements(
    context: ReflectionContext,
    inefficiencies: string[],
  ): string[] {
  ...
  private generateToolOptimizations(
    context: ReflectionContext,
    metrics: EfficiencyMetrics,
  ): string[] {
    ...
    if (
      context.toolsUsed.includes("Grep") &&
      context.toolsUsed.includes("Read")
    ) {
      optimizations.push(
        "Consider using Grep with content output instead of separate Read calls",
      );
    }
...
export function withSelfReflection<
...
>(
  ...
  return (async (...args: unknown[]) => {
    ...
    try {
      ...
      claudeSelfReflection.logError(
        error instanceof Error ? error.message : String(error),
      );

      // Complete reflection with failure
      await claudeSelfReflection.completeReflection(false, "poor", "low");
