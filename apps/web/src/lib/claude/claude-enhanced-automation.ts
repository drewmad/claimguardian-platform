...
class ClaudeEnhancedAutomation {
  ...
  async applyAutoOptimizations(taskContext: TaskContext): Promise<{
    ...
    const availableRules =
      await this.getApplicableOptimizationRules(taskContext);
    ...
    for (const rule of availableRules) {
      ...
      if (rule.confidence >= 0.85 && rule.successRate >= 0.8) {
      ...
      else if (rule.confidence >= 0.7 && rule.successRate >= 0.65) {
        suggestedOptimizations.push(rule);
      }
      // Skip low-confidence rules with reason
      else {
        skipReasons.push(
          `Skipped ${rule.pattern}: confidence ${rule.confidence.toFixed(2)}, success rate ${rule.successRate.toFixed(2)}`,
        );
      }
    }
  ...
  private async getApplicableOptimizationRules(
    ...
    return [...existingRules, ...newRules].filter(
      (rule) =>
  ...
  private async generateOptimizationRules(
    ...
    if (
      taskContext.framework === "react" ||
      taskContext.framework === "next.js"
    ) {
      rules.push({
  ...
  async generateProactiveSuggestions(
    ...
    const bottleneckAnalysis =
      await claudeAdvancedAnalytics.identifyBottlenecks("week");
    ...
    if (prediction.successProbability < 0.75) {
      suggestions.push({
        ...
        priority: "high",
        suggestion:
          "Consider breaking this complex task into smaller, manageable components",
    ...
    const relevantBottlenecks = bottleneckAnalysis.bottlenecks.filter(
      (b) => b.category === "tool-usage" && b.priority === "high",
    );
    ...
    if (
      taskContext.framework === "next.js" &&
      taskContext.taskType === "code-generation"
    ) {
      suggestions.push({
        ...
        priority: "medium",
        suggestion:
          "Apply Next.js 15 App Router patterns for optimal performance",
        reasoning:
          "Project uses Next.js 15 - applying modern patterns improves performance and maintainability",
        estimatedImpact: 15,
    ...
    if (prediction.riskFactors.length > 2) {
      suggestions.push({
        ...
        priority: "high",
        suggestion:
          "Implement additional validation steps to prevent common errors",
   ...
   */
  async determineSmartDelegation(
    taskContext: TaskContext,
  ): Promise<SmartDelegation> {
    ...
    if (
      taskContext.taskType === "debugging" &&
      taskContext.complexity === "complex"
    ) {
      delegation.bestApproach =
        "Systematic elimination with comprehensive logging and incremental testing";
      delegation.recommendedTools = [
        "Read",
        "Grep",
        "Bash",
        "Edit",
    ...
    if (
      ...
      delegation.bestApproach =
        "Component-driven development with TypeScript-first design";
  ...
  private identifyContextFactors(taskContext: TaskContext): string[] {
    ...
    if (taskContext.framework)
      factors.push(`Framework: ${taskContext.framework}`);
    if (taskContext.codeLanguage)
      factors.push(`Language: ${taskContext.codeLanguage}`);
    if (taskContext.filePath) factors.push(`File context available`);
    if (taskContext.constraints)
      factors.push(
        `Constraints: ${Object.keys(taskContext.constraints).join(", ")}`,
      );
  ...
  async processBatchLearning(
    ...
    logger.info("Processing batch learning session", {
      sessionId,
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      try {
        ...
        const result = await this.executeTaskWithBatchLearning(
          task,
          priorLearnings,
        );
        ...
        batchResults.push({
          ...
          toolsUsed: [],
          learningsExtracted: [
            `Error pattern: ${error instanceof Error ? error.message : "Unknown error"}`,
          ],
        });
      }
    }
  ...
  private async executeTaskWithBatchLearning(
    ...
    const learningsExtracted = [
      ...
      ...priorLearnings
        .slice(0, 2)
        .map((learning) => `Batch insight: ${learning}`),
    ];

    return { toolsUsed, learningsExtracted };
  }

  private consolidateBatchLearnings(
    batchResults: BatchLearningSession["tasks"],
  ): string[] {
    const allLearnings = batchResults.flatMap(
      (task) => task.learningsExtracted,
    );
  ...
  private categorizeLearning(learning: string): string {
    ...
    if (learning.includes("error") || learning.includes("Error"))
  ...
  private identifyBatchPatterns(
    ...
    const successRate =
      batchResults.filter((task) => task.result === "success").length /
      batchResults.length;
    if (successRate > 0.8) {
      patterns.push(
        `High success rate (${(successRate * 100).toFixed(1)}%) indicates effective batch learning`,
      );
    }
    ...
    const mostUsedTool = Array.from(toolUsage.entries()).sort(
      (a, b) => b[1] - a[1],
    )[0];
    if (mostUsedTool) {
      patterns.push(
        `Most effective tool: ${mostUsedTool[0]} (used in ${mostUsedTool[1]} tasks)`,
      );
    }
    ...
    const isImproving = executionTimes
      .slice(-3)
      .every((time, i, arr) => i === 0 || time <= arr[i - 1]);

    if (isImproving) {
      patterns.push(
        `Execution time improving throughout batch (avg: ${avgTime.toFixed(0)}ms)`,
      );
    }
  ...
  private findBatchOptimizations(
    batchResults: BatchLearningSession["tasks"],
  ): string[] {
    const optimizations: string[] = [];

    // Find tool sequence optimizations
    const toolSequences = batchResults.map((task) =>
      task.toolsUsed.join(" -> "),
    );
    ...
    toolSequences.forEach((sequence) => {
      sequenceFrequency.set(
        sequence,
        (sequenceFrequency.get(sequence) || 0) + 1,
      );
    });

    const mostCommonSequence = Array.from(sequenceFrequency.entries()).sort(
      (a, b) => b[1] - a[1],
    )[0];

    if (mostCommonSequence && mostCommonSequence[1] > 1) {
      optimizations.push(
        `Optimal tool sequence identified: ${mostCommonSequence[0]}`,
      );
    }

    // Find learning application optimizations
    const learningsCount = batchResults.reduce(
      (sum, task) => sum + task.learningsExtracted.length,
      0,
    );
    if (learningsCount > batchResults.length * 2) {
      optimizations.push(
        "High learning extraction rate - consider applying to similar future tasks",
      );
    }

    // Find time-based optimizations
    const timeVariance = this.calculateVariance(
      batchResults.map((task) => task.executionTime),
    );
    if (timeVariance < 1000) {
      // Low variance in execution times
      optimizations.push(
        "Consistent execution times indicate stable, optimized approach",
      );
    }
  ...
  private async updateGlobalLearningsFromBatch(
    session: BatchLearningSession,
  ): Promise<void> {
    ...
    for (const optimization of session.optimizationsFound) {
      if (optimization.includes("tool sequence")) {
        ...
        const rule: AutoOptimizationRule = {
          ...
          successRate:
            session.tasks.filter((t) => t.result === "success").length /
  ...
  async getAutomationStats(): Promise<{
    ...
    return {
      ...
      avgSuccessRateImprovement:
        rules.reduce((sum, rule) => sum + rule.avgImprovementPercent, 0) /
          rules.length || 0,
