...
class ClaudeCompleteLearningSystem {
  private activeLearningContexts: Map<string, CompleteLearningContext> =
    new Map();
  ...
  async initializeLearning(
    ...
    const context: CompleteLearningContext = {
      ...
      complexity:
        options.complexity || this.determineComplexity(description, taskType),
      startTime: Date.now(),
      tools: options.tools || [],
    ...
    if (context.enableAutoReflection) {
      ...
      reflectionTriggers.startTaskTracking(
        validTaskType as any,
        context.complexity,
      );
    }
  ...
  async executeWithLearning<T>(
    ...
    try {
      ...
      if (context.enableAutoReflection) {
        ...
        const validTaskType =
          context.taskType === "other" ? "analysis" : context.taskType;
        const wrappedFn = autoReflect(
          validTaskType as any,
          context.complexity,
          fn,
        );
    ...
    const metrics = await this.calculateFinalMetrics(
      context,
      !!error,
      executionTime,
    );
  ...
  async logError(
    ...
    logger.warn("Error logged to learning system", {
      taskId,
      errorId,
      errorType,
    });
  ...
  async getLearningStats(): Promise<{
    ...
    const resolutionRate =
      totalErrors > 0 ? (totalResolved / totalErrors) * 100 : 0;
    ...
    return {
      ...
      efficiencyTrend:
        reflectionStats.averageEfficiency > 75 ? "improving" : "stable",
  ...
  private determineComplexity(
    ...
    if (
      taskType === "planning" ||
      complexKeywords.some((k) => lowerDesc.includes(k))
    ) {
      return "complex";
    }
  ...
  private displayLearningInsights(
    analysis: any,
    context: CompleteLearningContext,
  ) {
    ...
    console.log(
      `   Success Estimate: ${((analysis.estimatedSuccessRate || 0) * 100).toFixed(1)}%`,
    );
    ...
    if (analysis.recommendations?.length > 0) {
      ...
      analysis.recommendations.slice(0, 3).forEach((rec: any) => {
        console.log(
          `     • ${rec.category?.toUpperCase() || "GENERAL"}: ${rec.summary?.substring(0, 100) || "No summary"}...`,
        );
      });
    }
    ...
    console.log(
      `   💡 Suggested Approach: ${analysis.suggestedApproach?.substring(0, 100)}...`,
    );
    console.log("");
  }
  ...
  private async calculateFinalMetrics(
    ...
    const timeEfficiency = Math.max(
      0,
      Math.min(1, expectedTime / executionTime),
    );
    ...
    const toolEfficiency = Math.max(
      0,
      Math.min(1, expectedTools / context.tools.length),
    );
    ...
    const efficiencyScore = Math.round(
      ((timeEfficiency + toolEfficiency) / 2 - errorPenalty) * 100,
    );
  ...
  private getExpectedTimeForComplexity(
    complexity: "simple" | "medium" | "complex",
  ): number {
    switch (complexity) {
      case "simple":
  ...
  private getExpectedToolsForTask(
    taskType: ClaudeErrorContext["taskType"],
  ): number {
    switch (taskType) {
      case "code-generation":
        return 2; // Write, maybe Read
      case "file-modification":
        return 3; // Read, Edit, maybe Grep
      case "debugging":
        return 4; // Read, Grep, Edit, maybe Bash
      case "analysis":
        return 3; // Read, Grep, maybe Glob
      case "planning":
        return 2; // Read, maybe Write
      default:
        return 3;
    }
  }
 ...
 */
export function withCompleteLearning<
...
>(
  ...
  return (async (...args: unknown[]) => {
    ...
    const result = await completeLearningSystem.executeWithLearning(
      context,
      async () => {
