...
class ClaudeABTestingFramework {
  ...
  private config: ABTestConfig = {
    ...
    treatmentPercentage: parseFloat(
      process.env.CLAUDE_AB_TEST_PERCENTAGE || "0.5",
    ),
    minimumSampleSize: parseInt(process.env.CLAUDE_AB_MIN_SAMPLE_SIZE || "30"),
    confidenceLevel: parseFloat(
      process.env.CLAUDE_AB_CONFIDENCE_LEVEL || "0.95",
    ),
    maxSessionDuration: parseInt(
      process.env.CLAUDE_AB_MAX_SESSION_DURATION || "3600000",
    ), // 1 hour
    allowedTaskTypes: [
      "code-generation",
      "file-modification",
  ...
  async executeTaskWithABTesting<T>(
    ...
    try {
      if (session.group === "treatment") {
        ...
        const optimization =
          await claudeEnhancedAutomation.applyAutoOptimizations({
  ...
  async generateABTestReport(
    ...
    const treatmentSessions = recentSessions.filter(
      (s) => s.group === "treatment",
    );
    ...
    const performanceImprovement =
      controlMetrics.avgExecutionTime > 0
        ? ((controlMetrics.avgExecutionTime -
            treatmentMetrics.avgExecutionTime) /
            controlMetrics.avgExecutionTime) *
          100
        : 0;

    const errorReduction =
      controlMetrics.errorRate > 0
    ...
    const estimatedROI =
      (performanceImprovement / 100) * avgTasksPerHour * hourlyRate;
    ...
    if (!statisticalAnalysis.sampleSizeSufficient) {
      ...
      statisticalAnalysis.statisticalSignificance >= 0.95 &&
      ...
      recommendation =
        performanceImprovement > 0 ? "continue_testing" : "inconclusive";
    }
    ...
    return {
      ...
      treatmentGroup: {
        ...treatmentMetrics,
        avgOptimizations: this.average(
          treatmentSessions.map((s) => s.learningPatternsApplied),
        ),
        avgConfidence: this.average(
          treatmentSessions.flatMap((s) => s.confidenceScores),
        ),
      },
      statisticalAnalysis,
      businessMetrics: {
  ...
  private calculateGroupMetrics(sessions: ABTestSession[]) {
    ...
    const totalExecutionTime = sessions.reduce(
      (sum, s) => sum + s.totalExecutionTime,
      0,
    );
  ...
  private performStatisticalAnalysis(
    controlSessions: ABTestSession[],
    treatmentSessions: ABTestSession[],
  ) {
    ...
    const controlMean = this.average(
      controlSessions.map(
        (s) => s.totalExecutionTime / Math.max(s.taskCount, 1),
      ),
    );
    const treatmentMean = this.average(
      treatmentSessions.map(
        (s) => s.totalExecutionTime / Math.max(s.taskCount, 1),
      ),
    );

    const pooledStdDev = this.calculatePooledStdDev(
      controlSessions,
      treatmentSessions,
    );
    const effectSize =
      pooledStdDev > 0
        ? Math.abs(controlMean - treatmentMean) / pooledStdDev
        : 0;

    // Simplified significance calculation
    const statisticalSignificance = Math.min(
      0.99,
      Math.max(0, (effectSize * minSampleSize) / 100),
    );
  ...
  private calculatePooledStdDev(
    controlSessions: ABTestSession[],
    treatmentSessions: ABTestSession[],
  ): number {
    const controlTimes = controlSessions.map(
      (s) => s.totalExecutionTime / Math.max(s.taskCount, 1),
    );
    const treatmentTimes = treatmentSessions.map(
      (s) => s.totalExecutionTime / Math.max(s.taskCount, 1),
    );
    ...
    const variance =
      allTimes.reduce((sum, time) => sum + Math.pow(time - mean, 2), 0) /
      (allTimes.length - 1);
  ...
  private cleanupExpiredSessions(): void {
    ...
    if (expiredSessions.length > 0) {
      logger.info("Cleaned up expired A/B test sessions", {
