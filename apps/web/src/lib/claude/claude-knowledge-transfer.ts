...
class ClaudeKnowledgeTransfer {
  ...
  private async gatherLearnings(
    options: ExportOptions,
  ): Promise<ExportedLearning[]> {
    ...
    if (options.filterByConfidence) {
      filtered = filtered.filter(
        (l) => l.confidence >= options.filterByConfidence!,
      );
    }
    ...
    if (options.filterByDateRange) {
      filtered = filtered.filter((l) => {
        ...
        return (
          date >= options.filterByDateRange!.from &&
  ...
  private async gatherPatterns(
    ...
    if (options.filterByConfidence) {
      filtered = filtered.filter(
        (p) => p.confidence >= options.filterByConfidence!,
      );
    }
  ...
  private calculateStatistics(
    ...
    const avgConfidence =
      [...learnings, ...patterns].reduce(
        (sum, item) => sum + item.confidence,
        0,
      ) / (learnings.length + patterns.length || 1);

    const avgImpact =
      learnings.reduce((sum, l) => sum + l.impact, 0) / (learnings.length || 1);
      ...
      if (existing) {
        if (options.mergeStrategy === "skip") {
          ...
          conflicts.push({
            ...
            resolution:
              options.mergeStrategy === "merge" ? "merged" : "used_import",
          });
        }
      }
  ...
  private async mergeLearnings(
    existing: any,
    imported: ExportedLearning,
  ): Promise<void> {
  ...
  private calculateImportSummary(
    ...
    const categoriesAdded = Array.from(
      new Set([
    ...
    const avgTimeReduction =
      data.patterns.reduce((sum, p) => sum + p.impact.timeReduction, 0) /
  ...
  getTransferHistory(): {
    ...
    const exports = Array.from(this.exportCache.entries()).map(
      ([id, data]) => ({
        exportId: id,
