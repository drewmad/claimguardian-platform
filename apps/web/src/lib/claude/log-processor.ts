  ...
  try {
    ...
    for (const pattern of patterns) {
      if (pattern.frequency > 3 || pattern.severity === 'critical') {
        ...
        if (pattern.statusCode === 400 && pattern.endpoint.includes('user_profiles')) {
    ...
    logger.error('Failed to process error logs', {}, error instanceof Error ? error : new Error(String(error)))
    throw error
  }
}
