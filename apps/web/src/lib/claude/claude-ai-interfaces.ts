...
export abstract class AILearningService {
  ...
  ): Promise<NLGenerationResponse>;
  abstract simplifyPattern(
    pattern: SharedPattern,
    style: string,
  ): Promise<string>;
  abstract translateDescription(
    description: NLDescription,
    targetLanguage: string,
  ): Promise<NLDescription>;

  // Bottleneck Resolution methods
  abstract analyzeBottlenecks(
    request: BottleneckAnalysisRequest,
  ): Promise<BottleneckAnalysisResponse>;
  abstract generateResolutions(
    bottleneck: Bottleneck,
  ): Promise<ResolutionSuggestion[]>;
  abstract prioritizeActions(
    suggestions: ResolutionSuggestion[],
  ): Promise<ResolutionSuggestion[]>;
}
