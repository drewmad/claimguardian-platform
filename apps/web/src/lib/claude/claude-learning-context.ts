...
class ClaudeLearningContext {
  ...
  async analyzeTask(context: TaskContext): Promise<PreTaskAnalysis> {
    try {
      ...
      logger.error(
        "Failed to analyze task context",
        { taskContext: context },
      ...
      return {
        ...
        commonMistakes: [],
        suggestedApproach:
          "Follow established patterns and test changes incrementally",
  ...
  private assessRiskLevel(
    ...
    const highRiskPatterns = learnings.filter(
      (l) =>
        l.success_rate < 0.6 ||
    ...
    const complexityFactors = [
      ...
      context.taskType === "code-generation" &&
        context.codeLanguage === "typescript",
  ...
  private extractWarnings(
    ...
    if (context.previousAttempts && context.previousAttempts > 1) {
      warnings.push(
        "⚠️ Multiple previous attempts detected - review approach carefully",
      );
    }

    if (
      context.taskType === "file-modification" &&
      !context.tools.includes("Read")
    ) {
      warnings.push(
        "⚠️ File modification without Read tool - may cause edit failures",
      );
    }
  ...
  private generateSuggestedApproach(
    ...
    if (context.taskType === "code-generation") {
      approach +=
        " Start with simple implementation and add complexity gradually.";
    } else if (context.taskType === "file-modification") {
      approach +=
        " Always read the file first to understand current structure.";
  ...
  private estimateSuccessRate(
    ...
    const avgConfidence =
      learnings.reduce((sum, l) => sum + l.confidence_score, 0) /
      learnings.length;
    ...
    if (
      context.framework &&
      learnings.some((l) =>
        l.context_tags.includes(`framework:${context.framework}`),
      )
    )
      adjustment += 0.1;
    ...
    return Math.max(
      0.1,
      Math.min(0.95, (avgSuccessRate + avgConfidence) / 2 + adjustment),
    );
  }
  ...
  private calculateRelevanceScore(
    ...
    // Task type match
    if (learning.context_tags.includes(`task:${context.taskType}`))
      score += 0.4;

    // Language match
    if (
      context.codeLanguage &&
      learning.context_tags.includes(`lang:${context.codeLanguage}`)
    )
      score += 0.3;
    ...
    if (
      context.framework &&
      learning.context_tags.includes(`framework:${context.framework}`)
    )
      score += 0.2;
  ...
  private isActionable(
    ...
    const hasSpecificSteps =
      learning.solution_pattern.includes("Use ") ||
   ...
   */
  async recordSuccessfulApplication(
    ...
    try {
      ...
      logger.error(
        "Failed to record learning application",
        { taskContext },
        error instanceof Error ? error : new Error(String(error)),
      );
    }
...
export function useLearningContext(taskContext?: TaskContext) {
  ...
  const analyzeTask = React.useCallback(async (context: TaskContext) => {
    ...
    try {
      ...
      logger.error(
        "Learning context analysis failed",
        { context },
        error instanceof Error ? error : new Error(String(error)),
      );
      throw error;
...
>(taskContext: TaskContext, fn: T): T {
  return (async (...args: unknown[]) => {
    ...
    console.log(
      `🧠 Claude Learning Context Analysis for ${taskContext.taskType}:`,
    );
    console.log(`Risk Level: ${analysis.riskLevel}`);
    console.log(
      `Success Rate Estimate: ${(analysis.estimatedSuccessRate * 100).toFixed(1)}%`,
    );
    ...
    try {
      ...
      await claudeErrorLogger.logError(error as Error, {
        taskType: taskContext.taskType,
