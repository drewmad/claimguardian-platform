...
class ProductionErrorMonitor {
  ...
  async performDatabaseHealthCheck(): Promise<DatabaseHealthCheck[]> {
    ...
    for (const obj of criticalObjects) {
      try {
        ...
        logger.error(
          `Health check failed for ${obj.type} ${obj.name}`,
          {},
   ...
   */
  async analyzeProductionErrorPatterns(
    logEntries: unknown[],
  ): Promise<ProductionErrorPattern[]> {
    ...
    for (const entry of logEntries) {
      ...
      if (typedEntry.status_code === 404) {
        if (typedEntry.path.includes("policy_documents_extended")) {
          ...
          pattern.rootCause =
            "Missing learnings table or search_learnings function";
        ...
        if (typedEntry.path.includes("user_profiles")) {
          ...
          pattern.resolution =
            "Create missing user profile records, verify RLS policies";
        } else if (typedEntry.path.includes("storage")) {
          pattern.rootCause =
            "Storage bucket missing or RLS policy misconfigured";
          pattern.resolution =
            "Verify bucket exists and RLS policies allow user access";
        }
      }
    }
  ...
  startHealthMonitoring(intervalMinutes: number = 60): void {
      ...
      async () => {
        try {
          ...
          if (missingObjects.length > 0) {
            logger.warn(
              `Health Check Alert: ${missingObjects.length} database objects missing`,
              {
                missing: missingObjects.map(
                  (obj) => `${obj.objectType}:${obj.objectName}`,
                ),
              },
            );
          }
    ...
    logger.info(
      `Started database health monitoring (${intervalMinutes}min intervals)`,
    );
  }
  ...
  private determineSeverity(
    ...
    if (statusCode === 404) {
      if (
        endpoint.includes("learnings") ||
        endpoint.includes("policy_documents_extended")
      ) {
