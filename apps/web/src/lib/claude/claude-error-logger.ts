...
class ClaudeErrorLogger {
  ...
  async logError(
    ...
    console.error(`[CLAUDE ERROR] ${fullContext.taskType} - ${fullContext.mistakeCategory}:`, {
   ...
   */
  async resolveError(errorId: string, resolutionMethod: string, lessonLearned?: string): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to resolve Claude error', { errorId }, error instanceof Error ? error : new Error(String(error)))
    }
  }
   ...
   */
  async getRelevantLearnings(context: Partial<ClaudeErrorContext>): Promise<ClaudeLearning[]> {
    if (!this.isEnabled || !this.supabase) return []
   ...
   */
  async getErrorPatterns(timeRange: 'day' | 'week' | 'month' = 'week'): Promise<any[]> {
    if (!this.isEnabled || !this.supabase) return []
   ...
   */
  private extractErrorDetails(error: Error, context: ClaudeErrorContext): string {
    const details = {
...
export const claudeErrorHelpers = {
  ...
  codeGeneration: {
    syntaxError: (error: Error, filePath: string, language: string, description: string) =>
    ...
    logicError: (error: Error, filePath: string, description: string, correctApproach?: string) =>
      claudeErrorLogger.logError(error, {
        taskType: 'code-generation',
        taskDescription: description,
  ...
  fileModification: {
    ...
    typeError: (error: Error, filePath: string, language: string, description: string) =>
      claudeErrorLogger.logError(error, {
        taskType: 'file-modification',
        taskDescription: description,
  ...
  // Analysis errors
  analysis: {
    misunderstanding: (description: string, userIntent: string, mistakeCategory: string) =>
      claudeErrorLogger.logError(new Error('Analysis misunderstanding'), {
        taskType: 'analysis',
        taskDescription: description,
        userIntent,
    ...
    incompleteAnalysis: (description: string, missingAspect: string) =>
      claudeErrorLogger.logError(new Error('Incomplete analysis'), {
        taskType: 'analysis',
        taskDescription: description,
