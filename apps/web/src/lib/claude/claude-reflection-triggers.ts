...
class ReflectionTriggers {
  ...
  private initializeDefaultTriggers() {
    this.triggers = [
      ...
      },
      {
        ...
        condition: (ctx) =>
          ctx.complexity === "complex" && ctx.success === false,
        priority: "high",
      ...
      },
      {
        ...
        priority: "medium",
        description:
          "Trigger reflection for successful but potentially inefficient tasks",
        enabled: true,
      },
    ];
  }
  ...
  startTaskTracking(
    ...
    logger.info("Task tracking started for reflection triggers", {
      taskId,
      taskType,
    });
    return taskId;
  ...
  async completeTaskTracking(success: boolean): Promise<boolean> {
    ...
    const triggeredReflections = this.triggers
      .filter(
        (trigger) => trigger.enabled && trigger.condition(this.activeTask!),
      )
      .sort(
        (a, b) =>
  ...
  private async executeReflection(trigger: ReflectionTrigger): Promise<void> {
    ...
    this.activeTask.toolsUsed.forEach((tool) => {
      claudeSelfReflection.logStep(
        `Used ${tool} tool`,
        "Tool usage during task execution",
        tool,
      );
    });
  ...
  private determineTaskQuality(
    ...
    const isEfficient =
      duration < 2 * 60 * 1000 &&
      context.toolsUsed.length <= 3 &&
      context.errorsCount === 0;
    const isGood =
      duration < 5 * 60 * 1000 &&
      context.toolsUsed.length <= 5 &&
      context.errorsCount <= 1;
  ...
  getTriggerStats(): {
    ...
    return {
      ...
      highPriorityTriggers: this.triggers.filter(
        (t) => t.enabled && t.priority === "high",
      ).length,
...
export function autoReflect<T extends (...args: unknown[]) => Promise<any>>(
  ...
  return (async (...args: unknown[]) => {
    ...
    console.log = (...logArgs: unknown[]) => {
      stepCount++;
      if (stepCount % 5 === 0) {
        // Log every 5th console.log as a step
