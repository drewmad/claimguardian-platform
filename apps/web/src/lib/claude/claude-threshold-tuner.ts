...
class ClaudeThresholdTuner {
  private config: TuningConfig = {
    ...
    maxThresholdChange: parseFloat(process.env.CLAUDE_MAX_THRESHOLD_CHANGE || '0.1'),
    validationPeriod: parseInt(process.env.CLAUDE_VALIDATION_PERIOD || '24'), // 1 day
    rollbackThreshold: parseFloat(process.env.CLAUDE_ROLLBACK_THRESHOLD || '0.05') // 5% performance drop
  }
  ...
  private analyzeThresholdPerformance(metrics: unknown[], threshold: number) {
    ...
    const falseNegativeRate = (falseNegatives + truePositives) > 0 ? falseNegatives / (falseNegatives + truePositives) : 0
  ...
  private generateThresholdRecommendation(
    ...
    const expectedImprovement = ((optimalAnalysis.f1Score - currentAnalysis.f1Score) / currentAnalysis.f1Score) * 100
    const thresholdChange = Math.abs(optimalAnalysis.threshold - currentThreshold)

    // Determine confidence in recommendation
    let confidence = 0.5
    if (optimalAnalysis.sampleSize >= this.config.minimumSampleSize) confidence += 0.2
    ...
    if (thresholdChange <= 0.05 && expectedImprovement > 0) riskAssessment = 'low'
    else if (thresholdChange > 0.15 || expectedImprovement < -2) riskAssessment = 'high'
  ...
  async autoTuneThreshold(): Promise<{
    ...
    if (!this.config.enabled) {
      return {
        ...
        previousThreshold: parseFloat(process.env.CLAUDE_CONFIDENCE_THRESHOLD || '0.8'),
        newThreshold: parseFloat(process.env.CLAUDE_CONFIDENCE_THRESHOLD || '0.8'),
    ...
    return {
      ...
      newThreshold: recommendedThreshold,
      expectedImprovement,
      reason: 'Automatic tuning - performance improvement opportunity identified'
    }
  }
  ...
  async validateThresholdChange(
    ...
    const now = new Date()
    const hoursSinceChange = (now.getTime() - changeTimestamp.getTime()) / (1000 * 60 * 60)
    ...
    const performanceChange = ((afterMetrics.f1Score - beforeMetrics.f1Score) / beforeMetrics.f1Score) * 100
  ...
  getTuningStatus(): {
    ...
    if (this.config.enabled && this.lastTuningTimestamp) {
      nextScheduledTuning = new Date(
        this.lastTuningTimestamp.getTime() + this.config.retuningInterval * 60 * 60 * 1000
      )
    }
  ...
  updateConfig(newConfig: Partial<TuningConfig>): void {
    ...
    logger.info('Threshold tuning configuration updated', { config: this.config })
  }
}
