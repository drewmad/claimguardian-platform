...
export class APIKeyManager {
   ...
   */
  async validateAPIKey(apiKey: string): Promise<{ userId: string; keyId: string; permissions: string[] } | null> {
    try {
      const supabase = await createClient()
  ...
  async getUsageStats(userId: string, days: number = 30): Promise<APIUsageStats> {
    try {
      ...
      const stats: APIUsageStats = {
        ...
        successful_requests: overallStats?.filter(s => s.status_code >= 200 && s.status_code < 400).length || 0,
        failed_requests: overallStats?.filter(s => s.status_code >= 400).length || 0,
        avg_response_time: overallStats && overallStats.length > 0 ? overallStats.reduce((sum, s) => sum + (s.response_time_ms || 0), 0) / overallStats.length : 0,
   ...
   */
  async updateAPIKeyPermissions(userId: string, keyId: string, permissions: string[]): Promise<void> {
    try {
      const supabase = await createClient()
  ...
  private aggregateEndpoints(endpointData: Array<{ endpoint: string }>): Array<{ endpoint: string; count: number }> {
    const counts = endpointData.reduce((acc, { endpoint }) => {
  ...
  private aggregateDailyUsage(usageData: Array<{ created_at: string }>): Array<{ date: string; count: number }> {
    const dailyCounts = usageData.reduce((acc, { created_at }) => {
