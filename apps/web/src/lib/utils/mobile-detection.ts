...
export class MobileDetectionService {
   ...
   */
  static async canDoFieldWork(): Promise<{
    suitable: boolean;
    issues: string[];
  }> {
  ...
  private static showPWAInstallBanner(
    deferredPrompt: BeforeInstallPromptEvent | null,
  ) {
    const banner = document.createElement("div");
    banner.id = "pwa-install-banner";
    banner.className =
      "fixed bottom-4 left-4 right-4 bg-blue-600 text-white p-4 rounded-lg shadow-lg z-50 flex items-center justify-between";
    banner.innerHTML = `
      <div>
  ...
  static async getDeviceRecommendations(): Promise<string[]> {
    ...
    if (!device.hasGPS) {
      recommendations.push(
        "Enable location services for automatic property location",
      );
    }
   ...
   */
  static setupOrientationListener(
