...
export class AIContentValidationService {
  ...
  async validateRealAPIUsage(config: RealAPITestConfig): Promise<ValidationReport> {
    ...
    if (totalCost > config.maxTestCost) {
      console.warn(`⚠️ Total test cost ($${totalCost.toFixed(4)}) exceeded limit ($${config.maxTestCost})`)
    }

    const endTime = Date.now()
    const report = this.generateValidationReport(config, startTime, endTime, results, totalCost)
    ...
    console.log(`📊 Success rate: ${report.summary.successfulTests}/${report.summary.totalTests} (${((report.summary.successfulTests / report.summary.totalTests) * 100).toFixed(1)}%)`)
  ...
  private async testOpenAI(config: RealAPITestConfig): Promise<APIValidationResult[]> {
    const results: APIValidationResult[] = []
  ...
  private async testOpenAIModel(model: string, prompt: string): Promise<APIValidationResult> {
    ...
    const actualCost = this.calculateOpenAICost(model, usage.prompt_tokens, usage.completion_tokens)
    const costAccuracy = Math.abs(1 - (Math.abs(actualCost - estimatedCost) / actualCost)) * 100
  ...
  private async testGemini(config: RealAPITestConfig): Promise<APIValidationResult[]> {
    const results: APIValidationResult[] = []
  ...
  private async testGeminiModel(model: string, prompt: string): Promise<APIValidationResult> {
    ...
    const actualCost = this.calculateGeminiCost(model, inputTokens, outputTokens)
    const costAccuracy = Math.abs(1 - (Math.abs(actualCost - estimatedCost) / actualCost)) * 100
  ...
  private calculateGeminiCost(model: string, inputTokens: number, outputTokens: number): number {
  ...
  private generateValidationReport(
    ...
    const providerCosts = results.reduce((acc, r) => {
      if (r.success) {
    ...
    const modelPerformance: Record<string, { averageLatency: number; costAccuracy: number; reliability: number }> = {}
    const modelGroups = results.reduce((acc, r) => {
    ...
    const recommendations = this.generateRecommendations(results, modelPerformance, averageCostAccuracy)
  ...
  private generateRecommendations(
    results: APIValidationResult[],
    modelPerformance: Record<string, { averageLatency: number; costAccuracy: number; reliability: number }>,
    averageCostAccuracy: number
  ): string[] {
    ...
    if (averageCostAccuracy < 90) {
      recommendations.push('Consider improving token estimation algorithms - cost accuracy is below 90%')
    } else if (averageCostAccuracy > 95) {
      recommendations.push('Excellent cost estimation accuracy - current implementation is reliable')
    }

    // Model performance recommendations
    const sortedModels = Object.entries(modelPerformance)
      .sort((a, b) => (b[1].reliability * b[1].costAccuracy) - (a[1].reliability * a[1].costAccuracy))

    if (sortedModels.length > 0) {
      recommendations.push(`Best performing model: ${sortedModels[0][0]} (${sortedModels[0][1].reliability.toFixed(1)}% reliability, ${sortedModels[0][1].costAccuracy.toFixed(1)}% cost accuracy)`)
    }
    ...
    const costPerToken = Object.entries(providerCosts).map(([provider, data]) => ({
      provider,
      costPerToken: data.tokens > 0 ? data.cost / data.tokens : 0
    })).sort((a, b) => a.costPerToken - b.costPerToken)

    if (costPerToken.length > 1) {
      recommendations.push(`Most cost-effective provider: ${costPerToken[0].provider} ($${(costPerToken[0].costPerToken * 1000).toFixed(6)}/1k tokens)`)
    }

    // Error rate recommendations
    const errorRate = (results.filter(r => !r.success).length / results.length) * 100
    if (errorRate > 10) {
      recommendations.push(`High error rate detected (${errorRate.toFixed(1)}%) - check API key configuration and rate limits`)
    }

    // Latency recommendations
    const avgLatency = results.filter(r => r.success).reduce((sum, r) => sum + r.latency, 0) / results.filter(r => r.success).length
    if (avgLatency > 5000) {
      recommendations.push(`High average latency detected (${avgLatency.toFixed(0)}ms) - consider implementing request caching or optimization`)
    }

    return recommendations
  }

  private async recordValidationResults(report: ValidationReport): Promise<void> {
    try {
  ...
  async validateAllAIFeatures(): Promise<ValidationReport[]> {
    ...
    results.forEach((report, index) => {
      const featureNames = ['Damage Analyzer', 'Policy Advisor', 'Inventory Scanner']
      console.log(`  ${featureNames[index]}: $${report.totalCost.toFixed(6)} (${report.summary.successfulTests}/${report.summary.totalTests} passed, ${report.summary.averageCostAccuracy.toFixed(1)}% accuracy)`)
    })
