...
export class RedisCacheManager {
  ...
  private async initializeRedisClient(): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to initialize Redis client, using fallback cache', error)
      this.client = null
    }
  }
  ...
  async ttl(key: string): Promise<number> {
    ...
    try {
      ...
      const fallbackItem = this.fallbackCache.get(fullKey)
      if (fallbackItem) {
        return Math.max(0, Math.floor((fallbackItem.expiresAt - Date.now()) / 1000))
      }
...
export class CachePatterns {
  ...
  static geoKey(type: 'property' | 'weather' | 'risk', identifier: string): string {
