...
export function withCacheMiddleware(
  ...
  return async function (
    ...
    const { endpoint, skipCache, customKey, customTTL, invalidateOnMutate } =
      options;
    ...
    try {
      ...
      const cacheKey =
        customKey || generateAPIKey(`${endpoint}_${method}`, params, userId);
      ...
      if (response.status >= 200 && response.status < 300) {
        try {
          ...
          console.warn(
            `[Cache] Could not cache response for ${endpoint}:`,
            error,
          );
        }
      }
      ...
      console.error(
        `[Cache] Error in cache middleware for ${endpoint}:`,
        error,
      );
