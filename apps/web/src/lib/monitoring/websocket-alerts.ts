...
export class WebSocketAlertManager {
  ...
  private async handleAlert(alert: WebSocketAlert): Promise<void> {
    ...
    console.log(`📤 Alert delivered to ${successfulDeliveries}/${eligibleChannels.length} channels`)
  }
  ...
  private async deliverAlert(alert: WebSocketAlert, channel: AlertChannel): Promise<void> {
    try {
      switch (channel.type) {
        case 'slack':
  ...
  private async deliverToSlack(alert: WebSocketAlert, channel: AlertChannel): Promise<void> {
  ...
  private async deliverToDiscord(alert: WebSocketAlert, channel: AlertChannel): Promise<void> {
    const webhookUrl = channel.config.webhookUrl as string
  ...
  private async deliverToEmail(alert: WebSocketAlert, channel: AlertChannel): Promise<void> {
    ...
    console.log(`📧 Email would be sent to ${emailPayload.to}:`, emailPayload.subject)
  }

  private async deliverToWebhook(alert: WebSocketAlert, channel: AlertChannel): Promise<void> {
  ...
  private async deliverToSMS(alert: WebSocketAlert, channel: AlertChannel): Promise<void> {
  ...
  private async deliverToPagerDuty(alert: WebSocketAlert, channel: AlertChannel): Promise<void> {
    ...
    console.log(`📟 PagerDuty event would be triggered:`, payload.payload.summary)
  }
  ...
  private performHealthCheck(): void {
    ...
    console.log(`💓 WebSocket service health: ${health.status} (${metrics.activeConnections} connections, ${metrics.errorRate.toFixed(2)} err/min)`)
  }
  ...
  private getSeverityColorInt(severity: WebSocketAlert['severity']): number {
    switch (severity) {
      case 'emergency': return 0x8B0000
      case 'critical': return 0xDC143C
      case 'warning': return 0xFF8C00
      case 'info': return 0x4169E1
...
`.trim()
  ...
  getAlertStats(): {
    ...
    this.alertHistory.forEach(entry => {
      // Count by type
      alertsByType[entry.alert.type] = (alertsByType[entry.alert.type] || 0) + 1
      
      // Count by severity
      alertsBySeverity[entry.alert.severity] = (alertsBySeverity[entry.alert.severity] || 0) + 1
    ...
    return {
      ...
      alertsByType,
      alertsBySeverity,
      deliverySuccessRate: totalAlerts > 0 ? (successfulDeliveries / totalAlerts) * 100 : 0,
      channelStats
    }
  }

  // Manual alert testing
  async testAlert(channelName?: string, severity: WebSocketAlert['severity'] = 'info'): Promise<void> {
