...
export class WebSocketMonitor extends EventEmitter {
  ...
  private calculateServiceMetrics(): void {
    ...
    // Connection metrics
    this.serviceMetrics.activeConnections = connections.filter(
    ...
    const recentConnections = connections.filter(
      (c) => c.connectedAt > oneMinuteAgo,
    );
    const recentDisconnections = connections.filter(
      (c) => c.status === "disconnected" && c.lastActivity > oneMinuteAgo,
    );
    ...
    this.serviceMetrics.messageRate =
      (totalMessages / (now - this.serviceMetrics.uptime)) * 1000; // per second
    this.serviceMetrics.errorRate =
      totalErrors / ((now - this.serviceMetrics.uptime) / 60000); // per minute
    ...
    const totalBytes = connections.reduce(
      (sum, c) => sum + c.bytesTransferred,
      0,
    );
    this.serviceMetrics.bandwidth =
      (totalBytes / (now - this.serviceMetrics.uptime)) * 1000; // bytes per second
  ...
  private checkConnectionAlerts(metrics: WebSocketMetrics): void {
    ...
    if (metrics.errors > 10) {
      // More than 10 errors per connection
      this.emitAlert({
        type: "error_rate",
        severity: "warning",
  ...
  private checkErrorRateAlerts(): void {
    if (
      this.serviceMetrics.errorRate > this.alertThresholds.errorRateThreshold
    ) {
      this.emitAlert({
        type: "error_rate",
        severity: "critical",
  ...
  private emitAlert(alert: WebSocketAlert): void {
    ...
    // Log alert
    console.log(
      `🚨 WebSocket Alert [${alert.severity.toUpperCase()}]: ${alert.message}`,
    );
  }
  ...
  getMetricsHistory(
    duration?: number,
  ): Array<{ timestamp: number; metrics: WebSocketServiceMetrics }> {
