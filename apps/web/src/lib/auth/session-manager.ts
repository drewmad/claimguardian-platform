...
class SessionManager {
  ...
  async startMonitoring() {
    ...
    if (this.refreshTimer || this.warningTimer) {
      logger.warn(
        "Session monitoring already active, skipping duplicate start",
      );
      return;
    }
  ...
  private scheduleRefresh(expiresAt: number) {
    ...
    if (timeUntilRefresh > 300) {
      ...
      logger.warn(
        "Session expires very soon, skipping refresh to avoid logout loop",
        {
          timeUntilExpiry: Math.round(timeUntilExpiry / 60),
        },
      );
  ...
  private async refreshSession() {
    ...
    if (now - this.lastRefreshAttempt < 60000) {
      logger.warn("Refresh attempted too soon, rate limiting", {
        timeSinceLastAttempt: Math.round(
    ...
    try {
      ...
      logger.warn(
        "Session refresh failed, will retry on next scheduled refresh",
      );
    } finally {
