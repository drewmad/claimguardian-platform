...
class SocialAuthService {
  ...
  async getConnectedAccounts(): Promise<SocialAccount[]> {
    try {
      ...
      const socialProviders = user.identities.filter((identity) =>
      ...
      return socialProviders.map((identity) => ({
        ...
        name:
          ...
          "",
        picture:
          identity.identity_data?.picture || identity.identity_data?.avatar_url,
        isConnected: true,
  ...
  private extractProviderFromUser(user: any): string {
    ...
    if (lastIdentity?.provider) {
      const config = this.getProviderConfig(
        lastIdentity.provider as SocialProvider,
      );
  ...
  private formatAuthError(message: string): string {
    ...
    const errorMappings: Record<string, string> = {
      ...
      invalid_request:
        "There was an issue with the login request. Please try again.",
      server_error:
