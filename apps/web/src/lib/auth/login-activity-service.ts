  ...
  async getLoginStats(userId: string): Promise<LoginActivityStats> {
    try {
      ...
      const uniqueDevices = new Set(
        data.map(
          (activity) =>
            `${activity.device_type}-${activity.browser}-${activity.os}`,
        ),
      ).size;
  ...
  async checkSuspiciousActivity(userId: string): Promise<{
    ...
    try {
      ...
      const rapidLogins = recentActivity.filter((activity, index) => {
        ...
        const timeDiff =
          new Date(recentActivity[index - 1].created_at).getTime() -
      ...
      logger.error(
        "Error checking suspicious activity",
        { userId },
        err instanceof Error ? err : new Error(String(err)),
      );
