...
class SecurityQuestionsService {
  ...
  async getUserQuestions(userId: string): Promise<UserSecurityAnswer[]> {
    try {
      ...
      if (error) {
        logger.error(
          "Failed to fetch user security questions",
          {},
          error instanceof Error ? error : new Error(String(error)),
        );
        throw error;
      ...
      return (
        (data as unknown as UserSecurityAnswerRow[])?.map((item) => ({
          id: item.id,
      ...
      logger.error(
        "Error fetching user security questions",
        { userId },
        error instanceof Error ? error : new Error(String(error)),
      );
      throw error;
  ...
  async saveAnswers(
    ...
    try {
      ...
      if (deleteError) {
        logger.error(
          "Failed to delete existing security answers",
          { userId },
      ...
      if (insertError) {
        logger.error(
          "Failed to save security answers",
          { userId },
      ...
      logger.info("Security answers saved successfully", {
        userId,
  ...
  async verifyAnswer(
    ...
    try {
      ...
      if (error || !data) {
        logger.error(
          "Failed to fetch security answer",
          {},
          error instanceof Error ? error : new Error(String(error)),
        );
        return false;
  ...
  async verifyAnswers(
    ...
    try {
      ...
      logger.error(
        "Error verifying multiple security answers",
        { userId },
        error instanceof Error ? error : new Error(String(error)),
      );
      return false;
  ...
  async hasSecurityQuestions(userId: string): Promise<boolean> {
    try {
      ...
      if (error) {
        logger.error(
          "Failed to check security questions",
          {},
          error instanceof Error ? error : new Error(String(error)),
        );
        return false;
