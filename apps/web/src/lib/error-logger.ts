...
export class AIErrorLogger {
  ...
  private showUserNotification(
    ...
    switch (severity) {
      case "critical":
        toast.error(
          `${feature} is currently unavailable. Please try again later.`,
          {
  ...
  async logPerformance(
    ...
    if (duration > 30000) {
      // 30 seconds
      logger.warn(
        `Slow performance detected: ${feature} ${action} took ${duration}ms`,
      );
    }
  }
}
...
export const aiErrorHelpers = {
  damageAnalyzer: {
    log: (
      ...
      errorLogger.logError(
        error,
        errorLogger.getAIContext("DamageAnalyzer", action, userId, model),
        "medium",
      ),
    ...
    logAPIError: (
      ...
      errorLogger.logAPIError(
        ...
        status,
        message,
        errorLogger.getAIContext("DamageAnalyzer", "API Call", userId, model),
      ),
    ...
    logModelError: (
      ...
      errorLogger.logAIModelError(
        model,
        prompt,
        error,
        errorLogger.getAIContext(
          "DamageAnalyzer",
          "Model Analysis",
          userId,
          model,
  ...
  policyChat: {
    log: (
      ...
      errorLogger.logError(
        error,
        errorLogger.getAIContext("PolicyChat", action, userId, model),
        "medium",
      ),
    ...
    logAPIError: (
      ...
      errorLogger.logAPIError(
        ...
        status,
        message,
        errorLogger.getAIContext("PolicyChat", "API Call", userId, model),
      ),
    ...
    logModelError: (
      ...
      errorLogger.logAIModelError(
        ...
        prompt,
        error,
        errorLogger.getAIContext("PolicyChat", "Chat Response", userId, model),
      ),
  },
  ...
  inventoryScanner: {
    log: (
      ...
      errorLogger.logError(
        error,
        errorLogger.getAIContext("InventoryScanner", action, userId, model),
        "medium",
      ),
    ...
    logAPIError: (
      ...
      errorLogger.logAPIError(
        ...
        status,
        message,
        errorLogger.getAIContext("InventoryScanner", "API Call", userId, model),
      ),
