...
export class EnhancedDocumentExtractor {
  ...
  async categorizeAndScoreEvidence(
    ...
    try {
      ...
      logger.warn("Evidence categorization failed", {
        error: toError(error),
        fileName,
      });
      return {
        ...
        qualityScore: 50,
        suggestions: [
          "Could not automatically analyze - manual review recommended",
        ],
      };
    }
  }
    ...
    try {
      ...
      if (extractionResult.success && extractionResult.data) {
        ...
        if (options.enrichWithPublicData) {
          extractionResult.data = await this.enrichExtractedData(
            extractionResult.data,
          );
        }

        // Calculate overall confidence based on field completeness
        extractionResult.confidence = this.calculateConfidence(
          extractionResult.data,
        );
      }
    ...
    return {
      policyInformation: {
        policyNumber: {
          type: "string",
          required: true,
          patterns: ["Policy Number", "Policy #", "Contract Number"],
        },
        carrierName: {
          type: "string",
          required: true,
          patterns: ["Insurance Company", "Carrier", "Insurer"],
        },
        policyType: {
          type: "enum",
          values: [
          ...
          patterns: ["Policy Form", "Coverage Type"],
        },
      },
      coverageAmounts: {
        dwellingCoverage: {
          type: "number",
          patterns: ["Coverage A", "Dwelling", "Building Coverage"],
        },
        personalPropertyCoverage: {
          type: "number",
          patterns: ["Coverage C", "Personal Property", "Contents"],
        },
        liabilityCoverage: {
          type: "number",
          patterns: ["Coverage E", "Personal Liability"],
        },
      },
      deductibles: {
        allPerilsDeductible: {
          type: "number",
          patterns: ["All Other Perils", "Deductible"],
        },
        hurricaneDeductible: {
          type: "string",
          patterns: ["Hurricane Deductible", "Named Storm"],
        },
        windHailDeductible: {
          type: "string",
          patterns: ["Wind/Hail", "Windstorm"],
        },
      },
      dates: {
        effectiveDate: {
          type: "date",
          patterns: ["Effective Date", "Policy Period From", "Coverage Begins"],
        },
        expirationDate: {
          type: "date",
          patterns: ["Expiration Date", "Policy Period To", "Coverage Ends"],
        },
      },
      premiums: {
        annualPremium: {
          type: "number",
          patterns: ["Annual Premium", "Total Premium", "Policy Premium"],
        },
        monthlyPremium: {
          type: "number",
          patterns: ["Monthly Premium", "Installment Amount"],
        },
      },
    };
  }
   ...
   */
  validateExtractedData(data: ExtractedPolicyDataEnhanced): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    if (data.effectiveDate && data.expirationDate) {
      ...
      const daysDiff =
        (expiration.getTime() - effective.getTime()) / (1000 * 60 * 60 * 24);
    ...
    if (data.annualPremium && data.annualPremium < 500) {
      errors.push(
        "Annual premium seems unusually low for Florida property insurance",
      );
    }
    ...
    if (
      ...
      data.propertyAddress.state !== "FL" &&
      data.propertyAddress.state !== "Florida"
    ) {
  ...
  private async enrichExtractedData(
    data: ExtractedPolicyDataEnhanced,
  ): Promise<ExtractedPolicyDataEnhanced> {
    try {
      ...
      if (data.propertyAddress) {
        ...
        const { data: parcel } = await this.supabase
          ...
          .select(
            "PARCEL_ID, CO_NO, ACT_YR_BLT, TOT_LVG_AREA, LAND_VAL, BLDG_VAL",
          )
