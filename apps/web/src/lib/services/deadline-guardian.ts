...
export class DeadlineGuardianService {
   ...
   */
  async analyzeDocumentForDeadlines(
    documentText: string,
    documentId: string,
    userId: string,
  ): Promise<Deadline[]> {
   ...
   */
  private fallbackDeadlineDetection(
    documentText: string,
    documentId: string,
    userId: string,
  ): Deadline[] {
        ...
        if (potentialDate && isAfter(potentialDate, new Date())) {
          deadlines.push({
            ...
            metadata: {
              ...
              keywords_found: pattern.keywords.filter((keyword) =>
                text.includes(keyword),
              ),
            },
   ...
   */
  private processDeadlineAnalysis(
    analysis: any,
    documentId: string,
    userId: string,
  ): Deadline[] {
   ...
   */
  private selectAlertChannel(
    alertType: DeadlineAlert["alert_type"],
    priority: Deadline["priority"],
  ): DeadlineAlert["channel"] {
    ...
    try {
      ...
      deadlines.forEach((deadline) => {
        ...
        if (deadline.status in stats) {
          (stats as any)[deadline.status] =
            ((stats as any)[deadline.status] || 0) + 1;
        }
        // Update priority counts
        if (deadline.priority in stats.by_priority) {
          stats.by_priority[
            deadline.priority as keyof typeof stats.by_priority
          ]++;
        }
      });
