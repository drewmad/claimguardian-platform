...
class AIDocumentExtractionService {
  ...
  private async mockExtraction(): Promise<ExtractionResult> {
    ...
    const mockData: ExtractedPolicyData = {
      policyNumber: `HO-${Math.floor(Math.random() * 1000000)
        .toString()
        .padStart(6, "0")}`,
      carrierName: [
   ...
   */
  validateExtractedData(data: ExtractedPolicyData): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    if (
      data.coverageAmount &&
      (data.coverageAmount < 10000 || data.coverageAmount > 10000000)
    ) {
    ...
    if (
      data.deductible &&
      data.coverageAmount &&
      data.deductible > data.coverageAmount * 0.1
    ) {
