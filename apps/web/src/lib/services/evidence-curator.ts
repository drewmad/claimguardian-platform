...
Provide JSON response with:
{
  ...
  private async saveEvidenceItem(item: EvidenceItem): Promise<boolean> {
    try {
      const { error } = await this.supabase.from("evidence_items").insert(item);
  ...
  async identifyEvidenceGaps(claimId: string): Promise<EvidenceGap[]> {
    try {
      ...
      for (const [category, requirements] of Object.entries(requiredEvidence)) {
        const existingInCategory = existingEvidence.filter(
        ...
        if (existingInCategory.length === 0) {
          ...
          const avgQuality =
            existingInCategory.reduce((sum, e) => sum + e.quality_score, 0) /
   ...
   */
  private getRequiredEvidenceByClaimType(
    claimType: string,
    ...
    return (
      requirements[claimType as keyof typeof requirements] ||
   ...
   */
  async createEvidenceWorkflow(
    claimId: string,
    claimType: string,
  ): Promise<EvidenceWorkflow | null> {
    try {
      const template =
        this.WORKFLOW_TEMPLATES[
          claimType as keyof typeof this.WORKFLOW_TEMPLATES
        ] || this.WORKFLOW_TEMPLATES.hurricane;
  ...
  private generateStepGuidance(
    ...
    const guidance = {
      damage_photos:
        "Take clear, well-lit photos from multiple angles. Include close-ups of specific damage and wide shots for context. Ensure good lighting and sharp focus.",
      receipts_invoices:
        "Collect all financial documentation related to the claim. Include receipts, invoices, and proof of payments. Ensure all text is clearly readable.",
      estimates_quotes:
        "Obtain detailed written estimates from licensed contractors. Include itemized breakdowns and professional assessments.",
      weather_reports:
        "Gather official weather data and reports from the National Weather Service or local meteorological agencies.",
      legal_documents:
        "Collect all official reports and legal documentation related to the incident.",
    };

    return (
      guidance[category as keyof typeof guidance] ||
      "Follow standard documentation procedures for this evidence type."
    );
  }
  ...
  async updateWorkflowProgress(
    claimId: string,
    completedCategory: EvidenceCategory,
  ): Promise<void> {
    try {
      ...
      const updatedSteps = workflow.steps.map((step: WorkflowStep) => {
        if (
          step.category === completedCategory &&
