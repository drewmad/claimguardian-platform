...
export class IntelligentDocumentSearch {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (error) {
        logger.warn(
          "Vector extension not available, falling back to keyword search",
        );
      } else {
    ...
    keywordResults.forEach((result) => {
      if (resultMap.has(result.id)) {
        ...
        existing.relevanceScore =
          (existing.relevanceScore + result.relevanceScore * 0.3) / 1.3;
        existing.highlights = [
  ...
  private applyFilters(
    ...
    if (filters.propertyId) {
      filteredResults = filteredResults.filter(
        (result) => result.metadata.propertyId === filters.propertyId,
      );
    }

    if (filters.claimId) {
      filteredResults = filteredResults.filter(
        (result) => result.metadata.claimId === filters.claimId,
      );
    }
  ...
  private generateMockSummary(content: string, documentType: string): string {
    const summaries = {
      policy:
        "Insurance policy covering property damage with $500K coverage limit and $2,500 deductible.",
      claim:
        "Property damage claim filed for water damage with estimated repair costs of $15,000.",
      warranty:
        "Home warranty covering HVAC system, expires in 6 months, contractor response within 24 hours.",
      receipt:
        "Purchase receipt for home improvement materials totaling $3,450.",
      contract:
        "Service contract for roof repair work with 2-year labor warranty.",
      correspondence:
        "Email correspondence regarding claim status and required documentation.",
      other:
        "Document contains important information regarding your property and insurance coverage.",
    };
  ...
  async extractDocumentEntities(documentId: string): Promise<DocumentEntity[]> {
    ...
    try {
      ...
      const entities: DocumentEntity[] =
        data.extracted_entities?.map((entity: any) => ({
          type: entity.type,
          value: entity.value,
    ...
    try {
      ...
      if (policyDocs.length === 0) {
        insights.push({
          ...
          description:
            "Upload your insurance policies to get personalized coverage analysis.",
          severity: "high",
          actionItems: [
      ...
      if (claimDocs.length > 0) {
        insights.push({
          ...
          description:
            "Review previous claims to identify potential coverage improvements.",
          severity: "medium",
