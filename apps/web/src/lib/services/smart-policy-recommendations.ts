...
class SmartPolicyRecommendationsService {
    ...
    try {
      ...
      await this.supabase.from("audit_logs").insert({
        ...
        metadata: {
          ...
          total_potential_savings: recommendations.reduce(
  ...
  private async performMarketAnalysis(userData: any): Promise<any> {
    ...
    return {
      ...
      savings_opportunity: Math.max(
        0,
        (userData.policies[0]?.annual_premium || 3200) - 2800,
      ),
  ...
  private getCompetitiveOptions(marketAnalysis: any): CarrierOption[] {
    ...
    for (const [carrier, data] of Object.entries(
      marketAnalysis.market_rates,
    ) as [string, any][]) {
      options.push({
        carrier,
    ...
    return [
      {
        ...
        recommended_limit: Math.round(
          (policy.dwelling_coverage || 200000) * 1.25,
        ),
...
export const smartPolicyRecommendationsService =
  new SmartPolicyRecommendationsService();
