...
export class ClaraAICompanionService {
  ...
  async startAdminSession(
    ...
    try {
      ...
      const session: Omit<ClaraSession, "id" | "created_at" | "updated_at"> = {
        ...
        recommendations: [],
        follow_up_required:
          analysis.crisis_indicators.length > 0 || analysis.intensity > 80,
      };
      ...
      const claraResponse = await this.generateClaraResponse(
        data.id,
        initialMessage,
        analysis,
      );
  ...
  private async analyzeEmotionalState(
    ...
    let recommendedApproach: EmotionalAnalysis["recommended_approach"] =
      "empathetic";
    ...
    else if (stressIndicators.length > 0)
      recommendedApproach = "solution-focused";

    return {
      primary_emotion: primaryEmotion,
      secondary_emotions:
        stressIndicators.length > 0 ? ["stressed", "overwhelmed"] : [],
      intensity,
   ...
   */
  private buildClaraSystemPrompt(
    analysis: EmotionalAnalysis,
    session?: ClaraSession,
  ): string {
...
I'm here to listen and support you through this. What would feel most helpful for you in this moment?`;
   ...
   */
  private mapToEmotionalState(
    emotion: string,
  ): ClaraSession["emotional_state"] {
    switch (emotion) {
      case "crisis":
      case "distressed":
        return "distressed";
   ...
   */
  private calculateCrisisLevel(
    analysis: EmotionalAnalysis,
  ): ClaraSession["crisis_level"] {
  ...
  private async triggerCrisisIntervention(
    ...
    try {
      ...
      await this.supabase.from("crisis_alerts").insert(alert);
