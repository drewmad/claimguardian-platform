...
const MOCK_CLAIMS_EVENTS: ClaimsProcessingEvent[] = [
  ...
  },
  {
    ...
    data: {
      ...
      payout_date: addMinutes(new Date(), 2880).toISOString(),
    },
...
class RealtimeClaimsProcessor {
  private subscribers: Map<string, (event: ClaimsProcessingEvent) => void> =
    new Map();
  ...
  subscribe(
    ...
    callback: (event: ClaimsProcessingEvent) => void,
  ): void {
  ...
  async processValidation(claimId: string): Promise<ClaimsProcessingEvent> {
    ...
    const validationResult: ClaimValidationResult =
      await this.validateClaim(claimId);
  ...
  async assignClaim(
    ...
    const assignment: ClaimAssignment = {
      ...
      processor_name: processorId.includes("ai")
        ? "AI Assessment Engine"
        : `Claims Officer ${processorId.slice(-3)}`,
  ...
  async getProcessingMetrics(): Promise<ProcessingMetrics> {
    ...
    const todayEvents = this.eventHistory.filter(
      (event) => new Date(event.timestamp) >= today,
    );

    const totalClaimsToday = new Set(todayEvents.map((e) => e.claim_id)).size;
    const processedClaimsToday = todayEvents.filter(
      (e) => e.event_type === "approved" || e.event_type === "denied",
    ).length;
    ...
    // Priority breakdown
    const priorityBreakdown = this.processingQueue.reduce(
      (acc, item) => {
    ...
    // Processing stages
    const processingStages = this.processingQueue.reduce(
      (acc, item) => {
    ...
    return {
      ...
      queue_backlog: this.processingQueue.filter((q) => q.status === "pending")
        .length,
  ...
  private generateProcessingMetadata(claimData: Record<string, unknown>) {
    ...
    return {
      ...
      estimated_duration_hours:
        amount > 25000 ? 3 + Math.random() * 2 : 1 + Math.random(),
  ...
  private async validateClaim(claimId: string): Promise<ClaimValidationResult> {
    ...
    return {
      ...
      validation_errors: isValid
        ? []
        : ["Missing required documentation", "Inconsistent damage description"],
      required_documents: [
        "photo_evidence",
        "repair_estimates",
        "police_report",
      ...
      auto_approval_eligible:
        isValid && confidenceScore > 0.85 && Math.random() > 0.3,
    };
  }
  ...
  private async assignToHuman(claimId: string): Promise<void> {
    ...
    const assignedProcessor =
      availableProcessors[
        Math.floor(Math.random() * availableProcessors.length)
      ];
