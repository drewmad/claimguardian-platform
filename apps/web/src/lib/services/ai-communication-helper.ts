...
export interface CommunicationRequest {
  ...
  recipient: 'adjuster' | 'carrier' | 'agent' | 'contractor' | 'lawyer' | 'other'
  purpose: string
  context: {
    claimNumber?: string
    policyNumber?: string
  ...
  tone: 'professional' | 'firm' | 'friendly' | 'formal' | 'urgent' | 'diplomatic'
  keyPoints: string[]
  attachments?: Array<{
    name: string
    type: string
  ...
  "nextSteps": ["recommended next actions"]
      ...
      const response = await enhancedAIClient.enhancedChat({
        messages: [
          {
            role: 'system',
            content: 'You are a Florida property insurance claims expert specializing in communication strategy and negotiation.'
          },
          {
            role: 'user',
      ...
      return {
        ...
        suggestedReply: {
          ...
          keyMessages: ['Acknowledge receipt', 'Request clarification if needed'],
          draftContent: 'Thank you for your response. We will review and follow up accordingly.',
          urgency: 'within_24h'
        },
        nextSteps: ['Review response manually', 'Consider professional consultation']
      }
    }
  }
...
Provide strategy in JSON format:
{
    ...
    {"objection": "expected objection", "response": "prepared response"}
      ...
      const response = await enhancedAIClient.enhancedChat({
        messages: [
          {
            role: 'system',
            content: 'You are an expert insurance claim negotiator with 20+ years of experience in Florida property claims.'
          },
          {
            role: 'user',
        ...
        anticipatedObjections: [
          {
            ...
            response: 'Provide independent contractor quotes and market rate documentation'
          }
        ]
      }
    }
   ...
   */
  private async getCommunicationTemplates(type: string, recipient: string): Promise<{
    structure: string[]
    keyPhrases: string[]
    legalConsiderations: string[]
...
Generate comprehensive response in JSON format:
{
   ...
   */
  private async generateNegotiationTips(request: CommunicationRequest): Promise<string[]> {
    ...
    try {
      const gap = request.context.estimatedAmount - request.context.currentOffer
      const gapPercentage = (gap / request.context.estimatedAmount) * 100

      const tips = [
        `Current gap: $${gap.toLocaleString()} (${gapPercentage.toFixed(1)}%)`,
        gapPercentage > 30 ? 'Significant gap suggests need for strong evidence' : 'Reasonable starting point for negotiation',
   ...
   */
  private async calculateOptimalTiming(request: CommunicationRequest): Promise<GeneratedCommunication['timing']> {
    const urgencyMap = {
      'urgent': {
  ...
  async getCommunicationInsights(claimId?: string): Promise<{
    ...
    try {
      ...
      const avgResponseTime = this.calculateAvgResponseTime(communications || [])
      const successfulStrategies = this.identifySuccessfulStrategies(communications || [])
  ...
  private calculateResponseRates(communications: any[]): Record<string, number> {
    const rates: Record<string, number> = {}
    const typeGroups = communications.reduce((groups, comm) => {
    ...
    Object.entries(typeGroups).forEach(([type, comms]) => {
      const responded = (comms as any[]).filter(comm => comm.received_response).length
