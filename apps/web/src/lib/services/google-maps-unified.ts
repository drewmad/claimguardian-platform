...
export class GoogleMapsUnifiedService {
  ...
  private rateLimitTrackers: Map<string, { count: number; resetTime: number }> =
    new Map();
  ...
  private async makeRequest<T>(
    ...
    if (
      rateLimitTracker &&
      rateLimitTracker.count >= this.config.rateLimits[apiType]
    ) {
    ...
    try {
      ...
      logger.info(`[GoogleMapsUnified] Making ${apiType} request`, {
        url: url.toString(),
      });
      ...
      if (!response.ok) {
        throw new Error(
          `API request failed: ${data.error_message || response.statusText}`,
        );
      }
  ...
  async analyzeRoofFromAerial(
    location: PropertyLocation,
  ): Promise<UnifiedResponse<RoofAnalysis>> {
  ...
  generateStaticMap(
    ...
    if (options.markers) {
      options.markers.forEach((marker, index) => {
        params.append(
          "markers",
          `color:red|label:${marker.label || index + 1}|${marker.lat},${marker.lng}`,
        );
      });
    }
  ...
  async getCompletePropertyIntelligence(location: PropertyLocation): Promise<
    UnifiedResponse<{
      address: unknown;
  ...
  > {
    try {
      ...
      const staticMapUrl = this.generateStaticMap(location, {
        zoom: 18,
        mapType: "satellite",
      });
      ...
      const result = {
        address:
          addressValidation.status === "fulfilled"
            ? addressValidation.value.data
            : null,
        roofAnalysis:
          roofAnalysis.status === "fulfilled"
            ? roofAnalysis.value.data || null
            : null,
        environmental: {
          weather:
            weatherData.status === "fulfilled" ? weatherData.value.data : null,
          pollen:
            pollenData.status === "fulfilled" ? pollenData.value.data : null,
          airQuality:
        ...
        elevation:
          elevationData.status === "fulfilled"
            ? elevationData.value.data?.results?.[0]?.elevation || null
            : null,
        timezone:
          timezoneData.status === "fulfilled"
            ? timezoneData.value.data?.timeZoneId || null
            : null,
        solarPotential:
          solarData.status === "fulfilled" ? solarData.value.data : null,
        staticMapUrl,
        streetViewUrl,
      };
  ...
  getCacheStats(): { size: number; apis: string[] } {
    return {
      ...
      apis: Array.from(
        new Set(Array.from(this.cache.keys()).map((key) => key.split(":")[0])),
      ),
    };
  }
