...
export class SettlementPredictionEngine {
  ...
  async predictSettlement(claimData: ClaimData): Promise<{
    ...
    try {
      ...
      const comparableSettlements =
        await this.findComparableSettlements(claimData);

      // Get carrier-specific patterns
      const carrierPatterns = await this.getCarrierPatterns(
        claimData.policyInfo.carrier,
      );
  ...
  async getSettlementTrends(
    ...
    try {
      ...
      // Calculate trends
      const avgSettlement =
        settlements.reduce((sum, s) => sum + (s.settlement_amount || 0), 0) /
        settlements.length;
      const medianDays = this.calculateMedian(
        settlements.map((s) => s.days_to_settle || 0),
      );
      const successRate =
        settlements.filter((s) => s.settlement_amount > 0).length /
        settlements.length;
    ...
    try {
      ...
      if (comparables.length === 0) {
        return {
          ...
          recommendations: [
            "Consider consulting with a local adjuster for market insights",
          ],
        };
      }

      // Calculate percentile position
      const sortedAmounts = comparables
        .map((c) => c.amount)
        .sort((a, b) => a - b);
      const estimatedPosition = this.findPercentile(
        sortedAmounts,
        claimData.estimatedDamage,
      );
      ...
      return {
        ...
        recommendations: insights.filter(
          (insight) =>
            insight.includes("consider") || insight.includes("should"),
        ),
      };
    } catch (error) {
    ...
    try {
      ...
      const avgCostPerSqFt =
        costData.data && costData.data.length > 0
          ? costData.data.reduce(
              (sum, p) => sum + p.BLDG_VAL / p.TOT_LVG_AREA,
              0,
            ) / costData.data.length
          : 150; // Default estimate

      const recentWeatherEvents =
        weatherData.data?.map(
          (event) => `${event.event_type} on ${event.begin_date.split("T")[0]}`,
        ) || [];

      return {
        avgCostPerSqFt,
        contractorAvailability:
          recentWeatherEvents.length > 3
            ? "low"
            : recentWeatherEvents.length > 1
              ? "medium"
              : "high",
        recentWeatherEvents,
        marketConditions:
      ...
      logger.warn("Market intelligence gathering failed", {
        error: toError(error),
      });
      return {
        avgCostPerSqFt: 150,
  ...
  private async findComparableSettlements(
    ...
    try {
      ...
      logger.warn("Failed to find comparable settlements", {
        error: toError(error),
      });
      return [];
    }
  ...
  private async generateAIPrediction(
    ...
    COMPARABLE SETTLEMENTS: ${comparables
      .slice(0, 5)
      .map((c) => `$${c.amount.toLocaleString()}`)
      .join(", ")}
    ...
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a Florida property insurance claims expert with 20+ years of experience.",
          },
      ...
      const median =
        Math.min(
          claimData.estimatedDamage,
          claimData.policyInfo.coverageAmount,
        ) - claimData.policyInfo.deductible;
      return {
        settlement: {
          low: median * 0.7,
          median,
  ...
  private async calculateTimeline(
    claimData: ClaimData,
    carrierPatterns: any,
  ): Promise<SettlementPrediction["timeline"]> {
    const baseDays = {
      minor: 30,
      moderate: 45,
      severe: 75,
      ...
      milestones: [
        {
          ...
          estimatedDays: 7,
          description: "Claim filed and initial review completed",
        },
        {
          phase: "Investigation",
          estimatedDays: expectedDays * 0.4,
          description: "Adjuster inspection and investigation",
        },
        {
          phase: "Evaluation",
          estimatedDays: expectedDays * 0.3,
          description: "Damage assessment and coverage review",
        },
        {
          phase: "Negotiation",
          estimatedDays: expectedDays * 0.2,
          description: "Settlement negotiation and approval",
        },
        {
          phase: "Payment",
          estimatedDays: expectedDays * 0.1,
  ...
  private async assessRisks(
    claimData: ClaimData,
    carrierPatterns: any,
  ): Promise<SettlementPrediction["riskAssessment"]> {
    // Simple risk assessment logic (could be enhanced with ML models)
    const disputeRisk =
      claimData.estimatedDamage > claimData.policyInfo.coverageAmount * 0.8
        ? "high"
        : claimData.estimatedDamage > claimData.policyInfo.coverageAmount * 0.5
          ? "medium"
          : "low";

    const underpaymentRisk =
      claimData.severity === "catastrophic"
        ? "high"
        : claimData.severity === "severe"
          ? "medium"
          : "low";

    const delayRisk =
      claimData.historicalClaims && claimData.historicalClaims.length > 2
        ? "high"
        : "low";
  ...
  private calculateSimilarityScore(
    ...
    const valueRatio =
      Math.min(claimData.propertyValue, settlement.property_value) /
  ...
  private async generateBenchmarkInsights(
    ...
    return [
      ...
      percentile > 70
        ? "Consider documenting premium materials and upgrades"
  ...
  private async storePrediction(
    ...
    try {
      await this.supabase.from("settlement_predictions").insert({
