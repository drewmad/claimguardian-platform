...
export class WebhookManager {
  ...
  async emit(event: Omit<WebhookEvent, 'id' | 'timestamp'>): Promise<void> {
    try {
      ...
      logger.info(`Webhook event ${event.type} emitted to ${subscriptions?.length || 0} subscribers`)
    } catch (error) {
  ...
  private async attemptDelivery(
    ...
    try {
      ...
      const signature = await this.generateSignature(JSON.stringify(payload), subscription.secret_key)
  ...
  async testWebhook(subscriptionId: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const testEvent: WebhookEvent = {
        ...
        data: { test: true, message: 'This is a test webhook from ClaimGuardian' },
