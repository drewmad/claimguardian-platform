...
export function calculateDepreciation(
  ...
  const schedule =
    customSchedule ||
    DEPRECIATION_SCHEDULES[category] ||
    DEPRECIATION_SCHEDULES.electronics;
  ...
  if (schedule.method === "straight-line") {
    ...
    const annualDepreciation =
      (purchasePrice - salvageValue) / schedule.lifeYears;
    ...
    if (partialYear > 0) {
      currentValue = Math.max(
        currentValue * (1 - rate * partialYear),
        salvageValue,
      );
    }
    ...
    if (schedule.salvagePercent > 100) {
      ...
      const appreciationRate =
        (schedule.salvagePercent - 100) / 100 / schedule.lifeYears;
...
export function getDepreciationAlerts(
  ...
  const schedule =
    customSchedule ||
    DEPRECIATION_SCHEDULES[item.category] ||
    DEPRECIATION_SCHEDULES.electronics;
  ...
  if (schedule.alertThresholds.maintenanceYears) {
    for (const year of schedule.alertThresholds.maintenanceYears) {
      ...
      if (monthsUntil > -3 && monthsUntil <= 3) {
        // Within 3 months
        alerts.push({
          type: "maintenance",
          ...
          dueDate: new Date(
            new Date(item.purchaseDate).setFullYear(
              new Date(item.purchaseDate).getFullYear() + year,
            ),
          ),
        });
      }
  ...
  if (schedule.alertThresholds.replacementYear) {
    const yearsUntilReplacement =
      schedule.alertThresholds.replacementYear - depreciation.ageYears;
  ...
  if (schedule.alertThresholds.valueThresholds) {
    const currentValuePercent =
      (depreciation.currentValue / item.purchasePrice) * 100;
    for (const threshold of schedule.alertThresholds.valueThresholds) {
      if (
        currentValuePercent <= threshold &&
        currentValuePercent > threshold - 10
      ) {
        alerts.push({
