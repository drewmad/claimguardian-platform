      ...
      setState((prevState) => {
        ...
        if (update.type === "cost_update" && update.data.cost_delta) {
          newState.metrics = {
            ...newState.metrics,
            totalCostToday:
              newState.metrics.totalCostToday + update.data.cost_delta,
        ...
        newState.recentUpdates = [update, ...newState.recentUpdates].slice(
          0,
          maxUpdates,
        );
        ...
        if (
          ...
          newState.metrics.alertsCount = newState.alerts.filter(
            (a) => !a.acknowledged,
          ).length;
