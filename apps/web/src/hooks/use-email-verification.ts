...
export function useEmailVerification(): EmailVerificationState & EmailVerificationActions {
  ...
  const resendEmail = useCallback(async (email?: string): Promise<boolean> => {
    ...
    if (!state.canResend) {
      error(`Please wait ${Math.ceil(state.resendCooldown / 1000)} seconds before resending`)
      return false
    }
    ...
    try {
      ...
      const toastId = toastLoading('Sending verification email...', { persistent: true })
      ...
      if (resendError) {
        ...
        if (resendError.message.includes('rate limit') || resendError.message.includes('too many')) {
          errorMessage = 'Too many attempts. Please wait before requesting another email.'
          isRateLimited = true
        }
        ...
        logger.warn('Failed to resend verification email', { email: targetEmail }, resendError)
        return false
      }
      ...
      addNotification({
        ...
        actions: [{
          ...
          handler: () => {
            const emailDomain = targetEmail.split('@')[1]?.toLowerCase() || ''
            const url = emailDomain === 'gmail.com' ? 'https://gmail.com' : `mailto:${targetEmail}`
            window.open(url, '_blank')
          }
        }]
      })
  ...
  }, [state.email, state.canResend, state.resendCooldown, state.attempts, error, toastLoading, success, addNotification])
