...
import { getRealtimeManager, type RealtimeEvent, type RealtimeTable, type RealtimeEventType, type PresenceState, type BroadcastMessage } from '@/lib/database/realtime-manager'
...
export function useRealtimeTable<T extends { id: string }>(
    ...
    setData(currentData => {
      ...
      switch (event.type) {
        case 'INSERT':
          if (event.new) {
            // Avoid duplicates
            const exists = updatedData.find(item => item.id === event.new.id)
            if (!exists) {
  ...
  const { isConnected, eventCount, error: realtimeError } = useRealtime<T>(
    table,
    '*',
    {
      filter,
...
export function useBroadcast(
  ...
  useEffect(() => {
    ...
    const setupBroadcast = async () => {
      try {
        ...
        setError(error)
        setIsConnected(false)
        logger.error('Broadcast subscription failed', { channel, eventType }, error)
      }
    }
  ...
  const broadcast = useCallback(async (payload: any, userId?: string) => {
    try {
      await realtimeManager.current.broadcast(channel, eventType, payload, { userId })
    } catch (err) {
      setError(err as Error)
...
export function useRealtimeQuery<T = any>(
  ...
  const fetchData = useCallback(async (fromCache = true) => {
    if (!enabled) return
  ...
  useEffect(() => {
    const updateStatus = () => {
      ...
      setStatus({
        ...
        errors: info.subscriptions.filter(sub => sub.status === 'error').length
      })
    }
