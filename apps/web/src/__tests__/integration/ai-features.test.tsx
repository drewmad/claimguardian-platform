...
describe('AI Features Integration Tests', () => {
  ...
  describe('ClaimPredictor Component', () => {
    it('renders claim predictor interface', () => {
      ...
      expect(screen.getByText('AI-Powered Claim Predictor')).toBeInTheDocument();
      expect(screen.getByText('Predict claim approval likelihood')).toBeInTheDocument();
    });
    ...
    it('analyzes claim when form is submitted', async () => {
      ...
      await waitFor(() => {
        expect(toast.success).toHaveBeenCalledWith(expect.stringContaining('Analysis complete'));
      }, { timeout: 3000 });
    ...
    it('displays risk factors after analysis', async () => {
      ...
      expect(screen.getByText(/documentation completeness/i)).toBeInTheDocument();
  ...
  describe('FraudDetectionDashboard Component', () => {
    it('renders fraud detection dashboard', () => {
      ...
      expect(screen.getByText(/multi-layered AI analysis/i)).toBeInTheDocument();
    });
    ...
    it('runs fraud scan when triggered', async () => {
      ...
      const scanButton = screen.getByRole('button', { name: /run fraud scan/i });
      ...
      await waitFor(() => {
        expect(toast.success).toHaveBeenCalledWith(expect.stringContaining('Fraud scan complete'));
      }, { timeout: 4000 });
    });
    ...
    it('displays fraud alerts after scan', async () => {
      ...
      expect(screen.getByText(/suspicious patterns detected/i)).toBeInTheDocument();
    });
    ...
    it('shows risk indicators properly', () => {
      ...
      const indicatorsTab = screen.getByRole('tab', { name: /risk indicators/i });
  ...
  describe('ComplianceChecker Component', () => {
    it('renders compliance checker interface', () => {
      ...
      expect(screen.getByText(/florida insurance compliance checker/i)).toBeInTheDocument();
      expect(screen.getByLabelText(/select claim to check/i)).toBeInTheDocument();
    });
    ...
    it('runs compliance check when claim is selected', async () => {
      ...
      await waitFor(() => {
        expect(toast.error).toHaveBeenCalledWith(expect.stringContaining('critical compliance issues'));
      }, { timeout: 3000 });
    ...
    it('shows Florida statute references', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/relevant florida statutes/i)).toBeInTheDocument();
      });
    ...
    it('enables auto-fix functionality', async () => {
      ...
      await waitFor(() => {
        const applyFixesButton = screen.getByRole('button', { name: /apply available auto-fixes/i });
  ...
  describe('SentimentAnalysisDashboard Component', () => {
    it('renders sentiment analysis dashboard', () => {
      ...
      expect(screen.getByText('Sentiment Analysis Dashboard')).toBeInTheDocument();
    ...
    it('analyzes test message sentiment', async () => {
      ...
      // Enter test message
      const messageInput = screen.getByPlaceholderText(/enter customer message/i);
      await user.type(messageInput, 'I am very frustrated with the slow response time');
      ...
      // Wait for analysis
      await waitFor(() => {
        expect(toast.success).toHaveBeenCalledWith('Sentiment analysis complete');
      }, { timeout: 2000 });
    });
  ...
  describe('PredictiveMaintenanceAlerts Component', () => {
    ...
    it('runs predictive scan', async () => {
      ...
      const scanButton = screen.getByRole('button', { name: /run predictive scan/i });
      await user.click(scanButton);
      ...
      await waitFor(() => {
        expect(toast.success).toHaveBeenCalledWith(expect.stringContaining('Predictive scan complete'));
      }, { timeout: 4000 });
    });
    ...
    it('displays maintenance alerts', () => {
      ...
      expect(screen.getByText(/schedule preventive maintenance/i)).toBeInTheDocument();
    });
    ...
    it('calculates cost savings correctly', () => {
      ...
      expect(screen.getByText(/potential savings this month/i)).toBeInTheDocument();
    ...
    it('schedules maintenance when requested', async () => {
      ...
      expect(toast.success).toHaveBeenCalledWith(expect.stringContaining('Maintenance scheduled'));
    });
  });
