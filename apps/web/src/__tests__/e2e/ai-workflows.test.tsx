...
describe('AI Workflows End-to-End Tests', () => {
    ...
    it('should perform full claim analysis from submission to approval prediction', async () => {
      ...
      mockSupabase.functions.invoke.mockResolvedValue({
        data: {
          ...
          recommendations: ['Submit plumber report', 'Include moisture readings'],
  ...
  describe('Compliance Auto-Fix Workflow', () => {
    it('should automatically fix compliance issues when enabled', async () => {
      ...
      await waitFor(() => {
        expect(toast.success).toHaveBeenCalledWith(expect.stringContaining('2 automatic fixes'));
      });
    });
  });
  ...
  describe('Predictive Maintenance Alert Workflow', () => {
    it('should predict failures and schedule preventive maintenance', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/critical maintenance required/i)).toBeInTheDocument();
      });
  ...
  describe('Multi-Component Integration Workflow', () => {
    it('should handle complete claim lifecycle with all AI features', async () => {
      ...
      mockSupabase.functions.invoke
        .mockResolvedValueOnce({
          data: {
            ...
            confidence: 88,
            recommendations: ['Strengthen documentation', 'Add weather reports']
          },
          error: null
        })
      ...
      const results = await Promise.all([
        mockSupabase.functions.invoke('claim-predictor-ai', { body: mockClaim }),
        mockSupabase.functions.invoke('fraud-detection-engine', { body: mockClaim }),
        mockSupabase.functions.invoke('compliance-checker', { body: mockClaim }),
        mockSupabase.functions.invoke('sentiment-analyzer', { body: { claim_id: mockClaim.id } }),
        mockSupabase.functions.invoke('predictive-maintenance', { body: { property_id: mockClaim.property_id } })
      ]);
  ...
  describe('Error Recovery Workflow', () => {
    ...
    it('should handle partial workflow failures gracefully', async () => {
      ...
      mockSupabase.functions.invoke
        .mockResolvedValueOnce({ data: { approval_likelihood: 80 }, error: null })
        .mockResolvedValueOnce({ data: null, error: { message: 'Fraud service unavailable' } })
