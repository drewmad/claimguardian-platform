...
import {
  ...
  ConsentManager,
  DataDeletionService,
} from "../../lib/compliance/privacy-compliance-framework";
import {
  SOXAuditTrailManager,
  FinancialControlsMonitor,
} from "../../lib/compliance/sox-audit-trail-system";
import {
  LegalDocumentGenerator,
  DocumentTemplateManager,
} from "../../lib/compliance/legal-document-generation-engine";
import {
  RegulatoryReportingManager,
  OIRReportGenerator,
} from "../../lib/compliance/automated-regulatory-reporting";
  ...
  createClient: vi.fn(() => ({
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
          single: vi.fn(() =>
            Promise.resolve({ data: mockClaimData, error: null }),
          ),
          order: vi.fn(() =>
            Promise.resolve({ data: [mockClaimData], error: null }),
          ),
...
describe("Compliance System Integration Tests", () => {
  ...
  describe("Florida Regulatory Compliance", () => {
    ...
    it("should check Florida Insurance Code compliance", async () => {
      const result =
        await floridaManager.checkInsuranceCodeCompliance("test-claim-id");
    ...
    it("should check Prompt Payment Act compliance", async () => {
      const result =
        await floridaManager.checkPromptPaymentActCompliance("test-claim-id");
    ...
    it("should run comprehensive compliance check", async () => {
      const result =
        await floridaManager.runComprehensiveComplianceCheck("test-claim-id");
      ...
      expect(["compliant", "at_risk", "violation"]).toContain(
    ...
    it("should validate Assignment of Benefits compliance", async () => {
      ...
      expect(["compliant", "incomplete", "non_compliant"]).toContain(
  ...
  describe("Privacy Compliance (GDPR/CCPA)", () => {
    ...
    it("should check CCPA compliance for California residents", async () => {
      // Mock user profile
      vi.mocked(privacyManager["supabase"].from)
        .mockReturnValueOnce({
          select: vi.fn(() => ({
            eq: vi.fn(() => ({
              single: vi.fn(() =>
    ...
    it("should execute data deletion", async () => {
      ...
      const result = await dataDeletionService.executeDataDeletion(
        "test-user-id",
        {
  ...
  describe("SOX Audit Trail System", () => {
    ...
    it("should log financial transactions with audit trail", async () => {
      ...
      const transactionId =
        await auditManager.logFinancialTransaction(transaction);
    ...
    it("should verify audit trail integrity", async () => {
      ...
      const result = await auditManager.verifyAuditTrailIntegrity(
        fromDate,
        toDate,
      );
    ...
    it("should monitor financial control effectiveness", async () => {
      ...
      expect(["effective", "needs_improvement", "material_weakness"]).toContain(
  ...
  describe("Legal Document Generation", () => {
    ...
    it("should generate Florida claim denial letter", async () => {
      ...
      expect(["draft", "under_review", "approved", "executed"]).toContain(
    ...
    it("should generate Assignment of Benefits form", async () => {
      ...
      expect(
        document.complianceChecks.some(
          (check) =>
            check.checkName.includes("Florida Statute") &&
            check.reference === "627.7152",
        ),
      ).toBe(true);
    });
  ...
  describe("Automated Regulatory Reporting", () => {
    ...
    it("should generate hurricane loss report", async () => {
      ...
      expect(Array.isArray((report.reportData as any).hurricaneEvents)).toBe(
        true,
      );
    });
    ...
    it("should submit report to regulatory body", async () => {
      ...
      // Then submit it
      const submissionResult = await reportingManager.submitReport(
        report.id,
    ...
    it("should generate specific OIR claim statistics", async () => {
      ...
      const oirReport = await oirGenerator.generateOIRClaimReport(
        startDate,
        endDate,
      );
      ...
      expect(compliance.promptPaymentCompliance).toHaveProperty(
        "compliantClaims",
      );
  ...
  describe("End-to-End Compliance Workflows", () => {
    ...
    it("should handle privacy request with audit logging", async () => {
      ...
      if (privacyRequest.status === "accepted") {
        const deletionResult =
          await dataDeletionService.executeDataDeletion("test-user-id");
    ...
    it("should generate regulatory report with compliance validation", async () => {
      ...
      expect(report).toHaveProperty("id");
      expect(
        report.validationResults.filter((v) => v.severity === "error").length,
      ).toBe(0);

      // 2. Verify compliance checks pass
      const compliancePassRate =
        report.complianceChecks.filter((c) => c.passed).length /
      ...
      if (report.validationResults.every((v) => v.severity !== "error")) {
        const submission = await reportingManager.submitReport(
          report.id,
          "compliance-officer",
        );
