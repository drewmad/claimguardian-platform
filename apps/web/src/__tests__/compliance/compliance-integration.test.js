...
describe("Compliance System Integration Tests", () => {
  beforeEach(() => {
    ...
    mockSOXAuditTrailManager.logAuditEvent.mockResolvedValue(
      "audit-event-id-123",
    );
    ...
    mockLegalDocumentGenerator.generateDocument.mockResolvedValue({
      ...
      complianceChecks: [
        {
          checkName: "Florida Statute Check",
          passed: true,
          details: "Compliant",
        },
      ],
  ...
  describe("Florida Regulatory Compliance", () => {
    it("should check Florida Insurance Code compliance", async () => {
      const result =
        await mockFloridaComplianceManager.checkInsuranceCodeCompliance(
          "test-claim-id",
        );
      ...
      expect(
        mockFloridaComplianceManager.checkInsuranceCodeCompliance,
      ).toHaveBeenCalledWith("test-claim-id");
    });

    it("should check Prompt Payment Act compliance", async () => {
      const result =
        await mockFloridaComplianceManager.checkPromptPaymentActCompliance(
          "test-claim-id",
        );
    ...
    it("should assess bad faith risk", async () => {
      const result =
        await mockFloridaComplianceManager.assessBadFaithRisk("test-claim-id");
    ...
    it("should run comprehensive compliance check", async () => {
      const result =
        await mockFloridaComplianceManager.runComprehensiveComplianceCheck(
          "test-claim-id",
        );
  ...
  describe("Privacy Compliance (GDPR/CCPA)", () => {
    it("should check GDPR compliance", async () => {
      const result =
        await mockPrivacyComplianceManager.checkGDPRCompliance("test-user-id");
  ...
  describe("SOX Audit Trail System", () => {
    it("should log SOX-compliant audit events", async () => {
      ...
      expect(mockSOXAuditTrailManager.logAuditEvent).toHaveBeenCalledWith(
        event,
      );
    });
  });
  ...
  describe("Legal Document Generation", () => {
    it("should generate Florida claim denial letter", async () => {
      ...
      const document =
        await mockLegalDocumentGenerator.generateDocument(request);
  ...
  describe("Automated Regulatory Reporting", () => {
    it("should generate OIR claim report", async () => {
      ...
      const report =
        await mockRegulatoryReportingManager.generateReport(request);
    ...
    it("should validate input parameters", async () => {
      ...
      mockLegalDocumentGenerator.generateDocument
        .mockRejectedValueOnce(
          new Error("Template not found: non-existent-template"),
        )
        .mockRejectedValueOnce(
          new Error(
            "Variable validation failed: Required variable companyName is missing",
          ),
        );
