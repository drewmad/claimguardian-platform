...
export async function GET() {
  ...
  try {
    // Test each table
    const { error: propError } = await supabase.from('properties').select('count').limit(0)
    results.tables.properties = !propError

    const { error: profError } = await supabase.from('user_profiles').select('count').limit(0)
    results.tables.user_profiles = !profError

    const { error: auditError } = await supabase.from('audit_logs').select('count').limit(0)
    results.tables.audit_logs = !auditError

    const { error: legalError } = await supabase.from('legal_documents').select('count').limit(0)
    results.tables.legal_documents = !legalError

    const { error: loginError } = await supabase.from('login_activity').select('count').limit(0)
