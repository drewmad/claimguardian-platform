  ...
  try {
    ...
    return NextResponse.json({
      ...
      results: {
        ...
        onboardingStatus: userPref?.onboarding_completed
          ? "completed"
          : "pending",
        nextSteps: [
          ...
          !userPref?.onboarding_completed &&
            "Onboarding flow will show on first dashboard visit",
        ].filter(Boolean),
      },
    });
  } catch (error) {
...
export async function GET() {
  return NextResponse.json({
    ...
    method: "POST",
    description:
      "Tests the complete signup flow including consent tracking and email verification",
  });
}
