  ...
  try {
    ...
    if (includeWeather) {
      ...
      const { data: currentWeather } = await supabase
        .rpc('get_current_conditions', { lat: latitude, lon: longitude });
      ...
      const { data: forecast } = await supabase
        ...
        .lte('valid_time_start', new Date(Date.now() + 48 * 60 * 60 * 1000).toISOString())
    ...
    if (includeAlerts) {
      ...
      const allAlerts = combineAndPrioritizeAlerts(weatherAlerts || [], ipawsAlerts || []);
...
function analyzeWeatherTrends(historical: any[]): any {
  ...
  const precipitation = historical.map(h => h.total_precipitation).filter(p => p != null);
...
function calculateDisasterRisk(disasters: any[], stats: any[]): number {
  ...
  if (stats && stats.length > 0) {
    const avgDisasters = stats.reduce((sum, s) => sum + s.disaster_count, 0) / stats.length;
...
function combineAndPrioritizeAlerts(weatherAlerts: any[], ipawsAlerts: any[]): any[] {
  const allAlerts = [
  ...
  return allAlerts.sort((a, b) => {
    ...
    const urgencyOrder = { immediate: 0, expected: 1, future: 2, past: 3, unknown: 4 };
    ...
    const aUrgency = urgencyOrder[a.urgency?.toLowerCase() as keyof typeof urgencyOrder] ?? 4;
    const bUrgency = urgencyOrder[b.urgency?.toLowerCase() as keyof typeof urgencyOrder] ?? 4;
...
async function generatePredictions(data: any): Promise<any> {
  ...
  // Medium-term (1 week)
  predictions.mediumTerm.disasterProbability = calculateDisasterProbability(data);
  predictions.mediumTerm.confidence = 0.70;

  // Long-term (1 month)
  predictions.longTerm.seasonalRisk = calculateSeasonalRisk(new Date());
  predictions.longTerm.confidence = 0.60;
...
function calculateComprehensiveRisk(data: any): any {
  ...
  // Prediction risk
  if (data.predictions) {
    const predictionRisk = data.predictions.mediumTerm.disasterProbability * 100;
...
function getPrimaryConcern(factors: any): string {
  ...
  switch (concern) {
    ...
    case 'alerts': return 'Emergency alerts active';
    case 'predictions': return 'High probability of future events';
    default: return 'General elevated risk';
  }
}
...
function generateRecommendations(riskAssessment: any, data: any): any {
  ...
  if (riskAssessment.riskLevel === 'critical' || riskAssessment.riskLevel === 'high') {
    recommendations.immediate.push('Review and update emergency contact information');
  ...
  if (data.fema?.activeDisasters?.length > 0) {
    ...
    recommendations.shortTerm.push('Register with FEMA if eligible for assistance');
  }

  // Long-term recommendations
  recommendations.longTerm.push('Consider flood insurance if not currently covered');
  recommendations.longTerm.push('Develop comprehensive emergency preparedness plan');
  recommendations.longTerm.push('Explore hazard mitigation grant opportunities');
