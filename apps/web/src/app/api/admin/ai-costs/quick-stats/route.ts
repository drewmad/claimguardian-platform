...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: yesterdayStats } = await supabase.rpc('get_daily_cost_stats', {
    ...
    const avgResponseTime = performanceData && performanceData.length > 0
      ? Math.round(performanceData.reduce((sum, log) => sum + (log.processing_time_ms || 0), 0) / performanceData.length)
      : 0
    ...
    const { data: toolData } = await supabase
      ...
      .lt('created_at', new Date(today.getTime() + 24 * 60 * 60 * 1000).toISOString().split('T')[0] + 'T00:00:00')
    ...
    const topCostTool = Object.entries(toolCosts)
    ...
    const yesterdayTotal = yesterdayStats?.[0] || { total_cost: 0, total_requests: 0 }

    const costChange = yesterdayTotal.total_cost > 0
      ? ((todayTotal.total_cost - yesterdayTotal.total_cost) / yesterdayTotal.total_cost) * 100
      : 0

    const requestChange = yesterdayTotal.total_requests > 0
      ? ((todayTotal.total_requests - yesterdayTotal.total_requests) / yesterdayTotal.total_requests) * 100
      : 0
