      ...
      if (!resultsError && results) {
        ...
        Object.values(metricsMap).forEach((metrics) => {
          ["model_a_metrics", "model_b_metrics"].forEach((key) => {
            const m = metrics[key as keyof ABTestMetrics] as Record<
              string,
              number
            >;
            if (m.requests > 0) {
              ...
              m.success_rate =
                Math.round((m.success_rate / m.requests) * 100 * 10) / 10;
              m.user_rating =
                Math.round((m.user_rating / m.requests) * 10) / 10;
            }
          });
        });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingTest) {
      return NextResponse.json(
        {
          error: "An active A/B test already exists for this feature",
          success: false,
        },
        { status: 409 });
    }
