...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      case 'health-check':
        const healthReport = await monitoringDeployment.performManualHealthCheck()
        return NextResponse.json({
          success: true,
          data: healthReport
        })
      ...
      case 'database-health':
        const dbHealthCheck = await productionErrorMonitor.performDatabaseHealthCheck()
        return NextResponse.json({
          success: true,
          data: dbHealthCheck
        })
      ...
      default:
        ...
        const [status, healthCheck, errorPatterns, dbHealth] = await Promise.all([
  ...
  try {
    ...
    switch (action) {
      ...
      case 'manual-health-check':
        const manualCheck = await monitoringDeployment.performManualHealthCheck()
        return NextResponse.json({
          success: true,
          data: manualCheck
        })
