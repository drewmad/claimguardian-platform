...
export async function GET(request: NextRequest) {
  try {
    ...
    if (scores && scores.length > 0) {
      ...
      const totalRating = scores.reduce((sum, score) => sum + score.numeric_rating, 0)
      summary.overall_rating = Math.round((totalRating / scores.length) * 10) / 10

      // Calculate satisfaction rate (ratings 4-5)
      const satisfiedRatings = scores.filter(score => score.numeric_rating >= 4).length
      summary.satisfaction_rate = Math.round((satisfiedRatings / scores.length) * 100)
      ...
      Object.entries(featureGroups).forEach(([featureId, featureScores]) => {
        const featureRating = featureScores.reduce((sum, score) => sum + score.numeric_rating, 0) / featureScores.length
        const latestFeedback = featureScores.find(score => score.feedback)?.feedback || ''
      ...
      summary.response_rate = Math.min(100, Math.round((scores.length / Math.max(scores.length * 1.15, 1)) * 100))
    }

    // Get recent feedback for display
    const recentFeedback = scores?.filter(score => score.feedback).slice(0, 10) || []
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate input
    if (!body.feature_id || !body.model || !body.rating || !body.numeric_rating || !body.response_time) {
      return NextResponse.json(
