  ...
  try {
    ...
    if (includePerformance && prompts?.length > 0) {
      ...
      if (!perfError && performance) {
        ...
        // Calculate averages
        Object.values(performanceMap).forEach(
          (metrics: Record<string, number>) => {
            if (metrics.total_uses > 0) {
              metrics.avg_time =
                Math.round(metrics.avg_time / metrics.total_uses) / 1000; // Convert to seconds
              metrics.success_rate = Math.round(
                (metrics.success_rate / metrics.total_uses) * 100,
              );
              metrics.user_rating =
                Math.round((metrics.user_rating / metrics.total_uses) * 10) /
                10;
            }
          },
        );
