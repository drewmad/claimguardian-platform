...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const [systemStatus, dbMetrics, cacheMetrics, appMetrics, alertMetrics] =
...
async function getQueryPerformance(): Promise<{
  ...
  try {
    ...
    const avgQueryTime =
      data.reduce((sum, log) => sum + (log.duration || 0), 0) / data.length;
...
async function getAlertMetrics(supabase: any): Promise<{
  ...
  try {
    ...
    const [activeAlertsResult, criticalAlertsResult, resolvedAlertsResult] =
      await Promise.allSettled([
        supabase
          .from("system_alerts")
          .select("*", { count: "exact", head: true })
    ...
    return {
      active:
        activeAlertsResult.status === "fulfilled"
          ? activeAlertsResult.value.count || 0
          : 0,
      critical:
        criticalAlertsResult.status === "fulfilled"
          ? criticalAlertsResult.value.count || 0
          : 0,
      resolved24h:
        resolvedAlertsResult.status === "fulfilled"
          ? resolvedAlertsResult.value.count || 0
          : 0,
    };
  } catch (error) {
