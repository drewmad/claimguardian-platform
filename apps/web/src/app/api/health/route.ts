...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const isMonitoringCheck =
    request.headers.get("x-monitoring-check") === "true";
  ...
  try {
    ...
    const [databaseResult, cacheResult, supabaseResult, systemMetrics] =
    ...
    if (failedChecks.length > 0) {
      ...
      const criticalFailures = ["database", "supabase"].some(
        (critical) => healthCheck.checks[critical]?.status === "fail",
      );
    ...
    return {
      status:
        failedServices.length === 0
          ? "pass"
          : failedServices.length < results.length
            ? "warn"
            : "fail",
      responseTime,
      details: {
        services: results.reduce(
          (acc, result) => {
            acc[result.service] = {
              ok: result.ok,
