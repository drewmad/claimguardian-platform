...
import { usePWA, useInstallPrompt, useOfflineStatus, useServiceWorker } from '@/hooks/use-pwa'
import { InstallPrompt } from '@/components/pwa/install-prompt'
import { OfflineIndicator, SyncStatus } from '@/components/pwa/offline-indicator'
...
const pwaFeatures = [
  {
    ...
    benefits: ['Home screen access', 'Full-screen experience', 'Faster launch times'],
    status: 'available'
  },
  {
    icon: WifiOff,
    ...
    benefits: ['Cached content access', 'Offline data entry', 'Auto-sync when online'],
    status: 'available'
  },
  {
    icon: Bell,
    ...
    benefits: ['Real-time alerts', 'Background updates', 'Customizable settings'],
    status: 'coming_soon'
  },
  {
    icon: Share,
    ...
    benefits: ['Share to any app', 'Quick property sharing', 'Document sharing'],
    status: 'available'
  },
  {
    icon: Camera,
    ...
    benefits: ['Direct file access', 'Drag & drop support', 'Auto-categorization'],
    status: 'available'
  },
  {
    icon: Zap,
...
const deviceSupport = [
  ...
  { platform: 'Android Chrome', icon: Smartphone, support: 'Full', color: 'green' },
  { platform: 'Windows Edge', icon: Monitor, support: 'Full', color: 'green' },
  { platform: 'macOS Safari', icon: Monitor, support: 'Partial', color: 'yellow' },
  { platform: 'Chrome Desktop', icon: Monitor, support: 'Full', color: 'green' },
  { platform: 'Firefox Desktop', icon: Monitor, support: 'Partial', color: 'yellow' }
]
        ...
        >
          <Card className="bg-white/10 border-white/20 backdrop-blur-sm">
            <CardContent className="p-4 text-center">
              ...
              <Badge className={`mt-2 ${pwaStatus.isInstalled ? 'bg-green-600' : 'bg-gray-600'}`}>
                {pwaStatus.platform}
              </Badge>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/10 border-white/20 backdrop-blur-sm">
            <CardContent className="p-4 text-center">
              <div className="text-2xl font-bold text-white mb-1">
                {offlineStatus.isOnline && !forceOfflineMode ? 'Online' : 'Offline'}
              </div>
              <div className="text-sm text-gray-300">Connection</div>
              <Badge className={`mt-2 ${offlineStatus.isOnline && !forceOfflineMode ? 'bg-green-600' : 'bg-red-600'}`}>
                {offlineStatus.isOnline && !forceOfflineMode ? 'Connected' : 'Offline Mode'}
              </Badge>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/10 border-white/20 backdrop-blur-sm">
            <CardContent className="p-4 text-center">
              ...
              <Badge className={`mt-2 ${isRegistered ? 'bg-green-600' : 'bg-gray-600'}`}>
                {hasUpdate ? 'Update Available' : isRegistered ? 'Registered' : 'Not Active'}
              </Badge>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/10 border-white/20 backdrop-blur-sm">
            <CardContent className="p-4 text-center">
              ...
              <Badge className={`mt-2 ${notificationsEnabled ? 'bg-green-600' : 'bg-gray-600'}`}>
          ...
          {hasUpdate && (
            <Card className="bg-blue-900/30 border-blue-500/30">
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-white font-medium">App Update Available</h3>
                    <p className="text-blue-200 text-sm">A new version is ready to install</p>
                  </div>
