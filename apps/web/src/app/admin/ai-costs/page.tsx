...
export default function AICostsDashboard() {
  ...
  // Prepare chart data
  const dailyChartData = dailySummary
    .reduce(
      (acc, item) => {
        const existing = acc.find(
          (d: Record<string, unknown>) => d.date === item.date,
        );
        if (existing) {
          (existing as Record<string, unknown>)[item.provider] =
            (((existing as Record<string, unknown>)[item.provider] ||
              0) as number) + item.daily_cost;
        } else {
          acc.push({
            date: item.date,
  ...
  const hourlyChartData = hourlyUsage
    .reduce(
      (acc, item) => {
        const hourStr = new Date(item.hour).toLocaleTimeString("en-US", {
          hour: "numeric",
        });
        const existing = acc.find(
          (d: Record<string, unknown>) => d.hour === hourStr,
        );
        if (existing) {
          (existing as Record<string, unknown>).calls =
            ((existing as Record<string, unknown>).calls as number) +
            item.calls;
        } else {
          acc.push({
            hour: hourStr,
            ...
            <div className="space-y-2">
              {alerts.map((alert, idx) => (
                <Alert key={idx} className="bg-yellow-900/20 border-yellow-900">
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    {(alert as Record<string, unknown>).alert_message as string}
                  </AlertDescription>
                </Alert>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <Card className="bg-gray-800 border-gray-700">
              ...
              <CardContent>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Projected: $
                  {costProjection?.projected_monthly_cost?.toFixed(2) || "0.00"}
                </p>
              </CardContent>
            </Card>
                      ...
                      <div className="grid grid-cols-3 gap-2 text-xs">
                        <div>
                          ...
                          <span className="text-gray-300">
                            {Math.round(model.avg_response_time)}ms
                          </span>
                        </div>
                        <div>
                          <span className="text-gray-500">P95: </span>
                          <span className="text-gray-300">
                            {Math.round(model.p95_response_time)}ms
                          </span>
                        </div>
                        <div>
