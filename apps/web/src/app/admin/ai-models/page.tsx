...
export default function AIModelsAdminPage() {
  ...
  const [featureMappings, setFeatureMappings] =
    useState<FeatureModelMapping[]>(FEATURE_MAPPINGS);
  ...
  const [qualityMetrics, setQualityMetrics] = useState({
    ...
    ratings_by_feature: {} as Record<
      string,
      { rating: number; count: number; latest_feedback: string }
    >,
  });
    ...
    try {
      ...
      if (abTestsResponse.ok) {
        ...
        if (abTestsData.success && abTestsData.data) {
          ...
          const convertedTests = abTestsData.data.map(
            (test: ABTestApiResponse) => ({
              id: test.id,
              name: test.name,
      ...
      const promptsResponse = await fetch(
        "/api/admin/custom-prompts?include_performance=true",
      );
      if (promptsResponse.ok) {
        const promptsData = await promptsResponse.json();
        if (promptsData.success && promptsData.data?.prompts) {
          // Convert API response to UI format
          const convertedPrompts = promptsData.data.prompts.map(
            (prompt: CustomPromptApiResponse) => ({
              id: prompt.id,
    ...
    const recommendations: AIRecommendation[] = [
      {
        ...
        reason:
          "23% faster response time with 91% accuracy vs 94% - cost savings of $12.50/day",
        confidence: 87,
      },
      {
      ...
      },
      {
        ...
        reason:
          "Claude may produce more professional tone for legal communications",
        confidence: 64,
      },
    ];
  ...
  const loadPerformanceData = async () => {
    try {
      const response = await fetch(
        "/api/admin/ai-models/performance?range=week",
      );
      if (response.ok) {
        ...
        const totalRequests = Object.values(data).reduce(
          (sum: number, model: ModelUsageData) => sum + model.requests,
          0,
        );
        const totalCost = Object.values(data).reduce(
          (sum: number, model: ModelUsageData) => sum + model.cost,
          0,
        );
        const avgResponseTime =
          totalRequests > 0
            ? Object.values(data).reduce(
                (sum: number, model: ModelUsageData) =>
                  sum + model.avgTime * model.requests,
                0,
              ) / totalRequests
            : 0;
        const avgSuccessRate =
          Object.keys(data).length > 0
            ? Object.values(data).reduce(
                (sum: number, model: ModelUsageData) => sum + model.successRate,
                0,
  ...
  const checkModelAvailability = async () => {
    ...
    try {
      ...
      const updatedModels = models.map((model) => {
        let available = false;
        if (model.provider === "openai" && keyStatus.hasOpenAIKey)
          available = true;
        if (model.provider === "gemini" && keyStatus.hasGeminiKey)
          available = true;
        if (model.provider === "claude" && keyStatus.hasClaudeKey)
          available = true;
  ...
  const testModel = async (modelId: string) => {
    ...
    try {
      ...
      const response = await fetch("/api/ai/chat", {
        ...
        body: JSON.stringify({
          messages: [
            { role: "user", content: "Test message for model validation" },
          ],
      ...
      if (response.ok) {
        ...
        setModels((prev) =>
          prev.map((m) => (m.id === modelId ? { ...m, status: "active" } : m)),
        );
      } else {
  ...
  const updateFeatureModel = (
    featureId: string,
    field: "currentModel" | "fallbackModel",
    modelId: string,
  ) => {
  ...
  const testABModels = async (
    ...
    try {
      ...
      const testPrompt =
        "Analyze this sample insurance claim for testing purposes.";
      ...
      toast.success(
        `Test completed! Model A: ${mockResultA.responseTime.toFixed(0)}ms, Model B: ${mockResultB.responseTime.toFixed(0)}ms`,
      );
    } catch (error) {
  ...
  const applyAIRecommendation = async (recommendation: AIRecommendation) => {
    if (recommendation.type === "model_switch" && recommendation.recommended) {
      ...
      toast.success(
        `Applied recommendation: Switched ${recommendation.feature} to ${recommendation.recommended}`,
      );
    } else if (recommendation.type === "prompt_optimization") {
      toast.info(
        "Create a custom prompt based on this recommendation in the Custom Prompts tab",
      );
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-2">
            Configure AI models, monitor performance, and manage feature
            assignments
          </p>
        </div>
      ...
      <Tabs defaultValue="models" className="space-y-6">
        <TabsList className="bg-gray-800 border-gray-700">
          <TabsTrigger value="models" className="text-gray-300">
            Available Models
          </TabsTrigger>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <Progress
                            value={model.speedRating * 10}
                            className="h-2"
                          />
                          <span className="text-sm text-white">
                            {model.speedRating}/10
                          </span>
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <Progress
                            value={model.accuracyRating * 10}
                            className="h-2"
                          />
                          <span className="text-sm text-white">
                            {model.accuracyRating}/10
                          </span>
                        </div>
                      </div>
                      <div>
          ...
          <Card className="bg-gray-800 border-gray-700">
            <CardHeader>
              ...
              <p className="text-gray-400">
                Configure which AI models to use for each feature
              </p>
            </CardHeader>
            <CardContent>
                        ...
                        >
                          {models
                            .filter(
                              (m) =>
                                m.available &&
                                m.capabilities.includes(
                                  mapping.category === "reasoning"
                                    ? "reasoning"
                                    : mapping.category,
                                ),
                            )
                      ...
                      <div>
                        ...
                        >
                          {models
                            .filter(
                              (m) =>
                                m.available && m.id !== mapping.currentModel,
                            )
                            .map((model) => (
                              <option key={model.id} value={model.id}>
                                {model.name} ({model.provider})
        ...
        <TabsContent value="performance" className="space-y-6">
          <Card className="bg-gray-800 border-gray-700">
            <CardHeader>
              ...
              <p className="text-gray-400">
                Real-time performance and cost tracking per model
              </p>
            </CardHeader>
            <CardContent>
              <div className="text-center py-8 text-gray-400">
                ...
                <p className="text-sm mt-2">
                  Will include response times, success rates, and cost analytics
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <TabsContent value="ab-testing" className="space-y-6">
          <Card className="bg-gray-800 border-gray-700">
            <CardHeader>
              ...
              <p className="text-gray-400">
                Compare model performance for same requests
              </p>
            </CardHeader>
            <CardContent>
              {/* Create New A/B Test */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setNewABTest((prev) => ({
                          ...prev,
                          trafficSplit: parseInt(e.target.value) || 50,
                        }))
                      }
                      min="10"
                      max="90"
                  ...
                  <div>
                    ...
                    >
                      ...
                      {models
                        .filter((m) => m.available && m.id !== newABTest.modelA)
                        .map((model) => (
                          <option key={model.id} value={model.id}>
                            {model.name} ({model.provider})
                          </option>
                        ...
                        <div className="flex items-center justify-between mb-4">
                          <div>
                            ...
                            <p className="text-sm text-gray-400">
                              {test.feature} • {test.trafficSplit}% /{" "}
                              {100 - test.trafficSplit}% split
                            </p>
                          </div>
                          <div className="flex items-center gap-2">
                            <Badge
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="bg-gray-800 p-3 rounded-lg">
                            <h5 className="font-medium text-white mb-2">
                              Model A:{" "}
                              {models.find((m) => m.id === test.modelA)?.name}
                            </h5>
                            <div className="grid grid-cols-2 gap-2 text-sm">
                              <div>
                                <span className="text-gray-400">Requests:</span>
                                <span className="text-white ml-2">
                                  {test.metrics.modelA.requests}
                                </span>
                              </div>
                              <div>
                                <span className="text-gray-400">Avg Time:</span>
                                <span className="text-white ml-2">
                                  {test.metrics.modelA.avgTime}s
                                </span>
                              </div>
                              <div>
                                ...
                                <span className="text-white ml-2">
                                  {test.metrics.modelA.successRate}%
                                </span>
                              </div>
                              <div>
                                <span className="text-gray-400">
                                ...
                                <span className="text-white ml-2">
                                  {test.metrics.modelA.userRating}/5
                                </span>
                              </div>
                            </div>
                          </div>
                          <div className="bg-gray-800 p-3 rounded-lg">
                            <h5 className="font-medium text-white mb-2">
                              Model B:{" "}
                              {models.find((m) => m.id === test.modelB)?.name}
                            </h5>
                            <div className="grid grid-cols-2 gap-2 text-sm">
                              <div>
                                <span className="text-gray-400">Requests:</span>
                                <span className="text-white ml-2">
                                  {test.metrics.modelB.requests}
                                </span>
                              </div>
                              <div>
                                <span className="text-gray-400">Avg Time:</span>
                                <span className="text-white ml-2">
                                  {test.metrics.modelB.avgTime}s
                                </span>
                              </div>
                              <div>
                                <span className="text-gray-400">
                                ...
                                <span className="text-white ml-2">
                                  {test.metrics.modelB.successRate}%
                                </span>
                              </div>
                              <div>
                                <span className="text-gray-400">
                                ...
                                <span className="text-white ml-2">
                                  {test.metrics.modelB.userRating}/5
                                </span>
                              </div>
                            </div>
                          </div>
        ...
        <TabsContent value="ai-recommendations" className="space-y-6">
          <Card className="bg-gray-800 border-gray-700">
            <CardHeader>
              ...
              <p className="text-gray-400">
                Machine learning insights for optimal model selection
              </p>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {aiRecommendations.length === 0 ? (
                  <div className="text-center py-8 text-gray-400">
                    ...
                    <p>
                      Analyzing performance data to generate recommendations...
                    </p>
                  </div>
                ) : (
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <Badge
                                variant="outline"
                                className="text-gray-400 border-gray-600 capitalize"
                              >
                                {rec.type.replace("_", " ")}
                              </Badge>
        ...
        <TabsContent value="custom-prompts" className="space-y-6">
          <Card className="bg-gray-800 border-gray-700">
            <CardHeader>
              ...
              <p className="text-gray-400">
                Create and manage custom system prompts for AI features
              </p>
            </CardHeader>
            <CardContent>
              {/* Create New Prompt */}
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            ...
                            <p className="text-sm text-gray-400">
                              {
                                featureMappings.find(
                                  (f) => f.featureId === prompt.feature,
                                )?.featureName
                              }{" "}
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                          ...
                          <div className="text-center bg-gray-800 p-2 rounded">
                            <div className="text-white font-medium">
                              {prompt.performance.userRating}/5
                            </div>
        ...
        <TabsContent value="quality-scoring" className="space-y-6">
          <Card className="bg-gray-800 border-gray-700">
            <CardHeader>
              ...
              <p className="text-gray-400">
                User feedback loops to improve model selection
              </p>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <CardContent className="p-4 text-center">
                      ...
                      <div className="flex justify-center mt-2">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <Star
                            key={star}
                            className={`h-3 w-3 ${star <= Math.round(qualityMetrics.overall_rating) ? "text-yellow-400 fill-current" : "text-gray-600"}`}
                          />
                        ))}
                      </div>
                    </CardContent>
                  ...
                  <Card className="bg-gray-700 border-gray-600">
                    <CardContent className="p-4 text-center">
                      <div className="text-2xl font-bold text-white mb-1">
                        {qualityMetrics.total_ratings.toLocaleString()}
                      </div>
                  ...
                  <Card className="bg-gray-700 border-gray-600">
                    <CardContent className="p-4 text-center">
                      <div className="text-2xl font-bold text-white mb-1">
                        {qualityMetrics.satisfaction_rate}%
                      </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {featureMappings.map((feature) => {
                      const featureMetrics = qualityMetrics.ratings_by_feature[
                        feature.featureId
                      ] || {
                        rating: 4.2,
                        count: 0,
                            ...
                            <div className="flex items-center gap-2">
                              <div className="flex">
                                {[1, 2, 3, 4, 5].map((star) => (
                                  <Star
                                    key={star}
                                    className={`h-4 w-4 ${star <= Math.round(featureMetrics.rating) ? "text-yellow-400 fill-current" : "text-gray-600"}`}
                                  />
                                ))}
                              </div>
                              <span className="text-sm text-gray-400">
                                {featureMetrics.rating}
                              </span>
                              <span className="text-xs text-gray-500">
                                ({featureMetrics.count} ratings)
                              </span>
                            </div>
                          </div>
                  ...
                  <div className="space-y-3">
                    {qualityScores.length === 0 ? (
                      <div className="text-center py-8 text-gray-400">
                        ...
                        <p className="text-sm mt-2">
                          Feedback will appear here as users rate AI responses
                        </p>
                      </div>
                    ) : (
                      qualityScores.slice(0, 5).map((score, idx) => {
                        const featureName =
                          featureMappings.find(
                            (f) => f.featureId === score.feature_id,
                          )?.featureName || score.feature_id;
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <div className="flex">
                                    {[1, 2, 3, 4, 5].map((star) => (
                                      <Star
                                        key={star}
                                        className={`h-3 w-3 ${star <= score.numeric_rating ? "text-yellow-400 fill-current" : "text-gray-600"}`}
                                      />
                                    ))}
                                  </div>
                                </div>
