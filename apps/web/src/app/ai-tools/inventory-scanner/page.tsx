...
export default function InventoryScannerPage() {
  ...
  const [selectedItemForBarcode, setSelectedItemForBarcode] = useState<
    string | null
  >(null);
...
Analyze this image and identify all items visible. For each item, provide details in this JSON format:
{
  "items": [
    {
      ...
      "requires_appraisal": boolean (true for art, jewelry, antiques)
        ...
        try {
          ...
          try {
            ...
            const itemsWithIds = parsed.items.map(
              (item: Partial<InventoryItem>, idx: number) => ({
                ...item,
      ...
      const totalValue = allItems.reduce(
        (sum, item) => sum + item.estimated_value * item.quantity,
        0,
      );
      ...
      const recommendations = [
        totalValue > 100000
          ? "Consider increasing your personal property coverage limit"
          : "",
        highValueCount > 5
          ? "Schedule high-value items separately on your policy"
          : "",
        allItems.some((i) => i.requires_appraisal)
  ...
  const saveInventory = async () => {
    ...
    try {
      // Merge edited items
      const finalItems = scanResult.items.map(
        (item) => editingItems[item.id] || item,
      );
  ...
  const updateItem = (id: string, updates: Partial<InventoryItem>) => {
    setEditingItems((prev) => ({
      ...
      [id]: {
        ...(prev[id] ||
          scanResult?.items.find((i) => i.id === id) ||
          ({} as InventoryItem)),
        ...updates,
      },
    }));
            ...
            <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
              ...
              <div className="mb-6 sm:mb-8">
                <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
                  ...
                  <Badge
                    variant="outline"
                    className="text-gray-400 border-gray-600 w-fit"
                  >
                    AI Enhanced
                  </Badge>
                </div>
                <p className="text-sm sm:text-base text-gray-400 max-w-3xl">
                  Automatically catalog your belongings for insurance
                  documentation. Our AI identifies items, estimates values, and
                  helps ensure you have adequate coverage.
                </p>
              </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    <Card
                      onClick={() => {
                        ...
                        setFilterRoom("");
                        toast.info(
                          `Showing all ${scanResult.items.length} items`,
                        );
                      }}
                    ...
                    <Card
                      onClick={() => {
                        const breakdown = scanResult.items
                          .map((item) => ({
                            name: item.name,
                    ...
                    >
                      <CardContent className="p-3 sm:p-4">
                        <div className="flex items-center gap-2 sm:gap-3">
                          ...
                          <div>
                            <p className="text-lg sm:text-2xl font-bold text-white">
                              ${scanResult.total_value.toLocaleString()}
                            </p>
                            <p className="text-xs sm:text-sm text-gray-500">
                              Total Value
                            </p>
                    ...
                    <Card
                      onClick={() => {
                        setFilterCategory("");
                        setFilterRoom("");
                        const highValueItems = filteredItems.filter(
                          (item) => item.high_value,
                        );
                        if (highValueItems.length > 0) {
                          toast.info(
                            `Showing ${highValueItems.length} high value items`,
                          );
                        }
                      }}
                    ...
                    <Card
                      onClick={() => {
                        ...
                        toast.info(
                          `Items found in ${Object.keys(scanResult.rooms).length} rooms`,
                        );
                      }}
                    ...
                    >
                      <CardContent className="p-3 sm:p-4">
                        <div className="flex items-center gap-2 sm:gap-3">
                          ...
                          <div>
                            <p className="text-lg sm:text-2xl font-bold text-white">
                              {Object.keys(scanResult.rooms).length}
                            </p>
                            <p className="text-xs sm:text-sm text-gray-500">
                              Rooms Scanned
                            </p>
                      ...
                      <div className="space-y-3 sm:space-y-0 sm:flex sm:flex-wrap sm:items-center sm:gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setSortBy(
                                e.target.value as "value" | "name" | "room",
                              )
                            }
                            className="text-xs sm:text-sm bg-gray-700 border-gray-600 text-white rounded px-2 py-1 flex-1 sm:flex-none"
                          >
                  ...
                  <div className="space-y-4">
                    ...
                    {filteredItems.map((item) => (
                      <Card
                        key={item.id}
                        className="bg-gray-900/50 backdrop-blur-md border-gray-800/50 hover:border-gray-700/50 transition-all"
                      >
                        <CardContent className="p-3 sm:p-4">
                            ...
                            <div className="lg:col-span-2">
                              <div className="flex items-start gap-2">
                                <span className="text-xl sm:text-2xl flex-shrink-0">
                                  {CATEGORY_ICONS[
                                    item.category as keyof typeof CATEGORY_ICONS
                                  ] || "📦"}
                                </span>
