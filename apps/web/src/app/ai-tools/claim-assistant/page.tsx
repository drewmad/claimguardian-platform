...
const CLAIM_STEPS: ClaimStep[] = [
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        description:
          "Take reasonable steps to prevent additional damage (tarps, shut off water, etc.)",
        completed: false,
        required: true,
      },
      {
    ...
    tasks: [
      {
        ...
        description:
          "Report the claim within policy time limits (usually 24-72 hours)",
        completed: false,
        required: true,
      },
      {
...
export default function ClaimAssistantPage() {
  ...
  const [expandedCompletedSteps, setExpandedCompletedSteps] = useState<
    Set<number>
  >(new Set());
  ...
  useEffect(() => {
    async function loadModelConfig() {
      try {
        const modelConfig =
          await aiModelConfigService.getModelForFeature("claim-assistant");
        if (modelConfig) {
    ...
    try {
      const completedTasks = steps.flatMap((step) =>
        step.tasks
          ...
          .map((task) => `${step.title}: ${task.title}`),
      );
      ...
      try {
        ...
        const response = await aiClient.chat(
          [
            {
              role: "system",
              content: "You are a helpful insurance claim assistant.",
            },
            ...
            <div className="max-w-7xl mx-auto space-y-6">
              ...
              <div className="mb-8">
                ...
                <p className="text-gray-400 max-w-3xl">
                  Step-by-step guidance through the insurance claim process.
                  Follow each step carefully to ensure you don't miss any
                  critical actions or documentation.
                </p>
              </div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="text-right">
                      <p className="text-2xl font-bold text-white">
                        {steps.reduce(
                          (acc, step) =>
                            acc + step.tasks.filter((t) => t.completed).length,
                          0,
                        )}
                      </p>
                      <p className="text-sm text-gray-400">
                            ...
                            <div key={step.id}>
                              ...
                              >
                                <div className="flex items-start gap-3">
                                  <div
                                    className={`p-2 rounded-lg ${isActive ? "bg-blue-600/20" : isCompleted ? "bg-green-600/20" : "bg-gray-700"}`}
                                  >
                                    <Icon
                                      className={`h-4 w-4 ${isActive ? "text-blue-400" : isCompleted ? "text-green-400" : "text-gray-400"}`}
                                    />
                                  </div>
                        ...
                        {steps[currentStep].tasks.map((task) => (
                          ...
                          >
                            <div className="flex items-start gap-3">
                              <button
                                onClick={() =>
                                  toggleTask(steps[currentStep].id, task.id)
                                }
                                className="mt-0.5"
                              >
                                {task.completed ? (
                              ...
                              <div className="flex-1">
                                <div className="flex items-center gap-2">
                                  <h4
                                    className={`font-medium ${task.completed ? "text-gray-300 line-through" : "text-white"}`}
                                  >
                                    {task.title}
                                  </h4>
                                  {task.required && (
                                    <Badge
                                      variant="outline"
                                      className="text-xs text-orange-400 border-orange-600/30"
                                    >
                                      Required
                                    </Badge>
                                  )}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-2">
                          ...
                          <ul className="text-sm text-gray-300 space-y-1">
                            <li>
                              • Always document everything - photos,
                              conversations, receipts
                            </li>
                            <li>
                              • Never admit fault or sign anything without
                              reading carefully
                            </li>
                            <li>
                              • Keep a detailed log of all interactions with
                              your insurer
                            </li>
                            <li>
                              • Don't throw away damaged items until the
                              adjuster approves
                            </li>
                          </ul>
                        </div>
