    ...
    coverage_details: {
      ...
      clauses: {
        water_damage_roof: {
          ...
          description:
            "Covers sudden and accidental water damage from roof leaks.",
          covered: true,
        },
        wind_damage_siding: {
  ...
  },
  {
    ...
    coverage_details: {
      ...
      clauses: {
        flood_damage: {
          title: "Flood Damage",
          description: "Covers damage from rising waters.",
          covered: true,
        },
      },
    },
...
const MOCK_ANALYSIS: { [key: string]: AnalysisResult } = {
  "roof-leak": {
    ...
    severity: "Moderate",
    description:
      "Visible water stains on the ceiling and wall, indicating a potential roof leak. The damage appears to be recent.",
    coverage: {
      isCovered: true,
  ...
  "siding-damage": {
    ...
    severity: "Severe",
    description:
      "Multiple sections of vinyl siding have been detached or cracked, likely due to high winds.",
    coverage: {
      isCovered: true,
...
function DamageAnalyzerContent() {
  ...
  const [step, setStep] = useState<
    "select_policy" | "upload" | "capture" | "analyzing" | "result"
  >("select_policy");
  ...
  useEffect(() => {
    async function loadPolicies() {
      ...
      try {
        ...
        setPolicies(
          formattedPolicies.length > 0 ? formattedPolicies : MOCK_POLICIES,
        );
      } catch (error) {
...
Please analyze the damage carefully, considering the policy context provided.`,
  ...
  const handleQualityFeedback = async (
    type: "helpful" | "accuracy" | "comment",
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-400 mt-2 max-w-3xl mx-auto">
            Upload a photo of property damage for an instant AI analysis,
            including severity, description, and coverage details from your
            policy.
          </p>
          <div className="mt-4">
  ...
  return (
    <div>
      <h2 className="text-2xl font-semibold text-center text-white mb-6">
        Select a Policy to Analyze Against
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {policies.map((policy) => (
          <button
            ...
            className="p-6 rounded-lg border-2 border-gray-700 hover:border-cyan-400 hover:bg-gray-800 transition text-left"
          >
...
function UploadStep({
  ...
  return (
    <div className="text-center">
      <div
        {...getRootProps()}
        className={`p-12 border-2 border-dashed rounded-lg cursor-pointer transition ${isDragActive ? "border-cyan-400 bg-gray-800" : "border-gray-600 hover:border-cyan-500"}`}
      >
        <input {...getInputProps()} />
        ...
        <p className="text-gray-400 mt-2">
          Please upload one image at a time for the most accurate analysis.
        </p>
      </div>
  ...
  return (
    <div className="text-center">
      ...
      <p className="text-gray-400 mb-6">
        Our AI is assessing the damage and cross-referencing your policy. This
        may take a moment.
      </p>
...
function ResultStep({
  ...
  const getDamageIcon = (damageType: string) => {
    if (damageType.toLowerCase().includes("water"))
      return <Droplets className="w-6 h-6" />;
    if (damageType.toLowerCase().includes("wind"))
      return <Wind className="w-6 h-6" />;
    if (damageType.toLowerCase().includes("struct"))
      return <Home className="w-6 h-6" />;
    if (damageType.toLowerCase().includes("electric"))
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <CardHeader>
            <CardTitle className="flex items-center gap-3">
              <span className={getSeverityColor(result.severity)}>
                {getDamageIcon(result.damageType)}
              </span>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div>
                ...
                <p
                  className={`text-lg font-semibold ${getSeverityColor(result.severity)}`}
                >
                  {result.severity}
                </p>
              </div>
        ...
        <Card
          className={`border-2 ${result.coverage.isCovered ? "border-green-500 bg-green-900/20" : "border-red-500 bg-red-900/20"}`}
        >
          <CardHeader>
            <CardTitle className="flex items-center gap-3">
              {result.coverage.isCovered ? (
          ...
          <CardContent>
            <div className="space-y-4">
              <div>
                ...
                <p className="text-lg font-semibold text-white">
                  {result.coverage.policy_provider} -{" "}
                  {result.coverage.policy_number}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-400">Relevant Clause</p>
                <p className="text-lg font-semibold text-white">
                  {result.coverage.clause}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-400">Clause Details</p>
                <p className="text-white">
                  {result.coverage.clause_description}
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Badge
                    className={`${abTestInfo.variant === "A" ? "bg-green-600/20 text-green-300 border-green-600/30" : "bg-purple-600/20 text-purple-300 border-purple-600/30"}`}
                  >
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <Button
                      size="sm"
                      variant={
                        qualityFeedback.helpful === true ? "default" : "outline"
                      }
                      onClick={() => onQualityFeedback("helpful", true)}
                      disabled={feedbackSubmitted}
                      className={
                        qualityFeedback.helpful === true
                      ...
                      >
                        <Star
                          className={`w-4 h-4 ${
                            ...
                            rating <= qualityFeedback.accuracy
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      onQualityFeedback("comment", e.target.value)
                    }
                ...
                <Button
                  ...
                  disabled={
                    feedbackSubmitted ||
                    (!qualityFeedback.helpful && !qualityFeedback.accuracy)
                  }
