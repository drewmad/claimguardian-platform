...
const DOCUMENT_TEMPLATES: DocumentTemplate[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      {
        id: "new_damage_date",
        label: "Date New Damage Discovered",
        type: "date",
        required: true,
      },
      {
        id: "new_damage_description",
        label: "New Damage Description",
        type: "textarea",
        required: true,
      },
      {
      ...
      },
      {
        ...
        label: "Contractor Assessment Details",
        type: "textarea",
        required: false,
      },
    ],
  },
  {
    ...
    fields: [
      ...
      {
        id: "itemized_losses",
        label: "Itemized List of Losses",
        type: "textarea",
        required: true,
      },
      {
        id: "total_loss_amount",
  ...
  useEffect(() => {
    const initializeComponent = async () => {
      try {
        ...
        const modelConfig =
          await aiModelConfigService.getModelForFeature("document-generator");
        if (modelConfig) {
        ...
        logger.error(
          "Failed to initialize document generator:",
          toError(error),
        );
      }
    };
      ...
      const completeData = {
        ...
        sender_name:
          user?.user_metadata?.firstName +
            " " +
            user?.user_metadata?.lastName || "Policyholder",
...
The letter should be ready to send after adding the recipient's information.`;
      ...
      try {
          ...
          [
            {
              role: "system",
              content:
                "You are an expert insurance claim documentation specialist with deep knowledge of Florida property insurance laws and regulations.",
            },
        ...
        try {
          ...
          setShowPreview(true);
          toast.success(
            "Document generated successfully (using fallback model)!",
          );
        } catch (fallbackError) {
          ...
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="mb-8 relative">
              ...
              <div className="relative">
                <Link href="/ai-tools" className={liquidGlass.links.button}>
                  ← Back to AI Tools
                </Link>
                <div className="flex items-center gap-4 mb-6">
                  <div className={liquidGlass.iconContainers.large}>
                    <FileSearch
                      className={`h-8 w-8 text-indigo-300 ${liquidGlass.text.glowPrimary}`}
                    />
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h1
                        className={`text-4xl font-bold text-white ${liquidGlass.text.shadowLight}`}
                      >
                        Document Generator
                      </h1>
                      <Badge className={liquidGlass.badges.warning}>Beta</Badge>
                    </div>
                    <p
                      className={`text-gray-300 max-w-3xl ${liquidGlass.text.shadowDark}`}
                    >
                      Generate professional insurance claim documents, appeals,
                      and correspondence. Our AI ensures proper formatting,
                      legal language, and Florida-specific requirements.
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <CardHeader>
                    <CardTitle className="text-white flex items-center gap-2">
                      <div className={liquidGlass.iconContainers.small}>
                        <FileText
                          className={`h-5 w-5 text-indigo-300 ${liquidGlass.text.glowSecondary}`}
                        />
                      </div>
                      Document Templates
                    </CardTitle>
                ...
                {!selectedTemplate ? (
                  <Card className={liquidGlass.cards.default}>
                    <CardContent className="p-12 text-center">
                      <div
                        className={`${liquidGlass.iconContainers.large} mx-auto mb-4 w-fit`}
                      >
                        <FileText
                          className={`h-12 w-12 text-indigo-300 ${liquidGlass.text.glowSecondary}`}
                        />
                      </div>
            ...
            <Card className={liquidGlass.cards.info}>
              <CardContent className="p-6">
                <div className="flex items-start gap-4">
                  <div
                    className={`${liquidGlass.iconContainers.medium} flex-shrink-0 mt-0.5`}
                  >
                    <Shield
                      className={`h-6 w-6 text-blue-300 ${liquidGlass.text.glowSecondary}`}
                    />
                  </div>
                  <div>
