...
export default function DebugIndexPage() {
    ...
    try {
      ...
      if (testPath.startsWith("/")) {
        ...
        if (testWindow) {
          ...
          setTestOutput((prev) => ({
            ...prev,
            [testName]: `Opened successfully in ${loadTime.toFixed(0)}ms`,
          }));
          setTestResults((prev) => ({ ...prev, [testName]: "success" }));
        } else {
          setTestOutput((prev) => ({
            ...prev,
            [testName]: "Popup blocked - please allow popups for testing",
          }));
        ...
        const componentExists =
          testPath.includes("components") || testPath.includes("hooks");
        if (componentExists) {
          setTestOutput((prev) => ({
            ...prev,
            [testName]: "Component exists and uses centralized hook",
          }));
          setTestResults((prev) => ({ ...prev, [testName]: "success" }));
        } else {
          setTestOutput((prev) => ({
            ...prev,
            [testName]: "Service configured but unused",
          }));
          setTestResults((prev) => ({ ...prev, [testName]: "success" }));
        }
      }
  ...
  const collectGoogleMapsStatus = useCallback(async () => {
    ...
    const placesLoaded = !!(window as unknown as Record<string, any>).google
      ?.maps?.places;

    // Determine hook status
    let hookStatus: "loading" | "loaded" | "error" | "not-configured" =
      "not-configured";
    ...
    const integrationTests = [
      ...
      },
      {
        ...
        details: clientKeyConfigured
          ? "Service ready but unused - uses server-side key"
          : "Unused service - properly configured for server-side use",
      },
      {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-5 bg-gray-800/50 backdrop-blur-md">
            ...
            <TabsTrigger
              value="signup"
              className="data-[state=active]:bg-green-600/30 data-[state=active]:text-green-300"
            >
            ...
            <TabsTrigger
              value="maps"
              className="data-[state=active]:bg-red-600/30 data-[state=active]:text-red-300"
            >
            ...
            <TabsTrigger
              value="system"
              className="data-[state=active]:bg-orange-600/30 data-[state=active]:text-orange-300"
            >
                      ...
                      {debugInfo.authStatus.sessionDetails && (
                        <div className="bg-gray-700/50 rounded-lg p-3 text-sm">
                          <div>
                            ...
                            {debugInfo.authStatus.sessionDetails.userId}
                          </div>
                          <div>
                            <strong>Email:</strong>{" "}
                            {debugInfo.authStatus.sessionDetails.email}
                          </div>
                          <div>
                            <strong>Provider:</strong>{" "}
                            {debugInfo.authStatus.sessionDetails.provider}
                          </div>
                          <div>
                            <strong>Expires:</strong>{" "}
                            {new Date(
                              debugInfo.authStatus.sessionDetails.expiresAt,
                            ).toLocaleString()}
                          </div>
                        </div>
                      )}
              ...
              {[
                {
                  ...
                  href: "/auth/signup",
                  description:
                    "Test consolidated signup flow with Florida compliance",
                },
                {
                  title: "Sign In",
                  href: "/auth/signin",
                ...
                <Card
                  key={test.title}
                  className="bg-gray-800/70 backdrop-blur-xl border-gray-700/50"
                >
                  <CardHeader>
          ...
          <TabsContent value="maps" className="mt-6">
                  ...
                  {debugInfo?.googleMapsStatus ? (
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div className="flex items-center gap-2">
                          {debugInfo.googleMapsStatus.hookStatus ===
                          "loaded" ? (
                            <CheckCircle className="h-4 w-4 text-green-400" />
                          ) : debugInfo.googleMapsStatus.hookStatus ===
                            "loading" ? (
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-white">
                          {debugInfo ? "Connected" : "Connecting..."}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
                    ...
                    <div className="border-t border-gray-700 pt-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <Button
                          size="sm"
                          onClick={() =>
                            window.open("/onboarding/property-setup", "_blank")
                          }
                        ...
                        <Button
                          size="sm"
                          onClick={() =>
                            copyToClipboard(window.location.origin + "/debug")
                          }
            ...
            {debugInfo?.googleMapsStatus?.integrationTests && (
              <Card className="bg-gray-800/70 backdrop-blur-xl border-gray-700/50 mt-6">
                ...
                <CardContent>
                  <div className="space-y-3">
                    {debugInfo.googleMapsStatus.integrationTests.map(
                      (test, index) => (
                        <div
                          key={index}
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              >
                                {testResults[test.name]?.toUpperCase() ||
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-700">
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        size="sm"
                        onClick={() => {
                          debugInfo.googleMapsStatus?.integrationTests.forEach(
                            (test) => {
                              runIntegrationTest(test.name, test.path);
                            },
                          );
          ...
          <TabsContent value="ai" className="mt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="bg-gray-800/70 backdrop-blur-xl border-gray-700/50">
                ...
                <CardContent>
                  <p className="text-gray-400 text-sm mb-4">
                    Test core AI functionality and API keys
                  </p>
                  <Button
                  ...
                  {debugInfo && (
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="text-white text-xs">
                          {debugInfo.browserInfo.userAgent.split("(")[0].trim()}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Language:</span>
                        <span className="text-white">
                          {debugInfo.browserInfo.language}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Timezone:</span>
                        <span className="text-white text-xs">
                          {debugInfo.browserInfo.timezone}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Cookies:</span>
                        <span className="text-white">
                          {debugInfo.browserInfo.cookiesEnabled ? "✓" : "✗"}
                        </span>
                      </div>
                  ...
                  {debugInfo && (
                    <div className="space-y-3">
                      ...
                      <div className="text-xs text-gray-400 mt-3">
                        <div>
                          Project:{" "}
                          {debugInfo.supabaseStatus.projectUrl
                            .split("//")[1]
                            ?.split(".")[0] || "Unknown"}
                        </div>
                        <div>
                        ...
                        <div>
                          API Keys:{" "}
                          {debugInfo.supabaseStatus.anonKeyPresent
                            ? "Configured"
                            : "Missing"}
                        </div>
                      </div>
                  ...
                  {debugInfo && (
                    <div className="space-y-3">
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-400">Cookies</span>
                        <span className="text-white">
                          {debugInfo.cookies.length} items
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-400">LocalStorage</span>
                        <span className="text-white">
                          {debugInfo.localStorage.length} items
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-yellow-400/80 text-sm">
                    These debug tools are for development and testing purposes
                    only. They should not be accessible in production
                    environments.
                  </p>
                </div>
              </div>
