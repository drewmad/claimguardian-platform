    ...
    <CardContent className="p-6 flex items-center gap-6">
      ...
      <div>
        ...
        <p className="text-red-200 mt-1">
          Source: {alert.sender_name} (Effective:{" "}
          {new Date(alert.effective_at!).toLocaleString()})
        </p>
      </div>
    </CardContent>
  </Card>
  ...
  <Card className="bg-panel backdrop-blur-lg border-border">
    <CardHeader>
      <CardTitle className="flex items-center justify-between">
        ...
        <Badge
          variant={property.status === "At Risk" ? "destructive" : "default"}
        >
          {property.status}
        </Badge>
      </CardTitle>
            ...
            selectedProperty.longitude && (
              ...
              >
                <div>
                  ...
                  <button className="mt-2 w-full bg-accent text-white text-sm py-1 rounded">
                    Start Claim
                  </button>
                </div>
              </Popup>
...
const PreparednessChecklist = () => {
  const [checklist, setChecklist] = useState({
    prepare: [
      {
        id: "prep1",
        text: "Photograph property exterior and interior",
        completed: false,
        isEvidence: true,
        link: "/ai-tools/evidence-organizer",
      },
      {
        id: "prep2",
        text: "Record a video walkthrough of your home's interior",
        completed: false,
        isEvidence: true,
        link: "/ai-tools/evidence-organizer",
      },
      {
        id: "prep3",
        text: "Upload photos of high-value items",
        completed: false,
        isEvidence: true,
        link: "/ai-tools/evidence-organizer",
      },
      {
        id: "prep4",
        text: "Secure outdoor furniture and loose items",
        completed: false,
      },
      {
        id: "prep5",
        text: "Board up windows and glass doors",
        completed: false,
      },
      {
        id: "prep6",
        text: "Assemble emergency kit (water, food, batteries)",
        completed: false,
      },
      {
        id: "prep7",
        text: "Review insurance policy documents",
        completed: false,
    ...
    survive: [
      ...
      {
        id: "surv2",
        text: "Monitor official news and weather alerts",
        completed: false,
      },
    ...
    recover: [
      {
        id: "recov1",
        text: 'Wait for the "all clear" from officials',
        completed: false,
      },
      {
        id: "recov2",
        text: "Document all property damage with photos/videos",
        completed: false,
        isEvidence: true,
        link: "/ai-tools/evidence-organizer",
      },
      {
        id: "recov3",
        text: "Contact your insurance provider to start a claim",
        completed: false,
      },
      {
        id: "recov4",
        text: "Make temporary repairs to prevent further damage",
        completed: false,
      },
    ],
  });
  const [activeTab, setActiveTab] = useState<"prepare" | "survive" | "recover">(
    "prepare",
  );
  ...
  useEffect(() => {
    async function loadProgress() {
      ...
      const progressMap = new Map(
        data.map((item: { item_id: string; completed: boolean }) => [
          item.item_id,
          item.completed,
        ]),
      );
      ...
      setChecklist((prev) => ({
        prepare: prev.prepare.map((item) => ({
          ...item,
          completed: progressMap.get(item.id) ?? item.completed,
        })),
        survive: prev.survive.map((item) => ({
          ...item,
          completed: progressMap.get(item.id) ?? item.completed,
        })),
        recover: prev.recover.map((item) => ({
          ...item,
          completed: progressMap.get(item.id) ?? item.completed,
        })),
      }));
        ...
        <ul className="space-y-3">
          {checklist[activeTab].map((item) => (
            <li key={item.id} className="flex items-center gap-3">
              ...
              >
                <div
                  className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${item.completed ? "border-accent bg-accent" : "border-border"}`}
                >
                  {item.completed && (
              ...
              <span
                className={`flex-grow ${item.completed ? "text-text-secondary line-through" : "text-text-primary"}`}
              >
                {item.text}
              </span>
      ...
      <DashboardLayout>
        <div className="p-6 text-center">
          ...
          <h2 className="text-xl font-bold text-white">
            Failed to load Disaster Hub data
          </h2>
      ...
      <div id="disaster-hub-content" className="p-6 space-y-6 animate-fade-in">
        ...
        <div>
          ...
          <p className="text-text-secondary mt-1">
            Your command center for emergency preparedness and response.
          </p>
        </div>
        ...
        {alerts.length > 0 ? (
          alerts.map((alert) => (
            <DisasterAlertBanner key={alert.id} alert={alert} />
          ))
        ) : (
            ...
            <CardContent className="p-6 flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-green-200 mt-1">
                  There are no active disaster alerts for your property
                  locations.
                </p>
              </div>
            </CardContent>
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="space-y-4">
                {properties.length > 0 ? (
                  properties.map((prop) => (
                    <PropertyStatusCard key={prop.id} property={prop} />
                  ))
                ) : (
