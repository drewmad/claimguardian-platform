...
function HomeSystemsContent() {
  ...
  const systemsNeedingAttention = systems.filter(
    (s) => s.status === "needs_attention" || s.status === "critical",
  ).length;
  const totalValue = systems.reduce(
    (sum, system) => sum + system.estimatedValue,
    0,
  );
  const avgEfficiency =
    systems
      .filter((s) => s.efficiency)
      .reduce((sum, s) => sum + (s.efficiency || 0), 0) /
    systems.filter((s) => s.efficiency).length;
          ...
          <div className="flex justify-between items-start mb-8">
            <div>
              ...
              <p className="text-gray-400">
                Monitor and maintain your home's critical systems
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {systems.map((system) => {
                ...
                const lifePercentage =
                  (system.age / system.expectedLifespan) * 100;
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-start gap-4">
                          <div
                            className={`w-12 h-12 rounded-lg flex items-center justify-center ${getStatusBgColor(system.status)}`}
                          >
                          ...
                          <div>
                            ...
                            <Badge
                              className={`mt-2 ${getStatusColor(system.status)}`}
                            >
                              {system.status
                                .replace("_", " ")
                                .charAt(0)
                                .toUpperCase() +
                                system.status.slice(1).replace("_", " ")}
                            </Badge>
                          </div>
                        </div>
                        ...
                        <div>
                          <div className="flex justify-between text-sm mb-1">
                            ...
                            <span className="text-gray-300">
                              {system.age} / {system.expectedLifespan} years
                            </span>
                          </div>
