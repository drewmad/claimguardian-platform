...
export async function trackLegalAcceptance(data: LegalAcceptanceData) {
  try {
    ...
    if (consentError) {
      logger.warn("Failed to record consent", {
    ...
    try {
      ...
      logger.error(
        ...
        {},
        prefUpdateError instanceof Error
          ? prefUpdateError
          : new Error(String(prefUpdateError)),
      );
    }
    ...
    logger.error(
      "Failed to track legal acceptance",
      { userId: data.userId },
      error as Error,
    );
...
export async function trackSignupConsents(data: {
  ...
  try {
    ...
    if (error) {
      logger.warn("Failed to update consent preferences", {
        error: error.message,
        code: error.code,
      });
    }
