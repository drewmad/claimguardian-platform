...
import {
  ...
  ExtractedPolicyData,
} from "@/lib/services/ai-document-extraction";
...
export async function processDocumentExtraction(
  params: ProcessExtractionParams,
) {
  try {
    ...
    if (
      existingExtraction &&
      existingExtraction.processing_status === "completed"
    ) {
    ...
    const extractionResult =
      await aiDocumentExtractionService.extractPolicyData(
        urlData.signedUrl,
        document.file_name,
        {
          apiProvider: "gemini",
    ...
    const updateData = {
      processing_status: extractionResult.success
        ? ("completed" as const)
        : ("failed" as const),
 ...
 */
export async function applyExtractionToProperty(
  extractionId: string,
  propertyId: string,
) {
  try {
