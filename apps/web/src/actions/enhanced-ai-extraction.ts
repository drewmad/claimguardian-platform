 ...
 */
export async function processEnhancedExtraction(
  params: ProcessEnhancedExtractionParams,
) {
  try {
    ...
    if (
      existingExtraction &&
      existingExtraction.confidence_score >=
        (params.options?.confidenceThreshold || 0.7)
    ) {
    ...
    let processingStatus: "completed" | "failed" | "review_required" =
      "completed";
...
function generateExtractionSuggestions(
  ...
  if (!result.data.dwellingCoverage) {
    suggestions.push(
      "Dwelling coverage amount missing - verify Coverage A section",
    );
  }
  ...
  if (!result.data.hurricaneDeductible && !result.data.windHailDeductible) {
    suggestions.push(
      "Hurricane/wind deductible not found - required for Florida policies",
    );
  }
  ...
  if (result.data.expirationDate) {
    ...
    if (daysUntilExpiry < 30) {
      suggestions.push(
        `Policy expires in ${daysUntilExpiry} days - consider renewal`,
      );
    }
  }
  ...
  if (result.data.dwellingCoverage && result.data.dwellingCoverage < 200000) {
    suggestions.push(
      "Dwelling coverage seems low for Florida property - verify adequacy",
    );
  }
