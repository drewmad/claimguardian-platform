  ...
  try {
    ...
    if (error) {
      logger.error(
        ...
        { userId: data.userId, eventType: data.eventType },
        error,
      );
      throw error;
    }
...
export async function captureSignupData(data: SignupTrackingData) {
  try {
    ...
    try {
      ...
      if (captureError) {
        ...
        if (
          ...
          captureError.code === "PGRST202"
        ) {
          logger.info(
            "[USER TRACKING] Falling back to direct user_profiles update",
          );
      ...
      logger.error(
        "[USER TRACKING] Unexpected RPC error",
        {},
    ...
    try {
      const { error: prefsError } = await supabase.rpc(
        "create_user_preferences",
        {
          ...
          p_preferences: {
            ...
            gdpr_consent_date: data.preferences.gdprConsent
              ? new Date().toISOString()
              : null,
            marketing_emails: data.preferences.marketingConsent,
            data_processing_consent: data.preferences.dataProcessingConsent,
            data_processing_consent_date: data.preferences.dataProcessingConsent
              ? new Date().toISOString()
              : null,
            ai_processing_consent: data.preferences.aiProcessingConsent,
            ai_processing_consent_date: data.preferences.aiProcessingConsent
              ? new Date().toISOString()
              : null,
            timezone:
      ...
      logger.error(
        "[USER TRACKING] Unexpected preferences error",
        {},
        prefsError instanceof Error
          ? prefsError
          : new Error(String(prefsError)),
      );
      // Don't throw - continue with signup
    }
    ...
    try {
      const consentTypes = [
        ...
        { type: "marketing_consent", value: data.preferences.marketingConsent },
        {
          ...
          value: data.preferences.dataProcessingConsent,
        },
        {
          type: "ai_processing_consent",
          value: data.preferences.aiProcessingConsent,
        },
      ];
      ...
      logger.error(
        "[USER TRACKING] Unexpected audit error",
        {},
