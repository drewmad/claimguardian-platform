...
export async function getUserPolicies(): Promise<{
  data: PolicyWithProperty[] | null;
  error: string | null;
}> {
  try {
...
export async function getPolicyById(
  policyId: string,
): Promise<{ data: PolicyWithProperty | null; error: string | null }> {
  try {
    const supabase = await createClient();
...
export async function createPolicy(
  policy: Omit<InsurancePolicy, "id" | "user_id" | "created_at" | "updated_at">,
): Promise<{ data: InsurancePolicy | null; error: string | null }> {
  try {
    const supabase = await createClient();
...
export async function getPolicyClaims(
  policyId: string,
  ...
  try {
    ...
    const stats = {
      totalCoverage: policies.reduce(
        (sum, p) => sum + (p.coverage_dwelling || 0),
        0,
      ),
      totalPremium: policies.reduce(
        (sum, p) => sum + (p.premium_annual || 0),
        0,
      ),
