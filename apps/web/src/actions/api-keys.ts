...
export async function updateAPIKeyPermissions(params: UpdateAPIKeyParams) {
  try {
    ...
    await apiKeyManager.updateAPIKeyPermissions(
      user.id,
      params.keyId,
      params.permissions,
    );
...
export async function adminGetAPIAnalytics(days: number = 30) {
  try {
    ...
    const analytics = {
      ...
      failed_requests:
        usageLogs?.filter((log) => log.status_code >= 400).length || 0,
      avg_response_time:
        usageLogs?.reduce((sum, log) => sum + (log.response_time_ms || 0), 0) /
          (usageLogs?.length || 1) || 0,
      rate_limit_violations:
        rateLimits?.reduce(
          (sum, limit) => sum + limit.limit_exceeded_count,
          0,
        ) || 0,
    ...
    return {
      data: null,
      error:
        error instanceof Error ? error.message : "Failed to get API analytics",
    };
  }
}
...
export async function adminUpdateTierLimits(
  ...
  try {
    ...
    const { error } = await supabase.from("api_tier_limits").upsert({
      tier,
    ...
    return {
      data: null,
      error:
        error instanceof Error ? error.message : "Failed to update tier limits",
    };
  }
}
...
): Array<{ user: string; requests: number }> {
  const userCounts = usageLogs.reduce(
    (acc, log) => {
...
function calculateTopEndpoints(
  usageLogs: Record<string, unknown>[],
...
function calculateDailyUsage(
  ...
  const dailyCounts = usageLogs.reduce(
    (acc, log) => {
