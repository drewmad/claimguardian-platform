...
export async function getComplianceDashboardData(): Promise<{
  ...
  try {
    ...
    const transformedConsents: ConsentRecord[] = (consents || []).map(consent => ({
      id: consent.id,
    ...
    logger.error('Failed to fetch compliance dashboard data', {}, error as Error)
    return {
      metrics: null,
      recentConsents: [],
...
export async function exportConsentRecords(): Promise<{
  ...
  try {
    ...
    logger.info('Consent records exported', { count: transformedConsents.length })
