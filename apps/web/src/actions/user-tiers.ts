    ...
    for (const userId of userIds) {
      try {
        switch (operation) {
          case "change_tier":
            if (!tier)
              throw new Error("Tier is required for change_tier operation");
            const tierResult = await updateUserTier({ userId, tier, reason });
            results.push({
              userId,
              success: !tierResult.error,
          ...
          case "delete":
            ...
            results.push({
              userId,
              success: true,
              note: "Marked for deletion - manual cleanup required",
            });
            break;
  ...
  try {
    ...
    for (const tierData of tierPermissions) {
      ...
      const changedPermissions = [
        ...newPermissions
          .filter((p: PermissionType) => !oldPermissions.includes(p))
          .map((p: PermissionType) => ({ permission: p, granted: true })),
        ...oldPermissions
          .filter((p: PermissionType) => !newPermissions.includes(p))
          .map((p: PermissionType) => ({ permission: p, granted: false })),
      ];
    ...
    for (const tierChange of changedTiers) {
      ...
      if (users) {
        for (const userRecord of users) {
          const user = userRecord.users as unknown as {
            id: string;
            email: string;
          };
          if (user?.email) {
...
export async function getUserUsageStats(userId: string) {
  try {
    ...
    const storageUsed =
      files?.reduce((total, file) => total + (file.file_size || 0), 0) || 0;
...
export async function getPropertyPricing(userId: string) {
  try {
    ...
    const additionalPropertiesNeeded = Math.max(
      0,
      (currentCount || 0) - pricing.freeLimit,
    );
    const monthlyAdditionalCost =
      additionalPropertiesNeeded * pricing.pricePerProperty;
