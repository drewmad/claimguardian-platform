...
export function WeatherDashboard() {
  ...
  const getWindCategory = (speed: number) => {
    ...
    if (speed >= 39) return { label: 'Tropical Storm', color: 'text-orange-600' }
    ...
    <div className="space-y-6">
      ...
      {severity && (
        <Alert className={`border-2 ${
          severity.level === 'extreme' ? 'border-red-500' :
          severity.level === 'severe' ? 'border-orange-500' :
          severity.level === 'elevated' ? 'border-yellow-500' :
          'border-green-500'
        }`}>
          <AlertTriangle className="h-4 w-4" />
          <AlertTitle>Weather Severity: {severity.level.toUpperCase()}</AlertTitle>
          <AlertDescription className="mt-2">
            <div className="flex items-center gap-4 text-sm">
              <span>Max Wind: {severity.metrics.maxWindSpeed} kts</span>
              <span>Active Alerts: {severity.metrics.alertCount}</span>
              <span>Update Frequency: {severity.multiplier === 1 ? 'Normal' : `${Math.round(1/severity.multiplier)}x`}</span>
            </div>
          </AlertDescription>
        </Alert>
      )}
      ...
      {alerts.length > 0 && (
        <div className="space-y-3">
          ...
          {alerts.map((alert) => (
            <Alert key={alert.event_id} className="border-orange-200 bg-orange-50">
                ...
                <Card key={`${obs.station_id}-${obs.observation_time}`}>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    <div className="pt-2 border-t">
                      <Badge className={windCat.color.replace('text-', 'bg-').replace('600', '100')}>
                        {windCat.label}
                      </Badge>
                    </div>
                  </CardContent>
              ...
              <Card key={idx}>
                ...
                <CardContent>
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={tide.quality === 'v' ? 'default' : 'secondary'}>
