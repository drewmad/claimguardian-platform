...
export function EvidenceWorkflowDashboard() {
  ...
  const loadData = async () => {
    try {
      const [evidenceData, workflowData, gapsData, analyticsData] =
        await Promise.all([
  ...
  const handleFileUpload = useCallback(
    async (files: FileList) => {
      ...
      for (let i = 0; i < newUploads.length; i++) {
        ...
        try {
          const evidenceItem = await evidenceCuratorService.analyzeEvidence(
            upload.file,
            claimId,
          );
      ...
      setTimeout(() => {
        setFileUploads((prev) =>
          prev.filter((u) => u.uploading || !u.analyzed),
        );
      }, 3000);
  ...
  const createWorkflow = async (claimType: string = "hurricane") => {
    try {
      const workflow = await evidenceCuratorService.createEvidenceWorkflow(
        claimId,
        claimType,
      );
      if (workflow) {
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400">
              AI-powered evidence management with automated gap detection
            </p>
          </div>
        </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-400">
                    {analytics.average_quality_score.toFixed(1)}
                  </p>
                </div>
      ...
      <Card className="bg-gray-800 border-gray-700">
        <CardHeader>
          ...
          <CardDescription>
            Drag and drop files or click to upload. AI will automatically
            analyze and categorize your evidence.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div
        ...
        <TabsContent value="evidence" className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedCategory(
                    e.target.value as EvidenceCategory | "all",
                  )
                }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredEvidence.map((item) => (
              <Card
                key={item.id}
                className="bg-gray-800 border-gray-700 hover:border-emerald-500/50 transition-colors"
              >
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      className={getQualityColor(item.quality_score)}
                      variant="outline"
                    >
          ...
          {workflows.length > 0 ? (
            workflows.map((workflow) => (
              <Card key={workflow.id} className="bg-gray-800 border-gray-700">
                ...
                <CardContent>
                  <div className="mb-4">
                    <Progress
                      value={workflow.completion_percentage}
                      className="h-2 mb-2"
                    />
            ...
            <Card className="bg-gray-800 border-gray-700">
              <CardContent className="text-center py-12">
                ...
                <p className="text-gray-400 mb-4">
                  Create an evidence workflow to get AI-guided step-by-step
                  assistance
                </p>
                <Button
                  onClick={() => createWorkflow()}
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className="text-white">
                            Missing {gap.category.replace("_", " ")}
                          </CardTitle>
                          <CardDescription>
                  ...
                  <CardContent>
                    ...
                    <div className="bg-red-900/10 border border-red-800/20 rounded-lg p-3 mb-4">
                      ...
                      <p className="text-xs text-red-400 mt-2">
                        ...
                        {gap.estimated_value_impact.toLocaleString()}
                      </p>
                    </div>
                    ...
                    {Object.entries(analytics.by_category).map(
                      ([category, count]) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <Progress
                              value={(count / analytics.total_items) * 100}
