...
export function MobileFieldApp() {
  ...
  const [newDocType, setNewDocType] = useState<
    "damage" | "inspection" | "inventory" | "maintenance"
  >("damage");
  ...
  const startDocumentation = async (
    ...
    if (!canAccess) {
      alert(
        `Your ${userLimits.tier} plan doesn't include access to ${type} documentation. Please upgrade your plan.`,
      );
      return;
    }
    ...
    if (userLimits.storageUsedMB >= userLimits.limits.storage) {
      alert(
        `Storage limit reached (${userLimits.limits.storage}MB). Please upgrade or free up space.`,
      );
      return;
    }
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="bg-gray-800 px-4 py-2 flex items-center justify-between text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <Battery
            className={`h-4 w-4 ${appState.battery > 20 ? "text-green-500" : "text-red-500"}`}
          />
        </div>
      </div>
        ...
        <div className="bg-gray-800 border-b border-gray-700 p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <Badge
                variant="outline"
                className={getTierColor(userLimits.tier)}
              >
                <div className="flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="flex justify-between">
              <span>Storage:</span>
              <span>
                {userLimits.storageUsedMB}/{userLimits.limits.storage}MB
              </span>
            </div>
            <div className="flex justify-between">
              <span>AI Requests:</span>
              <span>
                {userLimits.aiRequestsThisMonth}/{userLimits.limits.aiRequests}
              </span>
            </div>
          </div>
        </div>
          ...
          <Card className="bg-gray-800 border-gray-700">
            <CardContent className="p-4 space-y-4">
              ...
              {activeDoc.media.length > 0 && (
                <div className="space-y-2">
                  <label className="block text-sm font-medium">
                    Captured Media ({activeDoc.media.length})
                  </label>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              },
              {
                ...
                permission: "access_inventory_scanner",
              },
              {
                type: "maintenance",
                icon: Home,
              ...
              const canAccess = userLimits
                ? userLimits.tier !== "free" || type === "damage"
                : false;
                ...
                >
                  <CardContent className="p-4 text-center">
                    <div className="flex flex-col items-center gap-2">
                      <div
                        className={`p-3 rounded-lg ${canAccess ? "bg-blue-600/20" : "bg-gray-700"}`}
                      >
                        <Icon
                          className={`h-6 w-6 ${canAccess ? "text-blue-400" : "text-gray-500"}`}
                        />
                      </div>
            ...
            <Card className="bg-gray-800 border-gray-700">
              <CardContent className="p-8 text-center">
                ...
                <p className="text-gray-400 mb-4">
                  Start documenting property damage, inspections, or maintenance
                  in the field.
                </p>
                <Button onClick={() => setShowNewDocForm(true)}>
                  <Plus className="h-4 w-4 mr-2" />
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          {doc.metadata.timestamp.toLocaleDateString()}
                        </span>
                      </div>
                      <div className="flex items-center gap-1">
