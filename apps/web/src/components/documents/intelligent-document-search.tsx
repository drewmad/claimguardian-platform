...
const DOCUMENT_TYPE_LABELS = {
  policy: { label: 'Insurance Policy', icon: '🛡️', color: 'bg-blue-100 text-blue-800' },
  claim: { label: 'Claim', icon: '📋', color: 'bg-green-100 text-green-800' },
  warranty: { label: 'Warranty', icon: '🔧', color: 'bg-purple-100 text-purple-800' },
  receipt: { label: 'Receipt', icon: '🧾', color: 'bg-yellow-100 text-yellow-800' },
  contract: { label: 'Contract', icon: '📄', color: 'bg-orange-100 text-orange-800' },
  correspondence: { label: 'Email/Letter', icon: '✉️', color: 'bg-pink-100 text-pink-800' },
  other: { label: 'Other', icon: '📄', color: 'bg-gray-100 text-gray-800' }
}

const SEARCH_MODE_OPTIONS = [
  { value: 'hybrid', label: 'Smart Search', description: 'AI + Keywords', icon: Brain },
  { value: 'semantic', label: 'AI Search', description: 'Meaning-based', icon: Sparkles },
  { value: 'keyword', label: 'Text Search', description: 'Exact matches', icon: Search }
]
...
export function IntelligentDocumentSearchComponent() {
  ...
  const relevanceColor = useMemo(() => (score: number) => {
  ...
  const insightSeverityColor = useMemo(() => (severity: string) => {
    switch (severity) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold text-white mb-2">Intelligent Document Search</h1>
        <p className="text-gray-400">AI-powered search across all your insurance documents</p>
      </div>
            ...
            <div className="flex gap-2">
              {SEARCH_MODE_OPTIONS.map(mode => {
                ...
                return (
                  <Button
                    key={mode.value}
                    variant={searchState.searchMode === mode.value ? 'default' : 'outline'}
                    size="sm"
                    onClick={() => setSearchState(prev => ({ ...prev, searchMode: mode.value as any }))}
          ...
          {searchState.searchStats && (
            <div className="flex items-center gap-6 text-sm text-gray-400 border-t border-gray-700 pt-4">
              ...
              <div className="flex items-center gap-2">
                ...
                {Math.round(searchState.searchStats.avgRelevanceScore * 100)}% avg relevance
              </div>
            </div>
          )}
        </CardContent>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        ...
                        {/* Summary */}
                        {result.summary && (
                          <p className="text-gray-300 mb-3 line-clamp-2">{result.summary}</p>
                        )}
                        ...
                        {result.highlights.length > 0 && (
                          <div className="space-y-1 mb-3">
                            {result.highlights.slice(0, 2).map((highlight, idx) => (
                              <div key={idx} className="text-sm text-gray-400 bg-gray-700/50 p-2 rounded">
                                "...{highlight}..."
                              </div>
                            ))}
                          </div>
                        ...
                        {result.actionableItems && result.actionableItems.length > 0 && (
                          <div className="flex flex-wrap gap-2 mb-3">
                            {result.actionableItems.slice(0, 3).map((action, idx) => (
                        ...
                        <div className="flex items-center gap-4 text-xs text-gray-500">
                          <div className="flex items-center gap-1">
                            ...
                            {new Date(result.metadata.dateCreated).toLocaleDateString()}
                          </div>
      ...
      {/* Empty State */}
      {!searchState.isSearching && searchState.query && searchState.results.length === 0 && (
        <Card className="bg-gray-800 border-gray-700">
          <CardContent className="text-center py-12">
            ...
            <Button variant="outline" onClick={() => setSearchState(prev => ({ ...prev, query: '' }))}>
              Clear Search
            </Button>
          </CardContent>
        </Card>
      ...
      {!searchState.query && searchState.results.length === 0 && (
        <Card className="bg-gray-800 border-gray-700">
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-white mb-2">Intelligent Document Search</h3>
            <p className="text-gray-400 mb-6 max-w-2xl mx-auto">
              Upload your insurance documents and use AI-powered search to quickly find information.
              Search using natural language like "What's my hurricane deductible?" or "Show me all warranties that expire this year."
            </p>
