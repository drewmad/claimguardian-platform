...
export function EnhancedExtractionReview({
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(['basic', 'coverage']))
  ...
  const completionRate = totalFields > 0 ? ((totalFields - missingFields) / totalFields) * 100 : 0
  ...
  const getConfidenceIcon = (score: number) => {
    ...
    if (score >= 0.7) return <AlertCircle className="h-5 w-5 text-yellow-500" />
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-gray-900 p-3 rounded-lg">
              ...
              <div className="text-2xl font-bold text-yellow-500">{missingFields}</div>
            </div>
            <div className="bg-gray-900 p-3 rounded-lg">
        ...
        <CardContent className="p-6">
          <Tabs defaultValue="overview" className="w-full">
            ...
            <TabsContent value="coverage" className="mt-6 space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <DataField
                      ...
                      type="currency"
                      isEditing={isEditing}
                      onEdit={(v) => handleFieldEdit('medicalPaymentsCoverage', v)}
                    />
                  </div>
                </div>
              </div>
              ...
              {editedData.endorsements && editedData.endorsements.length > 0 && (
                <div className="mt-6">
                  <h3 className="text-lg font-semibold text-white mb-3">Endorsements & Riders</h3>
                  <div className="bg-gray-900 rounded-lg p-4">
                    <div className="space-y-2">
                      {editedData.endorsements.map((endorsement, idx) => (
                        <div key={idx} className="flex justify-between items-center">
                          <div>
                            <span className="text-white font-medium">{endorsement.code}</span>
                            <span className="text-gray-400 ml-2">{endorsement.description}</span>
                          </div>
                          {endorsement.premium && (
                            <span className="text-green-400">{formatCurrency(endorsement.premium)}</span>
                          )}
                        </div>
                      ))}
                    </div>
            ...
            <TabsContent value="premium" className="mt-6 space-y-4">
              {editedData.premiumBreakdown && (
                <div>
                  ...
                  <div className="bg-gray-900 rounded-lg p-4">
                    <div className="space-y-2">
                      {Object.entries(editedData.premiumBreakdown).map(([key, value]) => (
              ...
              {editedData.discounts && editedData.discounts.length > 0 && (
                <div>
                  <h3 className="text-lg font-semibold text-white mb-3">Discounts Applied</h3>
                  <div className="bg-gray-900 rounded-lg p-4">
                    <div className="space-y-2">
                      {editedData.discounts.map((discount, idx) => (
                        <div key={idx} className="flex justify-between items-center">
                          <div>
                            <span className="text-white font-medium">{discount.type}</span>
                            <span className="text-gray-400 ml-2">{discount.description}</span>
                          </div>
