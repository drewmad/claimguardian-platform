...
import { Shield, Users, TrendingUp, Lock, Eye, AlertCircle, CheckCircle } from 'lucide-react'
...
export function PrivacyPreservingAnalytics({ onInsightsUpdate }: PrivacyPreservingAnalyticsProps) {
  ...
  const [contributionData, setContributionData] = useState<Partial<ClaimContribution>>({
  ...
  const loadInsights = async (filters?: any) => {
    setIsLoading(true)
    try {
      const { data, error } = await supabase.functions.invoke('community_analytics', {
        body: {
          action: 'get_insights',
          filters: filters || {
            damageType: 'all',
    ...
    return (
      <Card className="bg-gray-800 border-gray-700">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-white mb-2">Privacy Level</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-white mb-1">Damage Type *</label>
              <select
                value={contributionData.damageType || ''}
                onChange={(e) => setContributionData(prev => ({ ...prev, damageType: e.target.value }))}
            ...
            <div>
              <label className="block text-sm font-medium text-white mb-1">Settlement Amount *</label>
              <input
                type="number"
                value={contributionData.settlementAmount || ''}
                onChange={(e) => setContributionData(prev => ({ ...prev, settlementAmount: parseInt(e.target.value) }))}
                className="w-full p-2 bg-gray-700 border border-gray-600 rounded text-white"
            ...
            <div>
              <label className="block text-sm font-medium text-white mb-1">Time to Settle (days)</label>
              <input
                type="number"
                value={contributionData.timeToSettle || ''}
                onChange={(e) => setContributionData(prev => ({ ...prev, timeToSettle: parseInt(e.target.value) }))}
                className="w-full p-2 bg-gray-700 border border-gray-600 rounded text-white"
                placeholder="e.g., 45"
              />
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-white mb-1">County</label>
              <input
                type="text"
                value={contributionData.county || ''}
                onChange={(e) => setContributionData(prev => ({ ...prev, county: e.target.value }))}
                className="w-full p-2 bg-gray-700 border border-gray-600 rounded text-white"
            ...
            <div>
              <label className="block text-sm font-medium text-white mb-1">Property Value</label>
              <input
                type="number"
                value={contributionData.propertyValue || ''}
                onChange={(e) => setContributionData(prev => ({ ...prev, propertyValue: parseInt(e.target.value) }))}
                className="w-full p-2 bg-gray-700 border border-gray-600 rounded text-white"
            ...
            <div>
              <label className="block text-sm font-medium text-white mb-1">Claim Success</label>
              <select
                value={contributionData.claimSuccess ? 'true' : 'false'}
                onChange={(e) => setContributionData(prev => ({ ...prev, claimSuccess: e.target.value === 'true' }))}
                className="w-full p-2 bg-gray-700 border border-gray-600 rounded text-white"
              >
          ...
          <div className="flex gap-3">
            <Button
              ...
              disabled={isContributing || !contributionData.damageType || !contributionData.settlementAmount}
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-gray-800 border-gray-700">
        <CardHeader>
          ...
          <p className="text-gray-400 text-sm">
            All data is anonymized and protected with mathematical privacy guarantees
          </p>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-3 text-gray-400">Loading privacy-protected insights...</span>
            </div>
          ) : (
              ...
              {insights.map((insight, index) => (
                <div key={index} className="p-4 bg-gray-700/50 rounded-lg border border-gray-600">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h3 className="text-lg font-semibold text-white">{insight.damageType}</h3>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        ±{formatCurrency(insight.confidenceInterval.upper - insight.averageSettlement)}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-white">{insight.successRate}%</p>
                      <Progress value={insight.successRate} className="h-1 mt-1" />
                    </div>
                    <div>
