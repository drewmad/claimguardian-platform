...
export function AchievementCard({
  ...
  const levelColor =
    levelColors[
      Math.min(achievement.currentLevel, 4) as keyof typeof levelColors
    ];
  const levelGlow =
    levelGlows[
      Math.min(achievement.currentLevel, 4) as keyof typeof levelGlows
    ];
  const currentLevelName =
    achievement.levelNames[achievement.currentLevel - 1] || "Locked";
  ...
  const cardContent = (
    <Card
      ...
      onClick={onClick}
    >
      <CardHeader className={cn("pb-3", size === "small" && "p-0 pb-2")}>
          ...
          <div className="flex items-center gap-3">
            <div
              className={cn(
                ...
                achievement.currentLevel === 2 &&
                  "from-gray-500/30 to-gray-600/20",
                achievement.currentLevel === 3 &&
                  "from-yellow-600/30 to-yellow-700/20",
                achievement.currentLevel === 4 &&
                  "from-purple-600/30 to-purple-700/20",
                size === "small" && "p-2",
                size === "medium" && "p-3",
                size === "large" && "p-4",
              )}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="text-gray-300 font-medium">
                {achievement.currentPoints} / {achievement.nextLevelPoints}{" "}
                points
              </span>
            </div>
            ...
            {achievement.currentLevel < achievement.maxLevel && (
              <p className="text-xs text-gray-500">
                {achievement.nextLevelPoints - achievement.currentPoints} points
                to{" "}
                {achievement.levelNames[achievement.currentLevel] ||
                  "next level"}
              </p>
            )}
          </div>
