...
export function ParcelSearchMap({
  ...
  const [selectedParcel, setSelectedParcel] = useState<FloridaParcel | null>(
    null,
  );
  ...
  const mockParcels = useMemo(
    (): FloridaParcel[] => [
      ...
      },
      {
        ...
        coordinates: [-82.091, 26.9765] as [number, number],
  ...
  const filteredParcels = useMemo(() => {
    return mockParcels.filter((parcel) => {
      if (
        filters.county &&
        !parcel.county.toLowerCase().includes(filters.county.toLowerCase())
      ) {
        return false;
      }
      if (
  ...
  const mapProperties = useMemo(() => {
    return filteredParcels.map((parcel) => ({
      ...
      type: parcel.propertyUse.toLowerCase().includes("single")
        ? ("single_family" as const)
        : parcel.propertyUse.toLowerCase().includes("condo")
          ? ("condo" as const)
          : parcel.propertyUse.toLowerCase().includes("commercial")
            ? ("commercial" as const)
            : ("single_family" as const),
  ...
  const handleSearch = useCallback(async () => {
    ...
    const results = mockParcels.filter(
      (parcel) =>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFilters((prev) => ({
                          ...prev,
                          maxValue: parseInt(e.target.value) || 5000000,
                        }))
                      }
                      className="bg-gray-700 border-gray-600 text-white text-xs"
                    />
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFilters((prev) => ({
                          ...prev,
                          yearBuiltMin: parseInt(e.target.value) || 1900,
                        }))
                      }
                      className="bg-gray-700 border-gray-600 text-white text-xs"
                    />
            ...
            <CardContent className="space-y-2 text-xs">
              ...
              <div className="grid grid-cols-2 gap-4 pt-2 border-t border-gray-700">
                <div>
                  ...
                  <div className="text-white font-medium">
                    ${selectedParcel.justValue.toLocaleString()}
                  </div>
                </div>
                <div>
                  <div className="text-gray-400">Lot Size</div>
                  <div className="text-white font-medium">
                    {selectedParcel.lotSize} acres
                  </div>
                </div>
                <div>
              ...
              <div className="pt-2 border-t border-gray-700">
                ...
                <div className="text-white text-xs">
                  {selectedParcel.legalDescription}
                </div>
              </div>
        ...
        <div className="bg-gray-800/90 backdrop-blur-sm rounded-lg p-3 border border-gray-600">
          ...
          <div className="space-y-1 text-xs text-gray-300">
            ...
            <div>
              Avg Value: $
              {Math.round(
                filteredParcels.reduce((sum, p) => sum + p.justValue, 0) /
                  filteredParcels.length,
              ).toLocaleString()}
            </div>
            <div>
              Counties: {new Set(filteredParcels.map((p) => p.county)).size}
            </div>
          </div>
        </div>
      </div>
