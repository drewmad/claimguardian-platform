...
import {
  smartPolicyRecommendationsService,
  type PolicyRecommendation,
} from "@/lib/services/smart-policy-recommendations";
import {
  Card,
  CardContent,
  CardDescription,
...
export function SmartRecommendationsDashboard({
  userId,
  propertyId,
  className,
  ...
  const loadRecommendations = async () => {
    try {
      setLoading(true);
      const data =
        await smartPolicyRecommendationsService.getUserRecommendations(userId);
  ...
  const generateRecommendations = async () => {
    try {
      ...
      const newRecommendations =
        await smartPolicyRecommendationsService.generateRecommendations(
          userId,
          propertyId,
        );
  ...
  const updateRecommendationStatus = async (
    ...
    try {
      ...
      const success =
        await smartPolicyRecommendationsService.updateRecommendationStatus(
          recommendationId,
          status,
          notes,
        );
  ...
  const stats = {
    ...
    totalSavings: recommendations.reduce(
      (sum, r) => sum + (r.market_analysis.potential_savings || 0),
      0,
    ),
    avgConfidence:
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400">
            AI-powered analysis to optimize your insurance coverage and costs
          </p>
        </div>
        <Button
        ...
        <Card className="bg-gray-800 border-gray-700">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-400 mb-6">
              Generate AI-powered policy recommendations to optimize your
              ...
              >
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div
                        className={`p-2 rounded-lg ${getPriorityColor(recommendation.priority)} text-white`}
                      >
                ...
                <CardContent className="space-y-4">
                  ...
                  {(recommendation.impact.cost_savings ||
                    ...
                    <div className="bg-gray-900 rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                        ...
                        {recommendation.market_analysis.potential_savings >
                          0 && (
                          <div>
                            <p className="text-gray-400">Market Savings</p>
                            <p className="text-green-400 font-semibold">
                              $
                              {recommendation.market_analysis.potential_savings.toLocaleString()}
                            </p>
                          </div>
                        )}
                        {recommendation.impact.coverage_increase && (
                          <div>
                            ...
                            <p className="text-blue-400 font-semibold">
                              $
                              {recommendation.impact.coverage_increase.toLocaleString()}
                            </p>
                          </div>
                        )}
                      </div>
                  ...
                  <div className="bg-gray-900 rounded-lg p-4">
                    ...
                    {recommendation.recommendation.implementation_steps.length >
                      0 && (
                      <div>
                        ...
                        <ul className="space-y-1">
                          {recommendation.recommendation.implementation_steps.map(
                            (step, index) => (
                              <li
                                key={index}
                  ...
                  {/* Market Analysis */}
                  {recommendation.market_analysis.competitive_options.length >
                    0 && (
                    <div className="bg-gray-900 rounded-lg p-4">
                      <h4 className="font-semibold text-white mb-2">
                        Market Analysis
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="text-white">
                            $
                            {recommendation.market_analysis.current_rate.toLocaleString()}
                            /year
                          </span>
                        </div>
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="text-white">
                            $
                            {recommendation.market_analysis.market_average.toLocaleString()}
                            /year
                          </span>
                        </div>
                        {recommendation.market_analysis.competitive_options
                          .slice(0, 2)
                          .map((option, index) => (
                            <div
                              key={index}
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="text-white">
                          {recommendation.ai_analysis.model_used}
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-400">Processing Time</p>
                        <p className="text-white">
                          {recommendation.ai_analysis.processing_time_ms}ms
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-400">Data Points</p>
                        <p className="text-white">
                          {recommendation.ai_analysis.market_data_points}
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="text-white">
                          {
                            recommendation.ai_analysis.risk_assessment
                              .overall_risk_score
                          }
                          /100
                        </p>
                  ...
                  {recommendation.deadline && (
                    <div className="flex items-center space-x-2 text-sm">
                      ...
                      <span className="text-white">
                        {format(
                          new Date(recommendation.deadline),
                          "MMM d, yyyy",
                        )}
                      </span>
                    </div>
