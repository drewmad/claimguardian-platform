...
import {
  FileUploadEnhanced,
  type EnhancedFile,
} from "@/components/ui/file-upload-enhanced";
...
export function DocumentUploadWizard({
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<UploadDocumentCategory>();
  ...
  const processDocuments = useCallback(async () => {
    ...
    try {
      ...
      logger.track("document_processing_completed", {
        ...
        category: selectedCategory,
        averageConfidence:
          processed.reduce((acc, doc) => acc + doc.confidence, 0) /
          processed.length,
      });
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <FileUploadEnhanced
        ...
        acceptedTypes={
          selectedCategory
            ? DOCUMENT_CATEGORIES.find((cat) => cat.id === selectedCategory)
                ?.acceptedTypes || ["image/*", "application/pdf", ".pdf"]
      ...
      <Alert>
        ...
        <AlertDescription>
          Documents will be automatically processed with AI to extract key
          information and categorize content.
        </AlertDescription>
      </Alert>
    </motion.div>
  );
  ...
  return (
    ...
    >
      <div className="text-center">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Choose the category that best describes your {uploadedFiles.length}{" "}
          uploaded document(s)
        </p>
      </div>
    ...
    >
      <div className="text-center space-y-6">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Processing {uploadedFiles.length} documents in the{" "}
            {selectedCategory?.replace(/-/g, " ")} category
          </p>
        </div>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      Confidence: {doc.confidence}% • Category:{" "}
    ...
    >
      <div className="text-center space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Successfully processed {processedDocuments.length} documents with AI
            analysis
          </p>
        </div>
        ...
        <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-4">
          <p className="text-sm text-green-800 dark:text-green-200">
            Your documents have been analyzed and are ready for use in your
...
function generateMockExtractedData(categoryId: string) {
  switch (categoryId) {
    case "insurance-policy":
      return {
        policyNumber:
          "POL-" + Math.random().toString(36).substr(2, 9).toUpperCase(),
        carrier: "State Farm",
