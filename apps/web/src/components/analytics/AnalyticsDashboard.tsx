...
export function AnalyticsDashboard() {
  ...
  const processAnalyticsData = (
    ...
    const errorEvents = events.filter(
      (e) => e.event_category === "error",
    ).length;
    const errorRate =
      events.length > 0 ? (errorEvents / events.length) * 100 : 0;

    // Calculate conversion rate (mock data for demo)
    const conversionEvents = events.filter(
      (e) => e.event_name === "conversion",
    ).length;
    const conversionRate =
      uniqueUsers > 0 ? (conversionEvents / uniqueUsers) * 100 : 0;
    ...
    <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ...
              {Math.floor((metrics.avgSessionDuration % 60000) / 1000)}s
            </div>
            <p className="text-xs text-muted-foreground">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Most used features in the selected time period
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              AI-powered features and their usage
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
                ...
                <div key={index} className="space-y-2">
                  ...
                  <div className="flex items-center justify-between text-sm text-gray-500">
                    ...
                    <span className="flex items-center gap-1">
                      <DollarSign className="h-3 w-3" />${ai.cost.toFixed(2)}
                    </span>
                  </div>
                </div>
              ))}
