...
export function MLOperationsDashboard() {
  ...
  const avgLatency =
    metrics.reduce((sum, m) => sum + m.avg_latency, 0) / metrics.length || 0;
  const avgErrorRate =
    metrics.reduce((sum, m) => sum + m.error_rate, 0) / metrics.length || 0;
  ...
  const modelComparisonData = [
    {
      metric: "Accuracy",
      ...models.reduce((acc, m) => ({ ...acc, [m.name]: m.accuracy }), {}),
    },
    {
      metric: "F1 Score",
      ...models.reduce((acc, m) => ({ ...acc, [m.name]: m.f1_score }), {}),
    },
    {
      metric: "Precision",
      ...models.reduce((acc, m) => ({ ...acc, [m.name]: m.precision }), {}),
    },
    {
      metric: "Recall",
      ...models.reduce((acc, m) => ({ ...acc, [m.name]: m.recall }), {}),
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Select
            value={timeRange}
            onValueChange={(v: "24h" | "7d" | "30d") => setTimeRange(v)}
          >
          ...
          <CardContent>
            {activeJobs.map((job) => {
              ...
              return (
                <div
                  key={job.id}
                  className="space-y-2 p-4 bg-gray-900 rounded-lg"
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-medium text-white">{model?.name}</h4>
                      <p className="text-sm text-gray-400">
                        Epoch {job.epoch}/{job.total_epochs} • Loss:{" "}
                        {job.loss.toFixed(4)} • Accuracy:{" "}
      ...
      <Tabs defaultValue="models" className="space-y-4">
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>
                        {model.version} • {model.size_mb.toFixed(1)} MB •{" "}
                        {(model.parameters / 1000000).toFixed(1)}M params
                      </CardDescription>
                    </div>
                    <Badge
                      variant="outline"
                ...
                <CardContent>
                  ...
                  {model.last_prediction && (
                    <p className="text-xs text-gray-500 mt-2">
                      Last prediction:{" "}
                      {format(new Date(model.last_prediction), "MMM d, h:mm a")}
                    </p>
                  )}
                </CardContent>
              </Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-2xl font-bold text-white">
                      {selectedModel.training_loss.toFixed(4)}
                    </p>
                  </div>
                  <div className="space-y-2">
                    <p className="text-sm text-gray-400">Validation Loss</p>
                    <p className="text-2xl font-bold text-white">
                      {selectedModel.validation_loss.toFixed(4)}
                    </p>
                  </div>
                </div>
              </CardContent>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>
                        ...
                        {dataset.features} features • {dataset.size} GB
                      </CardDescription>
                    </div>
                    <div className="text-right">
                      <Badge
        ...
        <TabsContent value="monitoring" className="space-y-4">
          ...
          <Alert className="bg-blue-900/20 border-blue-900">
            ...
            <AlertDescription>
              Real-time monitoring shows live model performance across all
              deployed endpoints
            </AlertDescription>
          </Alert>
