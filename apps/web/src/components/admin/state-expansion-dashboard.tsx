...
import type {
  StateConfiguration,
  StateExpansionPlan,
} from "@/lib/expansion/state-manager";
...
export function StateExpansionDashboard() {
  ...
  const [activeView, setActiveView] = useState<
    "overview" | "states" | "plans" | "analytics"
  >("overview");
  ...
  const loadExpansionData = async () => {
    try {
      ...
      for (const stateCode of priorityStates) {
        const readiness =
          await stateExpansionManager.getExpansionReadiness(stateCode);
  ...
  const getDeploymentStatusInfo = (status: string) => {
    const Icon =
      DEPLOYMENT_STATUS_ICONS[status as keyof typeof DEPLOYMENT_STATUS_ICONS] ||
      Clock;
    const color =
      DEPLOYMENT_STATUS_COLORS[
        status as keyof typeof DEPLOYMENT_STATUS_COLORS
      ] || "#64748B";
  ...
  const expansionOpportunityData = US_STATES_DATA.filter(
    (state) => state.code !== "FL",
  )
    .map((state) => ({
      name: state.name,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400">
            Nationwide deployment planning and management
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="bg-gray-800 border-gray-700">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-white">
              {formatMarketSize(
                US_STATES_DATA.reduce(
                  (sum, s) =>
                    s.status === "production" ? sum + s.marketSize : sum,
                  0,
                ),
              )}
            </div>
        ...
        <Card className="bg-gray-800 border-gray-700">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-white">
              {
                US_STATES_DATA.filter((s) =>
                  ["development", "testing", "staging"].includes(s.status),
                ).length
              }
            </div>
            <p className="text-xs text-gray-500 mt-1">States in progress</p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-white">
              {Object.values(readinessScores).length > 0
                ? (
                    Object.values(readinessScores).reduce(
                      (sum, score) => sum + score,
                      0,
            ...
            <p className="text-xs text-gray-500 mt-1">Expansion readiness</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs
        value={activeView}
        onValueChange={(value) =>
          setActiveView(value as "overview" | "states" | "plans" | "analytics")
        }
        className="space-y-6"
      >
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <ScatterChart data={expansionOpportunityData}>
                    ...
                    <Tooltip
                      ...
                      labelFormatter={(code) => {
                        const state = expansionOpportunityData.find(
                          (s) => s.code === code,
                        );
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="text-white">
                            {formatMarketSize(state.marketSize)}
                          </span>
                        </div>
        ...
        <TabsContent value="states" className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6">
            {US_STATES_DATA.map((state) => {
              const config = stateConfigs.find(
                      ...
                      <div className="flex gap-2">
                        {config ? (
                          ...
                          <Button
                            size="sm"
                            onClick={() =>
                              handleInitializeState(state.code, state.name)
                            }
                          >
                            <Plus className="mr-2 h-4 w-4" />
                            Initialize
            ...
            <Card className="bg-gray-800 border-gray-700">
              <CardContent className="text-center py-12">
                ...
                <p className="text-gray-400 mb-4">
                  Create expansion plans to organize your multi-state rollout
                  strategy.
                </p>
                <Button
                  onClick={() =>
                    stateExpansionManager.createDefaultExpansionPlan()
                  }
                >
                  <Plus className="mr-2 h-4 w-4" />
                  Create Default Plan
                ...
                <Card key={plan.phase} className="bg-gray-800 border-gray-700">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        <CardTitle className="text-white">
                          Phase {plan.phase} Expansion
                        </CardTitle>
                        <p className="text-gray-400">
                          {plan.states.join(", ")} •{" "}
                          {plan.timeline.start.toLocaleDateString()} -{" "}
                          {plan.timeline.end.toLocaleDateString()}
                        </p>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between">
                            ...
                            <span className="text-white">
                              {plan.resources.engineeringEffort} weeks
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-400">
                            ...
                            <span className="text-white">
                              $
                              {plan.resources.dataAcquisitionCost.toLocaleString()}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-400">Compliance:</span>
                            <span className="text-white">
                              ${plan.resources.complianceCost.toLocaleString()}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            ...
                            <div className="flex justify-between font-medium">
                              ...
                              <span className="text-white">
                                ...
                                {(
                                  ...
                                  plan.resources.operationalCost
                                ).toLocaleString()}
                              </span>
                            </div>
                          </div>
                        ...
                        <div className="space-y-2">
                          {plan.risks.map((risk, index) => {
                            ...
                            return (
                              <div
                                key={index}
                                className="p-3 border border-gray-700 rounded text-sm"
                              >
        ...
        <TabsContent value="analytics" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="bg-gray-800 border-gray-700">
              <CardHeader>
                <CardTitle className="text-white">
                  Market Opportunity vs Readiness
                </CardTitle>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="text-center">
                  <div className="text-3xl font-bold text-blue-400">
                    {formatMarketSize(
                      US_STATES_DATA.reduce((sum, s) => sum + s.marketSize, 0),
                    )}
                  </div>
                  ...
                  <div className="text-3xl font-bold text-green-400">
                    {(
                      (US_STATES_DATA.filter(
                        (s) => s.status === "production",
                      ).reduce((sum, s) => sum + s.marketSize, 0) /
                        US_STATES_DATA.reduce(
                          (sum, s) => sum + s.marketSize,
                          0,
                        )) *
                      100
                    ).toFixed(1)}
                ...
                <div className="text-center">
                  <div className="text-3xl font-bold text-yellow-400">
                    {
                      US_STATES_DATA.filter((s) => s.status !== "production")
                        .length
                    }
                  </div>
                ...
                <div className="text-center">
                  <div className="text-3xl font-bold text-purple-400">
                    {expansionPlans.reduce(
                      (sum, plan) => sum + plan.resources.engineeringEffort,
                      0,
                    )}
                  </div>
