        ...
        <Card className="bg-gray-800 border-gray-700">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-400">
              {formatCost(optimizationSuggestions.reduce((sum, s) => sum + s.potentialSavings, 0))}
            </div>
            <p className="text-xs text-gray-400 mt-1">
              From optimization
            </p>
                      ...
                      <div key={tool} className="space-y-3 p-4 rounded-lg bg-gray-700/50">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                          <div>
                            ...
                            <div className="text-white font-medium">{formatUsage(data.requests)}</div>
                            <Progress value={requestPercentage} className="h-1 mt-1" />
                          </div>
                          <div>
                            <div className="text-gray-400">Avg Cost</div>
                            <div className="text-white font-medium">{formatCost(data.avgCost)}</div>
                          </div>
                          <div>
                            <div className="text-gray-400">Success Rate</div>
                            <div className="text-white font-medium">{(data.successRate * 100).toFixed(1)}%</div>
                          </div>
                        </div>
                      </div>
                    )
            ...
            <CardContent>
              <div className="space-y-3">
                {analytics.topUsers
                  ...
                  .map((user, index) => (
                    <div key={user.userId} className="flex items-center justify-between p-4 rounded-lg bg-gray-700/50">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-gray-400 text-sm">
                            {formatUsage(user.requests)} requests • Avg: {formatCost(user.avgCost)}
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-white font-bold">{formatCost(user.cost)}</div>
                        <div className="text-gray-400 text-sm">
                          {analytics.totalCost > 0 ? ((user.cost / analytics.totalCost) * 100).toFixed(1) : 0}% of total
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                {Object.entries(analytics.modelPerformance).map(([model, performance]) => (
                  <div key={model} className="p-4 rounded-lg bg-gray-700/50">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Badge variant={performance.successRate > 0.95 ? 'default' : 'destructive'}>
                      ...
                      <div key={date} className="flex items-center justify-between p-3 rounded bg-gray-700/30">
                        ...
                        <div className="text-right min-w-[120px]">
                          ...
                          <div className="text-gray-400 text-xs">
                            {formatUsage(data.requests)} requests • {data.uniqueUsers} users
                          </div>
                        </div>
                      </div>
                    )
              ...
              <div className="space-y-4">
                {optimizationSuggestions.length === 0 ? (
                  <div className="text-center py-8 text-gray-400">
                    ...
                    <h3 className="text-lg font-medium mb-2">No optimization opportunities found</h3>
