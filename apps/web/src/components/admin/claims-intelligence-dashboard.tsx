  ...
  const loadClaimsData = async () => {
    try {
      ...
      const [
        ...
        supabase.rpc("get_geographic_claims_data", {
      ...
      const mockSettlements: SettlementAnalysis[] = [
        {
          ...
          avg_claim_amount: 15750,
          avg_settlement: 12800,
          settlement_ratio: 0.81,
          processing_days: 18.5,
          approval_rate: 87.5,
          total_claims: 234,
        },
        {
          damage_type: "Water Damage",
          avg_claim_amount: 8900,
          avg_settlement: 7200,
          settlement_ratio: 0.81,
          processing_days: 12.3,
          approval_rate: 82.1,
          total_claims: 456,
        },
        {
          damage_type: "Fire Damage",
          avg_claim_amount: 22400,
          avg_settlement: 19800,
          settlement_ratio: 0.88,
          processing_days: 21.2,
          approval_rate: 91.2,
          total_claims: 89,
        },
        {
          damage_type: "Theft",
          avg_claim_amount: 3400,
          avg_settlement: 2900,
        ...
        },
        {
          ...
          avg_claim_amount: 2100,
          avg_settlement: 1800,
          settlement_ratio: 0.86,
          processing_days: 6.5,
          approval_rate: 84.3,
          total_claims: 123,
        },
      ...
      const mockProcessing: ProcessingMetrics[] = [
        ...
        {
          stage: "Adjuster Assignment",
          avg_duration_hours: 12.8,
          bottleneck_score: 45,
          completion_rate: 95.8,
          volume: 1031,
        },
        {
          stage: "Property Inspection",
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">
            Fraud detection, settlement analysis, and processing insights
          </p>
        </div>
      ...
      {fraudIndicators.filter((f) => f.fraud_score >= 80).length > 0 && (
        <Alert className="bg-red-900/20 border-red-900">
          ...
          <AlertDescription>
            <strong>
              {fraudIndicators.filter((f) => f.fraud_score >= 80).length}{" "}
              high-risk fraud cases detected
            </strong>{" "}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="bg-gray-800 border-gray-700">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-white">
              {overview?.total_claims.toLocaleString() || "0"}
            </div>
            <p className="text-xs text-gray-500 mt-1">
              {overview?.pending_claims || 0} pending •{" "}
              {overview?.approved_claims || 0} approved
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gray-800 border-gray-700">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-white">
              {formatCurrency(overview?.total_payout || 0)}
            </div>
            <p className="text-xs text-gray-500 mt-1">
        ...
        <Card className="bg-gray-800 border-gray-700">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-white">
              {overview?.avg_processing_time?.toFixed(1) || "0"} days
            </div>
            <p className="text-xs text-gray-500 mt-1">Industry avg: 21 days</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gray-800 border-gray-700">
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500 mt-1">
              {overview
                ? (
                    (overview.fraud_detected / overview.total_claims) *
                    100
                  ).toFixed(1)
                : "0"}
              % fraud rate
      ...
      <Tabs defaultValue="fraud" className="space-y-4">
              ...
              <CardContent>
                <div className="space-y-4">
                  {["critical", "high", "medium", "low"].map((level) => {
                    const count = fraudIndicators.filter(
                      (f) => getFraudLevel(f.fraud_score) === level,
                    ).length;
                    const percentage =
                      fraudIndicators.length > 0
                        ? (count / fraudIndicators.length) * 100
                        : 0;
                        ...
                        <div className="w-full bg-gray-700 rounded-full h-2">
                          <div
                            ...
                            style={{
                              ...
                              backgroundColor:
                                FRAUD_COLORS[
                                  level as keyof typeof FRAUD_COLORS
                                ],
                            }}
                          />
                        </div>
                    ...
                    {fraudIndicators.map((fraud, idx) => (
                      <tr key={idx} className="border-b border-gray-700/50">
                        ...
                        <td className="text-right py-3 text-gray-300">
                          {formatCurrency(fraud.amount_requested)}
                        </td>
                        ...
                        <td className="text-center py-3">
                          <Badge
                            ...
                            style={{
                              borderColor:
                                STATUS_COLORS[
                                  fraud.status as keyof typeof STATUS_COLORS
                                ],
                              color:
                                STATUS_COLORS[
                                  fraud.status as keyof typeof STATUS_COLORS
        ...
        <TabsContent value="settlements" className="space-y-4">
          <Card className="bg-gray-800 border-gray-700">
            <CardHeader>
              <CardTitle className="text-white">
                Settlement Analysis by Damage Type
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                    ...
                    {settlements.map((settlement, idx) => (
                      <tr key={idx} className="border-b border-gray-700/50">
                        <td className="py-3 text-gray-300 font-medium">
                          {settlement.damage_type}
                        </td>
                        <td className="text-right py-3 text-gray-300">
                          {settlement.total_claims}
                        </td>
                        <td className="text-right py-3 text-gray-300">
                          {formatCurrency(settlement.avg_claim_amount)}
                        </td>
                        <td className="text-right py-3 text-gray-300">
                          {formatCurrency(settlement.avg_settlement)}
                        </td>
                        <td className="text-right py-3">
                          <span
                            className={
                              settlement.settlement_ratio > 0.85
                                ? "text-green-400"
                                : settlement.settlement_ratio > 0.75
                                  ? "text-yellow-400"
                                  : "text-red-400"
                            }
                          >
                        ...
                        <td className="text-right py-3 text-gray-300">
                          {settlement.processing_days.toFixed(1)}
                        </td>
                        <td className="text-right py-3">
                          <span
                            className={
                              settlement.approval_rate > 85
                                ? "text-green-400"
                                : settlement.approval_rate > 75
                                  ? "text-yellow-400"
                                  : "text-red-400"
                            }
                          >
                ...
                {processing.map((stage, idx) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center space-x-4 text-sm">
                        ...
                        <Badge
                          variant="outline"
                          className={
                            stage.bottleneck_score > 70
                              ? "border-red-500 text-red-500"
                              : stage.bottleneck_score > 40
                    ...
                    <div className="flex items-center justify-between text-sm text-gray-400">
                      ...
                      <span>
                        {stage.completion_rate.toFixed(1)}% completion rate
                      </span>
                    </div>
                    <div className="mt-2">
                      <div className="w-full bg-gray-700 rounded-full h-2">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {geographic.map((geo, idx) => (
                      <tr key={idx} className="border-b border-gray-700/50">
                        <td className="py-3 text-gray-300 font-medium">
                          {geo.county}, {geo.state}
                        </td>
                        <td className="text-right py-3 text-gray-300">
                          {geo.claim_count}
                        </td>
                        <td className="text-right py-3 text-gray-300">
                          {formatCurrency(geo.avg_amount)}
                        </td>
                        <td className="text-right py-3">
                          <span
                            className={
                        ...
                        <td className="text-right py-3">
                          <span
                            className={
                              geo.approval_rate > 85
                                ? "text-green-400"
                                : geo.approval_rate > 75
                                  ? "text-yellow-400"
                                  : "text-red-400"
                            }
                          >
                        ...
                        <td className="text-right py-3 text-gray-300">
                          {geo.avg_processing_time.toFixed(1)} days
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  ...
                  <tbody>
                    {carriers.map((carrier, idx) => (
                      <tr key={idx} className="border-b border-gray-700/50">
                        ...
                        <td className="text-right py-3 text-gray-300">
                          {carrier.total_policies}
                        </td>
                        <td className="text-right py-3 text-gray-300">
                          {carrier.claims_count}
                        </td>
                        <td className="text-right py-3 text-gray-300">
                          {formatCurrency(carrier.avg_payout)}
                        </td>
                        <td className="text-right py-3">
                          <span
                            className={
                              carrier.approval_rate > 85
                                ? "text-green-400"
                                : carrier.approval_rate > 75
                                  ? "text-yellow-400"
                                  : "text-red-400"
                            }
                          >
                        ...
                        <td className="text-right py-3 text-gray-300">
                          {carrier.avg_processing_time.toFixed(1)} days
                        </td>
                        <td className="text-right py-3">
                          <div className="flex items-center justify-end space-x-1">
                            <span
                              className={
                                carrier.customer_satisfaction > 4.0
                                  ? "text-green-400"
                                  : carrier.customer_satisfaction > 3.5
                                    ? "text-yellow-400"
                                    : "text-red-400"
                              }
                            >
