...
export function RealtimeDashboard({
  ...
  const [selectedTable, setSelectedTable] = useState<RealtimeTable | "all">(
    "all",
  );
  const [isMonitoring, setIsMonitoring] = useState(autoStart);
  const [presenceUsers, setPresenceUsers] = useState<Map<string, any>>(
    new Map(),
  );
  ...
  const startMonitoring = async () => {
    try {
      ...
      try {
        ...
        logger.warn("Presence subscription failed", {
  ...
  const updateMetrics = () => {
    ...
    const eventsPerSecond =
      uptime > 0 ? metrics.totalEvents / (uptime / 1000) : 0;
  ...
  return (
    <div className={cn("space-y-6", className)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <MetricCard
          title="Uptime"
          value={`${Math.floor(metrics.uptime / 60)}:${Math.floor(
            metrics.uptime % 60,
          )
            .toString()
            .padStart(2, "0")}`}
        ...
        <Alert className="border-red-200 bg-red-50 dark:bg-red-950/20">
          ...
          <AlertDescription>
            <div className="flex items-center justify-between">
              ...
              <Badge variant="destructive">{metrics.errorCount} errors</Badge>
            </div>
          </AlertDescription>
        </Alert>
      )}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select
                    value={selectedTable}
                    onChange={(e) =>
                      setSelectedTable(e.target.value as RealtimeTable | "all")
                    }
                ...
                >
                  ...
                  <div>
                    <p className="font-medium text-sm">
                      {presence.userInfo?.name || "Unknown User"}
                    </p>
                    <p className="text-xs text-gray-600">
                      {presence.location?.page}
                    </p>
                  </div>
                  <Badge
                    variant="outline"
                    className={cn(
                      presence.activity?.status === "online" &&
                        "border-green-200 text-green-600",
                      presence.activity?.status === "away" &&
...
function SubscriptionCard({ subscription }: { subscription: any }) {
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
      ...
      <div className="flex items-center gap-2">
        <StatusIcon
          className={cn("w-4 h-4", getStatusColor(subscription.status))}
        />
