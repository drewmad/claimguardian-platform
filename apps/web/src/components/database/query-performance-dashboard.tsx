...
export function QueryPerformanceDashboard({
  ...
  const performanceInsights = useMemo(() => {
    ...
    if (
      performanceData.cacheHitRate > 90 &&
      performanceData.averageExecutionTime < 200
    ) {
      insights.push({
        type: "success" as const,
        title: "Excellent Performance",
        message:
          "Database performance is optimal with high cache hit rate and fast query times.",
          ...
          trend={
            historicalData.length > 1
              ? performanceData.totalQueries -
                historicalData[historicalData.length - 2].totalQueries
              : 0
          }
          icon={Database}
          color="text-blue-600"
          ...
          trend={
            historicalData.length > 1
              ? performanceData.averageExecutionTime -
                historicalData[historicalData.length - 2].averageExecutionTime
              : 0
          }
          icon={Clock}
          ...
          trend={
            historicalData.length > 1
              ? performanceData.cacheHitRate -
                historicalData[historicalData.length - 2].cacheHitRate
              : 0
          }
          icon={Zap}
          ...
          trend={
            historicalData.length > 1
              ? performanceData.slowQueries -
                historicalData[historicalData.length - 2].slowQueries
              : 0
          }
          icon={AlertTriangle}
          color="text-red-600"
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      {table.queryCount} queries • {table.avgTime.toFixed(0)}ms
                      avg
                    </p>
                  </div>
          ...
          <div className="space-y-4">
            {performanceData.recentOptimizations.length > 0 ? (
              performanceData.recentOptimizations.map((optimization, index) => (
                <div
                  key={index}
                  className="border-l-4 border-l-blue-500 pl-4 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-r-lg"
                >
              ...
              <p className="text-gray-600 dark:text-gray-400 text-center py-8">
                No optimization suggestions at this time. Your database is
                performing well!
              </p>
            )}
          </div>
    ...
    <Card>
      <CardContent className="p-6">
        <div className="flex items-center justify-between">
          ...
          <div
            className={cn(
              "w-12 h-12 bg-gray-100 dark:bg-gray-800 rounded-lg flex items-center justify-center",
              color,
            )}
          >
