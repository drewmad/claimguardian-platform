...
export function NIMSComplianceDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const incidentsResponse = await fetch('/api/nims/incidents?status=active&limit=10')
      ...
      // Load active alerts
      const alertsResponse = await fetch('/api/nims/alerts?status=sent&limit=10')
      ...
      const resourcesResponse = await fetch('/api/nims/resources?status=available&limit=50')
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      case 'flash': return 'bg-red-600'
      case 'immediate': return 'bg-orange-500'
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'contained': return 'bg-yellow-600'
                  ...
                  {incidents.map((incident) => (
                    <div key={incident.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="space-y-1">
                        <h4 className="font-medium">{incident.incident_name}</h4>
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          {incident.incident_number} • Type {incident.complexity_level} • {incident.incident_type}
                        </p>
                        <p className="text-xs text-gray-500">
                          Started: {new Date(incident.start_date).toLocaleString()}
                        </p>
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-sm">External Service Integration</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <CheckCircle className="h-5 w-5 text-green-600" />
                  ...
                  <div className="flex items-center space-x-3">
                    <CheckCircle className="h-5 w-5 text-green-600" />
                    <span className="text-sm">Database Infrastructure Ready</span>
                  </div>
                </div>
              </div>
