...
import {
  Database, MapPin, TrendingUp, Shield, AlertCircle, CheckCircle,
  Activity, Users, Home, DollarSign, Clock, Zap, RefreshCw,
  Eye, Settings, Download, Filter
} from 'lucide-react'
...
export function FloridaAnalyticsDashboard() {
  ...
  const topCountiesByValue = [...countyStats]
    .sort((a, b) => (b.median_property_value || 0) - (a.median_property_value || 0))
    .slice(0, 10)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">Florida Data Analytics</h1>
          <p className="text-gray-300">
            Comprehensive analysis of {totalParcels.toLocaleString()} Florida properties
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-gray-800/50 border-gray-700">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-400 text-sm font-medium">Total Properties</p>
                <p className="text-2xl font-bold text-white">{totalParcels.toLocaleString()}</p>
              </div>
              <Home className="h-8 w-8 text-blue-400" />
            </div>
            <div className="mt-2">
              <Badge variant="secondary" className="text-xs">67 Counties</Badge>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gray-800/50 border-gray-700">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-400 text-sm font-medium">Avg Property Value</p>
        ...
        <Card className="bg-gray-800/50 border-gray-700">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-400 text-sm font-medium">High Risk Properties</p>
                <p className="text-2xl font-bold text-white">{totalHighRisk.toLocaleString()}</p>
              </div>
        ...
        <Card className="bg-gray-800/50 border-gray-700">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-400 text-sm font-medium">Data Quality</p>
                <p className="text-2xl font-bold text-white">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-700">
                      ...
                      <th className="text-right p-2 text-gray-300">Properties</th>
                      <th className="text-right p-2 text-gray-300">Median Value</th>
                      <th className="text-right p-2 text-gray-300">High Risk</th>
                  ...
                  <tbody>
                    {countyStats.map((county) => (
                      ...
                      >
                        <td className="p-2 text-white font-medium">{county.county_name}</td>
        ...
        <TabsContent value="quality" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {pipelineStatus?.recent_quality_metrics?.map((metric) => (
              <Card key={metric.table_name} className="bg-gray-800/50 border-gray-700">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-white font-medium">{metric.table_name}</h3>
                    <Database className="h-5 w-5 text-blue-400" />
                  </div>
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span className="text-gray-400">Records</span>
                      <span className="text-white">{metric.total_records?.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Completeness</span>
                      <span className="text-white">{metric.completeness_rate?.toFixed(1)}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Issues</span>
                      <span className={metric.quality_issues > 10 ? "text-red-400" : "text-green-400"}>
                        {metric.quality_issues}
                      </span>
                    </div>
                    <Progress value={metric.completeness_rate} className="h-2" />
                  </div>
                </CardContent>
              </Card>
            ))}
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-white">
                      {pipelineStatus?.health.active_sources} / {pipelineStatus?.health.total_sources}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
