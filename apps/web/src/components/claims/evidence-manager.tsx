...
export function EvidenceManager({ claimId, onUpdate }: EvidenceManagerProps) {
    ...
    const mockEvidence: Evidence[] = [
      {
        ...
        aiAnalysis: {
          summary:
            "Significant water damage visible on kitchen ceiling with staining and potential structural damage",
          detectedDamage: [
            "Water staining",
            "Ceiling sagging",
            "Paint peeling",
  ...
  const handleCategoryUpdate = async (
    ...
    setEvidence((prev) =>
      prev.map((e) => (e.id === evidenceId ? { ...e, category } : e)),
    );
  ...
  const handleDescriptionUpdate = async (
    ...
    setEvidence((prev) =>
      prev.map((e) => (e.id === evidenceId ? { ...e, description } : e)),
    );
  };
  ...
  const renderTimelineView = () => {
    const groupedByDate = filteredEvidence.reduce(
      (acc, item) => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400">
            ...
            {(
              evidence.reduce((sum, e) => sum + e.fileSize, 0) /
              1024 /
              1024
            ).toFixed(2)}{" "}
            MB total
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1 flex gap-3">
          ...
          <Select
            ...
            onValueChange={(v) =>
              setSelectedCategory(v as EvidenceCategory | "all")
            }
          >
                  ...
                  <div className="bg-gray-800 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <ul className="mt-1">
                          {selectedFile.aiAnalysis.detectedDamage.map(
                            (damage, i) => (
                      ...
                      <div>
                        ...
                        <p className="text-gray-300">
                          $
                          {selectedFile.aiAnalysis.estimatedCost?.toLocaleString() ||
                            "N/A"}
                        </p>
