    ...
    items: [
      {
        ...
        description: 'Sworn proof of loss must be submitted within 60 days (FL Statute 627.70131)',
  ...
  const categories: ComplianceCategory[] = report ? [
    {
      ...
      itemCount: report.items.filter(i => i.category === 'documentation').length,
      compliantCount: report.items.filter(i => i.category === 'documentation' && i.status === 'compliant').length
    },
    {
      name: 'Timeline',
      icon: Clock,
      description: 'Statutory deadlines and timeframes',
      itemCount: report.items.filter(i => i.category === 'timeline').length,
      compliantCount: report.items.filter(i => i.category === 'timeline' && i.status === 'compliant').length
    },
    {
      name: 'Coverage',
      icon: Shield,
      description: 'Policy coverage requirements',
      itemCount: report.items.filter(i => i.category === 'coverage').length,
      compliantCount: report.items.filter(i => i.category === 'coverage' && i.status === 'compliant').length
    },
    {
      name: 'Procedure',
      icon: FileCheck,
      description: 'Claim handling procedures',
      itemCount: report.items.filter(i => i.category === 'procedure').length,
      compliantCount: report.items.filter(i => i.category === 'procedure' && i.status === 'compliant').length
    },
    {
      name: 'Legal',
      icon: Gavel,
      ...
      compliantCount: report.items.filter(i => i.category === 'legal' && i.status === 'compliant').length
    }
  ] : [];
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Ensure your claim meets all Florida statutory requirements and insurance regulations
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex-1">
                ...
                >
                  ...
                  <option value="CLM-2024-0892">CLM-2024-0892 - Wind Damage</option>
                  <option value="CLM-2024-0887">CLM-2024-0887 - Water Damage</option>
      ...
      {report && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className={`text-4xl font-bold ${
                    ...
                    report.overallScore >= 70 ? 'text-yellow-500' :
                    'text-red-500'
                  }`}>
                    {report.overallScore}%
                  </div>
            ...
            <Card>
              <CardContent className="p-6">
                ...
                <p className="text-xs text-gray-500 mt-1">Should be addressed</p>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <Badge className={
                    ...
                    report.status === 'needs_review' ? 'bg-yellow-500' :
                    'bg-red-500'
                  }>
          ...
          <Tabs defaultValue="all" className="space-y-4">
                                ...
                                {item.fix && item.status !== 'compliant' && (
                                  <div className="mt-2 p-2 bg-blue-50 rounded">
                                    <p className="text-sm text-blue-800">
                                      <span className="font-medium">Action Required:</span> {item.fix}
                                    </p>
                                  </div>
                                )}
                              </div>
            ...
            {['documentation', 'timeline', 'coverage', 'procedure', 'legal'].map((category) => (
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {report.floridaStatutes.map((statute) => (
                  <div key={statute} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                      <p className="font-medium">§ {statute}</p>
                      <p className="text-sm text-gray-600">{floridaStatutes[statute as keyof typeof floridaStatutes]}</p>
                    </div>
