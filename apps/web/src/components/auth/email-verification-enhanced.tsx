  ...
  const startPolling = useCallback(() => {
    ...
    pollingInterval.current = setInterval(async () => {
      try {
        ...
        logger.error(
          "Unexpected error during verification polling",
          {},
          err as Error,
        );
      }
  ...
  const sendVerificationEmail = useCallback(
    async (email: string) => {
      try {
        ...
        const toastId = loading("Sending verification email...", {
          persistent: true,
        });
        ...
        if (resendError) {
          if (
            ...
            setState((prev) => ({
              ...
              status: "error",
              message:
                resendError.message || "Failed to send verification email",
            }));
    ...
    switch (status) {
      ...
      case "error":
        return {
          ...
          description:
            state.message || "There was an error verifying your email.",
        };
      case "expired":
        return {
          icon: Clock,
        ...
        <CardContent className="space-y-4">
          ...
          {(state.status === "verifying" || isPolling) && (
            <div className="space-y-2">
              <Progress
                value={state.status === "verifying" ? 30 : 70}
                className="h-2"
              />
          ...
          {["pending", "error", "expired", "resent", "rate_limited"].includes(
            state.status,
          ) && (
