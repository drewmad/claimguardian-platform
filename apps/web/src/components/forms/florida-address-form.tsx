...
import {
  getFloridaCountiesFallback,
  validateAddress,
} from "@/actions/geographic";
...
export function FloridaAddressForm({
  value,
  onChange,
  disabled,
  className,
}: FloridaAddressFormProps) {
  ...
  const inferCountyFromCity = useCallback(
    async (cityName: string): Promise<string | null> => {
        ...
        try {
          ...
          if (data?.exactMatch) {
            ...
            setValidationMessage(
              `✓ Valid ZIP code for ${match.city}, ${match.county}`,
            );
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          {validationMessage && (
            <p
              className={`text-xs mt-1 ${validationMessage.startsWith("✓") ? "text-green-400" : "text-yellow-400"}`}
            >
              {validationMessage}
            </p>
          )}
