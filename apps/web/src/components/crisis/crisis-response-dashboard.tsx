...
import { crisisResponseCoordinator, type CrisisEvent, type CrisisResponse, type CrisisAction, type EmergencyResource } from '@/lib/services/crisis-response-coordinator'
...
export function CrisisResponseDashboard({ userId, className }: CrisisResponseDashboardProps) {
  ...
  const activateCrisisResponse = async (crisisId: string) => {
    try {
      const response = await crisisResponseCoordinator.activateCrisisResponse(userId, crisisId)
      if (response) {
  ...
  const getSeverityLabel = (severity: number) => {
    switch (severity) {
      ...
      case 4: return 'EMERGENCY WARNING'
  ...
  const getCrisisIcon = (type: CrisisEvent['type']) => {
    switch (type) {
      ...
      case 'tornado': return <RotateCcw className="h-5 w-5" />
      ...
      case 'severe_weather': return <CloudRain className="h-5 w-5" />
  ...
  const getPriorityIcon = (priority: CrisisAction['priority']) => {
    switch (priority) {
      case 'critical': return <Siren className="h-4 w-4 text-red-500" />
    ...
    return (
      <div className={`space-y-6 ${className}`}>
        <div className="flex items-center justify-center p-12">
          ...
          <span className="ml-2 text-gray-400">Loading crisis monitoring...</span>
        </div>
      </div>
    )
  }
          ...
          {crisisAlerts.map(alert => (
            <Alert key={alert.id} className="border-red-500/20 bg-red-900/10">
              ...
              <AlertTitle className="text-red-400 flex items-center justify-between">
                <span>{getSeverityLabel(alert.severity)} - {alert.title}</span>
              ...
              <AlertDescription className="text-red-300">
                ...
                <div className="flex items-center justify-between mt-2">
                  <span className="text-sm text-gray-400">
                    {alert.location} • {formatDistanceToNow(new Date(alert.timestamp))} ago
                  </span>
                  <Button
                    size="sm"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="bg-gray-800 border-gray-700">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-400">Active Crises</p>
                    <p className="text-2xl font-bold text-white">{activeCrises.length}</p>
                  </div>
            ...
            <Card className="bg-gray-800 border-gray-700">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-400">Response Status</p>
                    <p className="text-2xl font-bold text-white">
                      {activeResponse ? 'ACTIVE' : 'MONITORING'}
                    </p>
                  </div>
                  <div className={`p-3 rounded-full ${activeResponse ? 'bg-red-500/10' : 'bg-green-500/10'}`}>
                    <Shield className={`h-6 w-6 ${activeResponse ? 'text-red-500' : 'text-green-500'}`} />
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gray-800 border-gray-700">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-400">Resources Available</p>
                    <p className="text-2xl font-bold text-white">{emergencyResources.length}</p>
                  </div>
            ...
            <Card className="bg-gray-800 border-gray-700">
              ...
              <CardContent className="space-y-4">
                {activeCrises.map(crisis => (
                  <div key={crisis.id} className="border border-gray-700 rounded-lg p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          <h3 className="font-semibold text-white">{crisis.title}</h3>
                          <p className="text-gray-400 text-sm mt-1">{crisis.description}</p>
                          <div className="flex items-center space-x-4 mt-2 text-sm text-gray-500">
                            ...
                            <span className="flex items-center">
                              ...
                              {formatDistanceToNow(new Date(crisis.created_at))} ago
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    {crisis.timing.peak_impact && (
                      <div className="mt-3 p-3 bg-yellow-900/20 rounded-lg border border-yellow-600/20">
                        <p className="text-yellow-400 text-sm">
                          ...
                          Peak impact expected: {format(new Date(crisis.timing.peak_impact), 'MMM d, yyyy h:mm a')}
                        </p>
                      </div>
                    )}
                  </div>
          ...
          {activeCrises.length === 0 && (
            <Card className="bg-gray-800 border-gray-700">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-gray-400 mb-6">No active crisis events detected in your area.</p>
                <Button
        ...
        <TabsContent value="response" className="space-y-6">
          {activeResponse ? (
            <>
                ...
                <CardHeader>
                  <CardTitle className="text-white flex items-center justify-between">
                    ...
                    <Badge variant="outline" className="border-red-500/20 text-red-400">
                      {activeResponse.response_level.replace('_', ' ').toUpperCase()}
                    </Badge>
                  </CardTitle>
                  <CardDescription className="text-gray-400">
                    Activated {formatDistanceToNow(new Date(activeResponse.activation_time))} ago
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {activeResponse.progress.actions_completed} of {activeResponse.progress.actions_total} actions completed
                      </p>
                    </div>
                    <div>
                      ...
                      <div className="flex items-center space-x-2 mt-1">
                        ...
                        />
                        <span className="text-sm font-medium text-white">
                          {activeResponse.progress.critical_actions_completed}/{activeResponse.progress.critical_actions_total}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <Card className="bg-gray-800 border-gray-700">
                <CardHeader>
                  <CardTitle className="text-white">Emergency Action Plan</CardTitle>
                  <CardDescription>Follow these steps in order of priority</CardDescription>
                </CardHeader>
                      ...
                      <div className="flex items-start space-x-3">
                        <div className="flex items-center pt-1">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => updateActionStatus(action.id, !!checked)}
            ...
            <Card className="bg-gray-800 border-gray-700">
              <CardContent className="p-12 text-center">
                ...
                <h3 className="text-lg font-semibold text-white mb-2">No Active Response</h3>
                <p className="text-gray-400">Crisis response will activate automatically when emergency conditions are detected.</p>
              </CardContent>
            </Card>
          )}
        </TabsContent>
        ...
        <TabsContent value="resources" className="space-y-6">
          <Card className="bg-gray-800 border-gray-700">
            <CardHeader>
              ...
              <CardDescription>Essential resources and contacts in your area</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {emergencyResources.map(resource => (
                <div key={resource.id} className="border border-gray-700 rounded-lg p-4">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-3">
                      <div className="p-2 bg-blue-500/10 rounded-lg">
                        {resource.type === 'shelter' && <Home className="h-5 w-5 text-blue-500" />}
                        {resource.type === 'contractor' && <Building className="h-5 w-5 text-blue-500" />}
                        {resource.type === 'medical' && <Heart className="h-5 w-5 text-blue-500" />}
                        {resource.type === 'government' && <Shield className="h-5 w-5 text-blue-500" />}
                      </div>
                      <div>
          ...
          <Card className="bg-gray-800 border-gray-700">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-3">
                  <h3 className="font-medium text-white">Local Emergency Management</h3>
                  <div className="space-y-2">
                    <Button size="sm" variant="outline" className="w-full justify-start" asChild>
              ...
              <div className="border-t border-gray-700 pt-4">
                <h3 className="font-medium text-white mb-3">Personal Emergency Contacts</h3>
                <div className="text-center py-8">
                  <Users className="mx-auto h-12 w-12 text-gray-500 mb-4" />
                  <p className="text-gray-400 mb-4">No personal emergency contacts configured</p>
