    ...
    (event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
      ...
      if (offset.x > threshold && rightActions.length > 0) {
        const actionIndex = Math.min(
          Math.floor(
            ((offset.x - threshold) / (maxSwipe - threshold)) *
              rightActions.length,
          ),
        ...
        const actionIndex = Math.min(
          Math.floor(
            ((Math.abs(offset.x) - threshold) / (maxSwipe - threshold)) *
              leftActions.length,
          ),
  ...
  const handleDragEnd = useCallback(
    (event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
      ...
      const shouldTriggerAction =
        distance > threshold || Math.abs(velocity.x) > velocityThreshold;
      ...
      if (shouldTriggerAction) {
        if (offset.x > 0 && rightActions.length > 0) {
          ...
          if (action) {
            if (hapticFeedback)
              triggerHaptic(action.destructive ? "heavy" : "medium");
            setIsOpen("right");
...
export function SwipeActionItem({
  ...
  return (
    <SwipeAction
      ...
      rightActions={[SwipeActionPresets.delete(onDelete)]}
      className={className}
    >
