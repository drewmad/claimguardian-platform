...
export interface SkeletonProps extends Omit<React.HTMLAttributes<HTMLDivElement>, "onDrag" | "onDragStart" | "onDragEnd" | "onAnimationStart" | "onAnimationEnd"> {
...
export function Skeleton({
  ...
  return <div className={baseClasses} style={skeletonStyle} {...props} />;
}
  ...
  return (
    <div className={cn("space-y-2", className)}>
      {Array.from({ length: lines }).map((_, index) => {
        ...
        const lineWidth =
          isLastLine && lines > 1 ? "w-2/3" : widthClasses[width];
...
export function SkeletonCard({
  ...
  return (
    <div className={cn("p-6 space-y-4", className)}>
      ...
      {/* Featured image */}
      {showImage && (
        <Skeleton variant="rounded" className="w-full h-48" animation="wave" />
      )}
