...
const statusColors = {
  ...
  needs_attention: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400',
...
export function TouchPropertyCard({
  ...
  const renderPropertyIcon = () => {
    switch (property.type) {
      ...
      case 'multi-family': return <Home className="w-4 h-4" />
      case 'commercial': return <Home className="w-4 h-4" />
        ...
        {property.riskScore && (
          <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <div className="flex items-center justify-between">
              ...
              <span className={cn('text-sm font-medium', riskColors[getRiskLevel(property.riskScore)])}>
              ...
              <div className="space-y-2">
                {property.lastInspection && (
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-gray-900 dark:text-white">{property.lastInspection}</span>
                  </div>
                )}
                {property.insuranceValue && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600 dark:text-gray-400">Insurance Value</span>
  ...
  const cardContent = variant === 'compact' ? renderCompactView() : renderDetailedView()
  ...
  if (enableSwipeActions) {
    return (
      <SwipeAction
        ...
        rightActions={[
          SwipeActionPresets.delete(handleDeleteAction)
        ]}
