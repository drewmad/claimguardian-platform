...
const TYPICAL_FEATURES = [ "Garage", "Carport", "Fence", "Shed", "Seawall", "Dock", "Boat Lift", "Pool", "Pool Cage / Screen Enclosure", "Hurricane Shutters", "Impact-Rated Windows", "Metal Roof", "Whole-House Generator", "Solar Panels", "Irrigation / Sprinkler System", "Septic System", "Security System" ];
...
type WizardAction =
    ...
    | { type: 'UPDATE_ROOMS_PER_FLOOR'; payload: { index: number; value: number } }
...
// --- Validation Schema ---
const VALIDATION_SCHEMA: Record<number, string[] | ((state: PropertyData) => string[])> = {
    1: ['ownershipStatus', 'address.street1', 'address.city', 'address.state', 'address.zip', 'propertyType'],
...
const initialState: PropertyData = {
    ...
    ownershipStatus: '',
    address: {
        full: '', street1: '', street2: '', city: '', state: '', zip: ''
    },
...
function wizardReducer(state: PropertyData, action: WizardAction): PropertyData {
    switch (action.type) {
        case 'SET_ID':
        ...
        case 'SET_ROOMS_PER_FLOOR_ARRAY': {
            ...
            const newRoomsPerFloor = Array(floorCount).fill('').map((_, i) => state.propertyDetails.roomsPerFloor[i] || 0);
            return { ...state, propertyDetails: { ...state.propertyDetails, numberOfFloors: floorCount, roomsPerFloor: newRoomsPerFloor } };
        }
        case 'UPDATE_ROOMS_PER_FLOOR': {
             const {index, value} = action.payload;
             const newRoomsPerFloor = [...state.propertyDetails.roomsPerFloor];
             newRoomsPerFloor[index] = value;
             return { ...state, propertyDetails: { ...state.propertyDetails, roomsPerFloor: newRoomsPerFloor } }
        }
        case 'TOGGLE_FEATURE': {
            const feature = action.payload;
            const currentFeatures = state.propertyDetails.features || [];
            const newFeatures = currentFeatures.includes(feature) ? currentFeatures.filter(f => f !== feature) : [...currentFeatures, feature];
            return { ...state, propertyDetails: { ...state.propertyDetails, features: newFeatures } };
        }
        case 'NEXT_STEP':
...
const useFormValidation = (state: PropertyData) => {
    const validateStep = useCallback((step: number) => {
        ...
        if (typeof requiredFields === 'function') requiredFields = requiredFields(state);
        if (!requiredFields) return {};

        const errors: Record<string, string> = {};
        requiredFields.forEach(field => {
            let value: unknown;
            if (field.startsWith('address.')) value = state.address[field.split('.')[1] as keyof typeof state.address];
            else if (field === 'propertyType') value = state.basicInfo.propertyType;
            else if (['bedrooms', 'bathrooms', 'numberOfFloors'].includes(field)) value = state.propertyDetails[field as keyof typeof state.propertyDetails];
...
const useGooglePlaces = (onPlaceSelected: (address: PropertyData['address']) => void) => {
    const GOOGLE_MAPS_API_KEY = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || "";
    const [scriptState, setScriptState] = useState({ loaded: false, error: null as string | null });
    ...
    useEffect(() => {
        ...
        if (window.google && window.google.maps) {
            setScriptState({ loaded: true, error: null }); return;
        }
        const script = document.createElement('script');
        script.src = `https://maps.googleapis.com/maps/api/js?key=${GOOGLE_MAPS_API_KEY}&libraries=places`;
        script.async = true;
        script.onload = () => setScriptState({ loaded: true, error: null });
        script.onerror = () => setScriptState({ loaded: false, error: "Failed to load Google Maps script." });
    ...
    const initAutocomplete = useCallback((inputElement: HTMLInputElement) => {
        ...
        const autocomplete = new window.google.maps.places.Autocomplete(inputElement, { types: ['address'], componentRestrictions: { country: 'us' } });
    ...
    return { initAutocomplete, isScriptLoaded: scriptState.loaded, error: scriptState.error };
};
...
export function PropertyWizard({ open, onClose, onComplete }: PropertyWizardProps) {
    ...
    useEffect(() => { setIsNextDisabled(Object.keys(validateStep(state.step)).length > 0); }, [state, validateStep]);
            ...
            <div className="relative w-full max-w-3xl h-full max-h-[95vh]">
                <div className={`w-full h-full ${COLORS.panel} rounded-2xl shadow-2xl backdrop-blur-2xl ${COLORS.border} border flex flex-col overflow-hidden`}>
                    <header className={`flex items-center justify-between p-6 border-b ${COLORS.border} flex-shrink-0`}>
                        ...
                        <button onClick={onClose} className={`p-2 rounded-full text-slate-400 hover:bg-slate-700/50`}> <X size={20} /> </button>
                    </header>
                        ...
                        <div className="flex items-center justify-between">
                            {STEPS.map((s, index) => (
                                <React.Fragment key={s.id}>
                                    ...
                                    {index < STEPS.length - 1 && <div className={`flex-1 h-0.5 mx-2 transition-colors duration-300 ${state.step > s.id ? COLORS.accent : 'bg-slate-700'}`}></div>}
                                </React.Fragment>
                            ))}
                        </div>
                    </div>
                    ...
                    <footer className="fixed bottom-0 inset-x-0 p-4 bg-slate-900/80 backdrop-blur border-t border-slate-700 flex items-center justify-between">
                        <div className={`transition-opacity duration-300 ${showSavedToast ? 'opacity-100' : 'opacity-0'}`}>
                            <p className="text-sm text-green-400 flex items-center gap-2"><CheckCircle size={16}/> Saved</p>
                        </div>
...
// --- Helper Components ---
const SkeletonLoader = () => ( <div className="w-full h-full bg-slate-800/50 p-8 space-y-6 animate-pulse"> <div className="h-10 w-3/4 bg-slate-700 rounded-lg"></div> <div className="h-12 w-full bg-slate-700 rounded-lg"></div> <div className="h-12 w-full bg-slate-700 rounded-lg"></div> <div className="h-12 w-1/2 bg-slate-700 rounded-lg"></div> </div> );
...
const Input = ({ label, ...props }: InputProps) => (
    <div>
        ...
        <input {...props} className={`w-full p-3 bg-slate-900/50 border ${COLORS.border} rounded-lg focus:ring-2 focus:ring-indigo-400 ${COLORS.textPrimary} placeholder-slate-500`} />
    </div>
);
...
const AddressInput = ({ address, dispatch }: AddressInputProps) => {
    ...
    useEffect(() => {
        if (isScriptLoaded && autocompleteRef.current) initAutocomplete(autocompleteRef.current);
    }, [isScriptLoaded, initAutocomplete]);

    const handleManualChange = (field: keyof PropertyData['address'], value: string) => {
        dispatch({ type: 'SET_ADDRESS', payload: { ...address, [field]: value, full: '' } });
    };
        ...
        <div className="space-y-4">
            {!isManual ? (
                <div className="relative">
                    ...
                    <MapPin className={`absolute left-3 top-10 w-5 h-5 ${COLORS.textSecondary}`} />
                    <input ref={autocompleteRef} type="text" defaultValue={address.full} placeholder="Start typing your address..." className={`w-full p-3 pl-10 bg-slate-900/50 border ${COLORS.border} rounded-lg`} />
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in">
                    <Input label="Street 1" value={address.street1} onChange={e => handleManualChange('street1', e.target.value)} />
                    <Input label="Street 2 (Apt, etc.)" value={address.street2} onChange={e => handleManualChange('street2', e.target.value)} />
                    <Input label="City" value={address.city} onChange={e => handleManualChange('city', e.target.value)} />
                    <Input label="State" value={address.state} onChange={e => handleManualChange('state', e.target.value)} />
                    <Input label="Zip Code" value={address.zip} onChange={e => handleManualChange('zip', e.target.value)} />
                </div>
            )}
            <button onClick={() => setIsManual(!isManual)} className="text-sm text-indigo-400 hover:text-indigo-300">
            ...
            <div className="space-y-8 animate-fade-in">
                ...
                {state.address.street1 && (
                    <div className="animate-fade-in">
                        ...
                        <ButtonGroup options={[{label: 'Single Family', value: 'single-family'}, {label: 'Condo', value: 'condominium'}, {label: 'Townhouse', value: 'townhouse'}, {label: 'Apartment', value: 'apartment'}, {label: 'Multi-Family', value: 'multi-family'}, {label: 'Land', value: 'land'}]} columns={3} selectedValue={state.basicInfo.propertyType} onChange={(value) => dispatch({ type: 'UPDATE_FIELD', payload: { section: 'basicInfo', field: 'propertyType', value } })} />
                    </div>
                )}
            </div>
        )}
...
const Step2 = ({ state, dispatch }: StepProps) => {
    const [visibleQuestions, setVisibleQuestions] = useState({ beds: true, baths: false, floors: false, rooms: false, features: false });

    useEffect(() => {
        if (state.propertyDetails.bedrooms) setVisibleQuestions(v => ({ ...v, baths: true }));
        if (state.propertyDetails.bathrooms) setVisibleQuestions(v => ({ ...v, floors: true }));
        if (state.propertyDetails.numberOfFloors) setVisibleQuestions(v => ({ ...v, rooms: true }));
        const allRoomsEntered = state.propertyDetails.roomsPerFloor.every(r => r > 0);
        if (state.propertyDetails.numberOfFloors > 0 && allRoomsEntered) setVisibleQuestions(v => ({ ...v, features: true }));
    }, [state.propertyDetails]);

    if (state.basicInfo.propertyType === 'land') {
        return <div className="text-center"><h2 className={`text-2xl font-bold ${COLORS.textPrimary}`}>No additional details needed for land.</h2><p className={COLORS.textSecondary}>You can proceed to the next step.</p></div>;
    }
    ...
    <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            {visibleQuestions.floors && <div className="animate-fade-in"><NumericStepper label="Number of Floors" value={state.propertyDetails.numberOfFloors} min={1} max={4} onChange={(value) => dispatch({ type: 'SET_ROOMS_PER_FLOOR_ARRAY', payload: value })} /></div>}
        </div>

        {visibleQuestions.rooms && state.propertyDetails.numberOfFloors > 0 && (
            <div className="animate-fade-in">
                <label className={`block text-sm font-medium mb-2 ${COLORS.textSecondary}`}>Rooms per Floor</label>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {state.propertyDetails.roomsPerFloor.map((_, index) => (
                        <NumericStepper key={index} label={`Floor ${index + 1}`} value={state.propertyDetails.roomsPerFloor[index]} onChange={(value) => dispatch({type: 'UPDATE_ROOMS_PER_FLOOR', payload: {index, value}})} />
                    ))}
                </div>
            </div>
        )}
        ...
        {visibleQuestions.features && (
            <div className="animate-fade-in">
                <CheckboxGrid options={TYPICAL_FEATURES} selectedOptions={state.propertyDetails.features} onToggle={(feature) => dispatch({ type: 'TOGGLE_FEATURE', payload: feature })} />
            </div>
        )}
    </div>
)};
    ...
    <div className="space-y-8">
        ...
        <div>
            ...
            <ButtonGroup
                ...
                onChange={(value) => dispatch({ type: 'UPDATE_FIELD', payload: { section: 'financialInfo', field: 'hasMortgage', value }})}
            />
        </div>
    </div>
);
...
const ReviewItem = ({ label, value, isFullWidth }: ReviewItemProps) => (
    <div className={`py-4 border-b ${COLORS.border} ${isFullWidth ? 'col-span-2' : ''}`}>
        <p className={`text-sm ${COLORS.textSecondary}`}>{label}</p>
        <p className={`font-semibold ${COLORS.textPrimary} mt-1`}>{value || 'Not provided'}</p>
    </div>
);
...
const Step5 = ({ state }: Step5Props) => (
    <div>
        <h2 className={`text-2xl font-bold ${COLORS.textPrimary} mb-2`}>Does this look right?</h2>
        ...
        <div className="space-y-6">
            <ReviewSection title="Property Overview" icon={<Home size={20} />}>
                <ReviewItem label="Address" value={`${state.address.street1}${state.address.street2 ? ' ' + state.address.street2 : ''}, ${state.address.city}, ${state.address.state} ${state.address.zip}`} isFullWidth />
            ...
            {state.basicInfo.propertyType !== 'land' &&
                <ReviewSection title="Structural Details" icon={<Building size={20} />}>
                    ...
                    <ReviewItem label="Rooms per Floor" value={state.propertyDetails.roomsPerFloor.join(', ')} isFullWidth />
                    <ReviewItem label="Features" value={state.propertyDetails.features.join(', ')} isFullWidth />
                </ReviewSection>
            }

            <ReviewSection title="Coverage & Financials" icon={<Shield size={20} />}>
                <ReviewItem label="Home/Renters Insurance" value={state.insuranceInfo.hasHomeownersRenters.charAt(0).toUpperCase() + state.insuranceInfo.hasHomeownersRenters.slice(1)} />
                {state.insuranceInfo.hasHomeownersRenters === 'yes' && <ReviewItem label="Includes Flood" value={state.insuranceInfo.hasFlood.charAt(0).toUpperCase() + state.insuranceInfo.hasFlood.slice(1)} />}
                <ReviewItem label="Mortgage" value={state.financialInfo.hasMortgage === 'yes' ? 'Yes' : 'No'} />
            </ReviewSection>
        </div>
    </div>
);
