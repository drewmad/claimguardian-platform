  ...
  useEffect(() => {
    const checkOnboardingStatus = async () => {
      ...
      try {
        ...
        if (
          prefs?.user_type &&
          (prefs?.property_address || prefs?.professional_role)
        ) {
        ...
        const firstIncomplete = updatedSteps.findIndex(
          (step) => !step.completed,
        );
...
function UserProfileStep({
  ...
  const [propertyStories, setPropertyStories] = useState(
    data.propertyStories || 1,
  );
  const [propertyBedrooms, setPropertyBedrooms] = useState(
    data.propertyBedrooms || 1,
  );
  const [propertyBathrooms, setPropertyBathrooms] = useState(
    data.propertyBathrooms || 1,
  );
  const [propertyStructures, setPropertyStructures] = useState<string[]>(
  ...
  const userTypes = [
    ...
    },
    {
      ...
      label: "Landlord",
      icon: "🏘️",
      description: "I rent properties to others",
    },
    {
      value: "property-professional",
      label: "Property Professional",
      icon: "💼",
      description: "I work in real estate/property",
    },
  ] as const;
  ...
  const handleTypeSelect = (type: UserType) => {
    ...
    onUpdate({
      ...
      userType: type,
      propertyAddress:
        type !== "property-professional" ? propertyAddress : undefined,
      professionalRole:
        type === "property-professional" ? professionalRole : undefined,
    ...
    try {
      ...
      autocompleteRef.current.addListener("place_changed", () => {
        ...
        if (place?.formatted_address && place.geometry?.location) {
          ...
          logger.info("Address coordinates:", {
            lat,
            lng,
  ...
  const needsPropertyDetails =
    selectedType && !isProfessional && addressVerified;
                  ...
                  {landlordUnitRanges.map((range) => (
                    ...
                    >
                      <div className="flex flex-col items-center">
                        ...
                        <div
                          className={`w-4 h-4 rounded-full border-2 flex items-center justify-center mt-2 transition-all duration-200 ${
                            landlordUnits === range
                              ? "border-accent bg-accent shadow-lg"
                              : "border-border"
                          }`}
                    ...
                    {commonStructures.map((structure) => (
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div
                            className={`w-3.5 h-3.5 rounded border-2 flex items-center justify-center transition-all duration-200 flex-shrink-0 ${
                              propertyStructures.includes(structure)
                                ? "border-accent bg-accent shadow-lg"
                                : "border-border"
                            }`}
...
function InsuranceStatusStep({
  ...
  const [hasPropertyInsurance, setHasPropertyInsurance] = useState<
    boolean | null
  >(data.hasPropertyInsurance || null);
  const [hasFloodInsurance, setHasFloodInsurance] = useState<boolean | null>(
    data.hasFloodInsurance || null,
  );
  const [hasOtherInsurance, setHasOtherInsurance] = useState<boolean | null>(
    data.hasOtherInsurance || null,
  );
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              ...
              <div
                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mx-auto transition-all duration-200 ${
                  stepData.value === true
        ...
        {hasOtherInsurance && currentStep === 2 && (
          <div className="pt-4 space-y-4">
            ...
            >
              ...
              <option value="equipment-breakdown">
    ...
    <div className="space-y-8">
      ...
      <div className="bg-success/10 border border-success/20 rounded-xl p-6 text-center backdrop-blur-sm">
        ...
        <p className="text-success/80 text-sm">
          Your ClaimGuardian account is ready to help protect what matters most
          to you.
        </p>
      </div>
