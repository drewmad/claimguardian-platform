...
const PricingCard: React.FC<{
  plan: PricingPlan & { priceIds?: { monthly: string; annual: string } };
  onClick: () => void;
  cycle: "monthly" | "annual";
  isProcessing?: boolean;
}> = ({ plan, onClick, cycle, isProcessing }) => {
  ...
  return (
    <div
      className={`${plan.recommended ? liquidGlass.cards.primary : liquidGlass.cards.default} p-8 rounded-2xl flex flex-col h-full relative ${plan.recommended ? "border-2" : "border"}`}
    >
      {plan.recommended && (
        <span
          className={`absolute -top-3 right-6 px-3 py-1 rounded-full text-sm font-bold ${liquidGlass.badges.primary}`}
        >
          Recommended
        </span>
      )}
      {isAnnual && savings > 0 && (
        <span
          className={`absolute -top-3 left-6 px-3 py-1 rounded-full text-sm font-bold ${liquidGlass.badges.success}`}
        >
          Save 17%
        </span>
      )}
      ...
      <ul className="text-left space-y-3 my-8 flex-grow">
        {plan.features.map((f: string, i: number) => (
          <li
            key={i}
            className={`flex items-start gap-2 ${f.startsWith("Everything") ? "font-semibold" : ""}`}
          >
      ...
      {plan.comingSoon && (
        <div className="my-6">
          ...
          <ul className="text-left text-sm text-gray-300 space-y-1 mt-2">
            {plan.comingSoon.map((f: string, i: number) => (
              <li key={i}>{f}</li>
            ))}
          </ul>
        </div>
...
export function Pricing() {
  ...
  const plans: Record<
    string,
    PricingPlan & { priceIds?: { monthly: string; annual: string } }
  > = {
    free: {
      name: "Guardian Free",
      price: {},
      ...
      features: [
        ...
        "5 AI Damage Scans/month",
        "500MB Secure Evidence Vault",
      ],
      comingSoon: [
        "Mobile App (iOS)",
    ...
    essential: {
      ...
      features: [
        ...
        "50 AI Damage Scans/month",
        "Deep Policy Analysis & Q&A",
        "Settlement Amount Calculator",
        "25 AI-Generated Documents/month",
      ...
      ctaClass:
        "bg-primary text-black hover:opacity-90 transition-all hover:scale-105",
      priceIds: {
    ...
    plus: {
      ...
      features: [
        ...
        "2 Team Members",
        "150 AI Damage Scans/month",
        "75 AI Documents/month",
        "25GB Evidence Vault",
    ...
    <section id="pricing" className="px-4 md:px-8 py-20 bg-black/10">
      <AnimatedSection className="max-w-6xl mx-auto text-center">
        ...
        <p className="mt-6 max-w-3xl mx-auto text-xl text-gray-300">
          Your plan pays for itself with one avoided claim denial. Choose the
          protection level that matches your property portfolio.
        </p>

        <div className="flex justify-center my-8">
          <div
            className={`${liquidGlass.backgrounds.secondary} p-1 rounded-lg flex items-center gap-2`}
          >
            <button
              onClick={() => setBillingCycle("monthly")}
              className={`px-4 py-1.5 rounded-md text-sm font-semibold transition-colors ${billingCycle === "monthly" ? liquidGlass.buttons.primary : ""}`}
            >
              Monthly
            </button>
            <button
              onClick={() => setBillingCycle("annual")}
              className={`px-4 py-1.5 rounded-md text-sm font-semibold transition-colors ${billingCycle === "annual" ? liquidGlass.buttons.primary : ""}`}
            >
              Annual (Save 17%)
            </button>
          </div>
        ...
        <div
          className={`${liquidGlass.cards.info} my-12 p-6 rounded-2xl text-left text-sm`}
        >
          <h3 className="font-bold text-primary mb-2">Why we charge:</h3>
          <p className="text-gray-300">
            Running cutting-edge AI models and secure hosting costs thousands
            each month. We're a family-run Florida business, built during late
            nights while the baby sleeps, because we believe every Floridian
            deserves access to tools that protect their legacy. That's why we're
            committed to maintaining a free tier - no one should face insurance
            companies alone.
          </p>
          <p className="text-gray-300 mt-2">
            Living the Florida dream comes with hurricanes, floods, and storms -
            but protecting your family's future shouldn't be another battle.
            Your subscription helps us keep the servers running and continue
            building tools for homeowners like you.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
          ...
          <PricingCard
            ...
            onClick={() => handlePlanClick("essential")}
            cycle={billingCycle}
        ...
        <div className="mt-16">
          <h3 className="font-slab text-2xl font-bold">
            For Larger Portfolios & Developers
          </h3>
          <p className="text-gray-300 mt-2">
            Specialized plans for enterprises and builders are coming soon.
          </p>
        </div>

        <p className="text-xs text-gray-300 mt-12 italic">
          Prices in USD. Florida residents add applicable sales tax. Features,
          pricing, and availability subject to change. Coming Soon features are
          in development. ClaimGuardian is a documentation and organization tool
          - not a substitute for professional legal, insurance, or financial
          advice. Service provided subject to our Terms of Service.
        </p>
      </AnimatedSection>
    </section>
  );
