...
const PricingCard: React.FC<{ plan: PricingPlan & { priceIds?: { monthly: string; annual: string } }; onClick: () => void; cycle: 'monthly' | 'annual'; isProcessing?: boolean }> = ({ plan, onClick, cycle, isProcessing }) => {
  ...
  return (
    <div className={`${plan.recommended ? liquidGlass.cards.primary : liquidGlass.cards.default} p-8 rounded-2xl flex flex-col h-full relative ${plan.recommended ? 'border-2' : 'border'}`}>
      {plan.recommended && <span className={`absolute -top-3 right-6 px-3 py-1 rounded-full text-sm font-bold ${liquidGlass.badges.primary}`}>Recommended</span>}
      {isAnnual && savings > 0 && <span className={`absolute -top-3 left-6 px-3 py-1 rounded-full text-sm font-bold ${liquidGlass.badges.success}`}>Save 17%</span>}
    ...
    <section id="pricing" className="px-4 md:px-8 py-20 bg-black/10">
      <AnimatedSection className="max-w-6xl mx-auto text-center">
        ...
        <div className="flex justify-center my-8">
          <div className={`${liquidGlass.backgrounds.secondary} p-1 rounded-lg flex items-center gap-2`}>
            <button onClick={() => setBillingCycle('monthly')} className={`px-4 py-1.5 rounded-md text-sm font-semibold transition-colors ${billingCycle === 'monthly' ? liquidGlass.buttons.primary : ''}`}>Monthly</button>
            <button onClick={() => setBillingCycle('annual')} className={`px-4 py-1.5 rounded-md text-sm font-semibold transition-colors ${billingCycle === 'annual' ? liquidGlass.buttons.primary : ''}`}>Annual (Save 17%)</button>
          </div>
        </div>
