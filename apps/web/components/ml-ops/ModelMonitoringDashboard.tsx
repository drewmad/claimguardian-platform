            ...
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className={`font-medium ${getHealthColor(model.metrics.healthScore)}`}>
                    {model.metrics.healthScore}%
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Requests</span>
                  <span className="font-medium">{model.metrics.requestCount.toLocaleString()}</span>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      {selectedModel && metrics && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm text-gray-500">Average Latency</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">{metrics.averageLatency}ms</div>
                <p className="text-xs text-gray-500 mt-1">P95: {metrics.p95Latency}ms</p>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm text-gray-500">Model Confidence</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">{(metrics.averageConfidence * 100).toFixed(1)}%</div>
                <p className="text-xs text-gray-500 mt-1">Average prediction confidence</p>
              </CardContent>
            </Card>

            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm text-gray-500">Throughput</CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">
                        {driftAnalysis.driftDetected ? 'Drift Detected' : 'No Drift Detected'}
                      </span>
                    </div>
                    {driftAnalysis.driftDetected && (
                      <span className={`font-medium ${getSeverityColor(driftAnalysis.severity)}`}>
                  ...
                  {driftAnalysis.driftDetected && (
                    <>
                      <div className="grid grid-cols-3 gap-4">
                        <div className="text-center">
                          ...
                          <p className="text-lg font-bold">
                            {Object.keys(driftAnalysis.metrics.featureDrift).length} features
                          </p>
                        </div>
                        <div className="text-center">
                          <p className="text-sm text-gray-500">Prediction Drift</p>
                          <p className="text-lg font-bold">
                            {(driftAnalysis.metrics.predictionDrift * 100).toFixed(1)}%
                          </p>
                        </div>
                        <div className="text-center">
                          <p className="text-sm text-gray-500">Concept Drift</p>
                          <p className="text-lg font-bold">
                            {(driftAnalysis.metrics.conceptDrift * 100).toFixed(1)}%
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="bg-yellow-50 p-3 rounded">
                        ...
                        <p className="text-sm text-gray-700">{driftAnalysis.recommendation}</p>
                      </div>
                    </>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">Deployed 2 days ago</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">Initial production release</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="flex gap-3">
            ...
            {driftAnalysis?.driftDetected && driftAnalysis.severity === 'high' && (
