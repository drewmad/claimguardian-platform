...
describe("InputSanitizer", () => {
  describe("sanitizeHtml", () => {
    ...
    it("should allow safe HTML tags", () => {
      const safeInput =
        "<p>Hello <strong>world</strong>!</p><br><em>Safe content</em>";
  ...
  describe("sanitizeEmail", () => {
    it("should validate and clean valid emails", () => {
      ...
      expect(inputSanitizer.sanitizeEmail("user.name+tag@domain.co.uk")).toBe(
  ...
  describe("sanitizeUrl", () => {
    it("should validate and accept valid URLs", () => {
      ...
      expect(
        inputSanitizer.sanitizeUrl("http://subdomain.example.com/path?query=1"),
      ).toBe("http://subdomain.example.com/path?query=1");
    });
  ...
  describe("sanitizePhone", () => {
    it("should clean and validate phone numbers", () => {
      ...
      expect(inputSanitizer.sanitizePhone("+44 20 7946 0958")).toBe(
        "+442079460958",
      );
    });
  ...
  describe("sanitizeFileName", () => {
    it("should clean file names", () => {
      ...
      expect(
        inputSanitizer.sanitizeFileName("document/with\\invalid:chars.pdf"),
      ).toBe("document_with_invalid_chars.pdf");
    });

    it("should remove leading dots", () => {
      expect(inputSanitizer.sanitizeFileName("...hidden-file.txt")).toBe(
        "hidden-file.txt",
      );
    });
    ...
    it("should preserve valid characters", () => {
      expect(inputSanitizer.sanitizeFileName("valid-file_name.123.txt")).toBe(
  ...
  describe("sanitizeSearchQuery", () => {
    it("should clean search queries", () => {
      expect(inputSanitizer.sanitizeSearchQuery("search term")).toBe(
        "search term",
      );
      expect(inputSanitizer.sanitizeSearchQuery("  search   term  ")).toBe(
        "search   term",
    ...
    it("should remove dangerous characters", () => {
      expect(
        inputSanitizer.sanitizeSearchQuery("<script>alert(1)</script>"),
  ...
  describe("sanitizeSqlSearchTerm", () => {
    it("should remove SQL injection patterns", () => {
      expect(
        inputSanitizer.sanitizeSqlSearchTerm("test'; DROP TABLE users; --"),
      ).toBe("test users");
      expect(
        inputSanitizer.sanitizeSqlSearchTerm("UNION SELECT * FROM passwords"),
      ).toBe("");
      expect(inputSanitizer.sanitizeSqlSearchTerm("normal search term")).toBe(
        "normal search term",
      );
    });
