...
jest.mock("@claimguardian/db", () => ({
  createBrowserSupabaseClient: jest.fn(() => ({
    from: jest.fn(() => ({
      insert: jest
        .fn()
        .mockResolvedValue({ data: { id: "prop-123" }, error: null }),
...
describe("Property Onboarding - Critical Tests", () => {
  describe("Address Validation", () => {
    it("should validate Florida addresses", () => {
      const validateFloridaAddress = (address: string) => {
        ...
        return {
          ...
          issues: [
            ...
            ...(!hasStreetNumber
              ? ["Address must start with street number"]
              : []),
      ...
      expect(
        validateFloridaAddress("123 Ocean Drive, Miami, FL 33101"),
      ).toEqual({
        isValid: true,
        issues: [],
      });
      ...
      expect(
        validateFloridaAddress("456 Sunset Blvd, Tampa, Florida 33602"),
      ).toEqual({
        isValid: true,
        issues: [],
      });
      ...
      expect(
        validateFloridaAddress("789 Beach Road, Jacksonville 32202"),
      ).toEqual({
        isValid: true,
        issues: [],
      });
      ...
      expect(validateFloridaAddress("123")).toEqual({
        isValid: false,
        issues: [
          "Address must be in Florida",
          "Address must include street name",
        ],
      });
    });
  ...
  describe("Property Type Selection", () => {
    ...
    it("should calculate appropriate insurance coverage based on property type", () => {
      const calculateCoverage = (propertyType: string, homeValue: number) => {
        ...
        const multiplier =
          coverageMultipliers[
            propertyType as keyof typeof coverageMultipliers
          ] || 1.0;
  ...
  describe("Hurricane Risk Assessment", () => {
    it("should assess hurricane risk by location", () => {
      const assessHurricaneRisk = (lat: number, lng: number) => {
        const isFloridaCoastal =
          lat > 24 &&
          lat < 31 && // Florida latitude range
          lng > -88 &&
          lng < -79 && // Florida longitude range
          Math.abs(lng) < 82; // Rough coastal approximation

        const isInlandFlorida =
          lat > 24 && lat < 31 && lng > -88 && lng < -79 && Math.abs(lng) >= 82;
      ...
      const miamiBeach = assessHurricaneRisk(25.7907, -80.13);
    ...
    it("should calculate hurricane deductible options", () => {
      const calculateHurricaneDeductible = (
        homeValue: number,
        riskLevel: string,
      ) => {
        if (riskLevel === "low") {
  ...
  describe("Data Persistence", () => {
    it("should validate property data before saving", () => {
      ...
      const validatePropertyData = (data: Partial<PropertyData>) => {
        ...
        if (
          !data.homeValue ||
          data.homeValue < 50000 ||
          data.homeValue > 10000000
        ) {
        ...
        if (
          !data.buildYear ||
          data.buildYear < 1800 ||
          data.buildYear > new Date().getFullYear()
        ) {
        ...
        if (
          !data.squareFootage ||
          data.squareFootage < 200 ||
          data.squareFootage > 20000
        ) {
      ...
      expect(validation.errors).toContain(
        "Home value must be between $50,000 and $10,000,000",
      );
      expect(validation.errors).toContain("Build year must be valid");
      expect(validation.errors).toContain(
        "Square footage must be between 200 and 20,000",
      );
  ...
  describe("Insurance Calculation Integration", () => {
    it("should calculate comprehensive insurance recommendations", () => {
      ...
      const calculateInsuranceRecommendations = (property: PropertyInfo) => {
        ...
        const ageRiskMultiplier =
          propertyAge > 30 ? 1.1 : propertyAge > 15 ? 1.05 : 1.0;
        ...
        const typeMultiplier =
          typeMultipliers[
            property.propertyType as keyof typeof typeMultipliers
          ] || 1.0;

        // Calculate base coverage
        const baseCoverage =
          property.homeValue * typeMultiplier * ageRiskMultiplier;
        ...
        return {
          ...
          estimatedAnnualPremium: Math.round(
            baseCoverage * 0.008 * (isCoastal ? 1.5 : 1.2),
          ),
          riskFactors: [
            ...(hurricaneRisk === "high" ? ["High hurricane risk area"] : []),
            ...(propertyAge > 30 ? ["Older construction"] : []),
            ...(property.propertyType === "mobile-home"
      ...
      const coastalRecommendation =
        calculateInsuranceRecommendations(coastalHome);
      ...
      expect(coastalRecommendation.riskFactors).toContain(
        "High hurricane risk area",
      );
      ...
      const inlandRecommendation =
        calculateInsuranceRecommendations(inlandHome);
      ...
      expect(inlandRecommendation.riskFactors).not.toContain(
        "High hurricane risk area",
      );
      expect(inlandRecommendation.riskFactors).not.toContain(
        "Older construction",
