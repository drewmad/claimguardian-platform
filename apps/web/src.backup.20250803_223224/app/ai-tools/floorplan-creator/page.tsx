...
interface Room {
  id: string
  name: string
  type: 'bedroom' | 'bathroom' | 'kitchen' | 'living' | 'dining' | 'garage' | 'other'
...
export default function FloorplanCreatorPage() {
  ...
  const roomTypes = [
    ...
    { type: 'kitchen', label: 'Kitchen', icon: ChefHat, color: 'bg-orange-500' },
    { type: 'living', label: 'Living/Dining', icon: Sofa, color: 'bg-green-500' },
          ...
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="mb-8">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <h1 className="text-3xl font-bold text-white">AI Floorplan Creator</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="bg-gray-800 border-gray-700">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-white">Floor Plan Canvas</CardTitle>
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => setCurrentPlan({ ...currentPlan, gridEnabled: !currentPlan.gridEnabled })}
                        >
                        ...
                        <>
                          ...
                          <div className="relative w-full h-full">
                            {currentPlan.rooms.map((room) => {
                              const Icon = room.icon || Square
                              return (
                                <div
                                  key={room.id}
                                  className={`absolute ${room.color} bg-opacity-30 border-2 ${
                                    selectedRoom?.id === room.id ? 'border-white' : 'border-gray-400'
                                ...
                                >
                                  <div className="absolute inset-0 flex flex-col items-center justify-center text-white">
                                    <Icon className="h-6 w-6 mb-1 opacity-70" />
                                    <p className="text-sm font-medium">{room.name}</p>
                                    {showDimensions && (
                                      <p className="text-xs opacity-70">
                                        {room.dimensions.width} × {room.dimensions.height} {currentPlan.unit}
                                      </p>
                                    )}
                                  </div>
                    ...
                    {currentPlan.rooms.length > 0 && (
                      <div className="mt-4 grid grid-cols-3 gap-4">
                        ...
                        <div className="bg-gray-700 rounded-lg p-3">
                          ...
                          <p className="text-lg font-semibold text-white">{currentPlan.rooms.length}</p>
                        </div>
                        <div className="bg-gray-700 rounded-lg p-3">
              ...
              <div className="space-y-6">
                ...
                {selectedRoom && (
                  <Card className="bg-gray-800 border-gray-700">
                    <CardHeader>
                      <CardTitle className="text-white">Room Properties</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div>
                        <Label htmlFor="roomName" className="text-white">Name</Label>
                        <Input
                          id="roomName"
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                          <Label htmlFor="roomHeight" className="text-white">Height ({currentPlan.unit})</Label>
                          <Input
                            id="roomHeight"
                            type="number"
                ...
                <Card className="bg-blue-900/20 border-blue-600/30">
                  <CardHeader>
                    <CardTitle className="text-blue-400 text-lg">Pro Tips</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-2 text-sm">
                      <div className="flex items-start gap-2">
                        <CheckCircle className="h-4 w-4 text-green-400 mt-0.5" />
                        <span className="text-gray-300">Phone scanning uses AI to detect room boundaries</span>
                      </div>
                      <div className="flex items-start gap-2">
                        <CheckCircle className="h-4 w-4 text-green-400 mt-0.5" />
                        <span className="text-gray-300">Export plans for insurance documentation</span>
                      </div>
                      <div className="flex items-start gap-2">
                        <CheckCircle className="h-4 w-4 text-green-400 mt-0.5" />
                        <span className="text-gray-300">Accurate measurements help with claims</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
