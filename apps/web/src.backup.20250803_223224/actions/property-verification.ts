...
export async function verifyPropertyEnrichment(
  ...
  try {
    ...
    const capturedFields = {
      ...
      insuranceFactors: !!(
        enrichment.insurance_risk_factors &&
        enrichment.insurance_territory_code
      )
    }
    ...
    logger.error('Error verifying property enrichment', { propertyId }, error as Error)
...
export async function getUserEnrichmentStats(userId: string) {
  try {
    ...
    if (propError) {
      logger.error('Failed to fetch user properties', { userId, error: propError })
    ...
    const { data: enrichments, error: enrichError } = await supabase
      .from('property_enrichments')
      .select('property_id, api_costs, flood_risk_score, insurance_risk_factors')
      .in('property_id', propertyIds)
      .eq('is_current', true)

    if (enrichError) {
      logger.error('Failed to fetch enrichment data', { userId, error: enrichError })
      return { success: false, error: 'Failed to fetch enrichment data' }
    }
    ...
    return {
      ...
      data: {
        ...
        enrichmentPercentage: Math.round((enrichedCount / properties.length) * 100),
        totalApiCost: totalCost,
        averageCompleteness: enrichedCount > 0 ? Math.round(avgCompleteness / enrichedCount) : 0
      }
    }
...
export async function checkEnrichmentHealth(userId: string) {
  try {
    ...
    logger.error('Error checking enrichment health', { userId }, error as Error)
    return { success: false, error: 'Failed to check enrichment health' }
  }
}