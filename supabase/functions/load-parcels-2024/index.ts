...
const FLORIDA_GIO_BASE = 'https://services1.arcgis.com/pGfbNJoYypmNq86F/arcgis/rest/services'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Loading ${countyInfo.name} County parcels (2024 data) - Mode: ${mode}, Offset: ${offset}, Limit: ${limit}`)
    ...
    while (attempts < maxRetries) {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Florida GIO API error: ${response.status} - ${errorText}`)
        }
    ...
    for (const feature of features) {
      try {
        ...
        if (geometry?.rings) {
          ...
          if (rings.length === 1) {
            // Single ring polygon
            const ringStr = rings[0].map((coord: number[]) => `${coord[0]} ${coord[1]}`).join(',')
            wkt = `SRID=4326;POLYGON((${ringStr}))`
          } else if (rings.length > 1) {
            // Multi-ring polygon (with holes or multiple parts)
            const multiStr = rings.map((ring: number[][]) =>
        ...
        const record = {
          ...
          TV_NSD: parseFloat(attributes.TV_NSD || attributes.TAXABLE_VALUE || 0) || null,
          ...
          // Property address
          PHY_ADDR1: attributes.SITE_ADDR || attributes.PHY_ADDR1 || attributes.SITUS_ADDR || null,
          ...
          EFF_YR_BLT: parseInt(attributes.YEAR_BUILT || attributes.EFF_YR_BLT || 0) || null,
          ACT_YR_BLT: parseInt(attributes.ACT_YR_BLT || attributes.YEAR_BUILT || 0) || null,
          TOT_LVG_AR: parseFloat(attributes.HEATED_AREA || attributes.TOT_LVG_AR || 0) || null,
          LND_SQFOOT: parseFloat(attributes.LND_SQFOOT || attributes.LAND_SQFT || 0) || null,
          NO_BULDNG: parseInt(attributes.NO_BULDNG || attributes.BLDG_NUM || 0) || null,

          // Values
          LND_VAL: parseFloat(attributes.LAND_VAL || attributes.LND_VAL || 0) || null,
          IMP_VAL: parseFloat(attributes.BLDG_VAL || attributes.IMP_VAL || 0) || null,

          // Sale information
          SALE_PRC1: parseFloat(attributes.SALE_PRICE || attributes.SALE_PRC1 || 0) || null,
          SALE_YR1: parseInt(attributes.SALE_YEAR || attributes.SALE_YR1 || 0) || null,
          SALE_MO1: parseInt(attributes.SALE_MONTH || attributes.SALE_MO1 || 0) || null,
    ...
    // Batch upsert
    if (records.length > 0) {
      console.log(`Upserting ${records.length} records to florida_parcels table...`)
