...
serve(async (req) => {
  ...
  try {
    ...
    const { damageType, county, propertyType, requestType } =
      (await req.json()) as CommunityQuery;
    ...
    if (authorization) {
      ...
      if (user) {
        ...
        await supabaseClient.from("achievement_events").insert({
          user_id: user.id,
    ...
    if (!claims || claims.length < MIN_SAMPLE_SIZE) {
      return new Response(
        JSON.stringify({
          ...
          message:
            "We need at least 100 similar claims to provide insights while protecting privacy",
    ...
    await supabaseClient.from("community_insights").insert({
    ...
    return new Response(
      JSON.stringify({
        ...
        disclaimer:
          "These insights are based on anonymized community data and should not be considered legal or financial advice",
      }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
...
function applyDifferentialPrivacy(claims: any[]): AnonymizedClaim[] {
  return claims.map((claim) => ({
    ...
    settlementBracket: addLaplacianNoise(
      claim.settlement_bracket,
      10000 / EPSILON,
    ),
    daysToSettle: Math.max(
      1,
      Math.round(addLaplacianNoise(claim.days_to_settle, 10 / EPSILON)),
    ),
...
function generateComprehensiveInsights(
  ...
  const lowSettlement =
    sortedSettlements[Math.floor(sortedSettlements.length * 0.25)];
  const highSettlement =
    sortedSettlements[Math.floor(sortedSettlements.length * 0.75)];
