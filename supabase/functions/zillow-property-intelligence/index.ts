  ...
  try {
    ...
    console.log(
      JSON.stringify({
        level: "warn",
        timestamp: new Date().toISOString(),
        message: "Zillow API error, using mock data:",
        error,
      }),
    );
    return generateMockZillowData(address, lat, lng);
...
function generateMockZillowData(
  address: string,
  lat?: number,
  lng?: number,
): any {
  ...
  const isFloridaProperty =
    addressLower.includes("fl") || addressLower.includes("florida");
  const isMiami =
    addressLower.includes("miami") || addressLower.includes("dade");
  const isTampa =
    addressLower.includes("tampa") || addressLower.includes("hillsborough");
  const isOrlando =
    addressLower.includes("orlando") || addressLower.includes("orange");
  const isJacksonville =
    addressLower.includes("jacksonville") || addressLower.includes("duval");
...
function analyzePropertyValue(zillowData: any): any {
  ...
  return {
    ...
    marketTrend:
      appreciation > 5 ? "up" : appreciation > -2 ? "stable" : "down",
...
function analyzeMarketConditions(zillowData: any, comparableSales: any[]): any {
  const avgSoldPrice =
    comparableSales.reduce((sum, comp) => sum + comp.soldPrice, 0) /
    comparableSales.length;
  const avgDaysOnMarket =
    comparableSales.reduce((sum, comp) => sum + comp.daysOnMarket, 0) /
    comparableSales.length;
  const appreciation = zillowData.appreciation;
  ...
  return {
    ...
    inventoryLevel:
      avgDaysOnMarket < 30 ? "low" : avgDaysOnMarket < 60 ? "medium" : "high",
    marketHeat:
      appreciation > 8
        ? "hot"
        : appreciation > 6
...
function calculateInsuranceIntelligence(
  propertyData: any,
  marketData: any,
): any {
  ...
  if (yearBuilt < 1990)
    riskFactors.push(
      "Older construction - potential electrical/plumbing updates needed",
    );
  if (yearBuilt < 1975)
    riskFactors.push("Pre-1975 construction - potential asbestos concerns");
...
function generateRentalEstimates(propertyData: any): any {
  ...
  return {
    ...
    yieldEstimate:
      Math.round(((rentZestimate * 12) / zestimate) * 100 * 100) / 100,
  ...
  try {
    ...
    switch (analysisType) {
      ...
      case "market-analysis":
        intelligence.propertyValue = analyzePropertyValue(zillowData);
        intelligence.marketAnalysis = analyzeMarketConditions(
          zillowData,
          zillowData.comparableSales,
        );
        break;
      ...
      case "complete-property-intel":
        ...
        intelligence.priceHistory = zillowData.priceHistory;
        intelligence.rentalEstimates = generateRentalEstimates(zillowData);
        intelligence.insuranceIntelligence = calculateInsuranceIntelligence(
          zillowData,
    ...
    console.log(
      JSON.stringify({
        level: "error",
        timestamp: new Date().toISOString(),
        message: "[Zillow Intelligence] Error:",
        error,
      }),
    );
