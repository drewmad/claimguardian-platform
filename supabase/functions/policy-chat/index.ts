...
Deno.serve(async (req) => {
  ...
  try {
    const { messages, policyDocument, userId } =
      (await req.json()) as PolicyChatRequest;
    ...
    if (policyDocument) {
      ...
      if (policyDocument.fileUrl) {
        ...
        if (
          policyDocument.type === "application/pdf" ||
          policyDocument.fileUrl.endsWith(".pdf")
        ) {
      ...
      if (policyContent) {
        ...
        chatHistory.push({
          ...
          parts: [
            {
              text: "I've received and analyzed the insurance policy document. I'm ready to answer any questions you have about your coverage, deductibles, claim procedures, or any other aspects of your policy.",
            },
          ],
        });
      }
