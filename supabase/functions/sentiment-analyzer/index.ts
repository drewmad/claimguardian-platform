...
serve(async (req) => {
  ...
  try {
    const {
      ...
      context = "general",
      user_id,
    } = (await req.json()) as SentimentRequest;
    ...
    const highRiskIndicators = analysis.risk_indicators.filter(
      (r) => r.probability > 0.7,
    );
    if (highRiskIndicators.length > 0 && user_id) {
      await triggerRiskAlerts(
        supabaseClient,
        user_id,
        highRiskIndicators,
        text,
...
async function analyzeSentiment(
  ...
  if (normalizedScore > 0.3) sentiment = "positive";
    ...
    detectedKeywords.filter((k) => k.sentiment === "positive").length > 0 &&
  ...
  const confidence = Math.min(
    100,
    Math.abs(normalizedScore) * 100 + keywordCount * 10,
  );
...
function detectEmotions(text: string, context: string): EmotionScore[] {
  ...
  const angerScore =
    angerPhrases.filter((p) => lowerText.includes(p)).length * 30;
  if (angerScore > 0) {
  ...
  const frustrationScore =
    frustrationPhrases.filter((p) => lowerText.includes(p)).length * 25;
  if (frustrationScore > 0) {
    emotions.push({
      emotion: "frustration",
      intensity: Math.min(100, frustrationScore),
    });
  }
  ...
  const satisfactionScore =
    satisfactionPhrases.filter((p) => lowerText.includes(p)).length * 30;
  if (satisfactionScore > 0) {
    emotions.push({
      emotion: "satisfaction",
      intensity: Math.min(100, satisfactionScore),
    });
  }
  ...
  const confusionScore =
    confusionPhrases.filter((p) => lowerText.includes(p)).length * 25;
  if (confusionScore > 0) {
    emotions.push({
      emotion: "confusion",
      intensity: Math.min(100, confusionScore),
    });
  }
  ...
  const urgencyScore =
    urgencyPhrases.filter((p) => lowerText.includes(p)).length * 35;
  if (urgencyScore > 0) {
    emotions.push({
      emotion: "urgency",
      intensity: Math.min(100, urgencyScore),
    });
  }
  ...
  const distressScore =
    distressPhrases.filter((p) => lowerText.includes(p)).length * 40;
  if (distressScore > 0) {
    emotions.push({
      emotion: "distress",
      intensity: Math.min(100, distressScore),
    });
  }
...
function detectRiskIndicators(text: string, context: string): RiskIndicator[] {
  ...
  if (legalMatches.length > 0) {
    risks.push({
      ...
      recommended_action:
        "Escalate to legal team immediately. Document all communications.",
    });
  }
  ...
  if (churnMatches.length > 0) {
    risks.push({
      ...
      recommended_action:
        "Transfer to retention specialist. Offer immediate assistance.",
    });
  }
  ...
  const escalationMatches = escalationPhrases.filter((p) =>
  ...
  if (escalationMatches.length > 0) {
    risks.push({
      ...
      recommended_action:
        "Notify supervisor. Prepare for executive escalation.",
    });
  }
  ...
  if (fraudMatches.length > 0) {
    risks.push({
      ...
      recommended_action:
        "Flag for fraud investigation team. Preserve all evidence.",
    });
  }
...
function generateRecommendations(
  ...
  if (sentiment === "negative" || sentiment === "mixed") {
    recommendations.push(
      "Respond with empathy and acknowledge customer concerns",
    );
    recommendations.push("Offer specific solutions or timeline for resolution");
  }

  // Emotion-based recommendations
  const highAnger = emotions.find(
    (e) => e.emotion === "anger" && e.intensity > 60,
  );
  if (highAnger) {
  ...
  const highDistress = emotions.find(
    (e) => e.emotion === "distress" && e.intensity > 50,
  );
  if (highDistress) {
  ...
  const highConfusion = emotions.find(
    (e) => e.emotion === "confusion" && e.intensity > 40,
  );
  if (highConfusion) {
  ...
  const highLegalRisk = risks.find(
    (r) => r.type === "legal" && r.probability > 0.6,
  );
  if (highLegalRisk) {
  ...
  const highChurnRisk = risks.find(
    (r) => r.type === "churn" && r.probability > 0.5,
  );
  if (highChurnRisk) {
