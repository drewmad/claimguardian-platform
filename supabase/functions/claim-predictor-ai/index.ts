...
interface ClaimPredictionRequest {
  property_id: string
  damage_type?: 'hurricane' | 'flood' | 'fire' | 'theft' | 'liability' | 'other'
...
interface FloridaFactor {
  type: 'hurricane_season' | 'flood_zone' | 'building_code' | 'insurance_market' | 'legal_environment'
  impact: string
...
async function predictClaimOutcome(
  ...
  if (propertyAge > 30) {
    ...
    recommendations.push('Provide recent inspection reports to demonstrate maintenance')
  ...
  if (historicalClaims.length > 3) {
    ...
    recommendations.push('Consider hiring a public adjuster for complex claim history')
  ...
  const hasActivePolicy = property.policies?.some((p: any) => p.status === 'active')
  if (!hasActivePolicy) {
    approvalScore -= 30
    riskFactors.push({
  ...
  floridaFactors.push({
    ...
    consideration: 'Insurers must respond within specific timeframes per Florida law'
  })
  ...
  const timeline = calculateFloridaTimeline(request.damage_type || 'other', approvalScore)

  // Find similar claims for comparison
  const similarClaims = generateSimilarClaims(request.damage_type || 'other', baseAmount)

  // Calculate confidence score
  const confidenceScore = calculateConfidence(riskFactors, property, historicalClaims)

  // Add general recommendations
  if (approvalScore < 50) {
    recommendations.push('Consider consulting with a property insurance attorney')
    recommendations.push('Document all communications with insurance company')
  }

  if (approvalScore > 80) {
    recommendations.push('Claim has high approval likelihood - proceed with standard documentation')
  }

  recommendations.push('Take extensive photos and videos of all damage')
  recommendations.push('Keep all receipts for emergency repairs and temporary housing')
...
function calculateSettlementRange(
  ...
  const settlementRates: Record<string, { low: number; expected: number; high: number }> = {
...
function calculateFloridaTimeline(
  ...
  const baseTimelines: Record<string, { initial: number; investigation: number; settlement: number }> = {
  ...
  const speedMultiplier = approvalScore > 70 ? 0.8 : approvalScore > 50 ? 1.0 : 1.3
  ...
  return {
    ...
    total_days: adjustedDays.initial + adjustedDays.investigation + adjustedDays.settlement
  }
}

function generateSimilarClaims(damageType: string, amount: number): SimilarClaim[] {
  // Generate realistic similar claims based on Florida data
  const baseData: Record<string, { avgSettlement: number; avgDays: number; approvalRate: number }> = {
...
function calculateConfidence(
  ...
  const totalWeight = riskFactors.reduce((sum, rf) => sum + Math.abs(rf.weight), 0)
