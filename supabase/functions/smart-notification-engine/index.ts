...
serve(async (req) => {
  ...
  try {
    ...
    const { userId, type, urgency, subject, body, metadata, channels } =
...
async function calculateOptimalSendTime(
  ...
  history.forEach((notif: any) => {
    ...
    hourEngagement[hour] =
      (hourEngagement[hour] || 0) + (responseTime < 60 ? 1 : 0.5);
    dayEngagement[day] = (dayEngagement[day] || 0) + 1;
  });

  // Find best hour
  const bestHour =
    Object.entries(hourEngagement).sort(([, a], [, b]) => b - a)[0]?.[0] ||
    "10";
...
async function updateEngagementPatterns(supabase: any, userId: string) {
  ...
  if (recentActivity && recentActivity.length > 20) {
    ...
    console.log(
      `Analyzed ${recentActivity.length} notifications for user ${userId}`,
    );
  }
}
