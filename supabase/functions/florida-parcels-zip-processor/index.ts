...
async function listZipContents(supabase: any): Promise<Response> {
  ...
  return new Response(
    JSON.stringify({
      ...
      message: 'ZIP file contains multiple formats. GeoJSON files are preferred for processing.'
    }),
    { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
  );
}

async function extractCountyData(supabase: any, countyCode?: number): Promise<Response> {
  if (!countyCode) {
  ...
  return new Response(
    JSON.stringify({
      ...
      message: 'The ZIP file contains FileGeodatabase (.gdb) files that need to be converted to GeoJSON format.',
      next_steps: [
