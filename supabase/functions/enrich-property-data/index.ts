...
serve(async (req) => {
  try {
    const { propertyId, latitude, longitude, address, placeId } =
    ...
    if (currentEnrichment?.enriched_at) {
      ...
      const hoursSinceEnrichment =
        (Date.now() - enrichedDate.getTime()) / (1000 * 60 * 60);
      if (hoursSinceEnrichment < 24) {
        return new Response(
          JSON.stringify({
            message: "Property recently enriched",
            data: currentEnrichment,
          }),
    ...
    const [geocodeData, elevationData, streetViewData, aerialData, nearbyData] =
      await Promise.all([
    ...
    const enrichmentData = {
      ...
      insurance_territory_code: getFloridaTerritoryCode(
        geocodeData.county,
        geocodeData.zip,
      ),
    ...
    try {
        ...
        await fetch(`${SUPABASE_URL}/functions/v1/send-email`, {
          ...
          body: JSON.stringify({
            ...
            enrichmentData: {
              ...
              protectionClass:
                enrichmentData.fire_protection?.protection_class || 10,
            },
          }),
        });
      }
      ...
      console.log(
        JSON.stringify({
          ...
          message: "Failed to send enrichment email:",
          emailError,
        }),
      );
...
async function fetchNearbyEmergencyServices(lat: number, lng: number) {
  ...
  return {
    fireStation: processNearbyResult(
      results.find((r) => r.type === "fire_station"),
    ),
    ...
    fireProtection: {
      nearest_station: processNearbyResult(
        results.find((r) => r.type === "fire_station"),
      ),
      protection_class: calculateProtectionClass(
        results.find((r) => r.type === "fire_station"),
      ),
    },
    medicalServices: {
      nearest_hospital: processNearbyResult(
        results.find((r) => r.type === "hospital"),
      ),
    },
    policeServices: {
      nearest_station: processNearbyResult(
        results.find((r) => r.type === "police"),
      ),
    },
  };
}
...
function calculateProtectionClass(fireStationResult: any) {
  ...
  const distance = calculateDistance(
    fireStationResult.results[0].geometry.location,
  );
...
function calculateInsuranceRiskFactors(data: any) {
  ...
  return {
    ...
    premium_impact:
      overallScore > 7 ? "high" : overallScore > 4 ? "moderate" : "low",
  };
}

async function storePropertyImages(
  supabase: any,
  propertyId: string,
  images: any,
) {
