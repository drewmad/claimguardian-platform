...
Deno.test("ML Pipeline - Property Risk Assessment E2E", async () => {
  ...
  const analyzeResponse = await makeAuthenticatedRequest(
    SPATIAL_AI_URL,
    analyzeRequest,
  );
  ...
  const embeddingResponse = await makeAuthenticatedRequest(
    SPATIAL_AI_URL,
    embeddingRequest,
  );
  ...
  const riskResponse = await makeAuthenticatedRequest(
    SPATIAL_AI_URL,
    riskRequest,
  );
