  ...
  try {
    ...
    switch (action) {
      case "deploy": {
        ...
        const { data: deployment, error } = await supabase.rpc(
      ...
      case "drift_check": {
        ...
        // Run drift detection
        const { data: driftResult, error } = await supabase.rpc(
          "check_model_drift",
          {
            p_model_version_id: modelVersionId,
      ...
      case "explain": {
        ...
        // Store explanation
        const { error } = await supabase.from("model_explanations").insert({
...
async function getDeploymentMetrics(supabase: any, deploymentId: string) {
  ...
  return {
    ...
    averageConfidence:
      predictions?.reduce((sum, p) => sum + p.model_confidence, 0) /
...
async function generateExplanation(
  ...
  // Normalize
  const total = Object.values(featureImportance).reduce(
    (sum, val) => sum + val,
    0,
  );
  ...
  const textExplanation = `The ${prediction.decision_type} decision was primarily influenced by: ${topFeatures
    .map(
      ([feature, importance]) =>
  ...
  return {
    ...
    decisionPath: [
      ...
      { step: 4, description: "Decision threshold", confidence: 0.9 },
    ],
    counterfactuals,
  };
}
