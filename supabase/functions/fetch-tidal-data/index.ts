...
const NOAA_API_URL =
  "https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?product=water_level&application=ClaimGuardian&begin_date=20250730&range=24&station_type=tide&format=json&datum=STND&units=english&time_zone=gmt&state=FL";
...
serve(async (_req) => {
  try {
    ...
    return new Response(
      JSON.stringify({
        message: `Successfully processed ${stationUpdates.length} tidal station updates.`,
      }),
      { headers: { "Content-Type": "application/json" } },
    );
  } catch (error) {
    console.log(
      JSON.stringify({
        ...
        message: "Error fetching tidal data:",
        error,
      }),
    );
    return new Response(
