...
interface StaticMapRequest {
  ...
  analysisType: 'property-overview' | 'damage-assessment' | 'neighborhood-context' | 'custom'
  options?: {
    zoom?: number
    size?: string // '640x640'
    format?: 'png' | 'jpg' | 'gif'
  ...
  return {
    ...
    annotated: generateStaticMapUrl(location, 'hybrid', {
      ...
      markers: [
        ...
        { lat: location.lat + 0.005, lng: location.lng + 0.005, color: 'red', label: 'F', size: 'small' as const },
        { lat: location.lat - 0.008, lng: location.lng + 0.003, color: 'green', label: 'H', size: 'small' as const }
      ]
    })
  }
}
...
function generateDamageAssessmentMaps(location: { lat: number, lng: number }): any {
  ...
  const damageMarkers = [
    ...
    { lat: location.lat - 0.0001, lng: location.lng + 0.0002, color: 'orange', label: '2', severity: 'moderate' },
    { lat: location.lat + 0.0002, lng: location.lng - 0.0001, color: 'yellow', label: '3', severity: 'minor' }
  ]
  ...
  return {
    beforeStormUrl: generateStaticMapUrl(location, 'satellite', {
      ...baseOptions,
      markers: [{ lat: location.lat, lng: location.lng, color: 'blue', label: 'P' }],
...
function generateNeighborhoodContext(location: { lat: number, lng: number }): any {
  ...
  const nearbyStructures = [
    ...
    { type: 'Vacant lot', distance: 35, relevance: 'No wind protection from east' }
  ]
...
function analyzeMapIntelligence(location: { lat: number, lng: number }, analysisType: string, address?: string): string[] {
  const insights = []
  const isFloridaAddress = address?.toLowerCase().includes('fl') || address?.toLowerCase().includes('florida')
  ...
  switch (analysisType) {
    case 'property-overview':
      ...
      if (isFloridaAddress) {
        insights.push('Located in hurricane-prone region - wind coverage essential')
    ...
    case 'damage-assessment':
      ...
      insights.push('Before/after comparison shows clear storm impact patterns')
      if (isFloridaAddress) {
    ...
    case 'neighborhood-context':
      ...
      insights.push('Topographical features affecting wind patterns identified')
      break
  }
