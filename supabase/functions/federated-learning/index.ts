  ...
  try {
    ...
    switch (action) {
      case "register_node": {
        ...
        const { error } = await supabase
          ...
          .insert({
            ...
            differential_privacy_epsilon: calculatePrivacyBudget(
              dataVolumeEstimate as number,
            ),
      ...
      case "start_round": {
        ...
        const selectedNodes = selectNodesForRound(
          eligibleNodes || [],
        ...
        const { data: round, error } = await supabase
          ...
          .insert({
            ...
            round_number: await getNextRoundNumber(
              supabase,
              modelFamily as string,
            ),
      ...
      case "submit_update": {
        ...
        await supabase
          .from("federated_learning_nodes")
          .update({
            total_rounds_participated: supabase.raw(
              "total_rounds_participated + 1",
            ),
...
function selectNodesForRound(nodes: any[], targetCount: number): any[] {
  ...
  Object.values(nodesByType).forEach((typeNodes) => {
    const sorted = typeNodes.sort(
      (a, b) => b.total_contribution_score - a.total_contribution_score,
    );
...
async function rewardContributingNodes(supabase: any, round: any) {
  ...
  for (const nodeId of round.completed_nodes) {
    await supabase
      .from("federated_learning_nodes")
      .update({
        total_contribution_score: supabase.raw(
          `total_contribution_score + ${contributionBonus}`,
        ),
      })
      .eq("node_identifier", nodeId);
  }
