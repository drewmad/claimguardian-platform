...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { fileUrl, fileName, options, extractionSchema } =
      (await req.json()) as ExtractionRequest;

    console.log("Enhanced document extraction started", {
      fileName,
    ...
    extractedData = await postProcessExtraction(
      extractedData,
      options,
      supabase,
    );
...
async function extractWithOpenAI(
  ...
  if (isImage) {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert insurance policy document analyzer. Extract structured data from insurance documents with high accuracy.",
        },
        {
          role: "user",
          content: [
    ...
    throw new Error(
      "PDF processing with OpenAI requires text extraction first",
    );
  }
}
...
async function postProcessExtraction(data: any, options: any, supabase: any) {
  ...
  data.extractedFields = Object.keys(data).filter(
    (key) =>
      data[key] !== null &&
      data[key] !== undefined &&
      key !== "extractedFields",
  ...
  if (data.propertyAddress?.state) {
    ...
    data.propertyAddress.state =
      stateMap[data.propertyAddress.state] || data.propertyAddress.state;
  }
