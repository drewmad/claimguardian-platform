...
serve(async (req) => {
  ...
  try {
    ...
    if (!source || !DATA_SOURCES[source as keyof typeof DATA_SOURCES]) {
      throw new Error(`Invalid source: ${source}. Available: ${Object.keys(DATA_SOURCES).join(', ')}`)
    }
      ...
      for (const feature of features) {
        try {
          ...
          const { error } = await supabase
            .from('active_events')
            .upsert({
              external_id: attributes.FIRE_ID || `wildfire_${attributes.OBJECTID}`,
              event_type: 'wildfire',
              event_name: attributes.FIRE_NAME || 'Unnamed Fire',
              status: 'active',
              severity: attributes.ACRES > 100 ? 'high' : attributes.ACRES > 10 ? 'medium' : 'low',
              start_time: attributes.DISCOVERY_DATE ? new Date(attributes.DISCOVERY_DATE).toISOString() : new Date().toISOString(),
              attributes: {
                ...attributes,
        ...
        try {
          const records = batch.map((feature: any) => {
            ...
            return {
              ...
              data_version: new Date().toISOString().split('T')[0].replace(/-/g, ''),
              created_at: new Date().toISOString(),
              updated_at: new Date().toISOString()
            }
