...
function generalizePolicyType(policyType: string): string {
  ...
  if (policyType.toLowerCase().includes('hurricane')) return 'hurricane_coverage'
...
function generatePrivateInsights(contributions: any[], privacySettings: PrivacySettings): ClaimInsight[] {
  ...
  for (const [damageType, claims] of Object.entries(damageGroups)) {
    ...
    const settlements = claims.map(c => dequantizeSettlement(c.settlement_bucket))
    ...
    const noisySuccessRate = dp.addLaplaceNoise(successes / claims.length, 1 / claims.length) * 100

    // Calculate trend (simplified - would use historical data in production)
    const trend = Math.random() > 0.33 ? (Math.random() > 0.5 ? 'increasing' : 'decreasing') : 'stable'
  ...
  try {
    ...
    if (action === 'contribute') {
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: 'Thank you for contributing! Your data helps the community while maintaining your privacy.',
      ...
      if (filters?.damageType && filters.damageType !== 'all') {
        query = query.contains('anonymized_data', { damage_type: filters.damageType })
      }
