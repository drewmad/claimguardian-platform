...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Check rate limiting
    const rateLimitOk = await checkRateLimit(supabase, user.id, 'secure-edge-function')
    if (!rateLimitOk) {
    ...
    logSecure('info', 'Request completed successfully', {
      userId: user.id,
      action: body.action
    }, requestId)
    ...
    logSecure('error', 'Unexpected error', {
      error: error.message,
