...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const fdotUrl = new URL(
      `https://gis.fdot.gov/arcgis/rest/services/Parcels/FeatureServer/${CHARLOTTE_LAYER_ID}/query`,
    );
    ...
    for (const feature of data.features) {
      try {
        ...
        const parcelRecord = {
          ...
          property_address:
            attributes.SITEADDRESS || attributes.PHY_ADDR1 || null,
          owner_name: attributes.OWN_NAME || attributes.OWNERNAME || null,
          owner_address:
            attributes.MAILING_ADDRESS_1 || attributes.OWN_ADDR1 || null,
          // Property details
        ...
        if (existing) {
          ...
          // Insert new parcel
          const { error } = await supabase.from("parcels").insert(parcelRecord);
    ...
    const hasMore =
      data.exceededTransferLimit || data.features.length === limit;
