...
serve(async (req) => {
  ...
  try {
    ...
    const analysis = await performFraudAnalysis(request.analysis_type, {
      claim: claimData,
      user: userData,
...
function analyzeClaimTiming(claim: any): {
  score: number;
  suspicious: boolean;
  ...
  if (claim.policy_start_date) {
    ...
    const daysSincePolicyStart =
      (claimDate.getTime() - policyStart.getTime()) / (1000 * 60 * 60 * 24);

    if (daysSincePolicyStart < 30) {
      score += 25;
      suspicious = true;
      evidence.push(
        `Claim filed ${Math.round(daysSincePolicyStart)} days after policy start`,
      );
    }
  }
  ...
  if (claim.policy_end_date) {
    ...
    const claimDate = new Date(claim.created_at);
    const daysBeforePolicyEnd =
      (policyEnd.getTime() - claimDate.getTime()) / (1000 * 60 * 60 * 24);

    if (daysBeforePolicyEnd < 30 && daysBeforePolicyEnd > 0) {
      score += 15;
      suspicious = true;
      evidence.push(
        `Claim filed ${Math.round(daysBeforePolicyEnd)} days before policy expiration`,
      );
    }
  }
...
function analyzeDamagePatterns(description: string): {
  score: number;
  ...
  const exaggerationCount = exaggerationKeywords.filter((k) =>
  ...
  const matchedPhrases = commonFraudPhrases.filter((p) =>
...
function analyzeHistoricalPatterns(claims: any[]): any {
  ...
  const timeSpan =
    claims.length > 1
      ? (new Date(claims[0].created_at).getTime() -
          new Date(claims[claims.length - 1].created_at).getTime()) /
        (1000 * 60 * 60 * 24 * 365)
      : 1;
  ...
  const amounts = claims
    ...
    .sort((a, b) => a - b);
  if (amounts.length > 2) {
    const isEscalating = amounts.every(
      (amt, idx) => idx === 0 || amt >= amounts[idx - 1],
    );
    if (isEscalating) {
      score += 10;
      patterns.push({
...
function analyzeDocuments(documents: any[]): {
  ...
  documents.forEach((doc) => {
    ...
    if (doc.name) {
      ...
      if (
        docName.includes("template") ||
        docName.includes("sample") ||
        docName.includes("example")
      ) {
        score += 20;
        anomalies.push({
          document_type: doc.type || "Unknown",
...
function analyzeFloridaSpecificPatterns(data: any): {
  score: number;
  flags: FloridaSpecificFlag[];
} {
  let score = 0;
  ...
  if (data.claim?.has_aob || data.additional?.contractor_involved) {
    const hasMultipleAOBs =
      data.historical?.filter((c: any) => c.has_aob).length > 2;
    if (hasMultipleAOBs) {
      score += 25;
      flags.push({
        type: "aob_abuse",
        ...
        risk_level: "high",
        legal_implications:
          "May violate Florida Statute 627.7152 regarding AOB agreements",
      });
    }
  }
  ...
  if (
    data.claim?.damage_type === "hurricane" ||
    data.claim?.damage_type === "wind"
  ) {
    ...
    if (isPostStorm && data.additional?.contractor_from_out_of_state) {
      ...
      flags.push({
        ...
        risk_level: "high",
        legal_implications:
          "Potential violation of Florida emergency contractor regulations",
      });
    }
  }
  ...
  if (data.claim?.amount && data.property?.value) {
    ...
    if (claimToValueRatio > 0.5) {
      ...
      flags.push({
        ...
        risk_level: "medium",
        legal_implications:
          "May constitute insurance fraud under Florida Statute 817.234",
      });
    }
  }
...
function analyzeBehavior(
  user: any,
  claims: any[],
): { score: number; patterns: BehavioralPattern[] } {
  let score = 0;
  const patterns: BehavioralPattern[] = [];
  ...
  if (user.last_updated && claims.length > 0) {
    ...
    const daysBetween =
      (firstClaim.getTime() - lastUpdate.getTime()) / (1000 * 60 * 60 * 24);
...
function calculateConfidence(
  ...
  const highConfidenceIndicators = indicators.filter(
    (i) => i.confidence > 80,
  ).length;
