...
serve(async (req) => {
  ...
  try {
    const { property_id, scan_type = "quick" } =
      (await req.json()) as MaintenanceRequest;
...
async function generateMaintenancePredictions(property: any, scanType: string) {
  ...
  const hasHurricaneRisk =
    isCoastal ||
    ["Miami-Dade", "Broward", "Palm Beach"].includes(property.county);
  ...
  if (hvacYears > 10) {
    ...
    if (hvacHealth < 40) {
      alerts.push({
        ...
        system: "HVAC",
        component: isCoastal
          ? "AC Compressor (Salt Corrosion)"
          : "AC Compressor",
        urgency: hvacHealth < 30 ? "critical" : "high",
        predictedFailure: new Date(
          Date.now() + hvacHealth * 24 * 60 * 60 * 1000,
        ).toISOString(),
        currentCondition: hvacHealth,
        estimatedCost: isCoastal ? 5500 : 4500,
        preventiveCost: 450,
        recommendation:
          "Schedule preventive maintenance to avoid complete failure",
  ...
  if (roofYears > 15 || hasHurricaneRisk) {
    const roofHealth = Math.max(
      30,
      100 - roofYears * 3 - (hasHurricaneRisk ? 10 : 0),
    );
    ...
    systemHealth.push({
      ...
      nextScheduled:
        roofHealth < 50 ? "Before hurricane season" : "Annual inspection",
    });
    ...
    if (roofHealth < 60) {
      alerts.push({
        id: `roof-${Date.now()}`,
        system: "Roof",
        component: hasHurricaneRisk
          ? "Hurricane Straps & Shingles"
          : "Shingles",
        urgency: roofHealth < 40 ? "high" : "medium",
        predictedFailure: new Date(
          Date.now() + roofHealth * 30 * 24 * 60 * 60 * 1000,
        ).toISOString(),
  ...
  const foundationHealth = Math.max(
    70,
    100 - propertyAge * 0.5 - (isCoastal ? 10 : 0),
  );
  systemHealth.push({
    system: "Foundation",
  ...
  if (hasHurricaneRisk) {
    ...
    systemHealth.push({
      ...
      trend: "stable",
      lastMaintenance: "3 months ago",
      nextScheduled: property.details?.impact_windows
        ? "Annual"
        : "Upgrade recommended",
    });
    ...
    if (!property.details?.impact_windows && scanType === "comprehensive") {
      alerts.push({
        ...
        preventiveCost: 8000,
        recommendation:
          "Upgrade to impact-resistant windows for insurance discount",
  ...
  return {
    ...
    predictions: {
      totalSavings: alerts.reduce(
        (sum, a) => sum + (a.estimatedCost - a.preventiveCost),
        0,
      ),
      ...
      nextMaintenance:
        alerts[0]?.recommendation || "No immediate action required",
    },
  };
}

function generateMaintenanceTimeline(
