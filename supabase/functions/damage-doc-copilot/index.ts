...
const DOCUMENTATION_GUIDANCE = {
  overview: {
    instruction:
      "Capture a wide shot showing the entire damage area and surrounding context.",
    qualityChecks: ["good lighting", "entire damage visible", "stable framing"],
  },
  close_up: {
    instruction:
      "Take a detailed close-up photo of the specific damage. Get close enough to see material details.",
    qualityChecks: [
  ...
  context: {
    instruction:
      "Show how the damage relates to the surrounding area. Step back to include more context.",
    qualityChecks: [
      "damage and surroundings visible",
      "scale evident",
      "good perspective",
    ],
  },
  surrounding_area: {
    instruction:
      "Photograph undamaged areas nearby for comparison. This helps establish the extent of damage.",
    qualityChecks: [
      "undamaged areas clear",
      "good contrast with damage",
      "comparative perspective",
    ],
  },
  supporting_evidence: {
    instruction:
      "Capture any serial numbers, measurements, or reference objects that help document the damage.",
    qualityChecks: [
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { imageUrl, sessionProgress, previousImages }: RequestBody =
      await req.json();
    ...
    const nextRequiredAngle = allAngles.find(
      (angle) => !sessionProgress.completedAngles.includes(angle),
    );
...
async function analyzeImage(
  ...
  if (isComplete) {
    return {
      ...
      quality,
      completionMessage:
        "Excellent work! Your damage documentation is thorough and well-captured.",
      suggestions: [
  ...
  const guidance =
    DOCUMENTATION_GUIDANCE[
      expectedAngle as keyof typeof DOCUMENTATION_GUIDANCE
    ];
