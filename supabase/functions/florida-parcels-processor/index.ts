...
async function getProcessingStatus(supabase: any): Promise<Response> {
  ...
  const statuses = Object.entries(FLORIDA_COUNTIES).map(([code, name]) => {
    ...
    return {
      ...
      progress:
        log?.total_parcels > 0
          ? Math.round((count / log.total_parcels) * 100)
          : 0,
  ...
  const summary = {
    ...
    processing_counties: statuses.filter((s) => s.status === "processing")
      .length,
    pending_counties: statuses.filter((s) => s.status === "pending").length,
    error_counties: statuses.filter((s) => s.status === "error").length,
    total_parcels_processed: Array.from(countsByCounty.values()).reduce(
      (sum, count) => sum + count,
      0,
    ),
  };

  return new Response(JSON.stringify({ summary, counties: statuses }), {
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  });
}
...
async function processCounty(
  ...
  console.log(
    `Processing ${countyName} County (${countyCode}) from ${storagePath}`,
  );
  ...
  try {
    ...
    const features = geoJson.features.filter(
      (f: any) => f.properties.CO_NO === countyCode,
    );
