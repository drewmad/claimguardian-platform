...
const FLORIDA_DATA_SOURCES = {
  'fl_dor_statewide': {
    base_url: 'https://floridarevenue.com/property/Documents/PropertyDataPortal/',
    data_format: 'csv',
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`Florida parcel monitor action: ${action}`)
    ...
    console.error('Parcel monitor error:', error)
...
async function checkDataSources(targetSource?: string) {
  ...
  for (const source of sourcesToCheck) {
    const config = FLORIDA_DATA_SOURCES[source as keyof typeof FLORIDA_DATA_SOURCES]
    if (!config) {
      results.push({
        source,
        status: 'error',
    ...
    try {
      ...
      console.error(`Error checking source ${source}:`, error)
      results.push({
        source,
        status: 'error',
...
async function triggerIngest(dataSource?: string, forceRefresh = false) {
  ...
  const config = FLORIDA_DATA_SOURCES[dataSource as keyof typeof FLORIDA_DATA_SOURCES]
  if (!config) {
    throw new Error(`Unknown data source: ${dataSource}`)
  }

  console.log(`Triggering ingest for ${dataSource}`)
  ...
  const { data: runningImports } = await supabase
    ...
    .in('status', ['pending', 'downloading', 'validating', 'transforming', 'importing'])
  ...
  const ingestResponse = await supabase.functions.invoke('florida-parcel-ingest', {
    body: {
...
async function findAvailableFiles(config: any): Promise<Array<{
  name: string
  size?: number
