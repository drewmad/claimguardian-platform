...
interface AerialRequest {
  ...
  analysisType: 'roof-condition' | 'damage-assessment' | 'solar-potential' | 'structural-analysis' | 'complete-analysis'
...
async function getAerialImagery(lat: number, lng: number, options: { zoom?: number, size?: string, mapType?: string } = {}): Promise<string> {
...
async function analyzeRoofFromAerialView(lat: number, lng: number): Promise<any> {
...
async function mockRoofAnalysis(lat: number, lng: number): Promise<any> {
  ...
  return {
    ...
    imageryDate: { year: 2024, month: Math.floor(Math.random() * 12) + 1, day: Math.floor(Math.random() * 28) + 1 },
    postalCode: "33101",
...
function analyzeRoofCondition(aerialData: any, location: { lat: number, lng: number }): any {
  ...
  const sunshineHours = aerialData.solarPotential?.maxSunshineHoursPerYear || 3000
...
function assessDamage(aerialData: any, damageReportDate?: string): any {
  ...
  if (damagePercentage > 25) {
    insuranceRecommendation = 'Significant damage detected. File insurance claim immediately and get professional inspection.'
  } else if (damagePercentage > 15) {
    insuranceRecommendation = 'Moderate damage visible. Consider insurance claim and get repair estimates.'
  } else if (damagePercentage > 5) {
    insuranceRecommendation = 'Minor damage detected. Monitor for worsening and consider preventive maintenance.'
  } else {
...
function analyzeSolarPotential(aerialData: any): any {
  ...
  const potentialSavings = annualEnergyPotential * 0.10 // $0.10 per kWh
...
function analyzeStructure(aerialData: any, location: { lat: number, lng: number }): any {
  const roofSegment = aerialData.roofSegmentStats?.[0]
  const buildingFootprint = roofSegment?.stats.groundAreaCoveredMeters2 * 10.764 || 0
  ...
  return {
    ...
    structuralConcerns: buildingFootprint > 4000 ? ['Large structure - check support systems'] : []
  }
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { location, analysisType, referenceDate, damageReportDate } = requestData
    ...
    intelligence.aerialUrls = {
      ...
      oblique: await getAerialImagery(location.lat, location.lng, { zoom: 19, mapType: 'hybrid' })
    }
    ...
    switch (analysisType) {
      ...
      case 'damage-assessment':
        intelligence.damageAssessment = assessDamage(aerialData, damageReportDate)
        break
      ...
      case 'structural-analysis':
        intelligence.structuralAnalysis = analyzeStructure(aerialData, location)
        break
