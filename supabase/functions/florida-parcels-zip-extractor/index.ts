...
async function listExtractedFiles(supabase: any): Promise<Response> {
  ...
  return new Response(
    JSON.stringify({
      ...
      files: groupedFiles,
      ready_for_processing: files?.some((f: any) =>
...
function getRecommendations(analysis: any): string[] {
  ...
  if (analysis.geodatabase_files.length > 0) {
    recommendations.push(
      "FileGeodatabase detected - will need ogr2ogr conversion after extraction",
    );
  }

  if (analysis.total_size_gb > 3) {
    recommendations.push(
      "Large file size - extraction may take several minutes",
    );
  }

  if (analysis.directories.length > 0) {
    recommendations.push(
      "Multiple directories found - data is organized by folder",
    );
  }
...
function getNextSteps(files: string[]): string[] {
  ...
  if (files.some((f) => f.endsWith(".geojson"))) {
    steps.push(
      "Process GeoJSON files directly using florida-parcels-processor",
    );
  }
