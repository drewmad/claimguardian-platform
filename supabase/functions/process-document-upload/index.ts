...
Deno.serve(async (req: Request) => {
  try {
    ...
    const floridaContext = await enrichWithFloridaData(
      consolidatedData,
      supabase,
    );
    ...
    await supabase
      ...
      .update({
        processing_status:
          processingRoute === "auto" ? "auto_confirmed" : "pending_review",
        extraction_results: consolidatedData,
        ai_metadata: suggestions,
        confidence_scores: {
          overall: confidenceScore,
          details: suggestions.confidences,
        },
...
function calculateConfidence(suggestions: any): number {
  ...
  totalScore +=
    (suggestions.confidences?.associations || 0) * weights.associations;
