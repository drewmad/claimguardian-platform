...
function analyzePerformance(feedbackData: any[]): any {
  ...
  const confirmed = feedbackData.filter(
    (f) => f.user_action === "confirm",
  ).length;
...
function identifyPatterns(feedbackData: any[]): any {
  ...
  feedbackData
    ...
    .forEach((f) => {
      ...
      patterns.commonCorrections[key] =
        (patterns.commonCorrections[key] || 0) + 1;
    });
  ...
  Object.keys(byCategory).forEach((cat) => {
    const avgConf =
      byCategory[cat].reduce((a, b) => a + b, 0) / byCategory[cat].length;
    if (avgConf < 0.7) {
      patterns.lowConfidenceAreas.push({
        category: cat,
...
function prepareTrainingData(feedbackData: any[], patterns: any): any[] {
  ...
  return feedbackData.map((feedback) => {
    ...
    return {
      ...
      expected: {
        name: isCorrect
          ? feedback.ai_suggested_name
          : feedback.user_corrected_name,
        category: isCorrect
          ? feedback.ai_suggested_category
          : feedback.user_corrected_category,
        associations: isCorrect
          ? feedback.ai_suggested_associations
          : feedback.user_corrected_associations,
      },
    ...
    messages: [
      {
        role: "system",
        content:
          "You are an expert document analyzer for insurance claims in Florida.",
      },
      {
        role: "user",
  ...
  return {
    ...
    improvements: {
      ...
      estimatedAccuracyGain:
        data.validationResult?.comparisonWithPrevious?.accuracyDelta || 0,
    },
...
function calculateTrainingWeight(feedback: any, patterns: any): number {
  ...
  const lowConfArea = patterns.lowConfidenceAreas.find(
...
function calculateEstimatedImprovement(metrics: any): number {
  ...
  const categoryImprovement =
    Object.values(metrics.categoryAccuracy).filter((acc) => acc < 0.8).length *
    0.02;
