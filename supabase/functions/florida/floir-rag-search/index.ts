...
serve(async (req: Request) => {
  ...
  try {
    ...
    const results: SearchResult[] = (searchResults || []).map(
      (result: any) => ({
        id: result.id,
        ...
        content_snippet: createContentSnippet(
          result.data_type,
...
Please provide a comprehensive answer based on the Florida insurance regulation data above.`,
    ...
    console.log(
      JSON.stringify({
        level: "error",
        timestamp: new Date().toISOString(),
        message: "FLOIR RAG search error:",
        error,
      }),
    );
...
function createContentSnippet(dataType: string, normalizedData: any): string {
  ...
  switch (dataType) {
    ...
    case "news_bulletins":
      return (
        `${normalizedData.Title || "N/A"} | ${normalizedData.Summary || "N/A"}`.substring(
          0,
          snippetLength,
        ) + "..."
      );
