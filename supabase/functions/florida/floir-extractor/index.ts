...
serve(async (req: Request) => {
  ...
  try {
    ...
    const {
      ...
      query = {},
      force_refresh = false,
    } = (await req.json()) as CrawlRequest;

    if (!data_type || !Object.values(FliorDataType).includes(data_type)) {
      throw new Error(
        `Invalid data_type. Must be one of: ${Object.values(FliorDataType).join(", ")}`,
      );
    }
    ...
    try {
            ...
            try {
              ...
              console.log(
                JSON.stringify({
                  level: "info",
                  timestamp: new Date().toISOString(),
                  message: `Failed to generate embedding for ${parsed.primary_key}:`,
                  embeddingError,
                }),
              );
              errors.push({
          ...
          if (upsertError) {
            console.log(
              JSON.stringify({
                level: "info",
                timestamp: new Date().toISOString(),
                message: `Failed to upsert record ${parsed.primary_key}:`,
                upsertError,
              }),
            );
            errors.push({
      ...
      await supabase.realtime.channel(`floir:${data_type}`).send({
        type: "broadcast",
