...
async function processChunk(
  ...
  for (const rawRecord of chunk) {
    try {
      ...
      const propertyRecord = await transformRecord(
        rawRecord,
        dataSource,
        batchId,
      );
      ...
      if (!validationResult[0].is_valid) {
        invalid++;
        errors.push(
          `Invalid record ${propertyRecord.parcel_id}: ${validationResult[0].errors.join(", ")}`,
        );
        continue;
      }
        ...
        try {
          ...
          console.log(
            JSON.stringify({
              level: "info",
              timestamp: new Date().toISOString(),
              message: `Failed to generate embedding for ${propertyRecord.parcel_id}:`,
              embeddingError,
            }),
          );
...
async function transformRecord(
  ...
  const transformed: any = {
    ...
    property_value: parseFloat(
      rawRecord.property_value ||
        rawRecord.PROPERTY_VALUE ||
        rawRecord.value ||
        "0",
    ),
    property_type:
      rawRecord.property_type || rawRecord.PROPERTY_TYPE || rawRecord.use_code,
    county:
      rawRecord.county ||
      rawRecord.COUNTY ||
      extractCountyFromSource(dataSource),
