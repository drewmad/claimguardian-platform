...
Deno.serve(async (req) => {
  ...
  if (fetchError) {
    return new Response(
      JSON.stringify({
        error: `Failed to fetch scraper runs: ${fetchError.message}`,
      }),
      { status: 500 },
    );
  }
  ...
  const lastRunMap = new Map(
    lastRuns.map((run) => [run.source, run.last_object_id]),
  );
  ...
  for (const result of results) {
    ...
    const recordsToInsert = await Promise.all(
      scrapeResult.data.map(async (record) => {
    ...
    if (insertError) {
      console.log(
        JSON.stringify({
          level: "info",
          timestamp: new Date().toISOString(),
          message: `[${scrapeResult.source}] Failed to insert data:`,
          insertError,
        }),
      );
      continue;
    ...
    const { error: updateError } = await supabaseClient
      ...
      .upsert(
        {
          source: scrapeResult.source,
          last_object_id: scrapeResult.lastObjectId,
        },
        { onConflict: "source" },
      );
    ...
    if (updateError) {
      console.log(
        JSON.stringify({
          level: "info",
          timestamp: new Date().toISOString(),
          message: `[${scrapeResult.source}] Failed to update scraper run state:`,
          updateError,
        }),
      );
    }
  ...
  return new Response(
    JSON.stringify({ message: "Scraping process completed." }),
    {
