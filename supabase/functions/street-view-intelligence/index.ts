...
interface StreetViewRequest {
  ...
  analysisType: 'property-facade' | 'damage-documentation' | 'access-assessment' | 'neighborhood-survey' | 'custom'
  options?: {
    size?: string // '640x640'
  ...
  const buildingCondition = {
    ...
    landscaping: {
      ...
      riskFactors: (() => {
        ...
        if (isFloridaProperty && Math.random() > 0.6) factors.push('Palm trees (wind hazard)')
        return factors
      })()
    }
  }
...
function generateDamageDocumentation(location: { lat: number, lng: number }, hasRecentDamage: boolean = false): any {
  ...
  if (hasRecentDamage || Math.random() > 0.7) {
    const damageTypes = [
      ...
      { type: 'Siding damage', location: 'south wall', severity: 'minor' as const },
  ...
  const recommendedAngles = [
    { heading: 45, pitch: 10, purpose: 'Front facade and roof line documentation' },
    { heading: 135, pitch: 0, purpose: 'Side elevation damage assessment' },
    { heading: 315, pitch: -5, purpose: 'Foundation and ground level inspection' }
  ]
...
function analyzeNeighborhoodContext(location: { lat: number, lng: number }): any {
  ...
  const emergencyAccess = {
    ...
    streetWidth: Math.random() > 0.7 ? 'wide (>24 feet)' : 'standard (20-24 feet)',
    obstructions: (() => {
      const obstructions = []
...
function generateStreetViewInsights(analysisType: string, available: boolean, address?: string): string[] {
  const insights = []
  const isFloridaProperty = address?.toLowerCase().includes('fl') || address?.toLowerCase().includes('florida')

  if (!available) {
    insights.push('Street View imagery not available for this location')
    insights.push('Consider alternative documentation methods (drone, ground photos)')
    return insights
  }
  ...
  switch (analysisType) {
    case 'property-facade':
      ...
      if (isFloridaProperty) {
        insights.push('Hurricane protection features (shutters, impact windows) assessable')
        insights.push('Building elevation and flood risk indicators visible')
      }
      break

    case 'damage-documentation':
      insights.push('Street-level damage assessment possible with current imagery')
      insights.push('Multiple viewing angles recommended for complete documentation')
      insights.push('Coordinate with aerial imagery for comprehensive damage mapping')
      break

    case 'access-assessment':
      insights.push('Emergency vehicle access routes clearly visible')
      insights.push('Property accessibility for inspectors and contractors documented')
      insights.push('Parking and staging areas for restoration work identified')
      break
    ...
    case 'neighborhood-survey':
      ...
      insights.push('Comparative damage assessment with neighboring properties possible')
      break
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    intelligence.primaryView = {
      ...
      insights: generateStreetViewInsights(analysisType, isAvailable, location.address),
      metadata: options.includeMetadata && isAvailable ? {
      ...
      switch (analysisType) {
        case 'property-facade':
          if (options.multipleAngles) {
            intelligence.multiAngleViews = generateMultiAngleViews(location, options)
          }
          intelligence.propertyAnalysis = analyzePropertyFromStreetView(location, location.address)
          break

        case 'damage-documentation':
          intelligence.damageDocumentation = generateDamageDocumentation(location, true)
          if (options.multipleAngles) {
            intelligence.multiAngleViews = generateMultiAngleViews(location, options)
          }
          break
        ...
        case 'neighborhood-survey':
          ...
          intelligence.neighborhoodContext = analyzeNeighborhoodContext(location)
          break
