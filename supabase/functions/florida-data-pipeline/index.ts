...
async function refreshData(
  ...
  if (!county_codes || county_codes.length === 0) {
    ...
    refreshResults.county_stats = await refreshCountyStats(
      supabase,
      county_codes,
    );
  }
...
async function refreshCountyStats(
  supabase: any,
  county_codes?: number[],
  ...
  // Process each county
  for (const countyCode of counties.slice(0, 10)) {
    // Limit to prevent timeout
    try {
      const { data: countyStats } = await supabase.rpc(
        "get_county_statistics",
        { county_code_param: countyCode },
      );
      ...
      if (countyStats && countyStats.length > 0) {
        ...
        await supabase.from("county_market_stats").upsert(
          {
...
async function refreshMarketTrends(
  ...
  if (recentData && recentData.length > 0) {
    ...
    for (const [countyCode, properties] of Object.entries(countyGroups)) {
      ...
      const avgValue =
        props.reduce((sum: number, p: any) => sum + (p.tot_val || 0), 0) /
        props.length;

      // Insert trend data
      await supabase.from("property_value_trends").insert({
        ...
        for (const record of tableData) {
          ...
          if (tableName === "florida_parcels") {
            const hasRequiredFields =
              record.parcel_id && record.co_no && record.tot_val;
        ...
        await supabase.from("data_quality_metrics").insert({
          ...
          geometry_quality_score:
            tableName === "florida_parcels"
              ? (totalRecords - invalidGeometries) / totalRecords
              : 1.0,
        });
...
async function performQualityCheck(
  ...
  return {
    ...
    results: {
      ...
      status:
        overallScore > 0.8 ? "GOOD" : overallScore > 0.6 ? "FAIR" : "POOR",
    },
    metadata: {
...
async function checkDataAnomalies(supabase: any): Promise<Record<string, any>> {
  ...
  results.score =
    1 -
    (results.extreme_values.count + results.duplicates.duplicate_count) / 1000;
...
async function checkDataFreshness(supabase: any): Promise<Record<string, any>> {
  ...
  results.freshness_score =
    1 - staleCount / Math.max(results.sources.length, 1);
...
async function checkDataCompleteness(
  ...
  if (parcels) {
    ...
    const complete = parcels.filter(
      (p: any) => p.parcel_id && p.co_no && p.tot_val && p.situs_addr_1,
    ).length;
...
async function getPipelineStatus(supabase: any): Promise<PipelineResult> {
  ...
  // Get pipeline health
  status.health = {
    active_sources: (sources || []).filter((s: any) => s.status === "ACTIVE")
      .length,
    total_sources: (sources || []).length,
    avg_quality_score: qualityMetrics
      ? qualityMetrics.reduce(
          (sum: number, m: any) => sum + (m.completeness_rate || 0),
          0,
