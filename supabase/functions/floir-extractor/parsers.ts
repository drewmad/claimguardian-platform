  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a data normalization expert for Florida insurance regulation data. Convert raw scraped data into clean, structured JSON format. Focus on standardizing dates, numbers, currency amounts, and categorical values. Return only valid JSON."
        },
        {
          role: "user",
          content: prompt
    ...
    console.error(`Failed to normalize ${dataType} data:`, error)
...
function createNormalizationPrompt(dataType: FliorDataType, rawData: any): string {
  const basePrompt = `
