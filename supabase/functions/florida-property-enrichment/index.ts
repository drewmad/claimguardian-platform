...
serve(async (req) => {
  try {
    ...
    if (
      ...
      return new Response(
        JSON.stringify({
          error: "Must provide parcel_id, objectid, coordinates, or address",
        }),
        {
          status: 400,
    ...
    const enrichmentResult = await enrichProperty(
      propertyData,
      supabaseClient,
      openai,
    );
...
async function enrichProperty(
  ...
  const infrastructureData = await analyzeInfrastructure(
    propertyData,
    supabase,
  );
...
async function generateRiskAssessment(
  ...
  return {
    ...
    risk_factors: {
      ...
      property_value_tier:
        propertyData.tot_val > 500000
          ? "HIGH"
          : propertyData.tot_val > 200000
            ? "MEDIUM"
            : "LOW",
    },
    mitigation_recommendations: generateMitigationRecommendations(
      hurricaneRisk,
      floodRisk,
      ageRisk,
    ),
...
async function generateMarketAnalysis(
  ...
  const pricePerSqft =
    propertyData.tot_lvg_area > 0
      ? propertyData.tot_val / propertyData.tot_lvg_area
      : 0;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a Florida property insurance and investment expert. Provide detailed, actionable analysis.",
        },
        {
          role: "user",
          content: prompt,
...
async function storeEnrichmentResults(
  ...
  await supabase.from("property_risk_assessments").upsert(
    {
  ...
  await supabase.from("property_market_analysis").upsert(
    {
      parcel_id: result.parcel_id,
  ...
  await supabase.from("infrastructure_proximity").upsert(
    {
      parcel_id: result.parcel_id,
...
function calculateHurricaneRisk(propertyData: any): number {
  const coastalCounties = [
    13, 15, 16, 21, 26, 27, 28, 45, 53, 54, 60, 66, 68, 74,
  ];
...
async function calculateCrimeRisk(
  propertyData: any,
  supabase: any,
): Promise<number> {
