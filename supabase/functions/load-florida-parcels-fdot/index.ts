...
serve(async (req) => {
  ...
  try {
    ...
    if (!layerId) {
      throw new Error(
        `Invalid county: ${county}. Valid counties: ${Object.keys(COUNTY_LAYERS).join(", ")}`,
      );
    }

    console.log(
      `Loading Florida parcels from FDOT - County: ${countyUpper} (Layer ${layerId}), Offset: ${offset}, Limit: ${limit}`,
    );
    ...
    const url = new URL(
      `https://gis.fdot.gov/arcgis/rest/services/Parcels/FeatureServer/${layerId}/query`,
    );
    ...
    if (features.length > 0) {
      ...
      console.log(
        "First parcel ID:",
        features[0].attributes?.PARCEL_ID || features[0].attributes?.PARCELNO,
      );
    }
      ...
      try {
        const records = batch
          .map((feature: any) => {
            ...
            const ownerAddress = [attributes.OWN_ADDR1, attributes.OWN_ADDR2]
              .filter(Boolean)
              .join(" ")
              .trim();
            const propertyAddress = [attributes.PHY_ADDR1, attributes.PHY_ADDR2]
              .filter(Boolean)
              .join(" ")
              .trim();
            ...
            return {
              ...
              year_built:
                attributes.ACT_YR_BLT || attributes.EFF_YR_BLT || null,
              living_area: attributes.TOT_LVG_AR || null,
              land_area: attributes.LND_SQFOOT
                ? attributes.LND_SQFOOT / 43560
                : null, // Convert sq ft to acres
              geom: wkt,
              raw_data: {
                ...attributes,
                source_metadata: {
        ...
        console.log(
          `Batch ${i}-${i + batchSize}: Prepared ${records.length} records for insertion`,
        );
        if (records.length > 0) {
          ...
          for (const record of records) {
            try {
              ...
              if (error) {
                console.error(
                  `Failed to insert parcel ${record.parcel_id}:`,
                  error.message,
                );
                errors++;
              } else {
              ...
              console.error(
                `Exception inserting parcel ${record.parcel_id}:`,
                recordErr,
              );
              errors++;
            }
          ...
          console.log(
            `Batch complete: ${processed} succeeded, ${errors} failed`,
          );
        }
      } catch (err) {
