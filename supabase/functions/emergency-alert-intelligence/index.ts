  ...
  try {
    ...
    console.log(
      JSON.stringify({
        level: "warn",
        timestamp: new Date().toISOString(),
        message: "Emergency alerts API error, using mock data:",
        error,
      }),
    );
    return generateMockAlertData(lat, lng, options);
...
function generateMockAlertData(lat: number, lng: number, options: any): any {
  ...
  if (isFloridaLocation && isHurricaneSeason && Math.random() > 0.6) {
    alerts.push({
      ...
      description:
        "A Hurricane Watch has been issued for Southeast Florida including Miami-Dade, Broward, and Palm Beach counties. Hurricane conditions are possible within 48 hours.",
      instruction:
        "Preparations to protect life and property should be rushed to completion. Prepare for dangerous wind having possible significant impacts across Southeast Florida.",
  ...
  if (isFloridaLocation && Math.random() > 0.7) {
    alerts.push({
      ...
      description:
        "Heavy rainfall may cause flooding of small creeks and streams, urban areas, highways, streets and underpasses.",
      instruction:
  ...
  if (Math.random() > 0.8) {
    alerts.push({
      ...
      instruction:
        "Be ready to act quickly if a tornado warning is issued or you suspect a tornado is approaching.",
  ...
  if (!isHurricaneSeason || !isFloridaLocation || Math.random() > 0.4) {
    return {
      ...
      preparednessRecommendations: [
        "Monitor weather conditions during hurricane season",
      ],
    };
  }
...
function generateRiskSummary(
  alerts: any[],
  intelligence: EmergencyAlertIntelligence,
): any {
  ...
  let preparednessLevel: "minimal" | "standard" | "heightened" | "maximum" =
    "minimal";
  ...
  try {
    ...
    switch (monitoringType) {
      ...
      case "hurricane-tracking":
        intelligence.activeAlerts = alertData.alerts.filter(
          (alert: any) =>
            alert.type.toLowerCase().includes("hurricane") ||
            alert.type.toLowerCase().includes("tropical"),
        );
        intelligence.hurricaneTracking = generateHurricaneTracking(
          location.lat,
          location.lng,
        );
        break;
      ...
      case "flood-warnings":
        ...
        intelligence.floodMonitoring = generateFloodMonitoring(
          location.lat,
          location.lng,
        );
        break;
      ...
      case "comprehensive-monitoring":
        ...
        intelligence.emergencyServices = generateEmergencyServices(
          location.lat,
          location.lng,
        );
        intelligence.riskSummary = generateRiskSummary(
          alertData.alerts,
          intelligence,
        );
        break;
    ...
    console.log(
      JSON.stringify({
        level: "error",
        timestamp: new Date().toISOString(),
        message: "[Emergency Alert Intelligence] Error:",
        error,
      }),
    );
