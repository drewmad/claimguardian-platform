...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Loading ${countyInfo.name} County parcels from Storage - Offset: ${offset}, Limit: ${limit}`)
    ...
    if (useFullFile) {
      ...
      console.log(`Filtered to ${features.length} ${countyInfo.name} County features`)
    }

    // Apply pagination
    const paginatedFeatures = features.slice(offset, offset + limit)
    console.log(`Processing ${paginatedFeatures.length} features (offset: ${offset}, limit: ${limit})`)
    ...
    for (const feature of paginatedFeatures) {
      try {
        ...
        if (geometry && geometry.type === 'Polygon' && geometry.coordinates) {
          ...
          if (coords.length === 1) {
            // Simple polygon
            const ringStr = coords[0].map((c: number[]) => `${c[0]} ${c[1]}`).join(',')
            wkt = `SRID=4326;POLYGON((${ringStr}))`
          } else {
            // Polygon with holes
            const ringsStr = coords.map((ring: number[][]) => 
          ...
          // MultiPolygon
          const polysStr = geometry.coordinates.map((poly: number[][][]) => 
            `(${poly.map(ring => 
              `(${ring.map(c => `${c[0]} ${c[1]}`).join(',')})`
            ).join(',')})`
          ).join(',')
        ...
        const record = {
          ...
          TV_NSD: parseFloat(props.TV_NSD || props.TAXABLE_VALUE || '0') || null,
          ...
          // Building info
          EFF_YR_BLT: parseInt(props.EFF_YR_BLT || props.YEAR_BUILT || '0') || null,
          ACT_YR_BLT: parseInt(props.ACT_YR_BLT || props.ACTUAL_YEAR_BUILT || '0') || null,
          TOT_LVG_AR: parseFloat(props.TOT_LVG_AREA || props.TOT_LVG_AR || props.HEATED_AREA || '0') || null,
          LND_SQFOOT: parseFloat(props.LAND_SQ_FOOTAGE || props.LND_SQFOOT || '0') || null,
          NO_BULDNG: parseInt(props.NO_BULDNG || props.BLDG_NUM || '0') || null,
          
          // Sale info
          SALE_PRC1: parseFloat(props.SALE_PRC1 || props.SALE_PRICE || '0') || null,
