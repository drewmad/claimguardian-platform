...
interface SolarRequest {
  ...
  analysisType: 'solar-potential' | 'roof-analysis' | 'financial-analysis' | 'environmental-impact' | 'complete-assessment'
  options?: {
...
function analyzeSolarPotential(solarData: any): any {
  ...
  if (!potential) {
    return {
      ...
      wholeRoofStats: { areaMeters2: 0, sunshineQuantiles: [], groundAreaCoveredMeters2: 0 },
      roofSegmentStats: [],
      panelCapacityWatts: 250,
      panelHeightMeters: 1.65,
      panelWidthMeters: 0.992,
...
function calculateFinancialAnalysis(solarData: any, location: { lat: number, lng: number }, options: any = {}): any {
  const potential = solarData.solarPotential
  if (!potential) {
    return {
  ...
  return {
    ...
    financing: {
      federalIncentive: Math.floor(installationCost * 0.30),
      stateIncentive: isFloridaLocation ? Math.floor(installationCost * 0.06) : 0,
      utilityIncentive: 0,
...
function calculateEnvironmentalImpact(solarData: any, financialAnalysis: any): any {
  const potential = solarData.solarPotential
...
function analyzeRoofSuitability(solarData: any, location: { lat: number, lng: number }): any {
  ...
  if (!roofSegment) {
    return {
      ...
      installationConsiderations: ['Roof access and structural evaluation needed'],
  ...
  if (sunshineHours > 3000 && pitch > 15 && pitch < 40 && azimuth > 135 && azimuth < 225 && area > 100) {
  ...
  // Pitch analysis
  if (pitch < 10) {
    challenges.push('Low roof pitch may reduce efficiency and complicate drainage')
    recommendations.push('Consider ballasted racking system for flat roofs')
  } else if (pitch > 40) {
    challenges.push('Steep roof pitch increases installation complexity and cost')
    installationConsiderations.push('Additional safety equipment required for steep roof installation')
  } else {
  ...
  if (azimuth < 135 || azimuth > 225) {
    ...
    recommendations.push('Consider east-west panel layout or ground-mount system')
  } else {
    recommendations.push('Good south-facing roof orientation for solar production')
  }
  ...
  if (isFloridaLocation) {
    ...
    installationConsiderations.push('Consider storm damage insurance for system')
    maintenanceFactors.push('Regular cleaning needed due to humidity and pollen')
    maintenanceFactors.push('Inspect for hurricane damage after major storms')
    recommendations.push('Excellent solar resource in Florida - high annual production expected')
  } else {
  ...
  installationConsiderations.push('Verify roof structural capacity before installation')
  installationConsiderations.push('Check local permitting requirements and HOA restrictions')
  maintenanceFactors.push('Monitor system performance with production monitoring')
  maintenanceFactors.push('Professional maintenance check every 5 years recommended')
...
function calculatePropertyValueImpact(financialAnalysis: any, location: { lat: number, lng: number }): any {
  ...
  const percentageIncrease = Math.round((estimatedValueIncrease / averageHomeValue) * 100 * 10) / 10
  ...
  if (isFloridaLocation) {
    marketAppealFactors.push('Hurricane preparedness with backup power potential')
  ...
  if (isFloridaLocation) {
    resaleConsiderations.push('Florida solar tax exemptions transfer to new owners')
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    switch (analysisType) {
      ...
      case 'roof-analysis':
        intelligence.solarPotential = analyzeSolarPotential(solarData)
        intelligence.roofAnalysisIntelligence = analyzeRoofSuitability(solarData, location)
        break

      case 'financial-analysis':
        intelligence.solarPotential = analyzeSolarPotential(solarData)
        intelligence.financialAnalysis = calculateFinancialAnalysis(solarData, location, options)
        intelligence.propertyValueImpact = calculatePropertyValueImpact(intelligence.financialAnalysis, location)
        break

      case 'environmental-impact':
        intelligence.solarPotential = analyzeSolarPotential(solarData)
        const tempFinancial = calculateFinancialAnalysis(solarData, location, options)
        intelligence.environmentalImpact = calculateEnvironmentalImpact(solarData, tempFinancial)
        break

      case 'complete-assessment':
        intelligence.solarPotential = analyzeSolarPotential(solarData)
        intelligence.financialAnalysis = calculateFinancialAnalysis(solarData, location, options)
        intelligence.environmentalImpact = calculateEnvironmentalImpact(solarData, intelligence.financialAnalysis)
        intelligence.roofAnalysisIntelligence = analyzeRoofSuitability(solarData, location)
        intelligence.propertyValueImpact = calculatePropertyValueImpact(intelligence.financialAnalysis, location)
        break
