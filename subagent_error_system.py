...
class SubagentErrorSystem:
    ...
    def extract_agent_learnings(self, agent_name: str = None) -> List[Dict]:
        ...
        for match in matches:
            ...
            learning_pattern = r"- \*\*Agent-Based Learnings\*\*:(.*?)(?=\n- \*\*|\n---|\Z)"
            ...
            if learning_match:
                ...
                optimization_match = re.search(r"- Optimization: (.+)", learning_content)
                ...
                if subagent_match:
                    ...
                    learning_data = {
                        ...
                        "fix_recipe": fix_recipe_match.group(1).strip() if fix_recipe_match else "",
                        "optimization": optimization_match.group(1).strip() if optimization_match else "",
...
"""
    ...
    def handle_error(self, error_message: str, file_path: str, agent_name: str = None, context: str = ""):
        ...
        try:
            # Log error
            subprocess.run([
                "claude", "-p", f"/log_error {error_details}"
            ], check=True)
            ...
            subprocess.run([
                "claude", "-p", f"/analyze_error {analysis_context}"
            ], check=True)
...
def main():
    ...
    if len(os.sys.argv) > 2:
        ...
        print("Example: python subagent_error_system.py ui-developer 'Fix button component styling'")
