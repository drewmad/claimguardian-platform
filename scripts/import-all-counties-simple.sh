...
# Simple Florida Counties Import System
# Uses basic bash without associative arrays
...
# Create county list file (all counties except Charlotte which is already done)
...
echo "Starting parallel import of 66 counties..."
...
echo
echo "Top 10 counties by parcel count:"
PGPASSWORD="$DB_PASSWORD" psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "SELECT co_no, COUNT(*) as parcels FROM florida_parcels GROUP BY co_no ORDER BY COUNT(*) DESC LIMIT 10;"
echo
