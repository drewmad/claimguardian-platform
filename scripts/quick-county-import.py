...
def import_county(county_code, county_name):
    ...
    try:
        ...
        subprocess.run(f"awk -F',' 'NR==1 || !seen[$2]++' {clean_csv} > {unique_csv}", shell=True)
        ...
        copy_cmd = [
            ...
            '-c', f"\\COPY florida_parcels(co_no,parcel_id,file_t,asmnt_yr,bas_strt,atv_strt,grp_no,dor_uc,pa_uc,spass_cd,jv,jv_chng,jv_chng_cd,av_sd,av_nsd,tv_sd,tv_nsd,jv_hmstd,av_hmstd,jv_non_hms,lnd_val,imp_val,own_name,own_addr1,own_addr2,own_city,own_state,own_zipcd,phy_addr1,phy_addr2,phy_city,phy_zipcd,lnd_sqfoot,tot_lvg_ar,no_buldng,sale_prc1,sale_yr1,sale_mo1,or_book1,or_page1,clerk_no1,qual_cd1,sale_prc2,sale_yr2,sale_mo2,or_book2,or_page2,clerk_no2,qual_cd2,s_legal,twn,rng,sec) FROM '{unique_csv}' WITH (FORMAT csv, HEADER true);"
        ]
...
def print_progress():
    ...
    # Count statuses
    statuses = {'complete': 0, 'processing': 0, 'importing': 0, 'extracting': 0, 'failed': 0}
    ...
    print(f"Processing: {statuses['extracting'] + statuses['processing'] + statuses['importing']}")
