...
async function bulkInsertParcels(supabaseUrl, anonKey, parcels) {
  const response = await fetch(
    `${supabaseUrl}/functions/v1/bulk-insert-parcels`,
    {
      method: "POST",
      headers: {
  ...
  while (totalSuccess + totalErrors < maxRecords) {
    try {
      ...
      if (parcels.length > 0) {
        ...
        console.log(
          `✅ Batch result: ${result.successful} successful, ${result.errors} errors`,
        );
...
async function main() {
  ...
  try {
    ...
    if (result.errors > 0 && result.success === 0) {
      ...
      console.log(
        "   1. Log in to ClaimGuardian at https://claimguardianai.com",
      );
