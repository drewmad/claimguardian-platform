...
async function generateReport() {
  ...
  for (const dir of Object.keys(stats.filesByDirectory)) {
    ...
    dirStats.coverage = Math.round((dirStats.withMetadata / dirStats.total) * 100);
  }

  const overallCoverage = Math.round((stats.filesWithMetadata / stats.totalFiles) * 100);
  ...
  if (overallCoverage < 50) {
    ...
    console.log('🎉 Excellent coverage! Run `pnpm metadata:validate` to check quality');
  }

  const unknownOwners = stats.filesByOwner['unknown'] || 0;
  if (unknownOwners > 0) {
    console.log(`📝 ${unknownOwners} files have unknown owners - consider assigning teams`);
  }

  const experimentalFiles = stats.filesByStatus['experimental'] || 0;
  if (experimentalFiles > stats.filesWithMetadata * 0.2) {
    console.log(`🧪 ${experimentalFiles} experimental files - consider stabilizing mature code`);
  }
