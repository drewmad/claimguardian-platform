...
class ParcelProcessor:
    ...
    def prepare_for_ai(self, gdf: gpd.GeoDataFrame) -> pd.DataFrame:
        ...
        numeric_columns = [
            'JV', 'TV_NSD', 'LND_VAL', 'IMP_VAL', 'SALE_PRC1', 
            'TOT_LVG_AR', 'LND_SQFOOT', 'EFF_YR_BLT', 'ACT_YR_BLT'
        ]
        ...
        if all(col in df.columns for col in ['IMP_VAL', 'LND_VAL']):
            df['improvement_ratio'] = df['IMP_VAL'] / (df['IMP_VAL'] + df['LND_VAL'] + 1)
        ...
        if 'geometry' in df.columns:
            ...
            # Shape complexity
            df['shape_complexity'] = df['parcel_perimeter_m'] / (2 * np.sqrt(np.pi * df['parcel_area_sqm']))
        ...
        if 'SALE_YR1' in df.columns:
            df['years_since_sale'] = current_year - pd.to_numeric(df['SALE_YR1'], errors='coerce')
    ...
    def create_ai_embeddings(self, df: pd.DataFrame) -> np.ndarray:
        ...
        embedding_features = [
            'building_age', 'improvement_ratio', 'assessment_ratio',
            'price_per_sqft', 'parcel_area_sqm', 'shape_complexity',
            'years_since_sale', 'is_residential', 'is_commercial',
            'is_corporate_owner', 'centroid_lat', 'centroid_lon'
        ]
    ...
    def process_charlotte_county(self):
        ...
        self.export_for_supabase(
            df_ai, 
            self.data_dir / "charlotte_parcels_ai_ready.csv"
        )
        ...
        feature_info = {
            ...
            "ai_features": [
                "building_age", "improvement_ratio", "assessment_ratio",
                "price_per_sqft", "shape_complexity", "years_since_sale"
            ],
