...
async function main() {
  ...
  if (!command || command === 'help') {
    ...
    console.log('  load-parcels [county]   - Load Florida parcels (optionally by county)')
    console.log('\nExamples:')
    console.log('  node load-geospatial-via-edge-functions.js list')
    console.log('  node load-geospatial-via-edge-functions.js load fire_stations')
  ...
  switch (command) {
    ...
    case 'load':
      ...
      if (!source) {
        ...
        console.log('Run "node load-geospatial-via-edge-functions.js list" to see available sources')
        return
      }
      await loadDataSource(source)
      break
