...
class RateLimiter {
  ...
  async checkLimit() {
    ...
    // Check limits
    if (
      minuteRequests >= BATCH_CONFIG.settings.rate_limit.requests_per_minute
    ) {
      const waitTime =
        60000 -
        (now -
          this.requests[
            this.requests.length -
              BATCH_CONFIG.settings.rate_limit.requests_per_minute
          ]);
      console.log(
        `⏳ Rate limit reached (minute). Waiting ${Math.ceil(waitTime / 1000)} seconds...`,
      );
    ...
    if (hourRequests >= BATCH_CONFIG.settings.rate_limit.requests_per_hour) {
      ...
      console.log(
        `⏳ Rate limit reached (hour). Waiting ${Math.ceil(waitTime / 60000)} minutes...`,
      );
      await sleep(waitTime);
    }
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (response.status === 429) {
        ...
        console.warn(
          `🚦 Rate limited (429). Waiting ${waitTime / 1000} seconds...`,
        );
        await sleep(waitTime);
...
async function loadCountyParcels(county, tracker) {
  ...
  while (totalProcessed < maxRecords) {
    try {
      console.log(
        `   Batch ${offset / BATCH_CONFIG.settings.parcels_per_batch + 1}: Loading records ${offset + 1}-${offset + BATCH_CONFIG.settings.parcels_per_batch}...`,
      );

      const result = await callEdgeFunctionWithRetry(
        "load-florida-parcels-fdot",
        {
          county: county.name,
          offset,
      ...
      console.error(
        `❌ Error loading ${county.name} at offset ${offset}:`,
        error.message,
      );
  ...
  console.log(
    `✅ ${county.name} County complete: ${totalProcessed} parcels loaded`,
  );
...
async function main() {
  ...
  console.log("\n📋 Rate Limits:");
  console.log(
    `- ${BATCH_CONFIG.settings.rate_limit.requests_per_minute} requests/minute`,
  );
  console.log(
    `- ${BATCH_CONFIG.settings.rate_limit.requests_per_hour} requests/hour`,
  );
  ...
  console.log("\n📊 Progress Status:");
  console.log(
    `- Datasets completed: ${stats.completedDatasets}/${BATCH_CONFIG.small_datasets.length}`,
  );
  console.log(
    `- Counties completed: ${stats.completedCounties}/${BATCH_CONFIG.florida_counties.length}`,
  );
  ...
  try {
    ...
    console.log(
      `(Loading up to ${BATCH_CONFIG.settings.max_parcels_per_county.toLocaleString()} parcels per county for testing)`,
    );
    ...
    for (const county of BATCH_CONFIG.florida_counties) {
      ...
      showProgressBar(
        countyIndex - 1,
        BATCH_CONFIG.florida_counties.length,
        "Overall Progress",
      );
    ...
    console.log(
      `- Datasets loaded: ${finalStats.completedDatasets}/${BATCH_CONFIG.small_datasets.length}`,
    );
    console.log(
      `- Counties loaded: ${finalStats.completedCounties}/${BATCH_CONFIG.florida_counties.length}`,
    );
