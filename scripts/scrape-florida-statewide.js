...
const STATEWIDE_SERVICE = {
  ...
  counties: ['CHARLOTTE', 'LEE', 'SARASOTA', 'COLLIER', 'MIAMI-DADE', 'BROWARD', 'PALM BEACH'],
  fieldMapping: {
...
async function scrapeCounty(countyName) {
  const source = `fl_${countyName.toLowerCase().replace(/[^a-z]/g, '_')}_county`;
  console.log(`\n[${source}] Starting scrape for ${countyName} County...`);
  ...
  while (hasMore) {
    const queryParams = new URLSearchParams({
      where: `COUNTY='${countyName}' AND OBJECTID>${lastObjectId}`,
...
async function main() {
  ...
  if (countyArg) {
    ...
    if (STATEWIDE_SERVICE.counties.includes(county)) {
      await scrapeCounty(county);
    } else {
      console.error(`Invalid county: ${countyArg}`);
      console.error(`Valid counties: ${STATEWIDE_SERVICE.counties.join(', ')}`);
    }
  } else {
    // Scrape all counties
    for (const county of STATEWIDE_SERVICE.counties) {
      await scrapeCounty(county);
