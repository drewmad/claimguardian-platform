...
class FDOTParcelFetcher:
    ...
    def _load_progress(self):
        ...
        if resume_path.exists():
            try:
                ...
                logger.info(
                    f"Resumed from checkpoint: {self.progress.fetched_parcels} parcels fetched"
                )
    ...
    async def _save_county_data(self, county: str, parcels: List[Dict]):
        ...
        filename = (
            f"{county.lower()}_parcels_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    async def fetch_all_parcels(self) -> Dict[str, int]:
        ...
        remaining_counties = [
            c for c in counties if c not in self.progress.counties_completed
        ]
