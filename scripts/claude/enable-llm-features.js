...
async function enableLLMFeatures() {
  console.log(
    `${colors.bright}${colors.cyan}🚀 Claude Learning System - Enable LLM Features${colors.reset}`,
  );
  ...
  try {
    ...
    console.log(
      `${colors.yellow}⚠${colors.reset} Verifying Opus model access...`,
    );
    ...
    const configUpdates = [
      ...
      },
      {
        ...
        key: "ENABLE_NATURAL_LANGUAGE",
        value: "true",
      },
      {
        name: "Bottleneck Analysis",
        key: "ENABLE_BOTTLENECK_ANALYSIS",
        value: "true",
      },
    ...
    configUpdates.forEach((config) => {
      console.log(
        `  ${colors.green}✓${colors.reset} ${config.name}: ${config.value}`,
      );
    });
    ...
    console.log(
      `  Found ${colors.cyan}${synthesisResults.patternsFound}${colors.reset} patterns`,
    );
    console.log(
      `  High confidence: ${colors.green}${synthesisResults.highConfidence}${colors.reset}`,
    );
    ...
    console.error(
      `${colors.red}❌ Failed to enable LLM features: ${error.message}${colors.reset}`,
    );
    process.exit(1);
  }
}
...
async function estimateMonthlyCosts() {
  ...
  console.log(
    `  ${colors.bright}Total: $${totalCost.toFixed(2)}/month${colors.reset}`,
  );
}
