...
const mockPatterns = [
  ...
  },
  {
    ...
    solution:
      "Wrap errors with context including user action, state, and stack trace",
    confidence: 0.91,
    impact: {
      timeReduction: 40,
      errorReduction: 30,
...
export const completeLearningSystem = {
  ...
  applyLearning: async (taskContext) => {
    ...
    const relevantLearning = mockLearnings.find(
      (l) => l.category === "performance" || l.tags.includes("optimization"),
    );
    return {
      applied: true,
