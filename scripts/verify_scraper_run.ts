...
async function verify() {
  ...
  if (rawDataError) {
    console.error(
      "Error fetching from external_raw_fl.property_data:",
  ...
  const counts = allRawData.reduce(
    (acc, record) => {
  ...
  const formattedCounts = Object.entries(counts).map(
    ([source, record_count]) => ({ source, record_count }),
  );

  console.log(
    JSON.stringify(
      { scraper_runs: runs, raw_data_counts: formattedCounts },
      null,
      2,
    ),
  );
