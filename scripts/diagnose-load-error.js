...
const SUPABASE_ANON_KEY = keyMatch
  ? keyMatch[1].split("\\n")[0].replace(/["\s]/g, "")
  : null;
...
console.log(
  "Last 50 chars:",
  SUPABASE_ANON_KEY.substring(SUPABASE_ANON_KEY.length - 50),
);
...
async function testParcelLoad() {
  try {
    ...
    const fdotUrl =
      "https://gis.fdot.gov/arcgis/rest/services/Parcels/FeatureServer/44/query";
    ...
    console.log(
      ...
      feature.attributes.PARCEL_ID || feature.attributes.PARCELNO,
    );

    // Now try to load it via our Edge Function
    console.log("\n2. Testing Edge Function with single parcel...");
    const edgeResponse = await fetch(
      `${SUPABASE_URL}/functions/v1/load-florida-parcels-fdot`,
      {
        method: "POST",
        headers: {
