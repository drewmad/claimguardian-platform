  ...
  async runAggregation() {
    try {
      ...
      for (const window of windows) {
        ...
        console.log(
          `  ✓ Aggregated ${window.interval} metrics (lookback: ${window.lookback})`,
        );
      }
  ...
  async runAnomalyDetection() {
    try {
      ...
      for (const config of configurations) {
        ...
        console.log(
          `  ✓ ${config.name} detection: ${data || 0} anomalies (${config.lookback}h, σ=${config.sensitivity})`,
        );
      }
      ...
      for (const [key, metrics] of Object.entries(metricGroups)) {
        ...
        if (Math.abs(trend.slope) > 0.1) {
          insights.push({
            ...
            analysis_period_start: new Date(
...
program
  ...
  .argument(
    "<operation>",
    "Operation to run: aggregate, anomalies, insights, cleanup",
  )
