...
function calculateLoadTime() {
  ...
  // Small datasets
  const smallDatasetTime =
    (CONFIG.small_datasets * CONFIG.delay_between_sources) / 1000;
  console.log("📁 Small Datasets (fire stations, wildfires, flood zones):");
  console.log(`   - Datasets: ${CONFIG.small_datasets}`);
  console.log(
    `   - Time per dataset: ${CONFIG.delay_between_sources / 1000} seconds`,
  );
  ...
  const testBatchesPerCounty = Math.ceil(
    testParcelsPerCounty / CONFIG.parcels_per_batch,
  );
  const testTimePerCounty =
    (testBatchesPerCounty * CONFIG.delay_between_calls) / 1000;
  const testTotalTime =
    CONFIG.counties_to_load * testTimePerCounty +
    (CONFIG.counties_to_load * CONFIG.delay_between_sources) / 1000;

  console.log(`   - Counties: ${CONFIG.counties_to_load}`);
  console.log(
    `   - Parcels per county: ${testParcelsPerCounty.toLocaleString()}`,
  );
  console.log(`   - Batches per county: ${testBatchesPerCounty}`);
  console.log(`   - Time per county: ${formatTime(testTimePerCounty)}`);
  console.log(
    `   - Total parcels: ${(CONFIG.counties_to_load * testParcelsPerCounty).toLocaleString()}`,
  );
  console.log(
    `   - Total time: ${formatTime(testTotalTime + smallDatasetTime)}`,
  );
  ...
  Object.entries(CONFIG.parcel_estimates).forEach(([county, parcels]) => {
    ...
    console.log(
      `   ${county}: ${parcels.toLocaleString()} parcels, ${batches} batches, ${formatTime(timeSeconds)}`,
    );
  });
  ...
  const allFloridaBatches = Math.ceil(
    CONFIG.total_florida_parcels / CONFIG.parcels_per_batch,
  );
  const allFloridaTime =
    (allFloridaBatches * CONFIG.delay_between_calls) / 1000;

  console.log(
    `   - Total parcels: ${CONFIG.total_florida_parcels.toLocaleString()}`,
  );
  console.log(`   - Total batches: ${allFloridaBatches.toLocaleString()}`);
  console.log(
    `   - Time at current rate: ${formatTime(allFloridaTime + smallDatasetTime)}`,
  );
  ...
  console.log(
    `   - Current delay: ${CONFIG.delay_between_calls}ms between requests`,
  );
  console.log(`   - Requests per hour: ${Math.floor(requestsPerHour)}`);
  console.log(
    `   - Parcels per hour: ${Math.floor(parcelsPerHour).toLocaleString()}`,
  );
  console.log(
    `   - Days to load all Florida: ${(allFloridaTime / 86400).toFixed(1)} days`,
  );
  ...
  console.log("\n📋 Recommendations:");
  console.log(
    "1. Start with test mode (5,000 parcels/county) to validate the process",
  );
  console.log("2. Monitor for rate limit errors (429 responses)");
  console.log(
    "3. Consider increasing batch size if API allows (1000 instead of 500)",
  );
  console.log(
    "4. For production, implement parallel workers with different counties",
  );
  console.log(
    "5. Consider loading only populated areas first (top 20 counties = 80% of parcels)",
  );
  console.log("6. Run during off-peak hours if possible");
  console.log(
    "7. Contact Florida GIS services for higher rate limits if needed",
  );
}
