...
/**
 * Florida Statewide Parcels Scraper - Public Schema Version
 * Stores data in public.florida_properties table
 */
...
async function scrapeCounty(countyName, countyCode) {
  ...
  while (hasMore && totalProcessed < 10000) { // Limit to 10k records for testing
...
// Main function
async function main() {
  console.log('Florida Statewide Parcels Scraper (Public Schema)');
  console.log('=================================================\n');
  
  // First, create the table if it doesn't exist
  console.log('Ensuring florida_properties table exists...');
  ...
  if (countyArg) {
    ...
    console.log('Running test scrape for Charlotte County (limited to 10k records)...');
  ...
  if (summary) {
    console.log(`\nTotal records in database: ${summary.count}`);
  }
}
