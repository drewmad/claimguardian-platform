...
async function main() {
  ...
  const checkResponse = await fetch(`${supabaseUrl}/rest/v1/scraper_logs?limit=1`, {
    headers: {
      'apikey': supabaseServiceKey,
      'Authorization': `Bearer ${supabaseServiceKey}`,
    }
  ...
  if (checkResponse.status === 404) {
    ...
    console.log('   - supabase/migrations/20250717009_fix_scraper_logs_table.sql');
    ...
    if (testResponse.ok) {
      ...
      console.error('❌ Failed to insert test entry:', await testResponse.text());
    }
  }
  ...
  console.log('3. Check Supabase dashboard for any remaining security warnings');
}
