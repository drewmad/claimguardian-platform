...
async function runMigration() {
  ...
  const migrationPath = path.join(
    ...
    "supabase",
    "migrations",
    "20250724_property_schema_migration.sql",
  );
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log("\n2. Run migration:");
      console.log(
        `   psql -h db.tmlrvecuwgppbaynesji.supabase.co -U postgres -d postgres -f ${migrationPath}`,
      );
    } else {
      console.error(
        "❌ Connection failed:",
  ...
  console.log(
    "2. Apply migration locally: psql postgresql://postgres:postgres@localhost:54322/postgres -f " +
      migrationPath,
  );
