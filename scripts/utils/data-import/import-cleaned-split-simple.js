...
function showProgress(current, total, startTime) {
  ...
  console.log(
    `⏱️  Elapsed: ${Math.floor(elapsed / 60)}m ${Math.floor(elapsed % 60)}s`,
  );
...
async function main() {
  ...
  console.log(
    `⏱️  Duration: ${Math.floor(duration / 60)}m ${Math.floor(duration % 60)}s`,
  );
  console.log(
    `⚡ Average: ${(duration / csvFiles.length).toFixed(1)}s per file`,
  );
  console.log("\n✨ Done!");
}
