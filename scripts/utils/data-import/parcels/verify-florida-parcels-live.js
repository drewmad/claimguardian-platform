...
async function verifyFloridaParcels() {
  ...
  if (allSuccess) {
    ...
    console.log(
      "  • County Link: county_fips (12001-12133) → florida_counties",
    );
    console.log(
      "  • Major Fields: jv (Just Value), lnd_val (Land Value), own_name, phy_addr1",
    );

    console.log("\n🔍 Available Functions:");
    console.log(
      "  • search_parcels_by_owner(owner_name, county_fips?, limit?)",
    );
