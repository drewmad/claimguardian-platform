...
async function importCharlotteData(targetTable) {
  ...
  for (let i = 1; i < Math.min(101, lines.length); i += batchSize) {
    // Limit to first 100 for testing
    const batch = [];

    for (
      let j = i;
      j < Math.min(i + batchSize, Math.min(101, lines.length));
      j++
    ) {
      ...
      if (targetTable === "properties") {
        ...
        transformedRecord = {
          ...
          area_acres: record.LND_SQFOOT
            ? parseFloat(record.LND_SQFOOT) / 43560
            : null,
          address: [record.PHY_ADDR1, record.PHY_CITY]
            .filter(Boolean)
            .join(", "),
    ...
    if (batch.length > 0) {
      try {
        ...
        if (error) {
          ...
          console.log(
            `   ✅ Batch ${Math.floor(i / batchSize) + 1}: ${batch.length} records imported`,
          );
        }
      } catch (err) {
...
async function verifyImport(targetTable) {
  ...
  try {
    ...
    console.log(
      `   Charlotte County records found: ${properties.length} (showing first 5)`,
    );
    ...
    if (properties.length > 0) {
      ...
      console.log(
        `     Coordinates: ${sample.coordinates?.lat || sample.LATITUDE}, ${sample.coordinates?.lng || sample.LONGITUDE}`,
      );
      console.log(
        `     Property Value: $${(sample.property_value || sample.TV_NSD)?.toLocaleString() || "N/A"}`,
      );
    }
...
async function main() {
  ...
  try {
    ...
    console.log(
      "   Table Editor: https://supabase.com/dashboard/project/tmlrvecuwgppbaynesji/editor",
    );
  } catch (error) {
