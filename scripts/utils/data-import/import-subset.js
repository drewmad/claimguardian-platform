...
// Get files in range
const allFiles = fs.readdirSync('./CleanedSplit')
  .filter(f => f.endsWith('.csv'))
  .sort();

const files = allFiles.slice(start, end);

console.log(`📊 Processing ${files.length} files from position ${start}-${end}\n`);
...
async function importFile(filePath) {
  ...
  console.log(`[${process.argv[2]}-${process.argv[3]}] Processing ${fileName}...`);
  ...
  try {
    ...
    for (let i = 0; i < records.length; i += chunkSize) {
      ...
      process.stdout.write(`\r   ${i + chunk.length}/${records.length} records`);
    }
