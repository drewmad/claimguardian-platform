...
async function extractCharlotteCounty() {
  ...
  console.log(
    `   Charlotte County records: ${charlotteRecords.toLocaleString()}`,
  );
  ...
  return new Promise((resolve) => {
    ...
    fs.createReadStream(sampleFile)
      ...
      .on("data", (data) => {
        count++;
        const county =
          data.CO_NO || data.co_no || data.COUNTY_FIPS || data.county_fips;
        if (county) {
  ...
  extractCharlotteCounty()
    .then((count) => {
      if (count > 0) {
        ...
        console.log(
          "   1. Apply database schema: scripts/charlotte-county-schema.sql",
        );
