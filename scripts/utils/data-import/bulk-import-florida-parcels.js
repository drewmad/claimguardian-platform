    ...
    const stream = fs
      ...
      .on("data", async (row) => {
        ...
        if (records.length >= BATCH_SIZE) {
          ...
          const batchPromise = importBatch(batch, fileName).then(() => {
            process.stdout.write(
              `\r  Progress: ${recordCount} records processed...`,
            );
            stream.resume();
          });
...
async function calculateCosts() {
  ...
  console.log(
    `💾 Database size (with indexes): ~${estimatedDBSize.toFixed(2)} GB`,
  );
  ...
  const apiCostPer1M = 0.4; // $0.40 per million requests
  const monthlyAPICost = (estimatedMonthlyReads / 1000000) * apiCostPer1M;

  console.log(
    `🔍 Estimated monthly API calls: ${(estimatedMonthlyReads / 1000000).toFixed(2)}M`,
  );
...
async function main() {
  ...
  console.log(
    `✅ Records imported: ${stats.successfulRecords}/${stats.totalRecords}`,
  );
  console.log(`❌ Failed records: ${stats.failedRecords}`);
  console.log(`⏱️  Duration: ${(duration / 60).toFixed(2)} minutes`);
  console.log(
    `⚡ Speed: ${(stats.totalRecords / duration).toFixed(0)} records/second`,
  );
