...
async function uploadCSVFile(filePath, fileIndex, totalFiles) {
  ...
  try {
    ...
    if (!storageResponse.ok) {
      ...
      console.log(
        `${colors.yellow}⚠️  Storage upload failed, using batch API...${colors.reset}`,
      );
    ...
    const importResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/rpc/import_csv_from_storage`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`,
    ...
    const transferResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/rpc/transfer_florida_parcels_staging`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`,
    ...
    console.log(
      `${colors.green}✅ Success! Processed in ${duration.toFixed(1)}s${colors.reset}`,
    );
  ...
  return new Promise((resolve) => {
    ...
    fs.createReadStream(filePath)
      ...
      .on("end", async () => {
        ...
        console.log(
          `${colors.green}✅ Imported ${recordCount.toLocaleString()} records${colors.reset}`,
        );
        resolve({ success: true });
      })
      .on("error", (error) => {
        console.error(
          `${colors.red}❌ Parse error: ${error.message}${colors.reset}`,
        );
...
async function uploadViaSupabaseCLI(filePath) {
  ...
  try {
    ...
    console.log(
      `${colors.cyan}📤 Uploading via Supabase CLI...${colors.reset}`,
    );
...
async function main() {
  console.clear();
  console.log(
    `${colors.bright}${colors.cyan}🚀 Florida Parcels Direct File Import${colors.reset}`,
  );
  ...
  console.log(
    `📊 Found ${colors.bright}${totalFiles}${colors.reset} CSV files\n`,
  );
  ...
  try {
    ...
    console.log(
      `${colors.yellow}⚠️  Installing required dependencies...${colors.reset}`,
    );
  ...
  for (let i = 0; i < csvFiles.length; i++) {
    ...
    if (remaining > 0) {
      console.log(
        `${colors.gray}Progress: ${createProgressBar(i + 1, totalFiles)} | ETA: ${Math.ceil(eta / 60)} min${colors.reset}`,
      );
    }
  }
  ...
  console.log(`${"═".repeat(60)}`);
  console.log(
    `✅ Successful files: ${colors.green}${successCount}${colors.reset}/${totalFiles}`,
  );
  console.log(`⏱️  Total duration: ${(totalDuration / 60).toFixed(2)} minutes`);
  console.log(
    `⚡ Average: ${(totalDuration / successCount).toFixed(1)}s per file`,
  );
