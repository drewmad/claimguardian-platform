...
const logValidation = (success, details) => {
  ...
  const logPath = path.join(
    REPO_ROOT,
    ".ai-context/retrospectives/system-validations.jsonl",
  );
...
try {
  ...
  console.log(
    "Running pnpm install to ensure local dependencies are up-to-date...",
  );
  ...
  const statusOutput = execSync("git status --porcelain pnpm-lock.yaml", {
    encoding: "utf8",
  }).trim();

  if (statusOutput.includes("M pnpm-lock.yaml")) {
    console.error(
      '❌ pnpm-lock.yaml was modified by "pnpm install" and is not staged!',
    );
    console.error(
      'Please run "git add pnpm-lock.yaml" and commit the changes.',
    );
  ...
  console.error(
    'Please run "pnpm install" and commit "pnpm-lock.yaml" if it was modified.',
  );
