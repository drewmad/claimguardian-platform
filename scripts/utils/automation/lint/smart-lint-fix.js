...
const fixStrategies = [
  ...
  },
  {
    name: "React Entities",
    command:
      'pnpm eslint . --fix --rule "react/no-unescaped-entities: error" --no-eslintrc',
  ...
  },
  {
    name: "Formatting",
    command:
      'pnpm eslint . --fix --rule "quotes: [error, single]" --rule "semi: [error, always]" --rule "comma-dangle: [error, always-multiline]" --no-eslintrc',
    description: "Fix basic formatting",
  },
  {
    name: "Unused Variables",
    command:
      'pnpm eslint . --fix --rule "@typescript-eslint/no-unused-vars: [error, { argsIgnorePattern: ^_, varsIgnorePattern: ^_ }]" --no-eslintrc',
...
async function smartLintFix() {
  ...
  log(
    `\n📊 Initial State: ${initialErrors} errors, ${initialWarnings} warnings`,
    "yellow",
  );
  ...
  log("\n📊 Final Results:", "blue");
  log(
    `   Errors: ${initialErrors} → ${finalErrors} (fixed ${initialErrors - finalErrors})`,
    finalErrors === 0 ? "green" : "yellow",
  );
  log(
    `   Warnings: ${initialWarnings} → ${finalWarnings} (fixed ${initialWarnings - finalWarnings})`,
