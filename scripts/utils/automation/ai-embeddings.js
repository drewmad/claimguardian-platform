...
class EmbeddingGenerator {
  ...
  async generateForNewProperties() {
    ...
    try {
      ...
      console.log(
        `📊 Processing ${properties.length} properties for embeddings`,
      );
      ...
      for (let i = 0; i < properties.length; i += this.config.batchSize) {
        ...
        console.log(
          `   ✅ Batch ${Math.floor(i / this.config.batchSize) + 1}: ${batch.length} properties embedded`,
        );
      }
  ...
  async processBatch(properties) {
    ...
    try {
      ...
      const descriptionTexts = embeddingRequests.map(
        (req) => req.description_text,
      );
  ...
  generatePropertyDescription(property) {
    ...
    if (property.spatial_features) {
      const spatialDesc = this.describeSpatialFeatures(
        property.spatial_features,
      );
    ...
    if (property.property_features) {
      const propDesc = this.describePropertyFeatures(
  ...
  describePropertyFeatures(propertyFeatures) {
    ...
    if (propertyFeatures.square_feet) {
      descriptions.push(
        `${propertyFeatures.square_feet.toLocaleString()} square feet`,
      );
    }
  ...
  async testSimilaritySearch(queryText, limit = 10) {
    ...
    try {
      ...
      results.forEach((result, index) => {
        console.log(
          `   ${index + 1}. ${result.address} (similarity: ${result.similarity.toFixed(3)})`,
        );
      });
