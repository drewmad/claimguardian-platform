...
class FloridaGeospatialDataAcquisition:
    ...
    def _fetch_paginated_data(self, source_config: Dict) -> Optional[gpd.GeoDataFrame]:
        ...
        while True:
            try:
                ...
                logger.info(f"Fetched batch {offset}-{offset + len(features)} for {source_config['name']}")
    ...
    def process_parcels(self, gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        gdf_processed = gdf.rename(columns={
            'PARCEL_ID': 'parcel_id',
            'CO_NO': 'county_fips',
            'COUNTY': 'county_name',
    ...
    def process_hazard_zones(self, gdf: gpd.GeoDataFrame, hazard_type: str) -> gpd.GeoDataFrame:
        ...
        if hazard_type.startswith('FEMA_FLOOD'):
            ...
            gdf_processed['zone_attributes'] = gdf.apply(
                lambda x: {'base_flood_elevation': x.get('STATIC_BFE', None)},
                axis=1
            )
            
        elif hazard_type == 'STORM_SURGE':
            # Map storm surge categories
            gdf_processed['hazard_type_code'] = 'STORM_SURGE_' + gdf['CATEGORY'].astype(str)
            gdf_processed['zone_name'] = 'Category ' + gdf['CATEGORY'].astype(str) + ' Storm Surge'
            gdf_processed['zone_attributes'] = gdf.drop(columns=['geometry']).to_dict('records')
    ...
    def process_facilities(self, gdf: gpd.GeoDataFrame, facility_type: str) -> gpd.GeoDataFrame:
    ...
    def process_active_events(self, gdf: gpd.GeoDataFrame, event_type: str) -> gpd.GeoDataFrame:
        ...
        if event_type == 'wildfire':
            ...
            gdf_processed['start_time'] = pd.to_datetime(gdf.get('DISCOVERY_DATE', datetime.now()))
            gdf_processed['attributes'] = gdf.drop(columns=['geometry']).to_dict('records')
    ...
    def load_to_database(self, gdf: gpd.GeoDataFrame, table_name: str, if_exists='append'):
    ...
    def run_acquisition(self, sources: List[str] = None):
        ...
        for source_key in sources:
            ...
            if source_key == "florida_parcels":
                ...
                gdf_processed = self.process_hazard_zones(gdf, source_config['hazard_type'])
            elif "facility_type" in source_config:
                gdf_processed = self.process_facilities(gdf, source_config['facility_type'])
            elif "event_type" in source_config:
                gdf_processed = self.process_active_events(gdf, source_config['event_type'])
            else:
