...
def process_charlotte_import():
    ...
    with open(csv_file, 'r') as f:
        ...
        for row in reader:
            ...
            # Build value tuple
            val = f"({row.get('CO_NO', 18)}, {escape_sql(row.get('PARCEL_ID'))}, " \
...
ON CONFLICT DO NOTHING;"""
                ...
                sys.stdout.write(f'\r[{bar}] {progress:.1f}% | Batch {batch_num} | {row_num:,}/{actual_count:,} rows | ETA: {int(eta)}s    ')
                sys.stdout.flush()
    ...
    print(f'\r[{"=" * bar_length}] 100% | Import completed!                                    ')
    print()
    print()
    print("========================================")
