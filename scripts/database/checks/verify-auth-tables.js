...
async function verifyTables() {
  ...
  if (userProfilesError) {
    if (userProfilesError.message.includes("does not exist")) {
      ...
      console.log(
        "⚠️  user_profiles table: Error accessing",
  ...
  try {
    ...
    if (captureError) {
      if (
        ...
        captureError.message.includes("does not exist")
      ) {
        console.log("❌ capture_signup_data function: NOT FOUND");
      } else {
        console.log(
          "✅ capture_signup_data function: EXISTS (test call failed as expected)",
        );
      }
    } else {
  ...
  try {
    ...
    if (loginError) {
      if (
        loginError.message.includes("function") ||
        loginError.message.includes("does not exist")
      ) {
        console.log("❌ log_login_activity function: NOT FOUND");
      } else {
        console.log(
          "✅ log_login_activity function: EXISTS (test call failed as expected)",
        );
      }
    } else {
