...
async function investigate() {
  ...
  console.log('Result:', adminError ? `Error: ${adminError.message}` : `Success (${adminData?.length} rows)`)
  ...
  console.log('Result:', anonError ? `Error: ${anonError.message}` : `Success (${anonData?.length} rows)`)
  ...
  if (anonError?.message.includes('users')) {
    ...
    console.log('Result:', usersError ? `Error: ${usersError.message}` : 'Success')
