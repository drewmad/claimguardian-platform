...
function transformRow(row) {
  ...
  for (const [csvCol, dbCol] of Object.entries(columnMapping)) {
    if (row[csvCol] !== undefined && row[csvCol] !== "") {
      ...
      if (
        csvCol.includes("_") ||
        ["JV", "LND_VAL", "SALE_PRC1", "SALE_PRC2", "TOT_LVG_AR"].includes(
          csvCol,
        )
      ) {
    ...
    try {
      ...
      if (batch.length >= batchSize) {
        ...
        if (stats.processed % 1000 === 0) {
          ...
          console.log(
            `⏱️  Processed: ${stats.processed} | Rate: ${rate} records/sec | Errors: ${stats.errors}`,
          );
        }
      }
    } catch (error) {
...
if (require.main === module) {
  ...
  if (args.length === 0) {
    console.log(
      "Usage: node import-florida-parcels-csv.js <csv-file> [options]",
    );
    console.log("\nOptions:");
    ...
    console.log("\nExample:");
    console.log(
      "  node import-florida-parcels-csv.js parcels.csv --batch-size=500 --skip-errors",
    );
    process.exit(1);
  }
