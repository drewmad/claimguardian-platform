...
async function testUppercaseImport() {
  ...
  const insertResult = await executeSQL(
    testInsertSQL,
    "Test INSERT with UPPERCASE columns",
  );
  ...
  if (insertResult.success) {
    ...
    if (verifyResult.success && verifyResult.data.rows.length > 0) {
      ...
      console.log(
        "   county_fips was automatically derived:",
        verifyResult.data.rows[0].county_fips,
      );
    }
    ...
    console.log(
      "   Table Editor → florida_parcels_uppercase → Import data from CSV",
    );
  } else {
    ...
    const viewsResult = await executeSQL(
      listViewsSQL,
      "List all florida_parcels views",
    );
