      ...
      if (batch.length >= BATCH_SIZE) {
        ...
        if (Date.now() - lastProgressUpdate > 1000) {
          ...
          process.stdout.write(
            `\r  Progress: ${fileRecords} records | ${batchCount} batches | ~${percent}% complete`,
          );
      ...
      if (!hasErrors) {
        ...
        for (let i = 0; i < MAX_RETRIES; i++) {
          try {
            ...
            console.error(
              `  ⚠️  Transfer attempt ${i + 1} failed: ${err.message}`,
            );
            if (i < MAX_RETRIES - 1) {
...
async function main() {
  ...
  for (let i = 0; i < csvFiles.length; i++) {
    ...
    console.log(
      `  📊 Records/second: ${(result.recordCount / duration).toFixed(0)}`,
    );
    ...
    console.log(`\n📊 Overall Progress:`);
    console.log(
      `   Files: ${i + 1}/${csvFiles.length} (${filesRemaining} remaining)`,
    );
    console.log(
      `   Records: ${globalStats.successfulRecords.toLocaleString()} successful, ${globalStats.failedRecords.toLocaleString()} failed`,
    );
  ...
  console.log(
    `📈 Total records processed: ${globalStats.totalRecords.toLocaleString()}`,
  );
  console.log(
    `✅ Successful records: ${globalStats.successfulRecords.toLocaleString()}`,
  );
  console.log(
    `❌ Failed records: ${globalStats.failedRecords.toLocaleString()}`,
  );
  console.log(
  ...
  console.log(
    `⚡ Average speed: ${(globalStats.totalRecords / totalDuration).toFixed(0)} records/second`,
  );
  ...
  if (globalStats.successfulFiles > 0) {
    console.log(
      "📁 Successfully imported files moved to: CleanedSplit_imported/",
    );
  }
}
