...
class CSVStreamer {
  ...
  async processBatch() {
    ...
    process.stdout.write(
      `\r📊 Processed: ${this.stats.processed} | Success: ${this.stats.successful} | Failed: ${this.stats.failed} | Rate: ${rate.toFixed(0)}/sec`,
    );
  }
...
async function estimateCosts(totalRecords, totalSizeGB) {
  ...
  console.log(
    `   - Monthly cost: $${(totalDBSize * 0.125).toFixed(2)} ($0.125/GB)`,
  );
  ...
  console.log(
    `   - Estimated usage: ${monthlyBandwidthGB.toFixed(1)} GB/month`,
  );
  console.log(
    `   - Additional cost: $${Math.max(0, (monthlyBandwidthGB - 50) * 0.09).toFixed(2)} ($0.09/GB)`,
  );
  ...
  if (totalDBSize > 8) {
    console.log(
      `   ⚠️  Database size (${totalDBSize.toFixed(1)} GB) exceeds Free tier (8 GB)`,
    );
...
async function main() {
  ...
  if (!inputPath) {
    console.log(
      "Usage: node import-florida-parcels-streaming.js <csv-file-or-directory>",
    );
    console.log("\nExample:");
    console.log(
      "  node import-florida-parcels-streaming.js ./florida_parcels.csv",
    );
    console.log(
      "  node import-florida-parcels-streaming.js ./parcels_directory/",
    );
    process.exit(1);
  }
  ...
  if (stats.isDirectory()) {
    ...
    console.error(
      "❌ Input must be a CSV file or directory containing CSV files",
    );
    process.exit(1);
  }
  ...
  console.log(
    `⚡ Average speed: ${(totalRecords / duration).toFixed(0)} records/second`,
  );
