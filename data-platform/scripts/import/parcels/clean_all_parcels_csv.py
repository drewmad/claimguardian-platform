...
EXPECTED_COLUMNS = {"PARCEL_ID", "CO_NO", "JV", "OWN_NAME", "PHY_ADDR1", "SALE_PRC1"}


class CSVCleaner:
    """Clean CSV files for Supabase import."""
    ...
    def clean_csv_file(self, input_file, output_file):
        ...
        try:
            ...
            self.log(
                f"   ✅ Cleaned {rows_processed:,} rows, "
                f"{file_cells_cleaned:,} cells modified"
            )
            return True
    ...
    def print_summary(self):
        ...
        if self.stats["errors"] > 0:
            ...
            error_files = ", ".join(self.stats["files_with_errors"])
...
def main():
    """Process CSV files for cleaning."""
    ...
    print("🏘️  Florida Parcels CSV Batch Cleaner")
    print("=" * 60)
    ...
    if success and cleaner.stats["files_processed"] > 0:
        ...
        print("2. Select the 'florida_parcels' table")
        print("3. Click 'Import data from CSV'")
        print(f"4. Upload files from: {output_path}")
        print("\n💡 Tips:")
        print("   - Upload files one at a time for better error tracking")
        print("   - The import will handle duplicates with UPSERT on parcel_id")
        print("   - Empty strings in numeric columns will become NULL values")
    else:
