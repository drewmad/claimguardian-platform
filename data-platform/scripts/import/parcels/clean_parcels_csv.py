...
def clean_value(value, column_name):
    ...
    # If it's a numeric column and contains only spaces or is empty,
...
def clean_csv(input_file, output_file):
    ...
    with open(input_file, "r", newline="", encoding="utf-8") as infile:
        ...
        print(
            f"🔢 Found {len(numeric_cols_found)} numeric columns to clean: "
            f"{', '.join(numeric_cols_found[:5])}..."
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Clean Florida parcels CSV for Supabase import"
    )
    ...
    print("🏘️  Florida Parcels CSV Cleaner")
    print("=" * 50)
    ...
    if success:
        ...
        print(
            "\n💡 The cleaned file has empty strings instead of spaces "
            "in numeric columns,"
        )
        print("   which Supabase will correctly interpret as NULL values.")
    else:
        sys.exit(1)
