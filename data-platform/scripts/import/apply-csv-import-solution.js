...
async function applyImportSolution() {
  ...
  console.log(
    `✅ Successful: ${successCount}/${results.filter((r) => r.description !== "Statement").length}`,
  );
  ...
  if (successCount > 0) {
    ...
    console.log(
      "5. After import, run: SELECT * FROM import_parcels_from_temp(1000);",
    );
    console.log(
      "6. Check results: SELECT * FROM florida_parcels_import_summary;",
    );
