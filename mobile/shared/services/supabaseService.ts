...
class SupabaseService {
  ...
  async syncProperties(
    ...
    try {
      ...
      const remoteOnlyProperties =
        remoteProperties?.filter(
  ...
  async syncAssessments(
    localAssessments: DamageAssessment[],
  ): Promise<SyncResult<DamageAssessment[]>> {
    try {
      const user = await this.getCurrentUser();
      if (!user) {
        return { success: false, error: "User not authenticated" };
      ...
      const conflicts: Array<{
        localItem: DamageAssessment;
        remoteItem: DamageAssessment;
        field: string;
      }> = [];
      const merged: DamageAssessment[] = [];

      for (const localAssessment of toUpload) {
        const remoteAssessment = remoteAssessments?.find(
          (r) => r.id === localAssessment.id,
        );
      ...
      const remoteOnlyAssessments =
        remoteAssessments?.filter(
          (remote) => !localAssessments.find((local) => local.id === remote.id),
        ) || [];

      merged.push(
        ...remoteOnlyAssessments.map((a) => ({ ...a, synced: true })),
      );
  ...
  async syncDamageItems(
    localItems: DamageItem[],
  ): Promise<SyncResult<DamageItem[]>> {
    try {
      const user = await this.getCurrentUser();
      if (!user) {
        return { success: false, error: "User not authenticated" };
      ...
      for (const localItem of toUpload) {
        ...
        if (!remoteItem) {
          ...
          const { data, error } = await this.client
            ...
            .insert({
              ...
              measurements: localItem.measurements
                ? JSON.stringify(localItem.measurements)
                : null,
              synced: undefined,
            })
            .select()
          ...
          if (localUpdated > remoteUpdated) {
            ...
            merged.push({
              ...remoteItem,
              measurements: remoteItem.measurements
                ? JSON.parse(remoteItem.measurements)
                : null,
              synced: true,
            });
          }
