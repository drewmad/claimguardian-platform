...
class ConflictResolver {
  ...
  private determineConflictType(
    ...
    const hasTimestampConflict = conflictFields.some((field) =>
      timestampFields.includes(field),
    );
    const hasContentConflict = conflictFields.some(
      (field) => !timestampFields.includes(field),
    );
  ...
  private async attemptAutoResolution<T>(
    ...
    const mergeResult = this.attemptSmartMerge(
      local,
      remote,
    ...
    const canAutoResolve = conflict.conflictFields.every(
      (field) =>
    ...
    if (canAutoResolve) {
      return this.applyStrategyRules(
        local,
        remote,
        conflict.conflictFields,
        strategy,
  ...
  private resolveTimestampConflict<T>(
    local: T,
    remote: T,
  ...
  private resolveDeletedConflict<T>(
    local: T,
    remote: T,
  ): ConflictResolution<T> {
    ...
    try {
      ...
      for (const field of conflictFields) {
        ...
        if (strategy.autoMergeRules[field]) {
          ...
          (merged as any)[field] = strategy.autoMergeRules[field](
            localValue,
            remoteValue,
          );
          mergedFields++;
          ...
          (merged as any)[field] = this.mergeTextFields(
            localValue,
            remoteValue,
          );
          mergedFields++;
  ...
  private getStrategyForEntityType(
    entityType: ConflictData["entityType"],
  ): MergeStrategy {
    switch (entityType) {
      case "property":
        return {
          ...
          autoMergeRules: {
            estimated_value: (local: number, remote: number) =>
      ...
      case "damage_item":
        return {
          ...
          autoMergeRules: {
            measurements: (local: any, remote: any) => ({
              ...remote,
              ...local,
            }), // Merge measurement objects
          },
        };
  ...
  async cleanupOldConflicts(olderThanDays: number = 30): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Cleaned up resolved conflicts older than ${olderThanDays} days`,
      );
    } catch (error) {
