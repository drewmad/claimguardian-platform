...
class SyncService {
  ...
  async removeSyncItem(itemId: string): Promise<void> {
    ...
    try {
      await this.db.runAsync("DELETE FROM sync_queue WHERE rowid = ?", [
        itemId,
      ]);
    } catch (error) {
  ...
  async performIncrementalSync(
    ...
    try {
      ...
      for (const entityType of processingOrder) {
        ...
        for (const item of items) {
          try {
            ...
            progress.failed++;
            errors.push(
              `Error syncing ${item.entity_type}:${item.entity_id}: ${errorMessage}`,
            );
            this.reportProgress(progress);
          }
        }
      ...
      console.log(
        `Sync completed. Success: ${progress.completed}, Failed: ${progress.failed}`,
      );
  ...
  private async processSyncItem(item: SyncQueueItem): Promise<boolean> {
    try {
      ...
      switch (item.entity_type) {
        ...
        case "assessment":
          return await this.syncAssessment(
            item.operation,
            data as DamageAssessment,
          );
    ...
    try {
      if (operation === "create" || operation === "update") {
        if (photo.local_uri && photo.upload_status === "pending") {
          const result = await supabaseService.uploadPhoto(
            photo,
            photo.local_uri,
          );
          return result.success;
        ...
        if (photo.remote_url) {
          ...
          if (fileName) {
            await client.storage.from("damage-photos").remove([fileName]);
          }
        }
    ...
    try {
      if (operation === "create" || operation === "update") {
        if (voiceNote.local_uri && voiceNote.upload_status === "pending") {
          const result = await supabaseService.uploadVoiceNote(
            voiceNote,
            voiceNote.local_uri,
          );
          return result.success;
        ...
        if (voiceNote.remote_url) {
          ...
          if (fileName) {
            await client.storage.from("voice-notes").remove([fileName]);
          }
        }
