...
export const selectAssessmentWithItems = (assessmentId: string) => (state: RootState) => ({
  assessment: state.assessments.items.find(a => a.id === assessmentId),
  damageItems: state.damageItems.items.filter(d => d.assessment_id === assessmentId),
  photos: state.photos.items.filter(p => p.assessment_id === assessmentId),
  voiceNotes: state.voiceNotes.items.filter(v => v.assessment_id === assessmentId),
})

export const selectPropertyWithAssessments = (propertyId: string) => (state: RootState) => ({
  property: state.properties.items.find(p => p.id === propertyId),
  assessments: state.assessments.items.filter(a => a.property_id === propertyId),
})
