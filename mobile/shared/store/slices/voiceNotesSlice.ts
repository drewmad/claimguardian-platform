...
export const loadVoiceNotesFromDatabase = createAsyncThunk<
  ...
  { rejectValue: string }
>("voiceNotes/loadFromDatabase", async (filters, { rejectWithValue }) => {
  try {
    // Implementation would load from SQLite with optional filters
    return [];
  } catch (error) {
    return rejectWithValue(
      error instanceof Error ? error.message : "Failed to load voice notes",
    );
  }
});
...
export const stopRecording = createAsyncThunk<
  ...
  void,
  { rejectValue: string }
>("voiceNotes/stopRecording", async (_, { rejectWithValue, getState }) => {
  try {
...
export const transcribeVoiceNote = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>("voiceNotes/transcribe", async (voiceNoteId, { rejectWithValue }) => {
  try {
...
export const uploadVoiceNote = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>("voiceNotes/upload", async (voiceNoteId, { rejectWithValue, getState }) => {
  try {
    const state = getState() as any;
    const voiceNote = state.voiceNotes.items.find(
      (note: VoiceNote) => note.id === voiceNoteId,
    );
...
const voiceNotesSlice = createSlice({
  ...
  reducers: {
    ...
    removeVoiceNote: (state, action: PayloadAction<string>) => {
      ...
      state.transcribing = state.transcribing.filter(
    ...
    setUploadStatus: (
      ...
      action: PayloadAction<{
        voiceNoteId: string;
        status: VoiceNote["upload_status"];
      }>,
    ) => {
      const note = state.items.find(
        (note) => note.id === action.payload.voiceNoteId,
      );
      if (note) {
    ...
    setRemoteUrl: (
      state,
      action: PayloadAction<{ voiceNoteId: string; url: string }>,
    ) => {
      const note = state.items.find(
        (note) => note.id === action.payload.voiceNoteId,
      );
    ...
    setTranscription: (
      state,
      action: PayloadAction<{ voiceNoteId: string; transcription: string }>,
    ) => {
      const note = state.items.find(
        (note) => note.id === action.payload.voiceNoteId,
      );
    ...
    bulkAddVoiceNotes: (state, action: PayloadAction<VoiceNote[]>) => {
      const newNotes = action.payload.filter(
        (newNote) =>
          !state.items.some((existing) => existing.id === newNote.id),
      );
    ...
    setUploadStatusFilter: (
      state,
      action: PayloadAction<VoiceNote["upload_status"] | null>,
    ) => {
    ...
    setSortBy: (
      state,
      action: PayloadAction<{
        sortBy: VoiceNotesState["filters"]["sortBy"];
        sortOrder: "asc" | "desc";
      }>,
    ) => {
    ...
    updateSettings: (
      state,
      action: PayloadAction<Partial<VoiceNotesState["settings"]>>,
    ) => {
      state.settings = {
        ...state.settings,
        ...action.payload,
    ...
    setAudioQuality: (
      state,
      action: PayloadAction<VoiceNotesState["settings"]["audioQuality"]>,
    ) => {
    ...
    updateStats: (state) => {
      const totalSizeBytes = state.items.reduce(
        (sum, note) => sum + note.file_size,
        0,
      );
      const totalDurationSeconds = state.items.reduce(
        (sum, note) => sum + note.duration_seconds,
        0,
      );
      ...
      state.stats = {
        ...
        totalDurationMinutes:
          Math.round((totalDurationSeconds / 60) * 100) / 100,
        pendingUpload: state.items.filter(
          (note) => note.upload_status === "pending",
        ).length,
        uploadedNotes: state.items.filter(
          (note) => note.upload_status === "completed",
        ).length,
        failedUploads: state.items.filter(
          (note) => note.upload_status === "failed",
        ).length,
        transcribedNotes: state.items.filter((note) => note.transcription)
          .length,
    ...
    removeOrphanedVoiceNotes: (state) => {
      state.items = state.items.filter(
        (note) => note.assessment_id || note.damage_item_id,
      );
      voiceNotesSlice.caseReducers.updateStats(state);
    },
