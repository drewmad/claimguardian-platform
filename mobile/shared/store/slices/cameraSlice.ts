...
const cameraSlice = createSlice({
  ...
  reducers: {
    ...
    setPermissionStatus: (state, action: PayloadAction<CameraState['permissionStatus']>) => {
    ...
    setCameraType: (state, action: PayloadAction<CameraState['cameraType']>) => {
    ...
    setWhiteBalance: (state, action: PayloadAction<CameraState['settings']['whiteBalance']>) => {
    ...
    toggleVolumeButtonCapture: (state) => {
      state.settings.useVolumeButtonForCapture = !state.settings.useVolumeButtonForCapture
    },
    ...
    recordCaptureAttempt: (state, action: PayloadAction<{ success: boolean; captureTime?: number }>) => {
      ...
      if (action.payload.success) {
        ...
        if (action.payload.captureTime) {
          ...
          state.captureStats.averageCaptureTime = totalTime / state.captureStats.successfulCaptures
        }
      } else {
    ...
    updateCameraSettings: (state, action: PayloadAction<Partial<CameraState['settings']>>) => {
      state.settings = {
        ...state.settings,
        ...action.payload
      }
    ...
    setManualMode: (state, action: PayloadAction<{
      exposure?: number
      whiteBalance?: CameraState['settings']['whiteBalance']
      focus?: boolean
    }>) => {
      if (action.payload.exposure !== undefined) {
        state.settings.exposure = Math.max(-1, Math.min(1, action.payload.exposure))
      }
    ...
    initializeCamera: (state, action: PayloadAction<{
      hasPermission: boolean
      supportedRatios?: string[]
      supportedResolutions?: string[]
    }>) => {
