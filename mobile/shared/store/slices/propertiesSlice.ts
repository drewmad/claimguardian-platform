...
>(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error instanceof Error ? error.message : 'Failed to load properties')
    }
  }
)
...
export const syncPropertyToServer = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  'properties/syncToServer',
  async (propertyId, { rejectWithValue }) => {
    try {
      // Implementation would sync to server
      throw new Error('Not implemented')
    } catch (error) {
      return rejectWithValue(error instanceof Error ? error.message : 'Failed to sync property')
    }
  }
)
...
const propertiesSlice = createSlice({
  ...
  reducers: {
    ...
    setSortBy: (state, action: PayloadAction<{ sortBy: PropertiesState['filters']['sortBy'], sortOrder: 'asc' | 'desc' }>) => {
    ...
    bulkAddProperties: (state, action: PayloadAction<Property[]>) => {
      const newProperties = action.payload.filter(
        newProp => !state.items.some(existingProp => existingProp.id === newProp.id)
      )
