...
export const loadAssessmentsFromDatabase = createAsyncThunk<
  ...
  string | undefined,
  { rejectValue: string }
>("assessments/loadFromDatabase", async (propertyId, { rejectWithValue }) => {
  try {
...
const assessmentsSlice = createSlice({
  ...
  reducers: {
    ...
    updateEstimatedDamage: (
      state,
      action: PayloadAction<{ assessmentId: string; amount: number }>,
    ) => {
    ...
    updateOverallCondition: (
      ...
      action: PayloadAction<{
        assessmentId: string;
        condition: DamageAssessment["overall_condition"];
      }>,
    ) => {
      const assessment = state.items.find(
        (a) => a.id === action.payload.assessmentId,
    ...
    updatePriority: (
      ...
      action: PayloadAction<{
        assessmentId: string;
        priority: DamageAssessment["priority_level"];
      }>,
    ) => {
      const assessment = state.items.find(
        (a) => a.id === action.payload.assessmentId,
    ...
    addNotes: (
      state,
      action: PayloadAction<{ assessmentId: string; notes: string }>,
    ) => {
      const assessment = state.items.find(
        (a) => a.id === action.payload.assessmentId,
      );
    ...
    setConditionFilter: (
      state,
      action: PayloadAction<DamageAssessment["overall_condition"] | null>,
    ) => {
    ...
    setPriorityFilter: (
      state,
      action: PayloadAction<DamageAssessment["priority_level"] | null>,
    ) => {
    ...
    setSortBy: (
      state,
      action: PayloadAction<{
        sortBy: AssessmentsState["filters"]["sortBy"];
        sortOrder: "asc" | "desc";
      }>,
    ) => {
    ...
    bulkAddAssessments: (state, action: PayloadAction<DamageAssessment[]>) => {
      const newAssessments = action.payload.filter(
        (newAssessment) =>
          !state.items.some((existing) => existing.id === newAssessment.id),
      );
      ...
      state.stats = {
        ...
        avgDamageAmount:
          state.items.length > 0
            ? state.items.reduce(
                (sum, a) => sum + a.estimated_total_damage,
                0,
              ) / state.items.length
            : 0,
        highPriorityCount: state.items.filter(
          (a) => a.priority_level === "high" || a.priority_level === "critical",
        ).length,
      };
    },
  ...
  extraReducers: (builder) => {
    ...
    // Update progress
    builder.addCase(updateAssessmentProgress.fulfilled, (state, action) => {
