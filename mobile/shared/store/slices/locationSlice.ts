...
export const startLocationTracking = createAsyncThunk<
  ...
  void,
  { rejectValue: string }
>("location/startTracking", async (_, { rejectWithValue }) => {
  try {
...
const locationSlice = createSlice({
  ...
  reducers: {
    ...
    updateCurrentLocation: (state, action: PayloadAction<LocationData>) => {
      ...
      const totalAccuracy =
        state.stats.averageAccuracy * (state.stats.totalLocationUpdates - 1) +
        (newLocation.accuracy || 0);
      state.stats.averageAccuracy =
        totalAccuracy / state.stats.totalLocationUpdates;
    ...
    toggleHeadingTracking: (state) => {
      state.settings.enableHeadingTracking =
        !state.settings.enableHeadingTracking;
    },
    ...
    addGeofence: (
      ...
      action: PayloadAction<
        Omit<LocationState["geofences"][0], "id" | "lastTriggered">
      >,
    ) => {
      const geofence = {
        ...action.payload,
    ...
    updateGeofence: (
      ...
      action: PayloadAction<{
        id: string;
        updates: Partial<LocationState["geofences"][0]>;
      }>,
    ) => {
    ...
    setLastKnownLocation: (
      state,
      action: PayloadAction<LocationData | null>,
    ) => {
