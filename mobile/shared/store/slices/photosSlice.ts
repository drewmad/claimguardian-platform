...
export const loadPhotosFromDatabase = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  'photos/loadFromDatabase',
  async (filters, { rejectWithValue }) => {
    try {
      // Implementation would load from SQLite with optional filters
      return []
    } catch (error) {
      return rejectWithValue(error instanceof Error ? error.message : 'Failed to load photos')
    }
  }
)
...
>(
  ...
  async (_, { rejectWithValue, getState, dispatch }) => {
    try {
      ...
      return rejectWithValue(error instanceof Error ? error.message : 'Failed to retry uploads')
    }
  }
)
...
>(
  'photos/compress',
  async ({ photoId, quality }, { rejectWithValue }) => {
    try {
      // Implementation would compress photo using expo-image-manipulator
      throw new Error('Not implemented')
    } catch (error) {
      return rejectWithValue(error instanceof Error ? error.message : 'Failed to compress photo')
    }
  }
)
...
const photosSlice = createSlice({
  ...
  reducers: {
    ...
    setUploadStatus: (state, action: PayloadAction<{ photoId: string; status: Photo['upload_status'] }>) => {
    ...
    setRemoteUrl: (state, action: PayloadAction<{ photoId: string; url: string }>) => {
      const photo = state.items.find(p => p.id === action.payload.photoId)
      if (photo) {
    ...
    bulkAddPhotos: (state, action: PayloadAction<Photo[]>) => {
      const newPhotos = action.payload.filter(
        newPhoto => !state.items.some(existing => existing.id === newPhoto.id)
      )
    ...
    bulkUpdateUploadStatus: (state, action: PayloadAction<{ photoIds: string[]; status: Photo['upload_status'] }>) => {
    ...
    setUploadStatusFilter: (state, action: PayloadAction<Photo['upload_status'] | null>) => {
    ...
    setSortBy: (state, action: PayloadAction<{ sortBy: PhotosState['filters']['sortBy'], sortOrder: 'asc' | 'desc' }>) => {
    ...
    updateStats: (state) => {
      ...
      state.stats = {
        ...
        uploadedPhotos: state.items.filter(p => p.upload_status === 'completed').length,
        failedUploads: state.items.filter(p => p.upload_status === 'failed').length,
