...
export const performFullSync = createAsyncThunk<
  ...
  void,
  { rejectValue: string }
>("sync/performFullSync", async (_, { rejectWithValue }) => {
  try {
...
export const retryFailedSyncs = createAsyncThunk<
  ...
  void,
  { rejectValue: string }
>("sync/retryFailedSyncs", async (_, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(
      error instanceof Error ? error.message : "Retry sync failed",
    );
  }
});
  ...
  reducers: {
    ...
    addToQueue: (
      ...
      action: PayloadAction<
        Omit<SyncQueue, "id" | "retry_count" | "created_at">
      >,
    ) => {
      ...
      const existingIndex = state.queue.findIndex(
        (item) =>
          item.entity_type === queueItem.entity_type &&
    ...
    updateSyncSettings: (
      state,
      action: PayloadAction<Partial<SyncState["settings"]>>,
    ) => {
    ...
    addSyncEvent: (
      state,
      action: PayloadAction<Omit<SyncEvent, "timestamp">>,
    ) => {
    ...
    updateSyncStats: (
      state,
      action: PayloadAction<Partial<SyncState["stats"]>>,
    ) => {
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(performFullSync.fulfilled, (state, action) => {
        ...
        // Update success rate
        const total =
          state.stats.totalSyncedToday + state.stats.totalFailedToday;
        if (total > 0) {
          state.stats.syncSuccessRate =
            (state.stats.totalSyncedToday / total) * 100;
        }

        // Clear successfully synced items from queue
        if (action.payload.success) {
          state.queue = state.queue.filter(
            (item) => item.retry_count >= state.settings.maxRetries,
          );
        }
    ...
    builder
      .addCase(syncSingleItem.fulfilled, (state, action) => {
        if (action.payload.success) {
          state.queue = state.queue.filter(
            (item) => item.entity_id !== action.payload.itemId,
          );
