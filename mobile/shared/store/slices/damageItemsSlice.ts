...
interface DamageItemsState {
  ...
  filters: {
    ...
    sortBy: 'location' | 'estimated_cost' | 'severity' | 'repair_priority' | 'created_at'
    sortOrder: 'asc' | 'desc'
  }
  stats: {
    totalItems: number
  ...
  templates: {
    ...
    categoryDefaults: Record<DamageItem['category'], {
...
export const loadDamageItemsFromDatabase = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  'damageItems/loadFromDatabase',
  async (assessmentId, { rejectWithValue }) => {
    try {
      // Implementation would load from SQLite, optionally filtered by assessment
      return []
    } catch (error) {
      return rejectWithValue(error instanceof Error ? error.message : 'Failed to load damage items')
    }
  }
)
...
export const bulkCreateDamageItems = createAsyncThunk<
  DamageItem[],
  { assessmentId: string; items: Omit<DamageItem, 'id' | 'assessment_id' | 'created_at' | 'updated_at' | 'synced'>[] },
  { rejectValue: string }
>(
...
const damageItemsSlice = createSlice({
  ...
  reducers: {
    ...
    // Field updates
    updateEstimatedCost: (state, action: PayloadAction<{ itemId: string; cost: number }>) => {
    ...
    updateSeverity: (state, action: PayloadAction<{ itemId: string; severity: DamageItem['severity'] }>) => {
      const item = state.items.find(item => item.id === action.payload.itemId)
      if (item) {
    ...
    updateRepairPriority: (state, action: PayloadAction<{ itemId: string; priority: DamageItem['repair_priority'] }>) => {
      const item = state.items.find(item => item.id === action.payload.itemId)
      if (item) {
    ...
    updateDescription: (state, action: PayloadAction<{ itemId: string; description: string }>) => {
      const item = state.items.find(item => item.id === action.payload.itemId)
      if (item) {
    ...
    setCategoryFilter: (state, action: PayloadAction<DamageItem['category'] | null>) => {
      state.filters.category = action.payload
    },

    setDamageTypeFilter: (state, action: PayloadAction<DamageItem['damage_type'] | null>) => {
      state.filters.damageType = action.payload
    },

    setSeverityFilter: (state, action: PayloadAction<DamageItem['severity'] | null>) => {
      state.filters.severity = action.payload
    },

    setPriorityFilter: (state, action: PayloadAction<DamageItem['repair_priority'] | null>) => {
    ...
    setCostRangeFilter: (state, action: PayloadAction<{ min: number | null; max: number | null }>) => {
      state.filters.costRange = action.payload
    },

    setSortBy: (state, action: PayloadAction<{ sortBy: DamageItemsState['filters']['sortBy'], sortOrder: 'asc' | 'desc' }>) => {
    ...
    bulkUpdateSeverity: (state, action: PayloadAction<{ itemIds: string[]; severity: DamageItem['severity'] }>) => {
    ...
    bulkUpdatePriority: (state, action: PayloadAction<{ itemIds: string[]; priority: DamageItem['repair_priority'] }>) => {
      action.payload.itemIds.forEach(itemId => {
        const item = state.items.find(item => item.id === itemId)
        if (item) {
          item.repair_priority = action.payload.priority
    ...
    updateStats: (state) => {
      state.stats = {
        ...
        totalEstimatedCost: state.items.reduce((sum, item) => sum + item.estimated_cost, 0),
        itemsByCategory: {
          structural: state.items.filter(item => item.category === 'structural').length,
          exterior: state.items.filter(item => item.category === 'exterior').length,
          interior: state.items.filter(item => item.category === 'interior').length,
          electrical: state.items.filter(item => item.category === 'electrical').length,
          plumbing: state.items.filter(item => item.category === 'plumbing').length,
        ...
        itemsBySeverity: {
          ...
          moderate: state.items.filter(item => item.severity === 'moderate').length,
          major: state.items.filter(item => item.severity === 'major').length,
          total_loss: state.items.filter(item => item.severity === 'total_loss').length
        },
        itemsByPriority: {
          low: state.items.filter(item => item.repair_priority === 'low').length,
          medium: state.items.filter(item => item.repair_priority === 'medium').length,
          high: state.items.filter(item => item.repair_priority === 'high').length,
          emergency: state.items.filter(item => item.repair_priority === 'emergency').length
        },
    ...
    removeItemsByAssessment: (state, action: PayloadAction<string>) => {
      state.items = state.items.filter(item => item.assessment_id !== action.payload)
