...
const userSlice = createSlice({
  ...
  reducers: {
    ...
    updateSubscriptionTier: (state, action: PayloadAction<User['subscription_tier']>) => {
      if (state.current) {
    ...
    toggleAutoSync: (state) => {
      if (state.current) {
        state.current.preferences.auto_sync = !state.current.preferences.auto_sync
    ...
    toggleGpsEnabled: (state) => {
      if (state.current) {
        state.current.preferences.gps_enabled = !state.current.preferences.gps_enabled
        state.preferences.gps_enabled = state.current.preferences.gps_enabled
      }
    },

    setPhotoQuality: (state, action: PayloadAction<User['preferences']['photo_quality']>) => {
      if (state.current) {
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(signIn.fulfilled, (state, action) => {
        ...
        state.profile.isComplete = !!(action.payload.user.full_name && action.payload.user.phone)
        state.profile.lastUpdated = new Date().toISOString()
      })
    ...
    builder
      ...
      .addCase(updateProfile.fulfilled, (state, action) => {
        ...
        state.profile.isComplete = !!(action.payload.full_name && action.payload.phone)
        state.profile.lastUpdated = new Date().toISOString()
      })
