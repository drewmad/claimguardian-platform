...
>(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error instanceof Error ? error.message : 'Failed to check network state')
    }
  }
)
...
>(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error instanceof Error ? error.message : 'Failed to test connection speed')
    }
  }
)
...
const networkSlice = createSlice({
  ...
  reducers: {
      ...
      if (wasConnected !== action.payload.isConnected) {
        ...
        if (action.payload.isConnected) {
          ...
          if (wasConnected && state.lastOnlineTime) {
            const offlineDuration = Date.now() - new Date(state.lastOnlineTime).getTime()
    ...
    updateStats: (state) => {
      if (state.connectionHistory.length > 1) {
        ...
        if (connectionCount > 0) {
          state.stats.averageConnectionDuration = totalDuration / connectionCount
        }
      }

      // Calculate total online time
      if (state.isConnected && state.lastOnlineTime) {
        const currentOnlineTime = Date.now() - new Date(state.lastOnlineTime).getTime()
    ...
    recordConnectionEvent: (state, action: PayloadAction<{
    ...
    recordSpeedTest: (state, action: PayloadAction<{
