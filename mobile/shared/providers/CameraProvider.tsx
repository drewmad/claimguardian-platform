...
export function CameraProvider({ children }: CameraProviderProps) {
  ...
  useEffect(() => {
    const monitorCameraPerformance = () => {
      ...
      const { successfulCaptures, failedCaptures, totalPhotos } =
    ...
    if (
      cameraState.captureStats.totalPhotos > 0 &&
      cameraState.captureStats.totalPhotos % 20 === 0
    ) {
  ...
  useEffect(() => {
    const provideOptimizationTips = () => {
      const { averageCaptureTime, failedCaptures, totalPhotos } =
        cameraState.captureStats;

      if (averageCaptureTime > 3000) {
        // More than 3 seconds average
        console.log(
          "Camera capture time is slow - consider lowering quality setting",
        );
      }

      if (totalPhotos > 5 && failedCaptures / totalPhotos > 0.1) {
        // More than 10% failure rate
        console.log(
          "High camera failure rate - check device storage and restart app if needed",
        );
      }
    };
