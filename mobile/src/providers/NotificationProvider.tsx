...
interface NotificationContextType {
  ...
  updateSettings: (
    settings: Partial<NotificationContextType["notificationSettings"]>,
  ) => void;
}
...
export function NotificationProvider({ children }: NotificationProviderProps) {
  ...
  useEffect(() => {
    if (
      currentLocation &&
      notificationSettings.locationBasedAlerts &&
      hasPermission
    ) {
  ...
  const subscribeToLocationBasedAlerts = async (location: {
    latitude: number;
    longitude: number;
  }) => {
    try {
...
export function useDisasterAlerts() {
  ...
  return {
    ...
    criticalAlerts: disasterAlerts.filter(
      (alert) =>
...
export function useSystemNotifications() {
  ...
  return {
    ...
    assessmentUpdates: systemNotifications.filter(
      (n) => n.type === "assessment_update",
    ),
    syncNotifications: systemNotifications.filter(
      (n) => n.type === "sync_complete",
    ),
