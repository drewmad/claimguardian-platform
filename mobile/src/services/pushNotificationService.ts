...
class PushNotificationService {
  ...
  private async registerTokenWithServer(token: string): Promise<void> {
    try {
      ...
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL}/api/push-tokens`,
        {
          method: "POST",
          headers: {
  ...
  private handleBackgroundNotification(
    response: Notifications.NotificationResponse,
  ): void {
  ...
  async sendAssessmentUpdate(
    ...
    try {
      const notificationData: NotificationData = {
        ...
        actionButtons: [{ id: "open_assessment", title: "Open Assessment" }],
      };
  ...
  private async subscribeToDisasterAlerts(): Promise<void> {
    try {
      ...
      if (location && this.pushToken) {
        ...
        // Subscribe via API
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_API_URL}/api/disaster-alerts/subscribe`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
