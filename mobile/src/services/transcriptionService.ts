  ...
  private async validateAudioFile(audioUri: string): Promise<void> {
    try {
      ...
      if (fileInfo.size && fileInfo.size > this.maxFileSize) {
        throw new Error(
          `Audio file too large. Maximum size: ${this.maxFileSize / (1024 * 1024)}MB`,
        );
      }
      ...
      if (!extension || !this.supportedFormats.includes(extension)) {
        throw new Error(
          `Unsupported audio format. Supported: ${this.supportedFormats.join(", ")}`,
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          ...
          .catch(() => ({ error: "Unknown error" }));
        throw new Error(
          `Google API error: ${errorData.error || response.statusText}`,
        );
      }
