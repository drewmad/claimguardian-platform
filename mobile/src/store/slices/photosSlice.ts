...
const photosSlice = createSlice({
  ...
  reducers: {
    addPhoto: (
      state,
      action: PayloadAction<Omit<Photo, "id" | "createdAt" | "updatedAt">>,
    ) => {
      const photo: Photo = {
        ...action.payload,
      ...
      state.stats.pendingSyncPhotos = state.photos.filter(
    ...
    selectAllPhotos: (state) => {
      ...
      state.selectedPhotos = [
        ...new Set([...state.selectedPhotos, ...visiblePhotoIds]),
      ];
    },
    ...
    updatePhotoNotes: (
      state,
      action: PayloadAction<{ photoId: string; notes: string }>,
    ) => {
    ...
    updatePhotoTags: (
      state,
      action: PayloadAction<{ photoId: string; tags: string[] }>,
    ) => {
      const photo = state.photos.find((p) => p.id === action.payload.photoId);
      if (photo) {
    ...
    updatePhotoAnalysis: (
      state,
      action: PayloadAction<{ photoId: string; analysis: PhotoAnalysis }>,
    ) => {
      const photo = state.photos.find((p) => p.id === action.payload.photoId);
      if (photo) {
    ...
    setFilters: (
      state,
      action: PayloadAction<Partial<PhotosState["filters"]>>,
    ) => {
...
function updateStats(state: PhotosState) {
  ...
  const photosWithDamage = state.photos.filter(
    (p) => p.analysis?.estimatedCost,
  );
  ...
  state.stats.averageDamageScore =
    photosWithScore.length > 0
      ? photosWithScore.reduce(
          (sum, p) => sum + (p.analysis?.damageScore || 0),
          0,
        ) / photosWithScore.length
      : 0;
}
...
export const selectAllPhotos = (state: { photos: PhotosState }) =>
  state.photos.photos;
export const selectFilteredPhotos = (state: { photos: PhotosState }) =>
  getFilteredPhotos(state.photos);
...
export const selectPhotoById =
  (photoId: string) => (state: { photos: PhotosState }) =>
    state.photos.photos.find((p) => p.id === photoId);
export const selectPhotosStats = (state: { photos: PhotosState }) =>
  state.photos.stats;
export const selectPhotosLoading = (state: { photos: PhotosState }) =>
  state.photos.isLoading;
export const selectPhotosError = (state: { photos: PhotosState }) =>
  state.photos.error;
