  ...
  const recordingOptions = useMemo(() => {
    const baseOptions = {
      android: {
        ...
        numberOfChannels: 1,
        bitRate:
          quality === "high" ? 128000 : quality === "medium" ? 64000 : 32000,
        sampleRate:
          quality === "high" ? 44100 : quality === "medium" ? 22050 : 16000,
      },
      ios: {
        extension: ".m4a",
  ...
  useEffect(() => {
    if (isRecording) {
      const interval = setInterval(() => {
        const newWaveformData = Array(WAVEFORM_BARS)
          .fill(0)
          .map(() => Math.random() * (audioLevel + 0.1));
  ...
  const transcribeAudio = useCallback(async (uri: string) => {
    ...
    try {
      ...
      Alert.alert(
        ...
        "Could not transcribe audio. The recording will still be saved.",
      );
    } finally {
        ...
        {!isRecording && !audioUri && (
          <View style={styles.tipsContainer}>
            ...
            <View style={styles.tipsList}>
              ...
              <Text style={styles.tip}>
                • Describe damage details, locations, and observations
              </Text>
            </View>
          </View>
        )}
